...
|-----------|-----------|----------|-------|-------------|
| `model` | MODEL | 예 | - | 교란된 주의 유도를 적용할 확산 모델입니다. |
| `scale` | FLOAT | 아니요 | 0.0 - 100.0 | 교란된 주의 유도 효과의 강도입니다(기본값: 3.0). 0으로 설정하면 노드가 아무런 효과를 내지 않으며 원래의 잡음 제거 결과를 반환합니다. |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `model` | MODEL | 교란된 주의 유도가 적용된 수정된 모델입니다. |
