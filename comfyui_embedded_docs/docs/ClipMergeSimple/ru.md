...
|----------|------------|-----------|--------------|----------|----------|
| `clip1` | CLIP | ОБЯЗАТЕЛЬНЫЙ | - | - | Первая модель CLIP для слияния. Служит базовой моделью для процесса слияния. |
| `clip2` | CLIP | ОБЯЗАТЕЛЬНЫЙ | - | - | Вторая модель CLIP для слияния. Ее ключевые патчи, за исключением идентификаторов позиций и масштаба логитов, применяются к первой модели на основе указанного коэффициента. |
| `соотношение` | FLOAT | ОБЯЗАТЕЛЬНЫЙ | 1.0 | 0.0 - 1.0 (шаг: 0.01) | Определяет долю признаков второй модели, смешиваемых с первой моделью. Коэффициент 1.0 означает полное применение признаков второй модели, а 0.0 оставляет только признаки первой модели. |

## Выходы

| Имя выхода | Тип данных | Описание |
|------------|------------|----------|
| `clip` | CLIP | Результирующая объединенная модель CLIP, включающая признаки обеих входных моделей в соответствии с указанным коэффициентом. |
