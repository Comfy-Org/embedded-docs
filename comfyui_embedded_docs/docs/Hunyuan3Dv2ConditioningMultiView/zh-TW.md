...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `前視圖` | CLIP_VISION_OUTPUT | 否 | - | 前視角的 CLIP 視覺輸出 |
| `左視圖` | CLIP_VISION_OUTPUT | 否 | - | 左視角的 CLIP 視覺輸出 |
| `後視圖` | CLIP_VISION_OUTPUT | 否 | - | 後視角的 CLIP 視覺輸出 |
| `右視圖` | CLIP_VISION_OUTPUT | 否 | - | 右視角的 CLIP 視覺輸出 |
...
| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `負向` | CONDITIONING | 包含帶有位置編碼的組合多視角嵌入的正向條件 |
