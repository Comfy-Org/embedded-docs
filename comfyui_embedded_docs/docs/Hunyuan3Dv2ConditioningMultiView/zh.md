...
| 参数 | 数据类型 | 必需 | 取值范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `front` | CLIP_VISION_OUTPUT | 否 | - | 前视角的 CLIP 视觉输出 |
| `left` | CLIP_VISION_OUTPUT | 否 | - | 左视角的 CLIP 视觉输出 |
| `back` | CLIP_VISION_OUTPUT | 否 | - | 后视角的 CLIP 视觉输出 |
| `right` | CLIP_VISION_OUTPUT | 否 | - | 右视角的 CLIP 视觉输出 |
...
| 输出名称 | 数据类型 | 描述 |
|-------------|-----------|-------------|
| `positive` | CONDITIONING | 包含带位置编码的组合多视角嵌入的正向条件数据 |
