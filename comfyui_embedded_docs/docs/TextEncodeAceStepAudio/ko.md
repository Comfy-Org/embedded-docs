...
| `tags` | STRING | 예 | - | 오디오 컨디셔닝을 위한 텍스트 태그 또는 설명 (여러 줄 입력 및 동적 프롬프트 지원) |
| `lyrics` | STRING | 예 | - | 오디오 컨디셔닝을 위한 가사 텍스트 (여러 줄 입력 및 동적 프롬프트 지원) |
| `lyrics_strength` | FLOAT | 아니요 | 0.0 - 10.0 | 컨디셔닝 출력에 대한 가사의 영향 강도를 제어합니다 (기본값: 1.0, 단계: 0.01) |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `conditioning` | CONDITIONING | 가사 강도가 적용된 처리된 텍스트 토큰을 포함하는 인코딩된 컨디셔닝 데이터 |
