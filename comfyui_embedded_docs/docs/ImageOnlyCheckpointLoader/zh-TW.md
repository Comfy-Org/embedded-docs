...
| 欄位         | 資料類型      | 說明                                                                                   |
|--------------|---------------|---------------------------------------------------------------------------------------|
| `ckpt_name`  | COMBO[STRING] | 指定要載入的檢查點名稱，對於從預定義清單中識別和擷取正確的檢查點檔案至關重要。 |

## 輸出

| 欄位           | 資料類型       | 說明                                                                                                   |
|----------------|----------------|-------------------------------------------------------------------------------------------------------|
| `model`        | MODEL          | 傳回從檢查點載入的主要模型，該模型已配置為在影片生成環境中進行影像處理。 |
| `clip_vision`  | `CLIP_VISION`  | 提供來自檢查點的 CLIP 視覺元件，專為影像理解與特徵提取而設計。 |
| `vae`          | VAE            | 提供變分自編碼器（VAE）元件，對於影像操作與生成任務至關重要。 |