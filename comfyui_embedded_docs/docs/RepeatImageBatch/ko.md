...
| 필드 | 데이터 타입 | 설명 |
|---------|-------------|-------------|
| `이미지` | `IMAGE` | 'image' 매개변수는 복제할 이미지를 나타냅니다. 배치 전체에 걸쳐 중복될 콘텐츠를 정의하는 데 중요합니다. |
| `개수` | `INT` | 'amount' 매개변수는 입력 이미지를 복제할 횟수를 지정합니다. 출력 배치의 크기에 직접적인 영향을 미치며, 유연한 배치 생성을 가능하게 합니다. |

## 출력

| 필드 | 데이터 타입 | 설명 |
|-------|-------------|-------------|
| `이미지` | `IMAGE` | 출력은 입력 이미지와 각각 동일한 이미지들의 배치로, 지정된 'amount'에 따라 복제됩니다. |