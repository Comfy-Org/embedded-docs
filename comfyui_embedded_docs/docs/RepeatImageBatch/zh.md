...
| 字段    | 数据类型 | 描述                                                                 |
|---------|----------|----------------------------------------------------------------------|
| `image` | `IMAGE`  | 'image' 参数表示要复制的图像。它对于定义将在批次中重复的内容至关重要。 |
| `amount`| `INT`    | 'amount' 参数指定输入图像应被复制的次数。它直接影响输出批次的大小，从而实现灵活的批次创建。 |

## 输出

| 字段   | 数据类型 | 描述                                                               |
|--------|----------|--------------------------------------------------------------------|
| `image`| `IMAGE`  | 输出是一批图像，每个图像都与输入图像相同，根据指定的 'amount' 进行复制。 |