...
| `seed` | Semilla para usar en la generación, que ayuda a controlar la aleatoriedad del resultado (predeterminado: 0). | INT | No | 0 a 2147483647 |
...
| Parámetro | Descripción | Tipo de datos | Obligatorio | Rango |
| --- | --- | --- | --- | --- |
| `model.prompt` | Prompt que describe el video. Use identificadores como 'character1' y 'character2' para referirse a los personajes de referencia. Debe contener al menos un personaje. | STRING | Sí | - |
| `model.negative_prompt` | Prompt negativo que describe lo que se debe evitar (predeterminado: vacío). | STRING | No | - |
...
| Parámetro | Descripción | Tipo de datos | Obligatorio | Rango |
| --- | --- | --- | --- | --- |
| `model.reference_videos` | Ranura ampliable: conecte hasta 3 videos de referencia (ranuras `video1` a `video3`). Se requiere al menos un video o imagen de referencia en total. | VIDEO | No | 0 a 3 elementos |
| `model.reference_images` | Ranura ampliable: conecte hasta 5 imágenes de referencia (ranuras `image1` a `image5`). Se requiere al menos un video o imagen de referencia en total. | IMAGE | No | 0 a 5 elementos |
