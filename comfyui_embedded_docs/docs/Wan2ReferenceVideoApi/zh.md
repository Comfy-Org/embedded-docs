...
| `watermark` | 是否在结果中添加 AI 生成的水印（默认值：False）。这是一项高级设置。 | BOOLEAN | 是 | True<br>False |

### wan2.7-r2v 输入

| 参数 | 描述 | 数据类型 | 是否必需 | 范围 |
| --- | --- | --- | --- | --- |
| `prompt` | 描述视频的提示词。使用如 'character1' 和 'character2' 之类的标识符来指代参考角色。必须至少包含一个角色。 | STRING | 是 | - |
| `negative_prompt` | 描述应避免内容的负面提示词（默认值：空）。 | STRING | 否 | - |
| `resolution` | 输出视频的分辨率。 | COMBO | 是 | "720P"<br>"1080P" |
| `ratio` | 输出视频的长宽比。 | COMBO | 是 | "16:9"<br>"9:16"<br>"1:1"<br>"4:3"<br>"3:4" |
...
| 参数 | 描述 | 数据类型 | 是否必需 | 范围 |
| --- | --- | --- | --- | --- |
| `reference_videos` | 可增长插槽：最多连接 3 个参考视频（插槽 `video1` 至 `video3`）。整体至少需要 1 个参考视频或参考图像。 | VIDEO | 否 | 0 到 3 items |
| `reference_images` | 可增长插槽：最多连接 5 个参考图像（插槽 `image1` 至 `image5`）。整体至少需要 1 个参考视频或参考图像。 | IMAGE | 否 | 0 到 5 items |
