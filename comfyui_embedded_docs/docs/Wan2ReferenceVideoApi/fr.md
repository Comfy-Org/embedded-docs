...
Ce nœud génère une vidéo mettant en scène une personne ou un objet à partir des matériaux de référence fournis. Il utilise le modèle Wan 2.7 pour créer des vidéos à partir d'un prompt textuel, en prenant en charge les performances d'un seul personnage et les interactions multi-personnages. Vous devez fournir au moins une vidéo de référence ou une image de référence pour que la génération fonctionne.
...
| Paramètre | Description | Type de données | Requis | Plage |
| --- | --- | --- | --- | --- |
| `prompt` | Prompt décrivant la vidéo. Utilisez des identifiants tels que 'character1' et 'character2' pour faire référence aux personnages de référence. Doit contenir au moins un personnage. | STRING | Oui | - |
...
* Vous devez fournir au moins une vidéo de référence ou une image de référence dans les entrées `reference_videos` ou `reference_images`. Sinon, le nœud génère une erreur.
