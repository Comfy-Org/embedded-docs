...
Not: `object_indices` yalnızca virgülle ayrılmış rakamları kabul eder; sayısal olmayan girişler ve aralık dışı dizinler yok sayılır. `ref_track_data` sağlanmadığında, `reference_image_mask` çıktısı referans arka plan rengini kullanan düz bir dolgudur (Animasyon Modunda beyaz, Değiştirme Modunda siyah).
