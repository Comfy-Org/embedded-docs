...
| `object_indices` | Dahil edilecek kişi indekslerinin virgülle ayrılmış listesi (örn. '0,2,3'). Hem referans hem de poz videosu maskelerine uygulanır. Boş = tümü. (varsayılan: "") | STRING | Evet | - |
...
| `replacement_mode` | False = Animasyon Modu (pose_video_mask siyah arka plana sahiptir, reference_image_mask beyaz arka plana sahiptir). True = Değiştirme Modu (pose_video_mask beyaz arka plana sahiptir, reference_image_mask siyah arka plana sahiptir). (varsayılan: False) | BOOLEAN | Evet | False<br>True |
