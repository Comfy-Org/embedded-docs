...
참고: `object_indices`는 쉼표로 구분된 숫자만 허용합니다. 숫자가 아닌 항목과 범위를 벗어난 인덱스는 무시됩니다. `ref_track_data`가 제공되지 않으면 `reference_image_mask` 출력은 참조 배경색을 사용한 단색 채움입니다(애니메이션 모드에서는 흰색, 교체 모드에서는 검은색).
