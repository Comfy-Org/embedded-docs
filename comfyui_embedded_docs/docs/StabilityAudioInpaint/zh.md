...
| 参数 | 数据类型 | 是否必填 | 范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `model` | COMBO | 是 | "stable-audio-2.5" | 用于音频修复的 AI 模型。 |
| `prompt` | STRING | 是 |  | 指导音频如何转换的文本描述（默认：空）。 |
| `audio` | AUDIO | 是 |  | 要转换的输入音频文件。音频时长必须在 6 到 190 秒之间。 |
| `duration` | INT | 否 | 1-190 | 控制生成音频的时长（以秒为单位）（默认：190）。 |
| `seed` | INT | 否 | 0-4294967294 | 用于生成的随机种子（默认：0）。 |
| `steps` | INT | 否 | 4-8 | 控制采样步数（默认：8）。 |
| `mask_start` | INT | 否 | 0-190 | 要转换的音频部分的起始位置（以秒为单位）（默认：30）。 |
| `mask_end` | INT | 否 | 0-190 | 要转换的音频部分的结束位置（以秒为单位）（默认：190）。 |
...
| 输出名称 | 数据类型 | 描述 |
|-------------|-----------|-------------|
| `audio` | AUDIO | 转换后的音频输出，指定部分已根据提示进行了修改。 |
