...
|-----------|--------------|-------------|-------|-----------|
| `model` | COMBO | Sim | "stable-audio-2.5" | O modelo de IA a ser usado para inpainting de áudio. |
...
| `seed` | INT | Não | 0-4294967294 | A semente aleatória usada para geração (padrão: 0). |
| `steps` | INT | Não | 4-8 | Controla o número de etapas de amostragem (padrão: 8). |
| `mask_start` | INT | Não | 0-190 | Posição inicial em segundos da seção de áudio a ser transformada (padrão: 30). |
| `mask_end` | INT | Não | 0-190 | Posição final em segundos da seção de áudio a ser transformada (padrão: 190). |
