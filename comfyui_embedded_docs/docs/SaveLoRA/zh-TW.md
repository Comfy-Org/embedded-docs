...
The SaveLoRA 節點會將 LoRA（低秩適應）模型儲存為檔案。它會將 LoRA 模型以 `.safetensors` 檔案格式寫入輸出目錄。您可以指定檔名前綴以及選用的步數；當提供步數時，步數會包含在儲存的檔名中。

## 輸入

| 參數 | 說明 | 資料類型 | 必填 | 範圍 |
| --- | --- | --- | --- | --- |
| `lora` | 要儲存的 LoRA 模型。請勿使用已包含 LoRA 層的模型。 | LORA_MODEL | 是 | N/A |
...
| 輸出名稱 | 說明 | 資料類型 |
| --- | --- | --- |
| *None* | 此節點不會向工作流程輸出任何資料。它是一個輸出節點，負責將檔案儲存至磁碟。 | N/A |
