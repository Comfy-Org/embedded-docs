...
|-----------|-------------|-----------------|--------|-------|
| `model` | Le modèle à utiliser pour la génération vidéo. La sélection d’une option révèle des paramètres de configuration supplémentaires pour `resolution`, `duration` et `audio`. | COMBO | Oui | `"viduq3-pro"`<br>`"viduq3-turbo"` |
...
| `seed` | Une valeur de graine pour contrôler le caractère aléatoire de la génération (par défaut : 1). | INT | Non | De 0 à 2147483647 |
...
| Paramètre | Description | Type de données | Requis | Plage |
|-----------|-------------|-----------------|--------|-------|
| `model.resolution` | Résolution de la vidéo de sortie. Ce paramètre est révélé après la sélection d’un `model`. | COMBO | Oui | `"720p"`<br>`"1080p"` |
| `model.duration` | Durée de la vidéo de sortie en secondes (par défaut : 5). Ce paramètre est révélé après la sélection d’un `model`. | INT | Oui | De 1 à 16 |
| `model.audio` | Lorsqu’il est activé, génère une vidéo avec du son (y compris dialogues et effets sonores) (par défaut : False). Ce paramètre est révélé après la sélection d’un `model`. | BOOLEAN | Oui | `True`<br>`False` |
