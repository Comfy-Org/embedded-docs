...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `model` | COMBO | 是 | `"viduq3-pro"`<br>`"viduq3-turbo"` | 用於影片生成的模型。選擇一個選項會顯示 `resolution`、`duration` 和 `audio` 的額外配置參數。 |
| `model.resolution` | COMBO | 是 | `"720p"`<br>`"1080p"` | 輸出影片的解析度。此參數在選取 `model` 後顯示。 |
| `model.duration` | INT | 是 | 1 至 16 | 輸出影片的時長（秒），預設值為 5。此參數在選取 `model` 後顯示。 |
| `model.audio` | BOOLEAN | 是 | `True` / `False` | 啟用時，輸出包含聲音（包括對話與音效）的影片，預設值為 False。此參數在選取 `model` 後顯示。 |
| `first_frame` | IMAGE | 是 | - | 影片序列的起始圖片。 |
| `end_frame` | IMAGE | 是 | - | 影片序列的結束圖片。 |
