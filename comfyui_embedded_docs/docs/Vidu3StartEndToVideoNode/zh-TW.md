...
| `seed` | 用於控制生成隨機性的種子值（預設值：1）。 | INT | 否 | 0 to 2147483647 |
...
| `model.duration` | 輸出影片的持續時間（秒）（預設值：5）。選取 `model` 後會顯示此參數。 | INT | 是 | 1 to 16 |
| `model.audio` | 啟用時，輸出含有聲音的影片（包括對話與音效）（預設值：False）。選取 `model` 後會顯示此參數。 | BOOLEAN | 是 | `True`<br>`False` |
