...
| 参数 | 数据类型 | 必填 | 取值范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `width` | INT | 是 | 16 至 MAX_RESOLUTION | 潜视频的宽度（单位：像素，默认值：848，必须能被 16 整除） |
| `height` | INT | 是 | 16 至 MAX_RESOLUTION | 潜视频的高度（单位：像素，默认值：480，必须能被 16 整除） |
| `length` | INT | 是 | 7 至 MAX_RESOLUTION | 潜视频的帧数（默认值：25） |
| `batch_size` | INT | 否 | 1 至 4096 | 批次中生成的潜视频数量（默认值：1） |
