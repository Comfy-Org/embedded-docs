...
|-----------|-----------|----------|-------|-------------|
| `width` | INT | 是 | 16 至 MAX_RESOLUTION | 潜空间视频的宽度（像素），默认值：848，必须能被 16 整除 |
| `height` | INT | 是 | 16 至 MAX_RESOLUTION | 潜空间视频的高度（像素），默认值：480，必须能被 16 整除 |
| `length` | INT | 是 | 7 至 MAX_RESOLUTION | 潜空间视频的帧数，默认值：25，减 1 后必须能被 6 整除 |
| `batch_size` | INT | 否 | 1 至 4096 | 每批次生成的潜空间视频数量，默认值：1 |
