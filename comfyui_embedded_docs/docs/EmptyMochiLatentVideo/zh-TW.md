...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `寬度` | INT | 是 | 16 至 MAX_RESOLUTION | 潛在影片的寬度（像素），預設值為 848，必須能被 16 整除 |
| `高度` | INT | 是 | 16 至 MAX_RESOLUTION | 潛在影片的高度（像素），預設值為 480，必須能被 16 整除 |
| `長度` | INT | 是 | 7 至 MAX_RESOLUTION | 潛在影片的幀數，預設值為 25，減去 1 後必須能被 6 整除 |
| `批次大小` | INT | 否 | 1 至 4096 | 批次中要生成的潛在影片數量，預設值為 1 |
