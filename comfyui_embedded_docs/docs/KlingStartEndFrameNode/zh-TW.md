...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `start_frame` | IMAGE | 是 | - | 參考影像 - URL 或 Base64 編碼字串，不得超過 10MB，解析度不低於 300*300px，長寬比介於 1:2.5 ~ 2.5:1 之間。Base64 不應包含 data:image 前綴。 |
...
| `aspect_ratio` | COMBO | 否 | "16:8"<br>"9:16"<br>"1:1" | 生成影片的長寬比（預設值："16:9"） |
| `mode` | COMBO | 否 | 提供多個選項 | 用於影片生成的配置，格式為：模式 / 時長 / 模型名稱。（預設值：可用模式中的第七個選項） |
