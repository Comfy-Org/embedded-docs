...
| 参数 | 数据类型 | 是否必填 | 范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `start_frame` | IMAGE | 是 | - | 参考图像 - URL 或 Base64 编码字符串，大小不能超过 10MB，分辨率不低于 300*300px，宽高比在 1:2.5 ~ 2.5:1 之间。Base64 不应包含 data:image 前缀。 |
...
| `aspect_ratio` | COMBO | 否 | "16:8"<br>"9:16"<br>"1:1" | 生成视频的宽高比（默认值："16:9"） |
| `mode` | COMBO | 否 | 提供多个选项 | 用于视频生成的配置，格式为：模式 / 时长 / 模型名称。（默认值：可用模式中的第七个选项） |
