...
|-----------|-----------|----------|-------|-------------|
| `prompt` | STRING | 是 | 无 | 用于生成的文本提示词（默认值：空字符串） |
| `start_frame` | IMAGE | 是 | 无 | 用于视频的起始帧 |
| `end_frame` | IMAGE | 是 | 无 | 用于视频的结束帧。仅 gen3a_turbo 模型支持此参数。 |
| `duration` | COMBO | 是 | `"5"`<br>`"10"` | 视频时长（秒）（默认值："5"） |
| `ratio` | COMBO | 是 | `"16:9"`<br>`"9:16"`<br>`"1:1"` | 生成视频的宽高比（默认值："16:9"） |
| `seed` | INT | 否 | 0 至 4294967295 | 用于生成的随机种子。设为 0 则使用随机种子（默认值：0）。 |
