...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `提示詞` | STRING | 是 | 不適用 | 生成用的文字提示（預設：空字串） |
| `起始幀` | IMAGE | 是 | 不適用 | 用於影片的起始幀 |
| `結束幀` | IMAGE | 是 | 不適用 | 用於影片的結束幀。僅支援 gen3a_turbo 模型。 |
| `持續時間` | COMBO | 是 | `"5"`<br>`"10"` | 影片長度（秒）（預設："5"） |
| `比例` | COMBO | 是 | `"16:9"`<br>`"9:16"`<br>`"1:1"` | 生成影片的長寬比（預設："16:9"） |
| `種子值` | INT | 否 | 0 至 4294967295 | 生成用的隨機種子。設為 0 使用隨機種子（預設：0）。 |
