...
* The number of items in the `texts` list must be 0, 1, or exactly match the number of items in the `images` list. If it is 0, an empty string is used for all images. If it is 1, that single text is repeated for all images. If the count is anything else, the node raises an error.
