...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `圖片` | IMAGE | 是 | N/A | 要編碼的影像列表。 |
| `vae` | VAE | 是 | N/A | 用於將影像編碼為潛在表示的 VAE 模型。 |
| `clip` | CLIP | 是 | N/A | 用於將文字編碼為條件資料的 CLIP 模型。 |
| `文字` | STRING | 否 | N/A | 文字說明列表。長度可為 n（與影像數量相符）、1（套用至所有影像），或省略（使用空字串）。 |
...
| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `conditioning` | LATENT | 潛在字典列表。 |
| `conditioning` | CONDITIONING | 條件列表列表。 |
