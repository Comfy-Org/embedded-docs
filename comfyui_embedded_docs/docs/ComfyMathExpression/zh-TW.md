...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `expression` | STRING | 是 | 不適用 | 要評估的數學公式。您可以使用對應於輸入值的變數名稱（預設值："a + b"）。 |
| `values` | FLOAT, INT, BOOLEAN | 否 | 不適用 | 一組可動態添加的數值或布林值輸入。每個輸入會被分配一個字母（a、b、c...），以便在表達式中作為變數使用。 |
