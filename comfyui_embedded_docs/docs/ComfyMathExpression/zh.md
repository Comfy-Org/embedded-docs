...
| 参数 | 数据类型 | 是否必填 | 范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `expression` | STRING | 是 | N/A | 要计算的数学公式。您可以使用与输入值对应的变量名（默认值："a + b"）。 |
| `values` | FLOAT, INT, BOOLEAN | 否 | N/A | 一组可动态添加的数值或布尔输入。每个输入被分配一个字母（a, b, c, ...），用作表达式中的变量。 |
