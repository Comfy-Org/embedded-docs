...
| 参数 | 数据类型 | 描述 |
|-------------|-------------|---------------------------------------------------------------------------------------------|
| `steps` | INT | 指定扩散过程中的步数。它影响生成的 sigma 序列的长度，从而影响噪声应用的粒度。 |
| `sigma_max` | FLOAT | 定义最大 sigma 值，设定扩散过程中噪声强度的上限。它在确定所应用的噪声水平范围方面起着关键作用。 |
| `sigma_min` | FLOAT | 设置最小 sigma 值，建立噪声强度的下限。此参数有助于微调噪声应用的起始点。 |
