...
| 參數 | 資料類型 | 說明 |
|-------------|-------------|---------------------------------------------------------------------------------------------|
| `steps` | INT | 指定擴散過程中的步驟數。它會影響生成的 sigma 序列的長度，進而影響噪聲應用的粒度。 |
| `sigma_max` | FLOAT | 定義最大 sigma 值，設定擴散過程中噪聲強度的上限。它在決定應用的噪聲水平範圍方面起著關鍵作用。 |
| `sigma_min` | FLOAT | 設定最小 sigma 值，建立噪聲強度的下限。此參數有助於微調噪聲應用的起始點。 |

## 輸出

| 參數 | 資料類型 | 說明 |
|-----------|-------------|---------------------------------------------------------------------------------------------|
| `sigmas` | SIGMAS | 根據指數排程生成的 sigma 值序列。這些值用於控制擴散過程中每一步的噪聲水平。 |