...
| --- | --- | --- | --- | --- |
| `num_frame_per_block` | Quadros por bloco autorregressivo. Um valor de 1 significa que o modelo gera um quadro por vez (quadro a quadro), enquanto um valor de 3 significa que ele gera três quadros juntos (em blocos). Essa configuração deve corresponder ao modo de treinamento do checkpoint. Padrão: 1. | INT | Sim | 1 to 64 |
