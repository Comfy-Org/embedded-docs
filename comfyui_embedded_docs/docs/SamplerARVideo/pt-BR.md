...
O nó Sampler AR Video fornece um método de amostragem especializado para modelos de vídeo autorregressivos, como aqueles que utilizam técnicas de Causal Forcing ou Self-Forcing. Ele gerencia todos os parâmetros relacionados ao loop autorregressivo (AR) diretamente no fluxo de trabalho, facilitando a configuração de como o modelo gera quadros de vídeo um passo de cada vez.

## Entradas

| Parâmetro | Tipo de Dado | Obrigatório | Intervalo | Descrição |
|-----------|---------------|-------------|-----------|-----------|
| `num_frame_per_block` | INT | Sim | 1 a 64 | Quadros por bloco autorregressivo. Um valor de 1 significa que o modelo gera um quadro por vez (quadro a quadro), enquanto um valor de 3 significa que ele gera três quadros juntos (por bloco). Esta configuração deve corresponder ao modo de treinamento do checkpoint. Padrão: 1. |

## Saídas

| Nome da Saída | Tipo de Dado | Descrição |
|---------------|---------------|-----------|
| `SAMPLER` | SAMPLER | Um objeto de amostrador configurado que utiliza a função de amostragem "ar_video" com os parâmetros autorregressivos especificados. |