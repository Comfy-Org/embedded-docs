...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `mesh` | MESH 或 FILE3D | 是 | - | 要儲存的網格或 3D 檔案。接受網格資料或包含 GLB、GLTF、OBJ、FBX、STL 和 USDZ 在內的 3D 檔案格式 |
| `filename_prefix` | STRING | 否 | - | 輸出檔案名稱的前綴（預設值："3d/ComfyUI"） |

## 輸出

| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `ui` | UI | 在使用者介面中顯示已儲存的 3D 檔案，包含檔案名稱、子資料夾和類型資訊 |
