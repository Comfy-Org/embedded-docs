...
| `largeur` | INT | Oui | 16 à MAX_RESOLUTION | Largeur de la vidéo de sortie (par défaut : 832, pas : 16) |
| `hauteur` | INT | Oui | 16 à MAX_RESOLUTION | Hauteur de la vidéo de sortie (par défaut : 480, pas : 16) |
...
| `sortie_vision_clip` | CLIP_VISION_OUTPUT | Non | - | Sortie CLIP vision facultative pour un conditionnement supplémentaire |
| `image_de_départ` | IMAGE | Non | - | Image de départ facultative pour initialiser la génération vidéo |
...
|---------------|-----------------|-------------|
| `négatif` | CONDITIONING | Conditionnement positif modifié avec les données d'image et visuelles intégrées |
| `latent` | CONDITIONING | Conditionnement négatif modifié avec les données d'image et visuelles intégrées |
