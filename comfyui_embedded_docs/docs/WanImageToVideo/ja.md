...
| `length` | INT | はい | 1 ～ MAX_RESOLUTION | ビデオのフレーム数（デフォルト: 81, ステップ: 4） |
| `batch_size` | INT | はい | 1 ～ 4096 | バッチで生成するビデオの数（デフォルト: 1） |
...
| 出力名 | データ型 | 説明 |
|-------------|-----------|-------------|
| `positive` | CONDITIONING | 画像とビジョンデータが組み込まれた修正済み正の条件付け |
| `negative` | CONDITIONING | 画像とビジョンデータが組み込まれた修正済み負の条件付け |
