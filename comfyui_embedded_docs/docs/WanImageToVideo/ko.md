> 이 문서는 AI에 의해 생성되었습니다. 오류를 발견하거나 개선 제안이 있으시면 기여해 주세요! [GitHub에서 편집](https://github.com/Comfy-Org/embedded-docs/blob/main/comfyui_embedded_docs/docs/WanImageToVideo/ko.md)

WanImageToVideo 노드는 비디오 생성 작업을 위한 컨디셔닝 및 잠재 표현을 준비합니다. 비디오 생성을 위한 빈 잠재 공간을 생성하며, 선택적으로 시작 이미지와 CLIP 비전 출력을 통합하여 비디오 생성 과정을 안내할 수 있습니다. 이 노드는 제공된 이미지와 비전 데이터를 기반으로 양수 및 음수 컨디셔닝 입력을 모두 수정합니다.

## 입력

| 매개변수 | 데이터 타입 | 필수 | 범위 | 설명 |
|-----------|-----------|----------|-------|-------------|
| `긍정 조건` | CONDITIONING | 예 | - | 생성을 안내하는 양수 컨디셔닝 입력 |
| `부정 조건` | CONDITIONING | 예 | - | 생성을 안내하는 음수 컨디셔닝 입력 |
| `vae` | VAE | 예 | - | 이미지를 잠재 공간으로 인코딩하는 VAE 모델 |
| `너비` | INT | 예 | 16 ~ MAX_RESOLUTION | 출력 비디오의 너비 (기본값: 832, 단계: 16) |
| `높이` | INT | 예 | 16 ~ MAX_RESOLUTION | 출력 비디오의 높이 (기본값: 480, 단계: 16) |
| `길이` | INT | 예 | 1 ~ MAX_RESOLUTION | 비디오의 프레임 수 (기본값: 81, 단계: 4) |
| `배치 크기` | INT | 예 | 1 ~ 4096 | 한 배치에서 생성할 비디오 수 (기본값: 1) |
| `clip_vision 출력` | CLIP_VISION_OUTPUT | 아니요 | - | 추가 컨디셔닝을 위한 선택적 CLIP 비전 출력 |
| `시작 이미지` | IMAGE | 아니요 | - | 비디오 생성을 초기화하는 선택적 시작 이미지 |

**참고:** `start_image`가 제공되면 노드는 이미지 시퀀스를 인코딩하고 컨디셔닝 입력에 마스킹을 적용합니다. `clip_vision_output` 매개변수가 제공되면 양수 및 음수 입력 모두에 비전 기반 컨디셔닝을 추가합니다.

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `부정 조건` | CONDITIONING | 이미지 및 비전 데이터가 통합된 수정된 양수 컨디셔닝 |
| `잠재 비디오` | CONDITIONING | 이미지 및 비전 데이터가 통합된 수정된 음수 컨디셔닝 |
| `latent` | LATENT | 비디오 생성을 위해 준비된 빈 잠재 공간 텐서 |

---
**Source fingerprint (SHA-256):** `e9f4350c43e48351523c04d82675c24f868df7b2109530c32b8e752a3ab61e8b`
