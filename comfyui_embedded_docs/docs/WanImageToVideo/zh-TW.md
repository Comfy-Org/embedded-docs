...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `positive` | CONDITIONING | 是 | - | 用於引導生成的正向條件化輸入 |
| `negative` | CONDITIONING | 是 | - | 用於引導生成的負向條件化輸入 |
| `vae` | VAE | 是 | - | 用於將影像編碼至潛在空間的 VAE 模型 |
| `width` | INT | 是 | 16 至 MAX_RESOLUTION | 輸出影片的寬度（預設值：832，步進：16） |
| `height` | INT | 是 | 16 至 MAX_RESOLUTION | 輸出影片的高度（預設值：480，步進：16） |
| `length` | INT | 是 | 1 至 MAX_RESOLUTION | 影片的影格數（預設值：81，步進：4） |
| `batch_size` | INT | 是 | 1 至 4096 | 每批次生成的影片數量（預設值：1） |
| `clip_vision_output` | CLIP_VISION_OUTPUT | 否 | - | 可選的 CLIP 視覺輸出，用於額外的條件化 |
| `start_image` | IMAGE | 否 | - | 可選的起始影像，用於初始化影片生成 |
...
| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `positive` | CONDITIONING | 已結合影像與視覺資料的修改後正向條件化 |
| `negative` | CONDITIONING | 已結合影像與視覺資料的修改後負向條件化 |
