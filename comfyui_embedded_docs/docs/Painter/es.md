...
|-----------|-----------|----------|-------|-------------|
| `image` | IMAGE | No | - | Imagen base opcional sobre la que pintar. Si no se proporciona, se crea un lienzo en blanco usando el color de fondo, ancho y alto especificados. |
...
| `width` | INT | Sí | 64 a 4096 | El ancho del lienzo en píxeles, utilizado cuando no se proporciona una `image` base. El valor debe ser múltiplo de 64. Valor predeterminado: 512. |
| `height` | INT | Sí | 64 a 4096 | La altura del lienzo en píxeles, utilizada cuando no se proporciona una `image` base. El valor debe ser múltiplo de 64. Valor predeterminado: 512. |
| `bg_color` | COLOR | Sí | - | El color de fondo del lienzo, especificado como código hexadecimal (ej. #000000). Solo se usa cuando no se proporciona una `image` base. Valor predeterminado: negro (#000000). |
...
|-------------|-----------|-------------|
| `IMAGE` | IMAGE | La imagen final compuesta. Es el resultado de fusionar las áreas pintadas (de la `mask`) sobre la `image` base proporcionada o el fondo de color. |
