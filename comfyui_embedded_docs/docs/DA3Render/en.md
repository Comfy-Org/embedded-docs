...
| `output` | The type of visualization to render.<br>- depth: normalised greyscale depth image.<br>- depth_colored: depth mapped through the Turbo colormap.<br>- sky_mask: sky probability in [0, 1] (for Mono/Metric models only).<br>- confidence: normalised depth confidence (for Small/Base models only). | COMBO | Yes | "depth"<br>"depth_colored"<br>"sky_mask"<br>"confidence" |
...
| Parameter | Description | Data Type | Required | Range |
|-----------|-------------|-----------|----------|-------|
| `normalization` | Depth normalization method.<br>- v2_style: mean/std normalisation for perceptually balanced results (default).<br>- min_max: stretches the full depth range to [0, 1] for maximum contrast.<br>- raw: no scaling, preserves metric units for Metric model. | COMBO | Yes | "v2_style"<br>"min_max"<br>"raw" |
| `apply_sky_clip` | Clip sky-region depth to the 99th percentile of foreground depth before normalisation. Requires a sky key in the da3_geometry input (for Mono/Metric models only). Default: False | BOOLEAN | Yes | True<br>False |
