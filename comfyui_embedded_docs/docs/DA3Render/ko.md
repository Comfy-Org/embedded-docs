...
| `output` | 렌더링할 시각화 유형입니다.<br>- depth: 정규화된 회색조 깊이 이미지.<br>- depth_colored: Turbo 컬러맵으로 매핑된 깊이.<br>- sky_mask: [0, 1] 범위의 하늘 확률(Mono/Metric 모델 전용).<br>- confidence: 정규화된 깊이 신뢰도(Small/Base 모델 전용). | COMBO | 예 | "depth"<br>"depth_colored"<br>"sky_mask"<br>"confidence" |
...
| 매개변수 | 설명 | 데이터 타입 | 필수 | 범위 |
|-----------|-------------|-----------|----------|-------|
| `normalization` | 깊이 정규화 방법입니다.<br>- v2_style: 지각적으로 균형 잡힌 결과를 위한 평균/표준편차 정규화(기본값).<br>- min_max: 최대 대비를 위해 전체 깊이 범위를 [0, 1]로 늘립니다.<br>- raw: 스케일링 없이 Metric 모델의 미터법 단위를 유지합니다. | COMBO | 예 | "v2_style"<br>"min_max"<br>"raw" |
