...
| 参数 | 描述 | 数据类型 | 必填 | 范围 |
|-----------|-------------|-----------|----------|-------|
| `image` | 要添加所描述对象的图像。上传图像前会丢弃 alpha 通道。 | IMAGE | 是 | - |
| `instruction` | 要添加什么以及在哪里添加，例如 'Place a red vase with flowers on the table'。不能为空。默认值：""（空字符串）。 | STRING | 是 | - |
| `seed` | Bria 在此处不接受种子，每次调用都会重新构想编辑，因此重复运行可能产生不同结果。该值从不会被发送：它只会改变此节点的缓存键，这样在其他方面完全相同的工作流图会再次运行编辑，而不是返回缓存结果。默认值：42。 | INT | 是 | 0 到 2147483647 |
| `moderation` | 审核设置。选择 "true" 以显示下方的审核标志。 | DYNAMIC_COMBO | 是 | "false"<br>"true" |
