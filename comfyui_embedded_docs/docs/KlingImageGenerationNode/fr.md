...
Le nœud Kling Image Generation génère des images à partir de prompts textuels avec l'option d'utiliser une image de référence comme guide. Il crée une ou plusieurs images en fonction de votre description textuelle et des paramètres de référence, puis renvoie les images générées en sortie.

## Entrées

| Paramètre | Description | Type de données | Requis | Plage |
|-----------|-------------|-----------------|--------|-------|
| `prompt` | Prompt textuel positif. Requis, doit contenir entre 1 et 500 caractères. | STRING | Oui | Maximum 500 caractères |
| `negative_prompt` | Prompt textuel négatif. | STRING | Oui | Maximum 500 caractères |
...
| `human_fidelity` | Similarité de la référence de sujet (par défaut : 0.45, avancé) | FLOAT | Oui | 0.0 - 1.0 |
| `model_name` | Sélection du modèle pour la génération d'images (par défaut : "kling-v3") | COMBO | Oui | `"kling-v3"` |
| `aspect_ratio` | Rapport d'aspect pour les images générées (par défaut : "16:9") | COMBO | Oui | `"16:9"`<br>`"9:16"`<br>`"1:1"`<br>`"4:3"`<br>`"3:4"`<br>`"21:9"`<br>`"9:21"` |
| `n` | Nombre d'images générées (par défaut : 1) | INT | Oui | 1 - 9 |
| `image` | Image de référence facultative | IMAGE | Non | - |
| `seed` | La graine contrôle si le nœud doit être réexécuté ; les résultats sont non déterministes indépendamment de la graine (par défaut : 0) | INT | Non | 0 - 2147483647 |
...
- Le paramètre `n` contrôle le nombre d'images produites et détermine également le prix de la requête.
