...
| `negative_prompt` | STRING | 是 | - | 反向文本提示 |
| `image_type` | COMBO | 是 | `"subject_reference"`<br>`"style_reference"` | 图像参考类型选择（高级）。当提供参考图像时必填。 |
| `image_fidelity` | FLOAT | 是 | 0.0 - 1.0 | 用户上传图像的参考强度（默认值：0.5，高级） |
| `human_fidelity` | FLOAT | 是 | 0.0 - 1.0 | 主体参考相似度（默认值：0.45，高级） |
| `model_name` | COMBO | 是 | `"kling-v3"`<br>`"kling-v2"`<br>`"kling-v1-5"` | 图像生成模型选择（默认值："kling-v3"） |
| `aspect_ratio` | COMBO | 是 | `"16:9"`<br>`"9:16"`<br>`"1:1"`<br>`"4:3"`<br>`"3:4"`<br>`"21:9"`<br>`"9:21"` | 生成图像的宽高比（默认值："16:9"） |
| `n` | INT | 是 | 1 - 9 | 生成图像数量（默认值：1） |
| `image` | IMAGE | 否 | - | 可选的参考图像 |
| `seed` | INT | 否 | 0 - 2147483647 | 种子控制节点是否应重新运行；无论种子如何，结果均非确定性（默认值：0） |
