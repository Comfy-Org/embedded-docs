...
| `이미지 종류` | COMBO | 예 | `"subject_reference"`<br>`"style_reference"` | 이미지 참조 유형 선택(고급). 참조 이미지가 제공될 때 필요합니다. |
| `이미지 충실도` | FLOAT | 예 | 0.0 - 1.0 | 사용자 업로드 이미지의 참조 강도(기본값: 0.5, 고급) |
| `사람 충실도` | FLOAT | 예 | 0.0 - 1.0 | 피사체 참조 유사도(기본값: 0.45, 고급) |
| `모델 명` | COMBO | 예 | `"kling-v3"`<br>`"kling-v2"`<br>`"kling-v1-5"` | 이미지 생성을 위한 모델 선택(기본값: "kling-v3") |
| `종횡비` | COMBO | 예 | `"16:9"`<br>`"9:16"`<br>`"1:1"`<br>`"4:3"`<br>`"3:4"`<br>`"21:9"`<br>`"9:21"` | 생성된 이미지의 화면 비율(기본값: "16:9") |
| `개수` | INT | 예 | 1 - 9 | 생성할 이미지 수(기본값: 1) |
| `이미지` | IMAGE | 아니요 | - | 선택적 참조 이미지 |
