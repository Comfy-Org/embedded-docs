...
| `width`   | `INT`      | 指定在條件上下文中設定區域的寬度，影響調整的水平範圍。 |
| `height`  | `INT`      | 決定設定區域的高度，影響條件修改的垂直範圍。 |
| `x`       | `INT`      | 設定區域的水平起始點，在條件上下文中定位調整位置。 |
| `y`       | `INT`      | 區域調整的垂直起始點，在條件上下文中建立其位置。 |
| `strength`| `FLOAT`    | 定義指定區域內條件修改的強度，允許對調整影響進行細微控制。 |
