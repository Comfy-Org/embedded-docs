...
|-----------|---------------|-------------|-----------|-----------|
| `modelo` | COMBO | Sim | "wan2.5-i2v-preview"<br>"wan2.6-i2v" | Modelo a ser usado (padrão: "wan2.6-i2v") |
| `imagem` | IMAGE | Sim | - | Imagem de entrada que serve como o primeiro quadro para a geração do vídeo. Exatamente uma imagem é necessária. |
...
| `resolução` | COMBO | Não | "480P"<br>"720P"<br>"1080P" | Qualidade da resolução do vídeo (padrão: "720P"). O modelo Wan 2.6 não suporta 480P. |
| `duração` | INT | Não | 5-15 (passo: 5) | Duração do vídeo gerado em segundos. Uma duração de 15 segundos é suportada apenas pelo modelo Wan 2.6 (padrão: 5). |
| `áudio` | AUDIO | Não | - | O áudio deve conter uma voz clara e alta, sem ruídos estranhos ou música de fundo. Quando fornecido, a duração do áudio deve estar entre 3,0 e 29,0 segundos. |
...
| `marca d'água` | BOOLEAN | Não | - | Se deve adicionar uma marca d'água gerada por IA ao resultado (padrão: False). |
