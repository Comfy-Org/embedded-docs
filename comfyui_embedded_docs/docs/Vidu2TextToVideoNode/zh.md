...
|-----------|-----------|----------|-------|-------------|
| `model` | COMBO | 是 | `"viduq2"` | 用于视频生成的AI模型。目前仅支持一个可用模型。 |
| `prompt` | STRING | 是 | - | 用于视频生成的文本描述，最大长度为2000个字符。 |
| `duration` | INT | 否 | 1 至 10 | 生成视频的时长（秒）。可通过滑块调整数值（默认值：5）。 |
| `seed` | INT | 否 | 0 至 2147483647 | 用于控制生成随机性的数值，可实现可重复的结果。生成后可进行控制（默认值：1）。 |
| `aspect_ratio` | COMBO | 否 | `"16:9"`<br>`"9:16"`<br>`"3:4"`<br>`"4:3"`<br>`"1:1"` | 视频宽度与高度的比例关系。 |
| `resolution` | COMBO | 否 | `"720p"`<br>`"1080p"` | 生成视频的像素尺寸。这是一个高级参数。 |
| `background_music` | BOOLEAN | 否 | - | 是否为生成的视频添加背景音乐（默认值：False）。这是一个高级参数。 |
