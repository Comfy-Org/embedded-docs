...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `model` | MODEL | 是 | - | 包含取樣參數和處理函數的模型 |
| `noise` | NOISE | 是 | - | 產生基礎噪聲模式的噪聲生成器 |
| `sigmas` | SIGMAS | 是 | - | 控制噪聲縮放強度的 Sigma 值。若為空，節點將返回未經修改的原始潛在影像。當提供多個 sigma 值時，噪聲縮放計算為第一個與最後一個 sigma 值之間的絕對差值。當僅提供一個 sigma 值時，該值將直接用作縮放值。 |
| `latent_image` | LATENT | 是 | - | 將添加噪聲的輸入潛在表示。空的潛在影像（僅包含零值）在處理過程中不會被偏移。 |

## 輸出

| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `LATENT` | LATENT | 已添加噪聲的修改後潛在表示。輸出中的任何 NaN 或無限值都會被轉換為零以確保穩定性。 |
