> Esta documentação foi gerada por IA. Se você encontrar erros ou tiver sugestões de melhoria, sinta-se à vontade para contribuir! [Editar no GitHub](https://github.com/Comfy-Org/embedded-docs/blob/main/comfyui_embedded_docs/docs/FluxProImageNode/pt-BR.md)

Gera imagens de forma síncrona com base no prompt e na resolução. Este nó cria imagens usando o modelo Flux 1.1 Pro, enviando requisições para um endpoint de API e aguardando a resposta completa antes de retornar a imagem gerada.

## Entradas

| Parâmetro | Tipo de Dado | Obrigatório | Faixa | Descrição |
|-----------|--------------|-------------|-------|-----------|
| `prompt` | STRING | Sim | - | Prompt para a geração da imagem (padrão: string vazia) |
| `prompt_upsampling` | BOOLEANO | Sim | - | Se deve realizar upsampling no prompt. Se ativo, modifica automaticamente o prompt para uma geração mais criativa, mas os resultados são não determinísticos (a mesma semente não produzirá exatamente o mesmo resultado). (padrão: Falso) |
| `width` | INTEIRO | Sim | 256-1440 | Largura da imagem em pixels (padrão: 1024, passo: 32) |
| `height` | INTEIRO | Sim | 256-1440 | Altura da imagem em pixels (padrão: 768, passo: 32) |
| `seed` | INTEIRO | Sim | 0-18446744073709551615 | A semente aleatória usada para criar o ruído. (padrão: 0) |
| `image_prompt` | IMAGEM | Não | - | Imagem de referência opcional para guiar a geração |

## Saídas

| Nome da Saída | Tipo de Dado | Descrição |
|---------------|--------------|-----------|
| `output` | IMAGEM | A imagem gerada retornada pela API |

---
**Source fingerprint (SHA-256):** `89316d84f364854541157b5b60bae3d4e25024bd4af61a47a1748c6671b463c1`
