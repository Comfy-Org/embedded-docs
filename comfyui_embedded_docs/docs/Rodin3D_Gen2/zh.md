...
| `Seed` | INT | 否 | 0-65535 | 用于生成的随机种子值（默认值：0） |
| `Material_Type` | COMBO | 否 | "PBR"<br>"Shaded" | 应用于 3D 模型的材质类型（默认值："PBR"） |
| `Polygon_count` | COMBO | 否 | "4K-Quad"<br>"8K-Quad"<br>"18K-Quad"<br>"50K-Quad"<br>"2K-Triangle"<br>"20K-Triangle"<br>"150K-Triangle"<br>"500K-Triangle" | 生成 3D 模型的目标多边形数量（默认值："500K-Triangle"） |
...
| 输出名称 | 数据类型 | 描述 |
|-------------|-----------|-------------|
| `3D Model Path` | STRING | 生成的 3D 模型文件路径（用于向后兼容） |
