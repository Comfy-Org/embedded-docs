...
| 参数 | 数据类型 | 是否必需 | 范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `images` | IMAGE | 是 | 不适用 | 要分割成片段的输入图像序列。 |
| `segment_length` | INT | 是 | 1 到 10000 | 每个片段的帧长度（默认值：149）。 |
| `num_segments` | INT | 是 | 1 到 100 | 作为列表输出的填充片段数量（默认值：1）。 |
| `audio` | AUDIO | 否 | 不适用 | 为每个输出片段切分的音频。 |

## 输出

| 输出名称 | 数据类型 | 描述 |
|-------------|-----------|-------------|
| `keyframes_sequence` | IMAGE | 填充后的关键帧序列列表，每个片段对应一个序列。 |
| `keyframes_mask` | MASK | 指示每个片段有效帧的遮罩列表。 |
