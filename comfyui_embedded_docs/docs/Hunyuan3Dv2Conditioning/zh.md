...
Hunyuan3Dv2Conditioning 节点处理 CLIP 视觉输出，为 3D 模型生成条件数据。它从视觉输出中提取最后的隐藏状态嵌入，并创建正条件和负条件对。正条件使用实际的嵌入，而负条件使用形状相同的零值嵌入。

## 输入

| 参数 | 描述 | 数据类型 | 是否必需 | 范围 |
|-----------|-------------|-----------|----------|-------|
| `clip视觉输出` | CLIP 视觉模型的输出，包含视觉嵌入 | CLIP_VISION_OUTPUT | 是 | - |
...
| `反向` | 包含与正条件嵌入形状相同的零值嵌入的负条件数据 | CONDITIONING |
