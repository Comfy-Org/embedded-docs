...
| `series_amount` | COMBO | Sim | `"disabled"`<br>`"2"`<br>`"3"`<br>`"4"`<br>`"5"`<br>`"6"`<br>`"7"`<br>`"8"`<br>`"9"` | Gera uma série de imagens. Este recurso não é suportado para o modelo `kling-image-o1`. (padrão: "disabled") |
