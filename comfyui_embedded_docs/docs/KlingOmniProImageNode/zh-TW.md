...
| `提示詞` | STRING | 是 | - | 描述圖片內容的文字提示。可包含正面和負面描述。文字長度必須在 1 到 2500 個字元之間。 |
| `解析度` | COMBO | 是 | `"1K"`<br>`"2K"`<br>`"4K"` | 生成圖片的目標解析度。注意：`kling-image-o1` 模型不支援 4K 解析度。 |
| `長寬比` | COMBO | 是 | `"16:9"`<br>`"9:16"`<br>`"1:1"`<br>`"4:3"`<br>`"3:4"`<br>`"3:2"`<br>`"2:3"`<br>`"21:9"` | 生成圖片的期望寬高比。 |
| `系列數量` | COMBO | 是 | `"disabled"`<br>`"2"`<br>`"3"`<br>`"4"`<br>`"5"`<br>`"6"`<br>`"7"`<br>`"8"`<br>`"9"` | 生成一系列圖片。`kling-image-o1` 模型不支援此功能。（預設值："disabled"） |
| `參考圖片` | IMAGE | 否 | - | 最多 10 張額外的參考圖片。每張圖片的寬度和高度必須至少為 300 像素，且寬高比必須在 1:2.5 到 2.5:1 之間。 |
| `種子` | INT | 否 | 0 到 2147483647 | 種子值控制節點是否應重新執行；無論種子值為何，結果都是非確定性的。（預設值：0） |
