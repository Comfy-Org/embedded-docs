...
|-----------|-----------|----------|-------|-------------|
| `conditioning` | CONDITIONING | 예 | | 기준 오디오 정보가 첨부될 컨디셔닝 데이터입니다. |
| `latent` | LATENT | 아니요 | | 기준 오디오의 선택적 잠재 표현입니다. 제공된 경우 해당 샘플이 컨디셔닝에 추가됩니다. |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `conditioning` | CONDITIONING | 선택적 `latent` 입력이 제공된 경우 기준 오디오 음색 잠재 정보를 포함하도록 수정된 컨디셔닝 데이터입니다. |
