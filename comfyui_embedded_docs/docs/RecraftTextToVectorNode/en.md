...
|-----------|-------------|-----------|----------|-------|
| `prompt` | Prompt for the image generation. (default: "", maximum length: 1000 characters) | STRING | Yes | - |
...
**Note:** The `prompt` is limited to a maximum of 1000 characters. An empty `negative_prompt` is treated as no negative prompt and is not sent to the API. The `seed` parameter only controls when the node re-runs but does not make the generation results deterministic.
