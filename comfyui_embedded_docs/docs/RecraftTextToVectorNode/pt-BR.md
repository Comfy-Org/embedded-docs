...
```markdown
Gera ilustrações vetoriais SVG de forma síncrona a partir de um prompt de texto usando o modelo Recraft V3. O nó envia o prompt e quaisquer configurações opcionais para a API Recraft e retorna a ilustração vetorial gerada como dados SVG.
...
**Nota:** O `prompt` é limitado a no máximo 1000 caracteres. Um `negative_prompt` vazio é tratado como ausência de prompt negativo e não é enviado à API. O parâmetro `seed` apenas controla quando o nó é executado novamente, mas não torna os resultados da geração determinísticos.
