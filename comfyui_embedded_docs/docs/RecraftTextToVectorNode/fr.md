...
Génère des illustrations vectorielles SVG de manière synchrone à partir d'un prompt texte et d'une résolution. Ce nœud envoie votre prompt à l'API Recraft et renvoie le contenu SVG généré.

## Entrées

| Paramètre | Description | Type de données | Requis | Plage |
|-----------|-------------|-----------------|--------|-------|
| `prompt` | Prompt pour la génération de l'image. (défaut : "", maximum 1000 caractères) | STRING | Oui | - |
...
| `taille` | La taille de l'image générée. (défaut : "1024x1024") | COMBO | Oui | `"1024x1024"`<br>`"1024x2048"`<br>`"2048x1024"`<br>`"2048x2048"`<br>`"512x512"`<br>`"512x1024"`<br>`"1024x512"`<br>`"2048x512"`<br>`"512x2048"` |
| `n` | Le nombre d'images à générer. (défaut : 1, min : 1, max : 6) | INT | Oui | 1-6 |
| `seed` | Graine (seed) pour déterminer si le nœud doit se ré-exécuter ; les résultats réels sont non déterministes quelle que soit la graine. (défaut : 0, min : 0, max : 18446744073709551615) | INT | Oui | 0-18446744073709551615 |
| `prompt négatif` | Une description textuelle facultative des éléments indésirables sur une image. (défaut : "", lorsqu'elle est vide, aucun prompt négatif n'est utilisé) | STRING | Non | - |
| `recraft_controls` | Contrôles supplémentaires facultatifs sur la génération via le nœud Recraft Controls. | CONTROLS | Non | - |

**Remarque :** Le paramètre `seed` contrôle uniquement le moment où le nœud se ré-exécute, mais ne rend pas les résultats de génération déterministes.
