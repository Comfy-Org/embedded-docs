...
| フィールド | データ型 | 説明 |
|----------------|-------------|-------------------------------------------------------------------------------|
| `image` | `IMAGE` | ブラーを適用する入力画像です。ブラー効果の主要な対象となります。 |
| `blur_radius` | `INT` | ブラー効果の半径を決定します。値が大きいほど、より顕著なブラーが適用されます。 |
| `sigma` | `FLOAT` | ブラーの広がりを制御します。シグマ値が高いほど、各ピクセルの周囲の広い範囲にブラーが影響します。 |

## 出力

| フィールド | データ型 | 説明 |
|-------|-------------|--------------------------------------------------------------------------|
| `image` | `IMAGE` | 入力画像にブラーを適用した結果です。ブラーの度合いは入力パラメータによって決定されます。 |