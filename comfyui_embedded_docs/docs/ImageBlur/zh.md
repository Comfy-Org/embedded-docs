...
| 字段 | 数据类型 | 描述 |
|------|----------|------|
| `image` | `IMAGE` | 待模糊处理的输入图像，是模糊效果的主要作用对象。 |
| `blur_radius` | `INT` | 决定模糊效果的半径。半径越大，模糊程度越明显。 |
| `sigma` | `FLOAT` | 控制模糊的扩散范围。sigma 值越高，模糊对每个像素周围区域的影响范围越广。 |

## 输出

| 字段 | 数据类型 | 描述 |
|------|----------|------|
| `image` | `IMAGE` | 输出为输入图像的模糊版本，模糊程度由输入参数决定。 |