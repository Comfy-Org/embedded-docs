...
This node generates or edits images by sending a text prompt to Google's Vertex AI API through the Gemini 3.1 Flash Image models. It creates new images from a description or modifies existing images using optional reference images.
...
|-----------|-------------|-----------|----------|-------|
| `prompt` | Text prompt describing the image to generate or the edits to apply. Include any constraints, styles, or details the model should follow. Must not be empty. | STRING | Yes | N/A |
| `model` | Selects the Gemini model to use for image generation. This parameter includes additional sub-parameters for resolution, aspect ratio, thinking level, and reference inputs. | DYNAMIC_COMBO | Yes | `"Nano Banana 2 (Gemini 3.1 Flash Image)"`<br>`"Nano Banana 2 Lite"` |
...
| `system_prompt` | Foundational instructions that dictate an AI's behavior. Defaults to a built-in prompt instructing the model to always produce an image. Advanced parameter. | STRING | No | N/A |
...
**Note:** A maximum of 14 reference images can be connected to the `images` input.
...
**Note:** The `STRING` output is empty when `response_modalities` is set to `IMAGE`.
