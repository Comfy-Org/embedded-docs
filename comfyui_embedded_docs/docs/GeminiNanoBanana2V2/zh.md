...
| `seed` | 当种子固定为特定值时，模型会尽力为重复请求提供相同的响应。但不保证确定性输出。此外，更改模型或参数设置（例如温度）即使使用相同的种子值也可能导致响应变化。默认使用随机种子值。（默认值：42） | INT | 是 | 0 to 18446744073709551615 |
...
| `temperature` | 控制生成中的随机性。较低的值更聚焦/更确定。（默认值：1.0）高级参数。 | FLOAT | 否 | 0.0 to 2.0 (step 0.01) |
| `top_p` | 核采样阈值。较低的值更聚焦，较高的值更多样。（默认值：0.95）高级参数。 | FLOAT | 否 | 0.0 to 1.0 (step 0.01) |
...
| 参数 | 描述 | 数据类型 | 必填 | 范围 |
|-----------|-------------|-----------|----------|-------|
| `images` | 可选的参考图像。最多共 14 张图像。可增长插槽：连接 `image_1` 至 `image_14`。 | IMAGE | 否 | 0 to 14 images |
