...
此節點透過將文字提示發送到 Google 的 Vertex AI API 來生成或編輯圖像。它使用特定的 Gemini 模型，根據您的指示創建新圖像或修改現有圖像。

## 輸入

| 參數 | 資料類型 | 必要 | 範圍 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `prompt` | STRING | 是 | N/A | 描述要生成的圖像或要應用的編輯的文字提示。包含模型應遵循的任何限制條件、風格或細節。 |
...
| `system_prompt` | STRING | 否 | N/A | 用於指示 AI 行為的基礎指令。這是一個進階參數。 |

**關於 `model` 參數的說明：** `model` 參數是一個動態組合，包含用於解析度、長寬比和思考級別的附加子參數。這些子參數在模型選擇中定義，未在此表格中列為單獨的輸入。
...
| `STRING` | STRING | 由模型生成的文字描述或標題。 |
| `thought_image` | IMAGE | 模型思考過程中的第一張圖像。僅在思考級別為 HIGH 且回應模式為 IMAGE+TEXT 時可用。 |