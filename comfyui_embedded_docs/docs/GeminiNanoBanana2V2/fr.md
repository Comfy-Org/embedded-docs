...
| `modèle` | Sélectionne le modèle Gemini à utiliser pour la génération d'images. Ce paramètre inclut des sous-paramètres supplémentaires pour la résolution, le rapport d'aspect, le niveau de réflexion et les entrées de référence. | COMBO | Oui | `"Nano Banana 2 (Gemini 3.1 Flash Image)"`<br>`"Nano Banana 2 Lite"` |
