...
| `init_image` | IMAGE | Sim | - | A imagem de entrada inicial a ser processada e codificada |
...
| `elevation_batch_increment` | FLOAT | Não | -180.0 a 180.0 | O incremento de elevação para cada item do lote (padrão: 0.0) |
| `azimuth_batch_increment` | FLOAT | Não | -180.0 a 180.0 | O incremento de azimute para cada item do lote (padrão: 0.0) |
