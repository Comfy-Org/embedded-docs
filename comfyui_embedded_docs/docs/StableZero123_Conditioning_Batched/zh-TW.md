...
| `初始影像` | IMAGE | 是 | - | 要處理和編碼的初始輸入影像 |
| `vae` | VAE | 是 | - | 用於將影像像素編碼為潛在空間的 VAE 模型 |
| `寬度` | INT | 否 | 16 至 MAX_RESOLUTION | 處理後影像的輸出寬度（預設：256，必須能被 8 整除） |
| `高度` | INT | 否 | 16 至 MAX_RESOLUTION | 處理後影像的輸出高度（預設：256，必須能被 8 整除） |
| `批次大小` | INT | 否 | 1 至 4096 | 批次中生成的條件樣本數量（預設：1） |
| `仰角` | FLOAT | 否 | -180.0 至 180.0 | 初始相機仰角，單位為度（預設：0.0） |
| `方位角` | FLOAT | 否 | -180.0 至 180.0 | 初始相機方位角，單位為度（預設：0.0） |
| `仰角批次遞增` | FLOAT | 否 | -180.0 至 180.0 | 每個批次項目的仰角增量（預設：0.0） |
| `方位角批次遞增` | FLOAT | 否 | -180.0 至 180.0 | 每個批次項目的方位角增量（預設：0.0） |
...
| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `負向` | CONDITIONING | 包含影像嵌入和相機參數的正向條件數據 |
| `潛在空間` | CONDITIONING | 嵌入初始化為零的負向條件數據 |
