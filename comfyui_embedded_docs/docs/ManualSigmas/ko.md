...
|-----------|-----------|----------|-------|-------------|
| `sigmas` | STRING | 예 | 쉼표 또는 공백으로 구분된 모든 숫자 | 시그마 값을 포함하는 문자열입니다. 노드는 이 문자열에서 모든 숫자를 추출합니다. 예를 들어, "1, 0.5, 0.1" 또는 "1 0.5 0.1"과 같이 입력할 수 있습니다. 기본값은 "1, 0.5"입니다. |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `sigmas` | SIGMAS | 입력 문자열에서 추출된 시그마 값 시퀀스를 포함하는 텐서입니다. |
