...
Le nœud `ManualSigmas` vous permet de définir manuellement une séquence personnalisée de niveaux de bruit (sigmas) pour le processus d’échantillonnage. Vous saisissez une liste de nombres sous forme de chaîne de caractères, et le nœud les convertit en un tenseur qui peut être utilisé par d’autres nœuds d’échantillonnage. Cela est utile pour tester ou créer des programmes de bruit spécifiques.

## Entrées

| Paramètre | Description | Type de données | Requis | Plage |
| --- | --- | --- | --- | --- |
| `sigmas` | Une chaîne contenant les valeurs sigma. Le nœud extraira tous les nombres de cette chaîne. Par exemple, "1, 0.5, 0.1" ou "1 0.5 0.1". La valeur par défaut est "1, 0.5". | STRING | Oui | Nombres séparés par des virgules ou des espaces |
