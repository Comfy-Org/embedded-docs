...
ManualSigmas 节点允许您手动为采样过程定义自定义的噪声水平（sigmas）序列。您以字符串形式输入一组数字，该节点会将其转换为可供其他采样节点使用的张量。这对于测试或创建特定的噪声调度非常有用。

## 输入

| 参数 | 描述 | 数据类型 | 是否必填 | 范围 |
| --- | --- | --- | --- | --- |
| `sigmas` | 包含 sigma 值的字符串。节点将从该字符串中提取所有数字。例如，"1, 0.5, 0.1" 或 "1 0.5 0.1"。默认值为 "1, 0.5"。 | STRING | 是 | 任意以逗号或空格分隔的数字 |
