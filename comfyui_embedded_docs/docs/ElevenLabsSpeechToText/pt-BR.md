...
|-----------|-----------|----------|-------|-------------|
| `áudio` | AUDIO | Sim | - | Áudio a ser transcrito. |
| `modelo` | COMBO | Sim | `"scribe_v2"` | Modelo a ser usado para transcrição. Selecionar este modelo revela parâmetros adicionais. |
...
| `código_idioma` | STRING | Não | - | Código de idioma ISO-639-1 ou ISO-639-3 (ex.: 'en', 'es', 'fra'). Deixe vazio para detecção automática. (padrão: "") |
| `num_locs` | INT | Não | 0 - 32 | Número máximo de falantes a prever. Defina como 0 para detecção automática. (padrão: 0) |
...
| `código_idioma` | STRING | O código do idioma detectado no áudio. |
