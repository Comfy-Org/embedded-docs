...
|-----------|-----------|----------|-------|-------------|
| `audio` | AUDIO | نعم | - | الصوت المراد نسخه. |
| `model` | COMBO | نعم | `"scribe_v2"` | النموذج المستخدم للنسخ. يؤدي اختيار هذا النموذج إلى إظهار معاملات إضافية. |
...
| `language_code` | STRING | لا | - | رمز اللغة وفقًا لمعيار ISO-639-1 أو ISO-639-3 (مثل 'en'، 'es'، 'fra'). اتركه فارغًا للكشف التلقائي. (الافتراضي: "") |
| `num_speakers` | INT | لا | 0 - 32 | الحد الأقصى لعدد المتحدثين المتوقعين. اضبط على 0 للكشف التلقائي. (الافتراضي: 0) |
| `seed` | INT | لا | 0 - 2147483647 | البذرة لإمكانية إعادة الإنتاج (لا يُضمن الحتمية). (الافتراضي: 1) |
...
|-------------|-----------|-------------|
| `text` | STRING | النص المنسوخ من الصوت. |
| `language_code` | STRING | رمز اللغة المكتشفة للصوت. |
