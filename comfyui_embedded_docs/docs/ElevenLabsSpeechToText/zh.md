...
| `model` | COMBO | 是 | `"scribe_v2"` | 用于转录的模型。选择此模型后会显示额外参数。 |
...
| `language_code` | STRING | 否 | - | ISO-639-1或ISO-639-3语言代码（例如'en'、'es'、'fra'）。留空则自动检测。(默认值："") |
| `num_speakers` | INT | 否 | 0 - 32 | 预测的最大说话者数量。设为0表示自动检测。(默认值：0) |
| `seed` | INT | 否 | 0 - 2147483647 | 用于结果复现的种子（不保证确定性）。(默认值：1) |
