...
|-----------|-----------|----------|-------|-------------|
| `model` | COMBO | Sim | `"grok-imagine-image-quality"`<br>`"grok-imagine-image-pro"`<br>`"grok-imagine-image"`<br>`"grok-imagine-image-beta"` | O modelo Grok específico a ser usado para geração de imagens. Diferentes modelos podem oferecer qualidade, velocidade ou recursos variados. |
| `prompt` | STRING | Sim | N/A | O prompt de texto usado para gerar a imagem. Esta descrição orienta a IA sobre o que criar. Deve ter pelo menos 1 caractere. |
