...
O nó Grok Image gera uma ou mais imagens a partir de uma descrição de texto usando o modelo Grok AI. Ele envia seu prompt a um serviço externo e retorna as imagens geradas como tensores que podem ser usados em seu fluxo de trabalho.
...
| `prompt` | O prompt de texto usado para gerar a imagem. Esta descrição orienta a IA sobre o que criar. Deve ter pelo menos 1 caractere (texto composto apenas por espaços em branco não é aceito). | STRING | Sim | N/A |
