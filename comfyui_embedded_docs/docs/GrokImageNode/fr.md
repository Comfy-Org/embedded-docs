...
Le nœud Grok Image génère une ou plusieurs images à partir d’une description textuelle à l’aide du modèle d’IA Grok. Il envoie votre prompt à un service externe et renvoie les images générées sous forme de tenseurs pouvant être utilisés dans votre flux de travail.
...
|---------------|-------------|-----------------|
| `output` | L’image générée ou un lot d’images. Si `number_of_images` vaut 1, un seul tenseur d’image est renvoyé. S’il est supérieur à 1, un lot de tenseurs d’image est renvoyé. | IMAGE |
