...
| 参数 | 数据类型 | 是否必填 | 范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `width` | INT | 是 | 16 至 MAX_RESOLUTION（步长：16） | 输出潜空间图像的宽度（像素），默认值：1024 |
| `height` | INT | 是 | 16 至 MAX_RESOLUTION（步长：16） | 输出潜空间图像的高度（像素），默认值：1024 |
| `batch_size` | INT | 是 | 1 至 4096 | 每批生成的潜空间图像数量，默认值：1 |
