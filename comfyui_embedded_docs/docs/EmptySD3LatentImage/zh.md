...
| 参数 | 数据类型 | 必填 | 取值范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `width` | INT | 是 | 16 至 MAX_RESOLUTION（步长：16） | 输出潜空间图像的宽度（单位：像素，默认值：1024） |
| `height` | INT | 是 | 16 至 MAX_RESOLUTION（步长：16） | 输出潜空间图像的高度（单位：像素，默认值：1024） |
| `batch_size` | INT | 是 | 1 至 4096 | 单批次生成的潜空间图像数量（默认值：1） |
