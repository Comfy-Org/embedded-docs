...
| パラメータ | データ型 | 必須 | 範囲 | 説明 |
|-----------|-----------|----------|-------|-------------|
| `width` | INT | はい | 16 から MAX_RESOLUTION (ステップ: 16) | 出力潜在画像の幅（ピクセル単位）（デフォルト: 1024） |
| `height` | INT | はい | 16 から MAX_RESOLUTION (ステップ: 16) | 出力潜在画像の高さ（ピクセル単位）（デフォルト: 1024） |
| `batch_size` | INT | はい | 1 から 4096 | バッチで生成する潜在画像の数（デフォルト: 1） |
