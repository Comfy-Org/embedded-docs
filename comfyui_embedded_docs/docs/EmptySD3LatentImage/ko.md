...
|-----------|-----------|----------|-------|-------------|
| `width` | INT | 예 | 16 ~ MAX_RESOLUTION (단위: 16) | 출력 잠재 이미지의 픽셀 단위 너비 (기본값: 1024) |
| `height` | INT | 예 | 16 ~ MAX_RESOLUTION (단위: 16) | 출력 잠재 이미지의 픽셀 단위 높이 (기본값: 1024) |
| `batch_size` | INT | 예 | 1 ~ 4096 | 배치로 생성할 잠재 이미지의 개수 (기본값: 1) |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `LATENT` | LATENT | SD3 호환 차원을 가진 빈 샘플이 포함된 잠재 텐서입니다. 이 텐서는 16개 채널을 가지며, 입력 너비와 높이에 비해 8배 공간적으로 다운스케일링됩니다. |
