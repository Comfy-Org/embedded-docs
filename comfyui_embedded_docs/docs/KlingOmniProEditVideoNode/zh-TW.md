...
| `提示詞` | STRING | 是 | | 描述影片內容的文字提示詞。可包含正面與負面描述。 |
| `影片` | VIDEO | 是 | | 用於編輯的影片。輸出影片的長度將與輸入相同。 |
| `保留原始音訊` | BOOLEAN | 是 | | 決定輸出影片是否保留輸入影片的原始音訊（預設值：True）。 |
| `參考圖片` | IMAGE | 否 | | 最多 4 張額外的參考圖片。 |
| `解析度` | COMBO | 否 | `"1080p"`<br>`"720p"` | 輸出影片的解析度（預設值：`"1080p"`）。 |
