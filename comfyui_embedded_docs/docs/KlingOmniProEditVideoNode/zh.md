...
| 参数 | 数据类型 | 是否必填 | 范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `model_name` | COMBO | 是 | `"kling-v3-omni"`<br>`"kling-video-o1"` | 用于视频编辑的 AI 模型（默认值：`"kling-v3-omni"`）。 |
| `prompt` | STRING | 是 | | 描述视频内容的文本提示词。可以包含正面和负面描述。 |
| `video` | VIDEO | 是 | | 用于编辑的视频。输出视频长度将与之相同。 |
| `keep_original_sound` | BOOLEAN | 是 | | 决定输出中是否保留输入视频的原始音频（默认值：True）。 |
