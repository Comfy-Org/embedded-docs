...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `模型` | COMBO | 是 | `viduq1` | 模型名稱 |
| `提示詞` | STRING | 是 | - | 用於影片生成的文字描述 |
| `持續時間` | INT | 否 | 5-5 | 輸出影片的持續時間（秒）（預設值：5） |
| `種子值` | INT | 否 | 0-2147483647 | 影片生成的隨機種子（0 表示隨機）（預設值：0） |
| `長寬比` | COMBO | 否 | `16:9`<br>`9:16`<br>`1:1` | 輸出影片的長寬比 |
| `解析度` | COMBO | 否 | `1080p` | 支援的解析度可能因模型和持續時間而異 |
| `移動幅度` | COMBO | 否 | `auto`<br>`small`<br>`medium`<br>`large` | 畫面中物體的移動幅度 |
