...
|-----------|-----------|----------|-------|-------------|
| `모델` | COMBO | 예 | `viduq1` | 모델 이름 |
| `프롬프트` | STRING | 예 | - | 비디오 생성을 위한 텍스트 설명 |
| `지속 시간` | INT | 아니요 | 5-5 | 출력 비디오의 길이(초) (기본값: 5) |
| `시드` | INT | 아니요 | 0-2147483647 | 비디오 생성을 위한 시드 (0은 무작위) (기본값: 0) |
| `화면비` | COMBO | 아니요 | `16:9`<br>`9:16`<br>`1:1` | 출력 비디오의 화면 비율 |
| `해상도` | COMBO | 아니요 | `1080p` | 지원되는 값은 모델 및 지속 시간에 따라 다를 수 있습니다 |
| `움직임 강도` | COMBO | 아니요 | `auto`<br>`small`<br>`medium`<br>`large` | 프레임 내 객체의 움직임 진폭 |
