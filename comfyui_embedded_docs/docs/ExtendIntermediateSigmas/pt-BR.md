...
| `steps` | INT | Sim | 1 a 100 | Número de passos intermediários a inserir entre sigmas existentes (padrão: 2) |
| `start_at_sigma` | FLOAT | Sim | -1.0 a 20000.0 | Limite superior sigma para extensão — estende apenas sigmas abaixo deste valor (padrão: -1.0, que significa infinito) |
| `end_at_sigma` | FLOAT | Sim | 0.0 a 20000.0 | Limite inferior sigma para extensão — estende apenas sigmas acima deste valor (padrão: 12.0) |
| `spacing` | COMBO | Sim | `"linear"`<br>`"cosseno"`<br>`"seno"` | O método de interpolação para espaçar os valores sigma intermediários (padrão: "linear") |
