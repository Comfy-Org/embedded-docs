...
| `steps` | INT | Sí | 1 a 100 | Número de pasos intermedios a insertar entre sigmas existentes (predeterminado: 2) |
| `start_at_sigma` | FLOAT | Sí | -1.0 a 20000.0 | Límite sigma superior para la extensión: solo extiende sigmas por debajo de este valor (predeterminado: -1.0, que significa infinito) |
| `end_at_sigma` | FLOAT | Sí | 0.0 a 20000.0 | Límite sigma inferior para la extensión: solo extiende sigmas por encima de este valor (predeterminado: 12.0) |
| `spacing` | COMBO | Sí | `"linear"`<br>`"cosine"`<br>`"sine"` | El método de interpolación para espaciar los valores sigma intermedios (predeterminado: "linear") |
