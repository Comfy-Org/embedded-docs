...
| `start_at_sigma` | FLOAT | 예 | -1.0 ~ 20000.0 | 확장을 위한 상한 시그마 경계 - 이 값 미만의 시그마만 확장합니다 (기본값: -1.0, 무한대를 의미함) |
| `end_at_sigma` | FLOAT | 예 | 0.0 ~ 20000.0 | 확장을 위한 하한 시그마 경계 - 이 값 초과의 시그마만 확장합니다 (기본값: 12.0) |
| `spacing` | COMBO | 예 | "linear"<br>"cosine"<br>"sine" | 중간 시그마 값의 간격을 지정하는 보간 방법 |
...
| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `sigmas` | SIGMAS | 추가적인 중간 값이 삽입된 확장된 시그마 시퀀스 |