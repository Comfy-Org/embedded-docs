...
| `스텝 수` | INT | 예 | 1 ~ 100 | 기존 시그마 사이에 삽입할 중간 단계 수 (기본값: 2) |
| `시그마 시작점` | FLOAT | 예 | -1.0 ~ 20000.0 | 확장을 위한 상위 시그마 경계 - 이 값보다 낮은 시그마만 확장합니다 (기본값: -1.0, 이는 무한대를 의미) |
| `시그마 종료점` | FLOAT | 예 | 0.0 ~ 20000.0 | 확장을 위한 하위 시그마 경계 - 이 값보다 높은 시그마만 확장합니다 (기본값: 12.0) |
| `간격 분포 방식` | COMBO | 예 | `"linear"`<br>`"cosine"`<br>`"sine"` | 중간 시그마 값의 간격을 위한 보간 방법 (기본값: "linear") |
...
| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `시그마 배열` | SIGMAS | 추가 중간 값이 삽입된 확장된 시그마 시퀀스 |
