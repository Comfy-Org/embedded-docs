...
| `steps` | Number of intermediate steps to insert between existing sigmas; with N steps, N-1 intermediate sigma values are inserted between each eligible pair (default: 2) | INT | Yes | 1 to 100 |
...
| `spacing` | The interpolation method for spacing the intermediate sigma values: "linear" distributes them evenly, "cosine" and "sine" apply curved spacing (default: "linear") | COMBO | Yes | `"linear"`<br>`"cosine"`<br>`"sine"` |
