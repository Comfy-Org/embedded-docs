...
| `steps` | INT | Evet | 1 ila 100 | Mevcut sigmalar arasına eklenecek ara adım sayısı (varsayılan: 2) |
| `start_at_sigma` | FLOAT | Evet | -1,0 ila 20000,0 | Uzatma için üst sigma sınırı - yalnızca bu değerin altındaki sigmaları genişletir (varsayılan: -1,0, yani sonsuz) |
| `end_at_sigma` | FLOAT | Evet | 0,0 ila 20000,0 | Uzatma için alt sigma sınırı - yalnızca bu değerin üzerindeki sigmaları genişletir (varsayılan: 12,0) |
| `spacing` | COMBO | Evet | `"linear"`<br>`"cosine"`<br>`"sine"` | Ara sigma değerlerini aralıklandırmak için enterpolasyon yöntemi (varsayılan: "linear") |
...
|-----------|-----------|----------|
| `sigmas` | SIGMAS | Ek ara değerler eklenmiş genişletilmiş sigma dizisi |
