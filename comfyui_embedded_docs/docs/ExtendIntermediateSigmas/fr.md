...
| `étapes` | INT | Oui | 1 à 100 | Nombre d'étapes intermédiaires à insérer entre les sigmas existants (par défaut : 2) |
| `commencer_à_sigma` | FLOAT | Oui | -1,0 à 20000,0 | Limite sigma supérieure pour l'extension - n'étend que les sigmas inférieurs à cette valeur (par défaut : -1,0, ce qui signifie l'infini) |
| `finir_à_sigma` | FLOAT | Oui | 0,0 à 20000,0 | Limite sigma inférieure pour l'extension - n'étend que les sigmas supérieurs à cette valeur (par défaut : 12,0) |
| `espacement` | COMBO | Oui | `"linear"`<br>`"cosine"`<br>`"sine"` | La méthode d'interpolation pour espacer les valeurs sigma intermédiaires (par défaut : "linear") |
