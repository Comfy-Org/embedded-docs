...
| `steps` | INT | Oui | 1-100 | Nombre d'étapes intermédiaires à insérer entre les sigmas existants (par défaut : 2) |
| `start_at_sigma` | FLOAT | Oui | -1.0 à 20000.0 | Limite sigma supérieure pour l'extension - n'étend que les sigmas en dessous de cette valeur (par défaut : -1.0, ce qui signifie l'infini) |
| `end_at_sigma` | FLOAT | Oui | 0.0 à 20000.0 | Limite sigma inférieure pour l'extension - n'étend que les sigmas au-dessus de cette valeur (par défaut : 12.0) |
| `spacing` | COMBO | Oui | "linear"<br>"cosine"<br>"sine" | La méthode d'interpolation pour l'espacement des valeurs sigma intermédiaires |
