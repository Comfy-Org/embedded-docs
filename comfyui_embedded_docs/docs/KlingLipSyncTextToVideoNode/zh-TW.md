...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `video` | VIDEO | 是 | - | 用於唇形同步的輸入影片檔案 |
| `text` | STRING | 是 | - | 唇形同步影片生成的文字內容。當模式為 text2video 時為必要。最大長度為 120 個字元。 |
| `voice` | COMBO | 否 | "Melody"<br>"Bella"<br>"Aria"<br>"Ethan"<br>"Ryan"<br>"Dorothy"<br>"Nathan"<br>"Lily"<br>"Aaron"<br>"Emma"<br>"Grace"<br>"Henry"<br>"Isabella"<br>"James"<br>"Katherine"<br>"Liam"<br>"Mia"<br>"Noah"<br>"Olivia"<br>"Sophia" | 唇形同步音訊的語音選擇（預設值："Melody"） |
| `voice_speed` | FLOAT | 否 | 0.8-2.0 | 語速。有效範圍：0.8~2.0，精確至小數點後一位。（預設值：1） |
