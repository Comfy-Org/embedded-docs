...
PrimitiveFloat 노드는 워크플로우에서 사용할 수 있는 부동 소수점(float) 숫자 값을 생성합니다. 단일 숫자 입력을 받아 동일한 값을 출력하므로, ComfyUI 파이프라인의 서로 다른 노드 간에 float 값을 정의하고 전달할 수 있습니다.

## 입력

| 매개변수 | 설명 | 데이터 타입 | 필수 | 범위 |
| --- | --- | --- | --- | --- |
| `value` | 출력할 부동 소수점 숫자 값 (기본값: 0.0) | FLOAT | 예 | -sys.maxsize to sys.maxsize (step: 0.1) |

## 출력

| 출력 이름 | 설명 | 데이터 타입 |
| --- | --- | --- |
| `output` | 입력된 부동 소수점 숫자 값 | FLOAT |
