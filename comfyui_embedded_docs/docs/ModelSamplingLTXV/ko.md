...
| `max_shift` | FLOAT | 예 | 0.0 ~ 100.0 | 선형 보간 계산에 사용되는 최대 시프트 값 (기본값: 2.05) |
| `base_shift` | FLOAT | 예 | 0.0 ~ 100.0 | 선형 보간 계산에 사용되는 기본 시프트 값 (기본값: 0.95) |
| `latent` | LATENT | 아니요 | - | 시프트 계산을 위한 토큰 수를 결정하는 데 사용되는 선택적 잠재 변수 입력입니다. 제공되지 않으면 기본 토큰 수 4096이 사용됩니다 |
