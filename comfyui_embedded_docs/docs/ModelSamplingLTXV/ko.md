...
| `최대 시프트` | FLOAT | 예 | 0.0 ~ 100.0 | 선형 보간 계산에 사용되는 최대 시프트 값 (기본값: 2.05) |
| `기반 시프트` | FLOAT | 예 | 0.0 ~ 100.0 | 선형 보간 계산에 사용되는 기본 시프트 값 (기본값: 0.95) |
| `잠재 비디오` | LATENT | 아니요 | - | 시프트 계산을 위한 토큰 수를 결정하는 데 사용되는 선택적 잠재 변수 입력입니다. 제공되지 않으면 기본 토큰 수 4096이 사용됩니다 |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-----------|-------------|-------------|
| `모델` | MODEL | 샘플링 매개변수가 적용된 수정된 모델 |
