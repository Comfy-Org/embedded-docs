...
| `max_shift` | FLOAT | 아니오 | 0.0 ~ 100.0 | 계산에 사용되는 최대 시프트 값 (기본값: 2.05) |
| `base_shift` | FLOAT | 아니오 | 0.0 ~ 100.0 | 계산에 사용되는 기본 시프트 값 (기본값: 0.95) |
| `latent` | LATENT | 아니오 | - | 시프트 계산을 위한 토큰 수 결정에 사용되는 선택적 잠재 변수 입력 |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `model` | MODEL | 적용된 샘플링 매개변수가 포함된 수정된 모델 |