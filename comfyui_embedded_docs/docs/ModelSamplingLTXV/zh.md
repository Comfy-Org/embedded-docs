...
| 参数 | 数据类型 | 是否必填 | 范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `model` | MODEL | 是 | - | 要应用采样参数的输入模型 |
| `max_shift` | FLOAT | 是 | 0.0 至 100.0 | 线性插值计算中使用的最大偏移值（默认值：2.05） |
| `base_shift` | FLOAT | 是 | 0.0 至 100.0 | 线性插值计算中使用的基础偏移值（默认值：0.95） |
