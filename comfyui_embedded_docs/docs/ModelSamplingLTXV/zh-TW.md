...
| `最大偏移` | FLOAT | 是 | 0.0 至 100.0 | 線性插值計算中使用的最大偏移值（預設值：2.05） |
| `基礎偏移` | FLOAT | 是 | 0.0 至 100.0 | 線性插值計算中使用的基礎偏移值（預設值：0.95） |
| `潛在空間` | LATENT | 否 | - | 可選的潛在空間輸入，用於決定偏移計算的 token 數量。若未提供，則使用預設的 4096 個 token 數量 |
