...
| `max_shift` | FLOAT | いいえ | 0.0 ～ 100.0 | 計算で使用される最大シフト値（デフォルト: 2.05） |
| `base_shift` | FLOAT | いいえ | 0.0 ～ 100.0 | 計算で使用される基本シフト値（デフォルト: 0.95） |
| `latent` | LATENT | いいえ | - | シフト計算のためのトークン数を決定するために使用されるオプションの潜在変数入力 |

## 出力

| 出力名 | データ型 | 説明 |
|-------------|-----------|-------------|
| `model` | MODEL | 適用されたサンプリングパラメータを持つ修正済みモデル |