...
| `longer_edge` | INT | 예 | 1 - 8192 | 긴 변의 목표 길이입니다. 짧은 변은 비례적으로 크기가 조정됩니다. (기본값: 1024) |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `image` | IMAGE | 크기가 조정된 이미지 또는 이미지 배치입니다. 출력은 입력과 동일한 수의 이미지를 가지며, 각 이미지의 긴 변이 지정된 `longer_edge` 길이와 일치합니다. |
