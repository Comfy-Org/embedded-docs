...
| パラメータ | データ型 | 必須 | 範囲 | 説明 |
|-----------|-----------|----------|-------|-------------|
| `clip_vision` | CLIP_VISION | はい | - | 画像特徴をエンコードするために使用されるCLIP Visionモデル |
| `init_image` | IMAGE | はい | - | 処理およびエンコードされる入力画像 |
| `vae` | VAE | はい | - | ピクセルを潜在空間にエンコードするために使用されるVAEモデル |
| `width` | INT | いいえ | 16 から MAX_RESOLUTION | 潜在表現の出力幅（デフォルト: 256、8で割り切れる必要があります） |
| `height` | INT | いいえ | 16 から MAX_RESOLUTION | 潜在表現の出力高さ（デフォルト: 256、8で割り切れる必要があります） |
| `batch_size` | INT | いいえ | 1 から 4096 | バッチで生成するサンプル数（デフォルト: 1） |
| `elevation` | FLOAT | いいえ | -180.0 から 180.0 | カメラの仰角（度単位、デフォルト: 0.0） |
| `azimuth` | FLOAT | いいえ | -180.0 から 180.0 | カメラの方位角（度単位、デフォルト: 0.0） |
...
| 出力名 | データ型 | 説明 |
|-------------|-----------|-------------|
| `positive` | CONDITIONING | 画像特徴とカメラ埋め込みを組み合わせたポジティブなコンディショニングデータ |
| `negative` | CONDITIONING | ゼロ初期化された特徴を持つネガティブなコンディショニングデータ |
