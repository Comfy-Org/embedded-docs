...
| `width` | Largura de saída para a representação latente (padrão: 256, passo: 8) | INT | Sim | 16 a MAX_RESOLUTION |
| `height` | Altura de saída para a representação latente (padrão: 256, passo: 8) | INT | Sim | 16 a MAX_RESOLUTION |
| `batch_size` | Número de amostras a gerar no lote (padrão: 1) | INT | Sim | 1 a 4096 |
| `elevation` | Ângulo de elevação da câmera em graus (padrão: 0.0, passo: 0.1) | FLOAT | Sim | -180.0 a 180.0 |
| `azimuth` | Ângulo de azimute da câmera em graus (padrão: 0.0, passo: 0.1) | FLOAT | Sim | -180.0 a 180.0 |
