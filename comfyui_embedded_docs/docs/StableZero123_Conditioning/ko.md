...
StableZero123_Conditioning 노드는 입력 이미지와 카메라 각도를 처리하여 3D 모델 생성을 위한 컨디셔닝 데이터와 잠재 표현을 생성합니다. CLIP 비전 모델을 사용하여 이미지 특징을 인코딩하고, 고도(elevation) 및 방위각(azimuth)을 기반으로 한 카메라 임베딩 정보와 결합한 다음, 다운스트림 3D 생성 작업을 위한 긍정 및 부정 컨디셔닝과 잠재 표현을 생성합니다.
...
**참고:** `width` 및 `height` 매개변수는 노드가 자동으로 8로 나누어 잠재 표현 차원을 생성하므로 8로 나누어 떨어져야 합니다.
