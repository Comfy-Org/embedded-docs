...
| `초기 이미지` | IMAGE | 예 | - | 처리 및 인코딩할 입력 이미지 |
| `vae` | VAE | 예 | - | 픽셀을 잠재 공간으로 인코딩하는 데 사용되는 VAE 모델 |
| `너비` | INT | 예 | 16 ~ MAX_RESOLUTION | 잠재 표현의 출력 너비 (기본값: 256, 8로 나누어 떨어져야 함) |
| `높이` | INT | 예 | 16 ~ MAX_RESOLUTION | 잠재 표현의 출력 높이 (기본값: 256, 8로 나누어 떨어져야 함) |
| `배치 크기` | INT | 예 | 1 ~ 4096 | 배치에서 생성할 샘플 수 (기본값: 1) |
| `고도` | FLOAT | 예 | -180.0 ~ 180.0 | 카메라 고도 각도 (도 단위, 기본값: 0.0) |
| `방위각` | FLOAT | 예 | -180.0 ~ 180.0 | 카메라 방위각 (도 단위, 기본값: 0.0) |
...
| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `부정 조건` | CONDITIONING | 이미지 특징과 카메라 임베딩을 결합한 양성 컨디셔닝 데이터 |
| `잠재 데이터` | CONDITIONING | 0으로 초기화된 특징을 가진 음성 컨디셔닝 데이터 |
