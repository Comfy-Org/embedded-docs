...
| 参数名 | 数据类型 | 必填 | 取值范围 | 描述 |
|--------|-----------|------|----------|------|
| `clip_vision` | CLIP_VISION | 是 | - | 用于编码图像特征的CLIP视觉模型 |
| `init_image` | IMAGE | 是 | - | 待处理和编码的输入图像 |
| `vae` | VAE | 是 | - | 用于将像素编码到潜在空间的VAE模型 |
| `width` | INT | 否 | 16至MAX_RESOLUTION | 潜在表示的输出宽度（默认：256，必须能被8整除） |
| `height` | INT | 否 | 16至MAX_RESOLUTION | 潜在表示的输出高度（默认：256，必须能被8整除） |
| `batch_size` | INT | 否 | 1至4096 | 批次中生成的样本数量（默认：1） |
| `elevation` | FLOAT | 否 | -180.0至180.0 | 相机仰角（单位：度，默认：0.0） |
| `azimuth` | FLOAT | 否 | -180.0至180.0 | 相机方位角（单位：度，默认：0.0） |
...
| `negative` | CONDITIONING | 零初始化特征的负向条件数据 |
