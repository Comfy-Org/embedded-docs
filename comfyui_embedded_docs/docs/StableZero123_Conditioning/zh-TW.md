...
| `初始影像` | IMAGE | 是 | - | 待處理和編碼的輸入圖像 |
| `vae` | VAE | 是 | - | 用於將像素編碼到潛在空間的 VAE 模型 |
| `寬度` | INT | 是 | 16 至 MAX_RESOLUTION | 潛在表示的輸出寬度（預設：256，必須能被 8 整除） |
| `高度` | INT | 是 | 16 至 MAX_RESOLUTION | 潛在表示的輸出高度（預設：256，必須能被 8 整除） |
| `批次大小` | INT | 是 | 1 至 4096 | 批次中生成的樣本數量（預設：1） |
| `仰角` | FLOAT | 是 | -180.0 至 180.0 | 相機仰角（度）（預設：0.0） |
| `方位角` | FLOAT | 是 | -180.0 至 180.0 | 相機方位角（度）（預設：0.0） |
...
| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `負向` | CONDITIONING | 結合圖像特徵和相機嵌入的正向條件數據 |
| `潛在空間` | CONDITIONING | 使用零初始化特徵的負向條件數據 |
