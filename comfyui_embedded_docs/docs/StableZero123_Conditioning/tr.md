...
|-----------|-----------|----------|-------|-------------|
| `clip_vision` | CLIP_VISION | Evet | - | Görüntü özelliklerini kodlamak için kullanılan CLIP görüş modeli |
| `init_image` | IMAGE | Evet | - | İşlenecek ve kodlanacak girdi görüntüsü |
| `vae` | VAE | Evet | - | Pikselleri gizli uzaya kodlamak için kullanılan VAE modeli |
| `width` | INT | Evet | 16 - MAX_RESOLUTION | Gizli temsil için çıktı genişliği (varsayılan: 256, 8'e bölünebilir olmalıdır) |
| `height` | INT | Evet | 16 - MAX_RESOLUTION | Gizli temsil için çıktı yüksekliği (varsayılan: 256, 8'e bölünebilir olmalıdır) |
| `batch_size` | INT | Evet | 1 - 4096 | Toplu işlemde oluşturulacak örnek sayısı (varsayılan: 1) |
| `elevation` | FLOAT | Evet | -180.0 - 180.0 | Derece cinsinden kamera yükseklik açısı (varsayılan: 0.0) |
| `azimuth` | FLOAT | Evet | -180.0 - 180.0 | Derece cinsinden kamera azimut açısı (varsayılan: 0.0) |
...
|-----------|-----------|-------------|
| `positive` | CONDITIONING | Görüntü özellikleri ve kamera gömmelerini birleştiren pozitif koşullandırma verileri |
| `negative` | CONDITIONING | Sıfır başlatmalı özelliklere sahip negatif koşullandırma verileri |
