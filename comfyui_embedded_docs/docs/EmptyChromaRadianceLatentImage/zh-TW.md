...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `寬度` | INT | 是 | 16 至 MAX_RESOLUTION | 潛在影像的寬度（像素為單位，預設值：1024，必須能被 16 整除） |
| `高度` | INT | 是 | 16 至 MAX_RESOLUTION | 潛在影像的高度（像素為單位，預設值：1024，必須能被 16 整除） |
| `批次大小` | INT | 否 | 1 至 4096 | 單一批次中要產生的潛在影像數量（預設值：1） |
