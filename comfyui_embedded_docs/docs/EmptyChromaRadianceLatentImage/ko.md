...
|-----------|-----------|----------|-------|-------------|
| `width` | INT | 예 | 16 ~ MAX_RESOLUTION | 잠재 이미지의 픽셀 단위 너비 (기본값: 1024, 16으로 나누어져야 함) |
| `height` | INT | 예 | 16 ~ MAX_RESOLUTION | 잠재 이미지의 픽셀 단위 높이 (기본값: 1024, 16으로 나누어져야 함) |
| `batch_size` | INT | 아니요 | 1 ~ 4096 | 배치로 생성할 잠재 이미지의 수 (기본값: 1) |
