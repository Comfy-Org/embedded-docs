...
|------------------|-----------------|-------------|
| `IMAGE` | IMAGE | La ou les images modifiées renvoyées par l'API Grok. Si une seule image est générée, elle est renvoyée directement. Si plusieurs images sont générées, elles sont concaténées en un seul tenseur de lot. |
