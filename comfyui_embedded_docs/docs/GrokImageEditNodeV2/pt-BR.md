...
| `model` | MODEL | Sim | Ver Descrição | O modelo de imagem Grok a ser usado. Este parâmetro possui várias sub-opções que aparecem após selecionar um modelo. Modelos disponíveis: `grok-imagine-image-quality`, `grok-imagine-image-pro`, `grok-imagine-image`. Cada modelo possui capacidades diferentes (veja a nota abaixo). |
...
- O parâmetro `model` é uma combinação dinâmica que inclui sub-opções para `resolution`, `number_of_images`, `images` e `aspect_ratio`.
...
- **Proporção de aspecto personalizada** (sub-opção `aspect_ratio`) só é permitida quando múltiplas imagens estão conectadas à entrada de imagem. Se apenas uma imagem for fornecida, a proporção de aspecto deve ser definida como "auto".
