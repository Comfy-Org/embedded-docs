...
| 매개변수 | 설명 | 데이터 타입 | 필수 | 범위 |
|-----------|-------------|-----------|----------|-------|
| `images` | 편집할 참조 이미지입니다. | IMAGE | 예 | 1 image |
| `resolution` | 편집된 이미지의 출력 해상도입니다. | STRING | 예 | "1K"<br>"2K" |
| `number_of_images` | 생성할 편집 이미지 수입니다. (기본값: 1) | INT | 예 | 1 ~ 10 |
...
| 매개변수 | 설명 | 데이터 타입 | 필수 | 범위 |
|-----------|-------------|-----------|----------|-------|
| `images` | 확장 가능한 슬롯: 편집할 참조 이미지를 1개 이상 연결합니다. `image_1`, `image_2`, `image_3`과 같은 번호가 매겨진 슬롯을 추가할 수 있습니다. 최대 이미지 수는 선택한 모델에 따라 다릅니다(위의 모델 섹션 참조). | IMAGE | 예 | 1 ~ 3 개 이미지, depending on the model |
