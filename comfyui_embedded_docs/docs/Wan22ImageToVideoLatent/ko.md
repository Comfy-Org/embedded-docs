...
| `width` | INT | 예 | 32 ~ MAX_RESOLUTION | 출력 비디오의 픽셀 단위 너비 (기본값: 1280, 단계: 32) |
| `height` | INT | 예 | 32 ~ MAX_RESOLUTION | 출력 비디오의 픽셀 단위 높이 (기본값: 704, 단계: 32) |
| `length` | INT | 예 | 1 ~ MAX_RESOLUTION | 비디오 시퀀스의 프레임 수 (기본값: 49, 단계: 4) |
| `batch_size` | INT | 예 | 1 ~ 4096 | 생성할 배치 수 (기본값: 1) |
| `start_image` | IMAGE | 아니요 | - | 비디오 잠재 표현에 인코딩할 선택적 시작 이미지 시퀀스 |
