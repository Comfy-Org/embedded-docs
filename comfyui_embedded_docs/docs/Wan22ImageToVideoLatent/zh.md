...
| `width` | INT | 是 | 32 至 MAX_RESOLUTION | 输出视频的像素宽度（默认值：1280，步长：32） |
| `height` | INT | 是 | 32 至 MAX_RESOLUTION | 输出视频的像素高度（默认值：704，步长：32） |
| `length` | INT | 是 | 1 至 MAX_RESOLUTION | 视频序列中的帧数（默认值：49，步长：4） |
| `batch_size` | INT | 是 | 1 至 4096 | 生成的批次数量（默认值：1） |
| `start_image` | IMAGE | 否 | - | 可选的起始图像序列，用于编码到视频潜在空间中 |
