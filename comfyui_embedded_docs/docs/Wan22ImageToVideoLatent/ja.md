...
| `width` | INT | はい | 32 ～ MAX_RESOLUTION | 出力ビデオの幅（ピクセル単位）（デフォルト：1280、ステップ：32） |
| `height` | INT | はい | 32 ～ MAX_RESOLUTION | 出力ビデオの高さ（ピクセル単位）（デフォルト：704、ステップ：32） |
| `length` | INT | はい | 1 ～ MAX_RESOLUTION | ビデオシーケンスのフレーム数（デフォルト：49、ステップ：4） |
| `batch_size` | INT | はい | 1 ～ 4096 | 生成するバッチ数（デフォルト：1） |
| `start_image` | IMAGE | いいえ | - | ビデオ潜在表現にエンコードするオプションの開始画像シーケンス |
