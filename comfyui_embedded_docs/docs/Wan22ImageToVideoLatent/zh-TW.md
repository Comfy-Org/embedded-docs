...
| `寬度` | INT | 是 | 32 至 MAX_RESOLUTION | 輸出影片的像素寬度（預設值：1280，步長：32） |
| `高度` | INT | 是 | 32 至 MAX_RESOLUTION | 輸出影片的像素高度（預設值：704，步長：32） |
| `長度` | INT | 是 | 1 至 MAX_RESOLUTION | 影片序列中的影格數量（預設值：49，步長：4） |
| `批次大小` | INT | 是 | 1 至 4096 | 要生成的批次數量（預設值：1） |
| `起始圖像` | IMAGE | 否 | - | 可選的起始影像序列，用於編碼到影片潛在空間中 |
