...
| `width` | INT | 否 | 32 至 MAX_RESOLUTION | 輸出影片的寬度（像素）（預設值：1280，間隔：32） |
| `height` | INT | 否 | 32 至 MAX_RESOLUTION | 輸出影片的高度（像素）（預設值：704，間隔：32） |
| `length` | INT | 否 | 1 至 MAX_RESOLUTION | 影片序列中的幀數（預設值：49，間隔：4） |
