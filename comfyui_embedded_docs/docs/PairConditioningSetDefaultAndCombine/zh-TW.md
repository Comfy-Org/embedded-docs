...
此節點設定預設的條件設定值，並將其與輸入的條件設定資料結合。它接收正向與負向的條件設定輸入及其對應的預設值，然後透過 ComfyUI 的掛鉤系統進行處理，以產生包含預設值的最終條件設定輸出。
...
| `positive_DEFAULT` | CONDITIONING | 是 | - | 用作備用值的預設正向條件設定值 |
| `negative_DEFAULT` | CONDITIONING | 是 | - | 用作備用值的預設負向條件設定值 |
