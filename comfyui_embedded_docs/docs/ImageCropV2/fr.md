...
| `crop_region` | BOUNDINGBOX | Oui | N/A | Définit la zone rectangulaire à extraire de l'image. Elle est spécifiée par `x` (départ horizontal), `y` (départ vertical), `width` (largeur) et `height` (hauteur). Si la région définie dépasse les bords de l'image, elle sera automatiquement ajustée pour s'adapter aux dimensions de l'image. |
...
|---------------|-----------------|-------------|
| `image` | IMAGE | La section recadrée de l'image d'entrée d'origine. |
