...
此節點從 OpenAI 模型生成文字回應。它會將您的文字提示，以及可選的圖片或檔案，發送到 OpenAI 模型，並返回生成的文字回應。

## 輸入

| 參數 | 說明 | 資料型別 | 必填 | 範圍 |
|-----------|-------------|-----------|----------|-------|
| `prompt` | 輸入給模型的文字，用於生成回應（預設：空） | STRING | 是 | - |
| `persist_context` | 此參數已棄用，沒有任何效果（預設：False） | BOOLEAN | 是 | - |
...
| `images` | 可選圖片，作為模型的上下文。若要包含多張圖片，您可以使用 Batch Images 節點 | IMAGE | 否 | - |
| `files` | 可選檔案，作為模型的上下文。接受來自 OpenAI Chat Input Files 節點的輸入 | OPENAI_INPUT_FILES | 否 | - |
| `advanced_options` | 可選的模型組態。接受來自 OpenAI Chat Advanced Options 節點的輸入 | OPENAI_CHAT_CONFIG | 否 | - |

## 輸出

| 輸出名 | 說明 | 資料型別 |
|-------------|-------------|-----------|
| `output_text` | 由 OpenAI 模型生成的文字回應 | STRING |
