...
| `model` | The model used to generate the response (default: `gpt-5`) | COMBO | Yes | `gpt-6-astra`<br>`gpt-6-sol`<br>`gpt-6-luna`<br>`gpt-5.6-sol`<br>`gpt-5.6-terra`<br>`gpt-5.6-luna`<br>`gpt-5.5-pro`<br>`gpt-5.5`<br>`gpt-5`<br>`gpt-5-mini`<br>`gpt-5-nano`<br>`gpt-4.1`<br>`gpt-4.1-mini`<br>`gpt-4.1-nano`<br>`o4-mini`<br>`o3`<br>`o1-pro`<br>`o1` |
...
Note: When an `advanced_options` configuration that sets a reasoning effort is connected, the selected `model` must support that effort value. For example, the gpt-4.1 family of models does not support any reasoning effort, `gpt-6-sol` and `gpt-6-luna` support none, low, medium, high, xhigh and max, `gpt-5.5` supports none, low, medium, high and xhigh, and `gpt-5.5-pro` supports medium, high and xhigh. If the reasoning effort is not supported by the selected model, the node raises an error.
