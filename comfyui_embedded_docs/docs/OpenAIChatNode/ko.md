...
|-----------|-----------|----------|-------|-------------|
| `prompt` | STRING | 예 | - | 모델에 전달할 텍스트 입력으로, 응답을 생성하는 데 사용됩니다 (기본값: 비어 있음) |
| `persist_context` | BOOLEAN | 예 | - | 이 매개변수는 더 이상 사용되지 않으며 효과가 없습니다 (기본값: False) |
| `model` | COMBO | 예 | 여러 OpenAI 모델 사용 가능 | 응답을 생성하는 데 사용되는 모델 |
| `images` | IMAGE | 아니요 | - | 모델의 컨텍스트로 사용할 선택적 이미지입니다. 여러 이미지를 포함하려면 이미지 배치 노드를 사용할 수 있습니다 |
| `files` | OPENAI_INPUT_FILES | 아니요 | - | 모델의 컨텍스트로 사용할 선택적 파일입니다. OpenAI 채팅 입력 파일 노드의 입력을 허용합니다 |
| `advanced_options` | OPENAI_CHAT_CONFIG | 아니요 | - | 모델의 선택적 구성입니다. OpenAI 채팅 고급 옵션 노드의 입력을 허용합니다 |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `output_text` | STRING | OpenAI 모델이 생성한 텍스트 응답 |
