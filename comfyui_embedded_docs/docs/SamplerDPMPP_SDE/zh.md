...
SamplerDPMPP_SDE 创建一个用于采样过程的 DPM++ SDE（随机微分方程）采样器。此采样器提供了一种随机采样方法，具有可配置的噪声参数和设备选择功能。它返回一个可在采样流程中使用的采样器对象。
...
| `设备` | 选择执行噪声计算的设备。设置为 "cpu" 时，创建 `dpmpp_sde` 采样器；设置为 "gpu" 时，创建 `dpmpp_sde_gpu` 采样器（默认值："gpu"） | COMBO | 是 | "gpu"<br>"cpu" |

注意：所有输入均被标记为高级参数。`noise_device` 的选择会改变所创建的采样器变体："cpu" 对应 `dpmpp_sde`，"gpu" 对应 `dpmpp_sde_gpu`。

## 输出

| 输出名称 | 描述 | 数据类型 |
| --- | --- | --- |
| `sampler` | 返回一个已配置的 DPM++ SDE 采样器对象，供采样流程使用 | SAMPLER |
