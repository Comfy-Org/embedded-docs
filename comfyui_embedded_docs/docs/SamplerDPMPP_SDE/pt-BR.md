...
| `noise_device` | Seleciona o dispositivo onde os cálculos de ruído são realizados (padrão: "gpu"). Quando definido como "cpu", o amostrador padrão `dpmpp_sde` é usado; quando definido como "gpu", o amostrador `dpmpp_sde_gpu` é usado. | COMBO | Sim | "gpu"<br>"cpu" |
