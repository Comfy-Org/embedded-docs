...
| `s_noise` | Controla a quantidade de ruído adicionado durante a amostragem (padrão: 1.0) | FLOAT | Sim | 0.0 - 100.0 |
...
Observação: Todas as entradas são marcadas como parâmetros avançados. A seleção de `noise_device` altera qual variante de amostrador é criada: "cpu" mapeia para `dpmpp_sde` e "gpu" mapeia para `dpmpp_sde_gpu`.
