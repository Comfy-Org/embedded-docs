...
此節點將 ControlNet 引導應用於 Stable Diffusion 3 的條件輸入。它接收正向和負向條件輸入，以及 ControlNet 模型和圖像，然後透過可調節的強度和時機參數來施加控制引導，以影響生成過程。
...
|-----------|-----------|----------|-------|-------------|
| `正向` | CONDITIONING | 是 | - | 要應用 ControlNet 引導的正向條件輸入 |
| `負向` | CONDITIONING | 是 | - | 要應用 ControlNet 引導的負向條件輸入 |
...
| 輸出名稱 | 資料類型 | 描述 |
|-------------|-----------|-------------|
| `負向` | CONDITIONING | 已應用 ControlNet 引導的修改後正向條件輸入 |
| `負向` | CONDITIONING | 已應用 ControlNet 引導的修改後負向條件輸入 |