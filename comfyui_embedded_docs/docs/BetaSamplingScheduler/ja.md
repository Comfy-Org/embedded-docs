...
| パラメータ | データ型 | 必須 | 範囲 | 説明 |
|-----------|-----------|----------|-------|-------------|
| `model` | MODEL | はい | - | サンプリングに使用するモデルです。モデルサンプリングオブジェクトを提供します |
| `steps` | INT | はい | 1 ～ 10000 | シグマを生成するサンプリングステップ数です（デフォルト：20） |
| `alpha` | FLOAT | はい | 0.0 ～ 50.0 | ベータスケジューラーのアルファパラメータで、スケジューリング曲線を制御します（デフォルト：0.6） |
| `beta` | FLOAT | はい | 0.0 ～ 50.0 | ベータスケジューラーのベータパラメータで、スケジューリング曲線を制御します（デフォルト：0.6） |
