...
|-----------|-----------|------------|---------|-------|-------------|
| `model` | MODEL | 必須 | - | - | サンプリングに使用されるモデル。モデルサンプリングオブジェクトを提供します |
| `steps` | INT | 必須 | 20 | 1-10000 | シグマを生成するサンプリングステップ数 |
| `alpha` | FLOAT | 必須 | 0.6 | 0.0-50.0 | ベータスケジューラのアルファパラメータ。スケジューリング曲線を制御します |
| `beta` | FLOAT | 必須 | 0.6 | 0.0-50.0 | ベータスケジューラのベータパラメータ。スケジューリング曲線を制御します |
