...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `model` | STRING | 是 | `"seedance-1-5-pro-251215"`<br>`"seedance-1-0-pro-250528"`<br>`"seedance-1-0-lite-t2v-250428"`<br>`"seedance-1-0-pro-fast-251015"` | 用於生成的 ByteDance 模型（預設值：`"seedance-1-0-pro-fast-251015"`）。 |
| `prompt` | STRING | 是 | - | 用於生成影片的文字提示。 |
| `resolution` | STRING | 是 | `"480p"`<br>`"720p"`<br>`"1080p"` | 輸出影片的解析度。 |
| `aspect_ratio` | STRING | 是 | `"16:9"`<br>`"4:3"`<br>`"1:1"`<br>`"3:4"`<br>`"9:16"`<br>`"21:9"` | 輸出影片的長寬比。 |
| `duration` | INT | 是 | 3 到 12 | 輸出影片的長度，以秒為單位（預設值：5）。 |
| `seed` | INT | 否 | 0 到 2147483647 | 用於生成的種子值（預設值：0）。 |
| `camera_fixed` | BOOLEAN | 否 | - | 指定是否固定攝影機。平台會在您的提示中附加固定攝影機的指令，但不保證實際效果（預設值：False）。 |
