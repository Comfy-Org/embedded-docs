...
|-----------|---------------|-------------|-------|-------------|
| `model` | STRING | Sí | `"seedance-1-5-pro-251215"`<br>`"seedance-1-0-pro-250528"`<br>`"seedance-1-0-lite-t2v-250428"`<br>`"seedance-1-0-pro-fast-251015"` | El modelo de ByteDance a utilizar para la generación (valor predeterminado: `"seedance-1-0-pro-fast-251015"`). |
...
| `aspect_ratio` | STRING | Sí | `"16:9"`<br>`"4:3"`<br>`"1:1"`<br>`"3:4"`<br>`"9:16"`<br>`"21:9"` | La relación de aspecto del video de salida. |
| `duration` | INT | Sí | 3 a 12 | La duración del video de salida en segundos (valor predeterminado: 5). |
| `seed` | INT | No | 0 a 2147483647 | Semilla a utilizar para la generación (valor predeterminado: 0). |
| `camera_fixed` | BOOLEAN | No | - | Especifica si se debe fijar la cámara. La plataforma añade una instrucción para fijar la cámara a tu descripción, pero no garantiza el efecto real (valor predeterminado: False). |
| `watermark` | BOOLEAN | No | - | Indica si se debe añadir una marca de agua "generado por IA" al video (valor predeterminado: False). |
