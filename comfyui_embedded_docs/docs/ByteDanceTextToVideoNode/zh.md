...
|-----------|-----------|----------|-------|-------------|
| `model` | STRING | 是 | `"seedance-1-5-pro-251215"`<br>`"seedance-1-0-pro-250528"`<br>`"seedance-1-0-lite-t2v-250428"`<br>`"seedance-1-0-pro-fast-251015"` | 用于生成的字节跳动模型（默认值：`"seedance-1-0-pro-fast-251015"`）。 |
| `prompt` | STRING | 是 | - | 用于生成视频的文本提示。 |
| `resolution` | STRING | 是 | `"480p"`<br>`"720p"`<br>`"1080p"` | 输出视频的分辨率。 |
| `aspect_ratio` | STRING | 是 | `"16:9"`<br>`"4:3"`<br>`"1:1"`<br>`"3:4"`<br>`"9:16"`<br>`"21:9"` | 输出视频的宽高比。 |
| `duration` | INT | 是 | 3 到 12 | 输出视频的时长（秒）（默认值：5）。 |
| `seed` | INT | 否 | 0 到 2147483647 | 用于生成的随机种子（默认值：0）。 |
| `camera_fixed` | BOOLEAN | 否 | - | 指定是否固定相机。平台会在您的提示后附加一条固定相机的指令，但不保证实际效果（默认值：False）。 |
