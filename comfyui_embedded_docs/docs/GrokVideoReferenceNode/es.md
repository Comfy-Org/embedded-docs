...
| `modelo` | El modelo a utilizar para la generación de video. | COMBO | Sí | `"grok-imagine-video-1.5"`<br>`"grok-imagine-video"` |
...
| Parámetro | Descripción | Tipo de dato | Requerido | Rango |
|-----------|-------------|--------------|-----------|-------|
| `voice_1` | Referencia de voz predefinida opcional; consúltala en el prompt como @Audio1. La API solo admite estas voces predefinidas, no audio personalizado (predeterminado: none). | COMBO | No | Opciones de voz predefinidas (incluye `"none"`) |
| `voice_2` | Segunda referencia de voz opcional; @Audio2 en el prompt (predeterminado: none). | COMBO | No | Opciones de voz predefinidas (incluye `"none"`) |
| `voice_3` | Tercera referencia de voz opcional; @Audio3 en el prompt (predeterminado: none). | COMBO | No | Opciones de voz predefinidas (incluye `"none"`) |
...
| Parámetro | Descripción | Tipo de dato | Requerido | Rango |
|-----------|-------------|--------------|-----------|-------|
| `reference_images` | Ranura ampliable: conecta de 1 a 7 imágenes de referencia para guiar la generación del video. Con `grok-imagine-video-1.5`, consúltalas en el prompt como @Image1 ... @Image7, numeradas en orden de entrada; una entrada por lotes cuenta una vez por imagen. | IMAGE | Sí | 1 a 7 imágenes |

**Nota:** Los subparámetros mostrados dependen del `model` seleccionado; `grok-imagine-video-1.5` añade las entradas `voice_1`, `voice_2` y `voice_3`. Se requiere al menos una imagen de referencia y el total está limitado a 7 (una entrada por lotes cuenta una vez por imagen). Con `grok-imagine-video-1.5`, el prompt puede hacer referencia a las imágenes conectadas como `@Image1` ... `@Image7` y a las voces habilitadas como `@Audio1`, `@Audio2`, `@Audio3`; hacer referencia a una imagen no conectada o a una voz configurada como `none` provoca un error. La API solo admite voces predefinidas, no audio personalizado.
