...
| `seed` | 用于确定节点是否应重新运行的种子；无论种子如何，实际结果都是不确定的（默认值：0）。 | INT | 否 | 0 到 2147483647 |
...
| `resolution` | 输出视频的分辨率。 | COMBO | 是 | `"480p"`<br>`"720p"` |
| `aspect_ratio` | 输出视频的宽高比。 | COMBO | 是 | `"16:9"`<br>`"4:3"`<br>`"3:2"`<br>`"1:1"`<br>`"2:3"`<br>`"3:4"`<br>`"9:16"` |
| `duration` | 输出视频的时长（秒）（默认值：6）。 | INT | 是 | 2 到 10 |

### 参考输入

| 参数 | 描述 | 数据类型 | 是否必填 | 范围 |
|-----------|-------------|-----------|----------|-------|
| `reference_images` | 可增长槽位：连接 1 到 7 张参考图像以引导视频生成。使用 `grok-imagine-video-1.5` 时，在提示中按输入顺序将它们引用为 @Image1 ... @Image7；批量输入每张图像计数一次。 | IMAGE | 是 | 1 到 7 images |
