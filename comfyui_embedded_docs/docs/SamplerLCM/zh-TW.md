...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `s_noise` | FLOAT | 是 | 0.0 至 64.0 (步長: 0.01) | 第一步的每步雜訊乘數。值為 1.0 時與模型的訓練雜訊規模一致。(預設值: 1.0) |
| `s_noise_end` | FLOAT | 是 | 0.0 至 64.0 (步長: 0.01) | 最後一步的每步雜訊乘數。設定為與 `s_noise` 相同可獲得恆定的雜訊排程。(預設值: 1.0) |
| `noise_clip_std` | FLOAT | 是 | 0.0 至 10.0 (步長: 0.01) | 將每步雜訊限制在 +/- N 個標準差範圍內。值為 0 時停用限制。(預設值: 0.0) |

## 輸出

| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `SAMPLER` | SAMPLER | 配置完成的 LCM 取樣器物件，可直接用於取樣工作流程中。 |
