...
|-----------|-----------|----------|-------|-------------|
| `s_noise` | FLOAT | 예 | 0.0 ~ 64.0 (단위: 0.01) | 첫 번째 단계의 단계별 노이즈 승수입니다. 값이 1.0이면 모델의 훈련 노이즈 스케일과 일치합니다. (기본값: 1.0) |
| `s_noise_end` | FLOAT | 예 | 0.0 ~ 64.0 (단위: 0.01) | 마지막 단계의 단계별 노이즈 승수입니다. 일정한 노이즈 일정을 위해 `s_noise`와 동일한 값으로 설정하십시오. (기본값: 1.0) |
| `noise_clip_std` | FLOAT | 예 | 0.0 ~ 10.0 (단위: 0.01) | 단계별 노이즈를 +/- N 표준편차 범위 내로 제한합니다. 값이 0이면 제한이 비활성화됩니다. (기본값: 0.0) |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `SAMPLER` | SAMPLER | 구성된 LCM 샘플러 객체로, 샘플링 워크플로우에서 사용할 준비가 되었습니다. |
