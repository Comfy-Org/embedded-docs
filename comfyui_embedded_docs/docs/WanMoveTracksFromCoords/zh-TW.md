...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `track_coords` | STRING | 否 | N/A | 包含軌跡座標資料的 JSON 格式字串。預設值為空列表（`"[]"`）。 |
| `track_mask` | MASK | 否 | N/A | 可選的遮罩。當提供時，節點會使用它來決定每個軌跡在每一幀的可見度。 |
...
| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `track_length` | TRACKS | 生成的軌跡資料，包含每個軌跡的路徑座標和可見度資訊。 |
| `track_length` | INT | 生成的軌跡中的總幀數。 |
