...
| --- | --- | --- | --- | --- |
| `coordonnées_piste` | Une chaîne au format JSON contenant les données de coordonnées pour les pistes. La valeur par défaut est une liste vide (`"[]"`). Cette entrée est une entrée forcée, elle doit donc être connectée dans l'interface utilisateur. | STRING | Non | N/A |
