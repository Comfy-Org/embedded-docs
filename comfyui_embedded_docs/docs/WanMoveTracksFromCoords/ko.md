...
|-----------|-----------|----------|-------|-------------|
| `트랙 좌표` | STRING | 아니요 | 해당 없음 | 트랙의 좌표 데이터를 포함하는 JSON 형식 문자열입니다. 기본값은 빈 목록(`"[]"`)입니다. |
| `트랙 마스크` | MASK | 아니요 | 해당 없음 | 선택적 마스크입니다. 제공된 경우 노드는 이를 사용하여 각 프레임별 트랙의 가시성을 결정합니다. |
...
| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `트랙 길이` | TRACKS | 생성된 트랙 데이터로, 각 트랙의 경로 좌표와 가시성 정보를 포함합니다. |
