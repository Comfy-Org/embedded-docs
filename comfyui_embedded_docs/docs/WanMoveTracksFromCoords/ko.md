...
|-----------|-----------|----------|-------|-------------|
| `track_coords` | STRING | 아니요 | 해당 없음 | 트랙의 좌표 데이터를 포함하는 JSON 형식 문자열입니다. 기본값은 빈 목록(`"[]"`)입니다. |
| `track_mask` | MASK | 아니요 | 해당 없음 | 선택적 마스크입니다. 제공된 경우 노드는 이를 사용하여 각 프레임별 트랙의 가시성을 결정합니다. |
