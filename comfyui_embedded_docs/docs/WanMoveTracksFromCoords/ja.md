...
| パラメータ | データ型 | 必須 | 範囲 | 説明 |
|-----------|-----------|----------|-------|-------------|
| `track_coords` | STRING | いいえ | N/A | トラックの座標データを含むJSON形式の文字列です。デフォルト値は空のリスト（`"[]"`）です。 |
| `track_mask` | MASK | いいえ | N/A | オプションのマスクです。指定された場合、ノードはこれを使用してフレームごとの各トラックの可視性を決定します。 |
...
| 出力名 | データ型 | 説明 |
|-------------|-----------|-------------|
| `tracks` | TRACKS | 生成されたトラックデータです。各トラックのパス座標と可視性情報を含みます。 |
