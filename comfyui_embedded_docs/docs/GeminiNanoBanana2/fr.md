...
Le nœud Nano Banana 2 génère ou modifie des images de manière synchrone via l'API Google Vertex en utilisant le modèle Gemini 3.1 Flash Image. Il envoie une invite textuelle, ainsi que des images ou fichiers de référence facultatifs, et renvoie l'image générée, tout texte d'accompagnement et, éventuellement, une image issue du processus de réflexion du modèle. Ce nœud est marqué comme obsolète.
...
| `thought_image` | Première image issue du processus de réflexion du modèle. Disponible uniquement lorsque `thinking_level` est défini sur HIGH et que la modalité IMAGE+TEXT est utilisée. | IMAGE |
