...
| `seed` | INT | 是 | 0 到 18446744073709551615 | 當種子固定為特定值時，模型會盡最大努力為重複的請求提供相同的回應。不保證輸出具有確定性。此外，即使使用相同的種子值，更改模型或參數設定（例如溫度）也可能導致回應產生變化。預設情況下，使用隨機種子值。（預設值：42） |
| `aspect_ratio` | COMBO | 是 | `"auto"`<br>`"1:1"`<br>`"2:3"`<br>`"3:2"`<br>`"3:4"`<br>`"4:3"`<br>`"4:5"`<br>`"5:4"`<br>`"9:16"`<br>`"16:9"`<br>`"21:9"` | 若設為 'auto'，則會匹配輸入圖像的長寬比；若未提供圖像，通常會生成 16:9 的方形。（預設值："auto"） |
| `resolution` | COMBO | 是 | `"1K"`<br>`"2K"`<br>`"4K"` | 目標輸出解析度。對於 2K/4K，會使用原生的 Gemini 放大工具。 |
| `response_modalities` | COMBO | 是 | `"IMAGE"`<br>`"IMAGE+TEXT"` | 決定模型將返回的內容類型。（進階） |
| `thinking_level` | COMBO | 是 | `"MINIMAL"`<br>`"HIGH"` | 控制模型推理過程的深度。 |
| `images` | IMAGE | 否 | N/A | 可選的參考圖像。若要包含多張圖像，請使用批次圖像節點（最多 14 張）。 |
| `files` | CUSTOM | 否 | N/A | 可選的檔案，用作模型的上下文。接受來自 Gemini 生成內容輸入檔案節點的輸入。 |
| `system_prompt` | STRING | 否 | N/A | 指示 AI 行為的基本指令。（進階） |
