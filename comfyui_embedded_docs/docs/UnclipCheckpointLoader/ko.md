...
| 필드         | Comfy 자료형      | 설명                                                                                  |
|-------------|------------------|--------------------------------------------------------------------------------------|
| `ckpt_name` | `COMBO[STRING]`  | 로드할 체크포인트의 이름을 지정하며, 미리 정의된 디렉터리에서 올바른 체크포인트 파일을 식별하고 검색하여 모델 및 구성의 초기화를 결정합니다. |

## 출력

| 필드          | Comfy 자료형    | 설명                                                              | Python 자료형        |
|--------------|----------------|------------------------------------------------------------------|---------------------|
| `model`      | `MODEL`        | 체크포인트에서 로드된 기본 모델을 나타냅니다.                        | `torch.nn.Module`   |
| `clip`       | `CLIP`         | 체크포인트에서 로드된 CLIP 모듈을 나타냅니다(사용 가능한 경우).      | `torch.nn.Module`   |
| `vae`        | `VAE`          | 체크포인트에서 로드된 VAE 모듈을 나타냅니다(사용 가능한 경우).        | `torch.nn.Module`   |
| `clip_vision`| `CLIP_VISION`  | 체크포인트에서 로드된 CLIP 비전 모듈을 나타냅니다(사용 가능한 경우).  | `torch.nn.Module`   |