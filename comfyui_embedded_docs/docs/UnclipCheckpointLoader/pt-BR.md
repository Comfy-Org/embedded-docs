...
|---------------|----------------|------------------------------------------------------------------------|----------------------|
| `model`       | `MODEL`        | Representa o modelo principal carregado do checkpoint.                 | `torch.nn.Module`    |
| `clip`        | `CLIP`         | Representa o módulo CLIP carregado do checkpoint, se disponível.       | `torch.nn.Module`    |
| `vae`         | `VAE`          | Representa o módulo VAE carregado do checkpoint, se disponível.        | `torch.nn.Module`    |
| `clip_vision` | `CLIP_VISION`  | Representa o módulo CLIP vision carregado do checkpoint, se disponível.| `torch.nn.Module`    |