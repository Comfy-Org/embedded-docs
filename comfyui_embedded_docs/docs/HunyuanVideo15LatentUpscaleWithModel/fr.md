...
| `méthode_d_agrandissement` | L'algorithme d'interpolation utilisé pour l'étape d'agrandissement initiale (par défaut : `"bilinear"`). | COMBO | Oui | `"nearest-exact"`<br>`"bilinear"`<br>`"area"`<br>`"bicubic"`<br>`"bislerp"` |
| `largeur` | La largeur cible pour le latent agrandi, en pixels. Une valeur de 0 calcule automatiquement la largeur en fonction de la hauteur cible et du ratio d'aspect d'origine. La largeur finale de sortie sera un multiple de 16 (par défaut : 1280). | INT | Oui | 0 à 16384 (pas : 8) |
| `hauteur` | La hauteur cible pour le latent agrandi, en pixels. Une valeur de 0 calcule automatiquement la hauteur en fonction de la largeur cible et du ratio d'aspect d'origine. La hauteur finale de sortie sera un multiple de 16 (par défaut : 720). | INT | Oui | 0 à 16384 (pas : 8) |
| `rogner` | Détermine comment le latent agrandi est recadré pour correspondre aux dimensions cibles. | COMBO | Oui | `"disabled"`<br>`"center"` |
