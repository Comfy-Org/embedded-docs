...
| `samples` | Büyütülecek latent görüntü temsili. | LATENT | Evet | N/A |
| `upscale_method` | İlk büyütme adımında kullanılan enterpolasyon algoritması (varsayılan: `"bilinear"`). | COMBO | Evet | `"nearest-exact"`<br>`"bilinear"`<br>`"area"`<br>`"bicubic"`<br>`"bislerp"` |
| `width` | Büyütülen latent için hedef genişlik, piksel cinsinden. 0 değeri, hedef yüksekliğe ve özgün en-boy oranına göre genişliği otomatik olarak hesaplar. Nihai çıktı genişliği 16'nın katı olur (varsayılan: 1280). | INT | Evet | 0 ile 16384 (step: 8) |
| `height` | Büyütülen latent için hedef yükseklik, piksel cinsinden. 0 değeri, hedef genişliğe ve özgün en-boy oranına göre yüksekliği otomatik olarak hesaplar. Nihai çıktı yüksekliği 16'nın katı olur (varsayılan: 720). | INT | Evet | 0 ile 16384 (step: 8) |
| `crop` | Büyütülen latentin hedef boyutlara sığdırılmak üzere nasıl kırpılacağını belirler. | COMBO | Evet | `"disabled"`<br>`"center"` |
