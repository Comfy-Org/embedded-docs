...
|-----------|-----------|----------|-------|-------------|
| `lora_name` | STRING | 예 | 여러 옵션 사용 가능 | loras 폴더에서 로드할 LoRA 파일의 이름입니다 |
| `strength_model` | FLOAT | 예 | -20.0 ~ 20.0 | 모델 구성 요소에 LoRA를 적용할 강도 승수입니다 (기본값: 1.0) |
| `prev_hooks` | HOOKS | 아니요 | - | 이 후크와 연결할 선택적 이전 후크입니다 |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `hooks` | HOOKS | 모델 처리에 적용할 수 있는 생성된 LoRA 후크입니다 |
