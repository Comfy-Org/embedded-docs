...
| `negative_prompt` | STRING | 是 | - | 反向文字提示詞 |
| `cfg_scale` | FLOAT | 否 | 0.0-1.0 | 控制輸出結果遵循提示詞的程度（預設值：0.75） |
| `aspect_ratio` | COMBO | 否 | "16:8"<br>"9:16"<br>"1:1"<br>"21:9"<br>"3:4"<br>"4:3" | 生成影片的畫面比例（預設值："16:9"） |
| `camera_control` | CAMERA_CONTROL | 否 | - | 可使用 Kling 相機控制節點建立。控制影片生成過程中的攝影機運鏡與動態效果。 |

## 輸出

| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `output` | VIDEO | 具有相機控制效果的生成影片 |
