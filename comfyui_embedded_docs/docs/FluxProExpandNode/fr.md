...
| `suréchantillonnage du prompt` | BOOLEAN | Non | - | Indique s'il faut effectuer un sur-échantillonnage de la description textuelle. Si activé, modifie automatiquement la description pour une génération plus créative, mais les résultats sont non déterministes (une même graine ne produira pas exactement le même résultat). (par défaut : False) |
