...
| 参数             | 数据类型         | 描述                                                                 |
|-----------------|-----------------|----------------------------------------------------------------------|
| `image`         | `IMAGE`         | 待放大的输入图像。此参数至关重要，因为它提供了将进行放大处理的基础图像。 |
| `upscale_method`| COMBO[STRING]   | 指定用于放大的插值方法。方法的选择会影响放大后图像的质量和特性。         |
| `scale_by`      | `FLOAT`         | 图像放大的倍数。此参数决定了输出图像相对于输入图像的尺寸增加比例。       |

## 输出

| 参数     | 数据类型 | 描述                                                               |
|----------|----------|--------------------------------------------------------------------|
| `image`  | `IMAGE`  | 放大后的图像，其尺寸根据指定的缩放因子和插值方法，比输入图像更大。 |