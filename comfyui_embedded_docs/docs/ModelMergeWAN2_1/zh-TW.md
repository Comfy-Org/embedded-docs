...
| `patch_embedding.` | FLOAT | 是 | 0.0 - 1.0 | 區塊嵌入元件的權重（預設值：1.0） |
| `time_embedding.` | FLOAT | 是 | 0.0 - 1.0 | 時間嵌入元件的權重（預設值：1.0） |
| `time_projection.` | FLOAT | 是 | 0.0 - 1.0 | 時間投影元件的權重（預設值：1.0） |
| `text_embedding.` | FLOAT | 是 | 0.0 - 1.0 | 文字嵌入元件的權重（預設值：1.0） |
| `img_emb.` | FLOAT | 是 | 0.0 - 1.0 | 圖像嵌入元件的權重，用於圖像轉影片模型（預設值：1.0） |
...
| `head.` | FLOAT | 是 | 0.0 - 1.0 | 輸出頭元件的權重（預設值：1.0） |
...
| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `model` | MODEL | 合併後的模型，根據指定的權重結合了兩個輸入模型的元件 |
