...
ModelMergeWAN2_1 節點透過加權平均的方式混合兩個模型的組件來進行模型合併。它支援不同的模型尺寸，包括具有 30 個區塊的 1.3B 模型和具有 40 個區塊的 14B 模型，並對包含額外圖像嵌入組件的圖像轉影片模型進行特殊處理。模型的每個組件都可以單獨加權，以控制兩個輸入模型之間的混合比例。
...
**注意：** 所有權重參數使用 0.0 到 1.0 的範圍，步長為 0.01。此節點支援最多 40 個區塊以適應不同的模型尺寸，其中 1.3B 模型使用 30 個區塊，14B 模型使用 40 個區塊。`img_emb.` 參數專門用於圖像轉影片模型。
