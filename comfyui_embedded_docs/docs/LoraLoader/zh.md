...
| `lora_name` | COMBO[STRING] | 选择要使用的 LoRA 模型名称 |
| `strength_model` | FLOAT | 取值范围为 -100.0 到 100.0，日常图像生成通常在 0~1 之间使用。数值越高，模型调整效果越明显 |
| `strength_clip` | FLOAT | 取值范围为 -100.0 到 100.0，日常图像生成通常在 0~1 之间使用。数值越高，模型调整效果越明显 |

## 输出

| 参数 | 数据类型 | 描述 |
| --- | --- | --- |
| `model` | MODEL | 应用了 LoRA 调整后的模型 |
