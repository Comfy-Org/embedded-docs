...
| `model` | El modelo GPT Image de OpenAI a utilizar. Al seleccionar un modelo, se muestran parámetros adicionales específicos de ese modelo. | COMBO | Sí | `"gpt-image-2"`<br>`"gpt-image-1.5"`<br>`"gpt-image-1"` |
...
| `seed` | Semilla para reproducibilidad (por defecto: 0). Nota: aún no está implementada en el backend. | INT | Sí | 0 a 2147483647 |
