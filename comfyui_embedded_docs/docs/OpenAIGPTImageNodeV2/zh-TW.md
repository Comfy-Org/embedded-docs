...
| 參數 | 資料類型 | 必要 | 範圍 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `prompt` | STRING | 是 | 不適用 | GPT 圖像的文字提示（預設：""）。 |
| `model` | COMBO | 是 | `"gpt-image-2"`<br>`"gpt-image-1.5"`<br>`"gpt-image-1"` | 要使用的 OpenAI GPT 圖像模型。選擇模型會顯示該模型專屬的額外參數。 |
...
| `n` | INT | 是 | 1 至 8 | 要生成的圖像數量（預設：1）。 |
| `seed` | INT | 是 | 0 至 2147483647 | 用於重現結果的種子（預設：0）。注意：後端尚未實作此功能。 |
