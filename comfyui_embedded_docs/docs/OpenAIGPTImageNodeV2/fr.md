...
| `model` | COMBO | Oui | `"gpt-image-2"`<br>`"gpt-image-1.5"`<br>`"gpt-image-1"` | Le modèle OpenAI GPT Image à utiliser. La sélection d'un modèle révèle des paramètres supplémentaires spécifiques à ce modèle. |
...
- Lors de l'utilisation de `gpt-image-2` avec un `model.size` défini sur "Personnalisé", les `custom_width` et `custom_height` doivent être des multiples de 16, le bord maximal doit être <= 3840, le rapport hauteur/largeur ne doit pas dépasser 3:1, et le nombre total de pixels doit être compris entre 655 360 et 8 294 400.
...
- Lorsqu'un masque est fourni, les dimensions du masque doivent correspondre aux dimensions de l'image d'entrée.
