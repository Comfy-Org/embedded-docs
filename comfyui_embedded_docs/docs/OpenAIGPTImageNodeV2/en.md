...
| --- | --- | --- | --- | --- |
| `model` | The OpenAI GPT Image model to use. Selecting a model reveals additional parameters specific to that model. | DYNAMIC_COMBO | Yes | `"gpt-image-2.5-flare"`<br>`"gpt-image-2.5-sunburst"`<br>`"gpt-image-2"`<br>`"gpt-image-1.5"`<br>`"gpt-image-1"` |
...
| `model.quality` | Image quality, affects cost and generation time (default: `"low"`). | COMBO | Yes | `"low"`<br>`"medium"`<br>`"high"`<br>`"xhigh"`<br>`"max"` |
