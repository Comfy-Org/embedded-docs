...
| `風格` | 描述音樂風格的文字。支援多行輸入和動態提示詞。 | STRING | 是 | Multiline text |
| `歌詞` | 生成音樂的歌詞。支援多行輸入和動態提示詞。 | STRING | 是 | Multiline text |
| `abc` | 連接 ABC 生成器，或提供已編輯的樂譜。留空以自動使用 off 模式。預設值："" | STRING | 是 | Multiline text |
| `種子` | 生成用的隨機種子。預設值：0 | INT | 是 | 0 至 18446744073709551615 |
| `模式` | full：生成旋律與和弦；melody：僅生成旋律，建議用於翻唱。預設值："full" | COMBO | 是 | "full"<br>"melody" |
| `max_duration` | 最大持續時間（秒）。對於長提示詞會自動縮短；生成可能會提前停止。預設值：360.0 | FLOAT | 是 | 0.04 至 900.0 |
| `溫度` | 生成用的取樣溫度。預設值：1.0（進階） | FLOAT | 是 | 0.0 至 5.0 |
