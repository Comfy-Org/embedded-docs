...
|-----------|-----------|----------|-------|-------------|
| `samples` | LATENT | 예 | - | 3D 복셀 데이터로 디코딩할 잠재 표현입니다 |
| `vae` | VAE | 예 | - | 잠재 샘플을 디코딩하는 데 사용되는 VAE 모델입니다 |
| `num_chunks` | INT | 예 | 1000-500000 | 메모리 관리를 위해 처리를 분할할 청크 수입니다 (기본값: 8000) |
| `octree_resolution` | INT | 예 | 16-512 | 3D 복셀 생성에 사용되는 옥트리 구조의 해상도입니다 (기본값: 256) |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `voxels` | VOXEL | 디코딩된 잠재 표현에서 생성된 3D 복셀 데이터입니다 |
