...
|---------|----------|--------|--------|---------|
| `switch` | BOOLEAN | بله | | شرط بولی که تعیین می‌کند کدام ورودی عبور داده شود. در صورت true بودن، ورودی `on_true` انتخاب می‌شود. در صورت false بودن، ورودی `on_false` انتخاب می‌شود. |
| `on_false` | MATCH_TYPE | خیر | | مقداری که در صورت false بودن شرط `switch` خروجی داده می‌شود. این ورودی اختیاری است، اما حداقل یکی از ورودی‌های `on_false` یا `on_true` باید متصل باشد. |
| `on_true` | MATCH_TYPE | خیر | | مقداری که در صورت true بودن شرط `switch` خروجی داده می‌شود. این ورودی اختیاری است، اما حداقل یکی از ورودی‌های `on_false` یا `on_true` باید متصل باشد. |
...
|-----------|----------|---------|
| `output` | MATCH_TYPE | مقدار انتخاب‌شده. این مقدار با نوع داده ورودی متصل‌شده `on_false` یا `on_true` مطابقت خواهد داشت. |
