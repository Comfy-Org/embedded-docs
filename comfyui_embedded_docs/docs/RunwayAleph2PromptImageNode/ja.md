...
| `位置` | この画像を出力ビデオのタイムライン上に配置する方法です。絶対時間（開始からの秒数）または割合時間（ビデオ長に対する割合）から選択します。オプションを選択すると、対応するサブパラメータが表示されます。 | COMBO | はい | `Absolute (seconds)`<br>`Fraction (0.0 to 1.0)` |
