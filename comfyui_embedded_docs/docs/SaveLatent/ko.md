...
|-----------|-----------|----------|-------|-------------|
| `samples` | LATENT | 예 | - | 디스크에 저장할 잠재 샘플입니다 |
| `filename_prefix` | STRING | 아니요 | - | 출력 파일 이름의 접두사입니다 (기본값: "latents/ComfyUI") |
| `prompt` | PROMPT | 아니요 | - | 메타데이터에 포함할 프롬프트 정보입니다 (숨김 매개변수) |
| `extra_pnginfo` | EXTRA_PNGINFO | 아니요 | - | 메타데이터에 포함할 추가 PNG 정보입니다 (숨김 매개변수) |
