...
|-----------|--------------|-------------|-------|-----------|
| `vídeo` | VIDEO | Sim | N/A | O vídeo de entrada para editar. Máximo de 240 quadros, máximo de ~2,77 megapixels por quadro. |
| `prompt` | STRING | Sim | N/A | Uma descrição textual da nova aparência desejada para a cena. |
| `alpha_mode` | COMBO | Sim | `"fill"`<br>`"select"`<br>`"custom"` | O modo de máscara alfa. O modo "fill" não possui máscara separada e preenche todo o quadro. O modo "select" usa uma única imagem de quadro-chave para definir a área a ser editada. O modo "custom" usa um vídeo alfa completo para definir a área a ser editada quadro a quadro. |
| `resolução_máxima` | COMBO | Sim | `"720p"`<br>`"1080p"` | A resolução máxima para o vídeo de saída (padrão: "1080p"). |
| `seed` | INT | Sim | 0 a 2147483647 | Um valor de semente para reprodutibilidade. Usar a mesma semente com as mesmas entradas produzirá o mesmo resultado. |
| `imagem_de_referência` | IMAGE | Não | N/A | Uma imagem de referência opcional que descreve a nova aparência desejada para a cena. |
...
|---------------|--------------|-----------|
| `vídeo` | VIDEO | O vídeo editado com as alterações de cena aplicadas. |
| `alpha` | VIDEO | A máscara alfa usada pelo Beeble. Fica vazia no modo "fill", que não possui máscara separada. |
