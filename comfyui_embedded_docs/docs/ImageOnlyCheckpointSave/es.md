...
El nodo ImageOnlyCheckpointSave guarda un archivo de checkpoint que contiene un modelo, un codificador de visión CLIP y una VAE. Crea un archivo safetensors con el prefijo de nombre de archivo especificado y lo almacena en el directorio de salida. Este nodo está diseñado específicamente para guardar componentes de modelo relacionados con imágenes juntos en un solo archivo de checkpoint.
...
| `vae` | La VAE (Autoencoder Variacional) que se guardará en el checkpoint | VAE | Sí | - |
| `filename_prefix` | El prefijo para el nombre del archivo de salida (por defecto: "checkpoints/ComfyUI") | STRING | Sí | - |
