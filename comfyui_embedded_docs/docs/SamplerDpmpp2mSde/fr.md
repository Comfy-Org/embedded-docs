...
| `eta`           | `FLOAT`         | Détermine la taille du pas dans l'intégration numérique, affectant la granularité du processus d'échantillonnage. Une valeur plus élevée indique une taille de pas plus grande. |
...
| `noise_device`  | COMBO[STRING]   | Indique le périphérique de calcul ('gpu' ou 'cpu') sur lequel le processus de génération de bruit est exécuté, affectant les performances et l'efficacité. |
