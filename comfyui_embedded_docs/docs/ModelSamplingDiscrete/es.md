...
| `sampling`| COMBO[STRING] | `str`           | Especifica el método de muestreo discreto que se aplicará al modelo. La elección del método afecta la forma en que el modelo genera muestras, ofreciendo diferentes estrategias de muestreo. |
...
| Parámetro | Tipo de dato | Python dtype     | Descripción |
|-----------|-------------|-------------------|-------------|
| `model`   | MODEL     | `torch.nn.Module` | El modelo modificado con la estrategia de muestreo discreto aplicada. Este modelo ahora está preparado para generar muestras utilizando el método y los ajustes especificados. |