...
|-----------|-----------|----------|-------|-------------|
| `prompt` | STRING | 是 | 不适用 | 描述视频接下来应发生什么的文本说明。 |
| `video` | VIDEO | 是 | 不适用 | 要扩展的源视频。MP4 格式，时长 2-15 秒。 |
| `model` | COMBO | 是 | `"grok-imagine-video"` | 用于视频扩展的模型。选中后，会显示一个嵌套的 `duration` 参数。 |
| `seed` | INT | 否 | 0 到 2147483647 | 用于确定节点是否应重新运行的种子；实际结果与种子无关，具有非确定性（默认值：0）。 |
