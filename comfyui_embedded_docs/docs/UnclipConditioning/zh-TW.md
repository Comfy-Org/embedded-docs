...
| 參數                   | Comfy 資料類型          | 說明 |
|------------------------|------------------------|-------------|
| `條件設定`         | `CONDITIONING`         | 基礎條件化數據，CLIP 視覺輸出將被添加至此數據中，作為後續修改的基礎。 |
| `clip_vision_output`   | `CLIP_VISION_OUTPUT`   | 來自 CLIP 視覺模型的輸出，提供整合至條件化過程中的視覺上下文。 |
| `強度`             | `FLOAT`                | 決定 CLIP 視覺輸出對條件化過程的影響強度。 |
| `雜訊增強`   | `FLOAT`                | 指定在將 CLIP 視覺輸出整合至條件化過程之前，要應用的噪聲增強程度。 |

## 輸出

| 參數                   | Comfy 資料類型          | 說明 |
|-----------------------|------------------------|-------------|
| `條件設定`         | `CONDITIONING`         | 經過強化的條件化數據，現已包含整合後的 CLIP 視覺輸出，並應用了指定的強度與噪聲增強。 |