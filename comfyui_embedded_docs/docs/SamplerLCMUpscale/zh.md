...
| 参数 | 数据类型 | 是否必填 | 范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `scale_ratio` | FLOAT | 否 | 0.1 - 20.0 | 放大过程中应用的缩放比例（默认值：1.0） |
| `scale_steps` | INT | 否 | -1 - 1000 | 放大过程使用的步数。设为 -1 可自动计算（默认值：-1） |
| `upscale_method` | COMBO | 是 | "bislerp"<br>"nearest-exact"<br>"bilinear"<br>"area"<br>"bicubic" | 用于放大图像的插值方法 |
