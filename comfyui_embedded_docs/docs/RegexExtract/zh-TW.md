...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `字串` | STRING | 是 | - | 要搜尋模式的輸入文字 |
| `正則表達式模式` | STRING | 是 | - | 要搜尋的正則表達式模式 |
| `模式` | COMBO | 是 | "First Match"<br>"All Matches"<br>"First Group"<br>"All Groups" | 提取模式決定返回匹配項的哪些部分（預設："First Match"） |
| `忽略大小寫` | BOOLEAN | 否 | - | 匹配時是否忽略大小寫（預設：True） |
| `多行模式` | BOOLEAN | 否 | - | 是否將字串視為多行（預設：False） |
| `點號匹配所有` | BOOLEAN | 否 | - | 點號 (.) 是否匹配換行符（預設：False） |
| `群組索引` | INT | 否 | 0-100 | 使用群組模式時要提取的捕獲群組索引（預設：1） |
