...
| `regex_pattern` | STRING | 是 | - | 要搜尋的正則表達式模式 |
| `mode` | COMBO | 是 | "First Match"<br>"All Matches"<br>"First Group"<br>"All Groups" | 提取模式決定返回匹配項的哪些部分（預設："First Match"） |
| `case_insensitive` | BOOLEAN | 否 | - | 匹配時是否忽略大小寫（預設：True） |
| `multiline` | BOOLEAN | 否 | - | 是否將字串視為多行（預設：False） |
| `dotall` | BOOLEAN | 否 | - | 點號 (.) 是否匹配換行符（預設：False） |
| `group_index` | INT | 否 | 0-100 | 使用群組模式時要提取的捕獲群組索引（預設：1） |
