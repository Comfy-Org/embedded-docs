...
|-----------|---------------|-----------|-------|-------------|
| `cadena` | STRING | Sí | - | El texto de entrada en el que buscar patrones |
| `patrón_regex` | STRING | Sí | - | El patrón de expresión regular a buscar |
| `modo` | COMBO | Sí | "First Match"<br>"All Matches"<br>"First Group"<br>"All Groups" | El modo de extracción determina qué partes de las coincidencias se devuelven (predeterminado: "First Match") |
| `insensible_a_mayúsculas` | BOOLEAN | No | - | Si se ignora mayúsculas/minúsculas al coincidir (predeterminado: True) |
| `multilínea` | BOOLEAN | No | - | Si se trata la cadena como múltiples líneas (predeterminado: False) |
| `dotall` | BOOLEAN | No | - | Si el punto (.) coincide con saltos de línea (predeterminado: False) |
| `índice_de_grupo` | INT | No | 0-100 | El índice del grupo de captura a extraer al usar modos de grupo (predeterminado: 1) |
