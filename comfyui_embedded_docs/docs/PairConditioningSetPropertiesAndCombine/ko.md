...
|-----------|-----------|----------|-------|-------------|
| `긍정 조건` | CONDITIONING | 예 | - | 원본 긍정 조건화 입력 |
| `부정 조건` | CONDITIONING | 예 | - | 원본 부정 조건화 입력 |
| `새 긍정 조건` | CONDITIONING | 예 | - | 적용할 새로운 긍정 조건화 |
| `새 부정 조건` | CONDITIONING | 예 | - | 적용할 새로운 부정 조건화 |
| `강도` | FLOAT | 예 | 0.0 ~ 10.0 | 새로운 조건화를 적용하기 위한 강도 계수 (기본값: 1.0) |
| `조건 영역 설정` | STRING | 예 | "default"<br>"mask bounds" | 조건화 영역이 적용되는 방식을 제어 (기본값: "default") |
| `마스크` | MASK | 아니요 | - | 조건화 적용 영역을 제한하는 선택적 마스크 |
| `후크` | HOOKS | 아니요 | - | 고급 제어를 위한 선택적 후크 그룹 |
| `타임스텝 범위` | TIMESTEPS_RANGE | 아니요 | - | 선택적 타임스텝 범위 지정 |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `부정 조건` | CONDITIONING | 결합된 긍정 조건화 출력 |
| `부정 조건` | CONDITIONING | 결합된 부정 조건화 출력 |
