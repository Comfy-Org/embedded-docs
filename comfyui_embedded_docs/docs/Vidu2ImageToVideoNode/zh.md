...
|-----------|-----------|----------|-------|-------------|
| `model` | COMBO | 是 | `"viduq2-pro-fast"`<br>`"viduq2-pro"`<br>`"viduq2-turbo"` | 用于视频生成的 Vidu2 模型。不同模型在速度和质量上各有优劣。 |
...
| `resolution` | COMBO | 是 | `"720p"`<br>`"1080p"` | 生成视频的输出分辨率。此参数为高级选项。 |
| `movement_amplitude` | COMBO | 是 | `"auto"`<br>`"small"`<br>`"medium"`<br>`"large"` | 画面中物体的运动幅度。此参数为高级选项。 |
