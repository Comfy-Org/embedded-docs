...
LTXV 音訊 VAE 解碼節點可將音訊的潛在表示轉換回音訊波形。它使用專門的 Audio VAE 模型來執行此解碼過程，並產生具有特定取樣率的音訊輸出。

## 輸入

| 參數 | 描述 | 資料類型 | 必要的 | Range |
| --- | --- | --- | --- | --- |
...
| 輸出名稱 | 描述 | 資料類型 |
| --- | --- | --- |
| `Audio` | 解碼後的音訊波形及其相關的取樣率。該波形是移至與輸入潛在表示相同裝置的張量，取樣率則由 Audio VAE 模型決定。 | AUDIO |
