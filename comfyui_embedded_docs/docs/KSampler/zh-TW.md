...
| `步驟數` | INT | 是 | 20 | 1 ~ 10000 | 去噪過程中使用的步數，更多步數意味著更準確的結果 |
| `cfg` | FLOAT | 是 | 8.0 | 0.0 ~ 100.0 | 控制生成圖像與輸入條件的匹配程度，建議值為 6-8 |
| `取樣器` | UI Option | 是 | 無 | Multiple algorithms | 選擇用於去噪的採樣器，影響生成速度和風格 |
| `排程器` | UI Option | 是 | 無 | Multiple schedulers | 控制噪聲的移除方式，影響生成過程 |
...
| `去雜訊強度` | FLOAT | 否 | 1.0 | 0.0 ~ 1.0 | 決定噪聲移除比例，較低的值意味著與輸入圖像的關聯較少 |
| `生成後控制` | UI Option | 否 | 無 | Random/Inc/Dec/Keep | 提供在每個提示後更改種子的能力 |

## 輸出結果

| 參數 | 功能 |
| -------------- | ------------------------------------------ |
| `Latent` | 輸出採樣器去噪後的潛在圖像 |
