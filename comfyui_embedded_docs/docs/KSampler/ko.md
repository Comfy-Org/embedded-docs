...
| ----------------------- | -------------- | ---- | ------ | --------------------------- | --------------------------------------------------------------------------------------------- |
| Model                   | checkpoint     | 예   | 없음   | -                           | 디노이징 과정에 사용되는 입력 모델입니다.                                                     |
| seed                    | Int            | 예   | 0      | 0 ~ 18446744073709551615    | 무작위 노이즈를 생성하는 데 사용됩니다. 동일한 "시드"를 사용하면 동일한 이미지가 생성됩니다. |
| steps                   | Int            | 예   | 20     | 1 ~ 10000                   | 디노이징 과정에 사용할 단계 수입니다. 단계가 많을수록 더 정확한 결과를 얻을 수 있습니다.      |
| cfg                     | float          | 예   | 8.0    | 0.0 ~ 100.0                 | 생성된 이미지가 입력 조건과 얼마나 일치하는지 제어합니다. 6-8을 권장합니다.                   |
| sampler_name            | UI 옵션        | 예   | 없음   | 여러 알고리즘               | 디노이징에 사용할 샘플러를 선택합니다. 생성 속도와 스타일에 영향을 줍니다.                   |
| scheduler               | UI 옵션        | 예   | 없음   | 여러 스케줄러               | 노이즈가 제거되는 방식을 제어합니다. 생성 과정에 영향을 줍니다.                               |
| Positive                | conditioning   | 예   | 없음   | -                           | 디노이징을 안내하는 긍정 조건입니다. 이미지에 나타나길 원하는 내용입니다.                     |
| Negative                | conditioning   | 예   | 없음   | -                           | 디노이징을 안내하는 부정 조건입니다. 이미지에 나타나지 않길 원하는 내용입니다.                |
| Latent_Image            | Latent         | 예   | 없음   | -                           | 디노이징에 사용되는 잠재 이미지입니다.                                                        |
| denoise                 | float          | 아니요 | 1.0    | 0.0 ~ 1.0                  | 노이즈 제거 비율을 결정합니다. 값이 낮을수록 입력 이미지와의 연관성이 줄어듭니다.             |
| control_after_generate  | UI 옵션        | 아니요 | 없음   | Random/Inc/Dec/Keep         | 각 프롬프트 후에 시드를 변경할 수 있는 기능을 제공합니다.                                     |
