...
| `seed` | Int | 是 | 0 | 0 ~ 18446744073709551615 | 用于生成随机噪声，使用相同的“种子”将生成相同的图像 |
| `steps` | Int | 是 | 20 | 1 ~ 10000 | 去噪过程中使用的步数，步数越多，结果越精确 |
| `cfg` | float | 是 | 8.0 | 0.0 ~ 100.0 | 控制生成图像与输入条件的匹配程度，建议值为 6-8 |
| `sampler_name` | UI Option | 是 | 无 | 多种算法 | 选择用于去噪的采样器，影响生成速度和风格 |
| `scheduler` | UI Option | 是 | 无 | 多种调度器 | 控制噪声的移除方式，影响生成过程 |
...
| `denoise` | float | 否 | 1.0 | 0.0 ~ 1.0 | 决定噪声移除比例，值越低，与输入图像的关联性越小 |
| `control_after_generate` | UI Option | 否 | 无 | Random/Inc/Dec/Keep | 提供在每次提示后更改种子的能力 |
