...
| 필드               | 데이터 유형 | 설명                                                                                     |
|-------------------|-------------|-----------------------------------------------------------------------------------------|
| `이미지`          | `IMAGE`     | 애니메이션 PNG로 처리 및 저장될 이미지 목록입니다. 목록의 각 이미지는 애니메이션의 한 프레임을 나타냅니다. |
| `파일명 접두사` | `STRING`    | 출력 파일의 기본 이름을 지정하며, 생성된 애니메이션 PNG 파일의 접두사로 사용됩니다.               |
| `fps`             | `FLOAT`     | 애니메이션의 초당 프레임 수로, 프레임이 표시되는 속도를 제어합니다.                             |
| `압축 레벨`  | `INT`       | 애니메이션 PNG 파일에 적용되는 압축 수준으로, 파일 크기와 이미지 선명도에 영향을 줍니다.          |

## 출력

| 필드   | 데이터 유형 | 설명                                                                                     |
|--------|-------------|-----------------------------------------------------------------------------------------|
| `ui`   | 해당 없음   | 생성된 애니메이션 PNG 이미지를 표시하고, 애니메이션이 단일 프레임인지 다중 프레임인지 나타내는 UI 구성 요소를 제공합니다. |