...
| `text` | 虚拟形象要说的文本（最多 5000 个字符）。生成的语音必须至少 1 秒长。（默认值：空）
| `voice` | 脚本使用的语音（HeyGen 最受欢迎的语音）。 | COMBO | 是（当 speech 为 "script" 时） | 提供多个选项 |
| `custom_voice_id` | 可选的 HeyGen 语音 ID。设置后将覆盖上面选择的语音。可以使用 HeyGen 库中的任何语音（2000+）。（默认值：空）
...
注意：`speech` 是一个选择语音来源的动态输入。设置为 "script" 时，`text` 和 `voice` 为必填，`custom_voice_id` 和 `voice_speed` 为可选。设置为 "audio" 时，则需要 `audio`。
