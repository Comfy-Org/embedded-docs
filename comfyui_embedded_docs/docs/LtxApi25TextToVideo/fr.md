...
LTX 2.5 Text To Video est un nœud API qui génère des vidéos de qualité professionnelle à partir d'une description textuelle en utilisant le modèle LTX 2.5. Vous fournissez une invite et choisissez des paramètres de génération tels que le niveau du modèle, la durée, la résolution, le nombre d'images par seconde et l'inclusion de l'audio ; le nœud soumet la tâche à l'API LTX et renvoie la vidéo résultante.
...
Remarque : Les options disponibles pour `model.resolution` dépendent du `model` sélectionné. "LTX-2.5 (Fast)" prend en charge des résolutions jusqu'à 2160x3840, tandis que "LTX-2.5 (Pro)" prend en charge des résolutions jusqu'à 1920x1080.

## Sorties

| Nom de sortie | Description | Type de données |
|---------------|-------------|-----------------|
| `video` | La vidéo générée renvoyée par l'API LTX, prête à être utilisée dans le workflow. Si la génération audio est activée, la vidéo inclut un son synchronisé. | VIDEO |
