# LTX 2.5 Texto para Vídeo

LTX 2.5 Text To Video é um nó de API que gera vídeos de qualidade profissional a partir de uma descrição textual usando o modelo LTX 2.5. Você fornece um prompt e escolhe as configurações de geração, como o nível do modelo, a duração, a resolução, a taxa de quadros e se deseja incluir áudio; o nó envia a tarefa para a API LTX e retorna o vídeo resultante.
...
| `prompt` | A descrição textual do vídeo a ser gerado. É necessário um prompt não vazio de até 10.000 caracteres (padrão: ""). | STRING | Sim | 1 a 10000 caracteres |
| `semente` | Valor da semente usado para geração reproduzível (padrão: 42). | INT | Não | Inteiro |

Nota: As opções disponíveis de `model.resolution` dependem do `model` selecionado. O "LTX-2.5 (Fast)" suporta resoluções de até 2160x3840, enquanto o "LTX-2.5 (Pro)" suporta resoluções de até 1920x1080.
