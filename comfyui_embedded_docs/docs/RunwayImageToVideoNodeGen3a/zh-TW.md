...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `提示詞` | STRING | 是 | N/A | 生成用的文字提示（預設值：""） |
| `起始幀` | IMAGE | 是 | N/A | 用於影片的起始影格 |
| `持續時間` | COMBO | 是 | `"5"`<br>`"10"` | 影片持續時間（秒）（預設值："5"） |
| `比例` | COMBO | 是 | `"1280x720"`<br>`"720x1280"`<br>`"1920x1080"`<br>`"1080x1920"`<br>`"1080x1080"` | 生成影片的長寬比（預設值："1280x720"） |
| `種子值` | INT | 否 | 0 至 4294967295 | 生成用的隨機種子（預設值：0） |
