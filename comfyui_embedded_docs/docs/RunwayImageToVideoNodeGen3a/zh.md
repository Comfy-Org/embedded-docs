...
| 参数 | 数据类型 | 是否必填 | 范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `prompt` | STRING | 是 | 无 | 用于生成的文本提示（默认值：""） |
| `start_frame` | IMAGE | 是 | 无 | 用于视频的起始帧 |
| `duration` | COMBO | 是 | `"5"`<br>`"10"` | 视频时长（秒）（默认值："5"） |
| `ratio` | COMBO | 是 | `"1280x720"`<br>`"720x1280"`<br>`"1920x1080"`<br>`"1080x1920"`<br>`"1080x1080"` | 生成视频的宽高比（默认值："1280x720"） |
| `seed` | INT | 否 | 0 到 4294967295 | 用于生成的随机种子（默认值：0） |
