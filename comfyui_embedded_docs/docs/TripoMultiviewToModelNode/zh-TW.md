...
此節點透過處理最多四張顯示物件不同視角的圖片，使用 Tripo 的 API 同步生成 3D 模型。它需要一張正面圖片以及至少一張額外視角（左側、背面或右側）的圖片，以建立具有紋理和材質選項的完整 3D 模型。

## 輸入參數

| 參數名稱 | 資料類型 | 必填 | 數值範圍 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `image` | IMAGE | 是 | - | 物件的正面視角圖片（必填） |
...
**注意：** 正面圖片（`image`）始終為必填項。必須提供至少一張額外的視角圖片（`image_left`、`image_back` 或 `image_right`）以進行多視角處理。
