...
|-----------|-----------------|--------|-------|-------------|
| `largeur` | INT | Oui | 64 à 4096 (pas : 32) | La largeur de l'image latente en pixels (par défaut : 2048). Le modèle a été entraîné à environ 4 mégapixels ; des résolutions plus faibles s'écartent de la distribution et la qualité régresse de manière notable. |
| `hauteur` | INT | Oui | 64 à 4096 (pas : 32) | La hauteur de l'image latente en pixels (par défaut : 2048). Le modèle a été entraîné à environ 4 mégapixels ; des résolutions plus faibles s'écartent de la distribution et la qualité régresse de manière notable. |
| `taille_lot` | INT | Non | 1 à 64 | Le nombre d'images latentes à générer en un seul lot (par défaut : 1). |

## Sorties

| Nom de la sortie | Type de données | Description |
|------------------|-----------------|-------------|
| `samples` | LATENT | Un tenseur rempli de zéros représentant l'image latente vide, avec la forme (batch_size, 3, height, width). |
