...
TrainLoraNode 使用提供的潛在資料和條件資料，在擴散模型上建立並訓練 LoRA（低秩適應）模型。它允許您使用自訂訓練參數、優化器和損失函數來微調模型。該節點輸出應用 LoRA 後的已訓練模型、LoRA 權重、訓練損失指標以及完成的總訓練步數。
...
| `latents` | LATENT | 是 | - | 用於訓練的潛在資料，作為模型的資料集/輸入。 |
| `positive` | CONDITIONING | 是 | - | 用於訓練的正向條件資料。 |
| `batch_size` | INT | 是 | 1-10000 | 訓練時使用的批次大小（預設值：1）。 |
| `grad_accumulation_steps` | INT | 是 | 1-1024 | 訓練時使用的梯度累積步數（預設值：1）。 |
| `steps` | INT | 是 | 1-100000 | 訓練 LoRA 的總步數（預設值：16）。 |
...
| `existing_lora` | COMBO | 是 | 多個選項可用 | 要附加到的現有 LoRA。設定為 None 表示建立新的 LoRA（預設值："[None]"）。 |

**注意：** 正向條件資料的數量必須與潛在影像的數量相符。如果只提供一個正向條件資料但有多個影像，系統會自動將其重複應用於所有影像。
