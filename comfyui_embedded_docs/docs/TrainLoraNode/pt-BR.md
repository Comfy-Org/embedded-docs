...
| `seed` | INT | Sim | 0-18446744073709551615 | A semente a ser usada para o treinamento (usada no gerador para inicialização dos pesos LoRA e amostragem de ruído) (padrão: 0). |
...
| `gradient_checkpointing` | BOOLEAN | Sim | - | Usar checkpointing de gradiente para o treinamento (padrão: True). |
...
| `bucket_mode` | BOOLEAN | Sim | - | Ativar o modo de balde de resolução. Quando ativado, espera latentes pré-agrupados do nó ResolutionBucket (padrão: False). |
