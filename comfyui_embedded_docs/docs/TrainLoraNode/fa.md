...
| --- | --- | --- |
| `مدل` | وزن‌های LoRA آموزش‌دیده که می‌توانند ذخیره شوند یا به مدل‌های دیگر اعمال شوند. | LORA_MODEL |
| `وزن‌های LoRA` | یک دیکشنری حاوی مقادیر خطای آموزشی در طول زمان. | LOSS_MAP |
| `نقشه خطا` | تعداد کل مراحل آموزشی تکمیل‌شده (شامل هر مرحله قبلی از LoRA موجود). | INT |
