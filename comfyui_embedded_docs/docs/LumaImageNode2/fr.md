...
|-----------|-----------------|--------|-------|-------------|
| `prompt` | STRING | Oui | 1–6000 caractères | Description textuelle de l'image souhaitée. |
...
| `seed` | INT | Oui | 0 à 2147483647 | La graine détermine si le nœud doit être réexécuté ; les résultats sont non déterministes quelle que soit la graine. (par défaut : 0) |
...
| `web_search` | BOOLEAN | Oui | Vrai / Faux | Permet ou non au modèle de rechercher sur le web un contexte supplémentaire. (par défaut : Faux) |
