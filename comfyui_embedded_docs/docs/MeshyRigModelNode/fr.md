...
| `height_meters` | FLOAT | Oui | 0,1 à 15,0 | La hauteur approximative du modèle de personnage en mètres. Cela facilite la mise à l'échelle et la précision de l'armature (valeur par défaut : 1,7). |
| `texture_image` | IMAGE | Non | N/A | L'image de texture de couleur de base du modèle, dépliée en UV. |
...
|---------------|-----------------|-------------|
| `model_file` | STRING | Une sortie héritée pour la rétrocompatibilité, contenant le nom du fichier du modèle GLB. |
| `rig_task_id` | STRING | L'identifiant unique de tâche pour cette opération d'armature, pouvant être utilisé pour référencer le résultat. |
| `GLB` | FILE3DGLB | Le modèle de personnage 3D armaturé enregistré au format de fichier GLB. |
