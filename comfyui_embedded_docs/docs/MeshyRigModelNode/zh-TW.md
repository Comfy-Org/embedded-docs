...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `meshy_task_id` | STRING | 是 | 不適用 | 先前 Meshy 操作（例如文字轉 3D 或圖片轉 3D）所產生的唯一任務 ID，用於指定要進行骨架綁定的模型。 |
| `height_meters` | FLOAT | 是 | 0.1 至 15.0 | 角色模型的近似高度（單位：公尺）。這有助於提高縮放和骨架綁定的準確性（預設值：1.7）。 |
| `texture_image` | IMAGE | 否 | 不適用 | 模型的 UV 展開基礎顏色紋理圖片。 |
...
| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `model_file` | STRING | 為向後相容性保留的舊版輸出，包含 GLB 模型的檔案名稱。 |
| `rig_task_id` | STRING | 此骨架綁定操作的唯一任務 ID，可用於引用結果。 |
| `GLB` | FILE3DGLB | 以 GLB 檔案格式儲存的已綁定 3D 角色模型。 |
| `FBX` | FILE3DFBX | 以 FBX 檔案格式儲存的已綁定 3D 角色模型。 |
