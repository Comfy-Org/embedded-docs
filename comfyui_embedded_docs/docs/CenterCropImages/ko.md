...
|-----------|-----------|----------|-------|-------------|
| `image` | IMAGE | 예 | - | 자르기를 수행할 입력 이미지입니다. |
| `width` | INT | 예 | 1~8192 | 자르기 영역의 너비입니다(기본값: 512). |
| `height` | INT | 예 | 1~8192 | 자르기 영역의 높이입니다(기본값: 512). |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `image` | IMAGE | 중앙 자르기 작업 후 생성된 결과 이미지입니다. |
