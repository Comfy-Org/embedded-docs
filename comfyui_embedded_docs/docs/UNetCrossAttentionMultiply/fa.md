...
| `q` | FLOAT | خیر | 0.0 - 10.0 | فاکتور مقیاس‌دهی برای مؤلفه‌های query در توجه متقاطع (پیش‌فرض: 1.0) |
| `k` | FLOAT | خیر | 0.0 - 10.0 | فاکتور مقیاس‌دهی برای مؤلفه‌های key در توجه متقاطع (پیش‌فرض: 1.0) |
| `v` | FLOAT | خیر | 0.0 - 10.0 | فاکتور مقیاس‌دهی برای مؤلفه‌های value در توجه متقاطع (پیش‌فرض: 1.0) |
| `out` | FLOAT | خیر | 0.0 - 10.0 | فاکتور مقیاس‌دهی برای مؤلفه‌های خروجی در توجه متقاطع (پیش‌فرض: 1.0) |

## خروجی‌ها

| نام خروجی | نوع داده | توضیحات |
|-------------|-----------|-------------|
| `model` | MODEL | مدل UNet اصلاح‌شده با مؤلفه‌های توجه متقاطع مقیاس‌دهی‌شده |
