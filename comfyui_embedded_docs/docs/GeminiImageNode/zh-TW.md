...
| `seed` | INT | 必填 | 42 | 0 到 18446744073709551615 | 當種子固定為特定值時，模型會盡力為重複請求提供相同的回應。不保證輸出具有確定性。此外，更改模型或參數設置（例如溫度）即使使用相同的種子值也可能導致回應變化。預設情況下使用隨機種子值 |
...
*注意：此節點包含由系統自動處理的隱藏參數（`auth_token`、`comfy_api_key`、`unique_id`），無需使用者輸入。*
