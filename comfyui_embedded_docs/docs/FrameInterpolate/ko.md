...
|-----------|-----------|----------|-------|-------------|
| `interp_model` | MODEL | 예 | - | 중간 프레임을 생성하는 데 사용할 프레임 보간 모델입니다 |
| `images` | IMAGE | 예 | - | 보간할 연속 이미지(프레임) 배치입니다. 최소 2개의 이미지가 필요합니다. |
| `multiplier` | INT | 예 | 2 ~ 16 | 프레임 수를 곱할 횟수입니다. 예를 들어, 승수가 2이면 프레임 수가 두 배가 됩니다. (기본값: 2) |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `IMAGE` | IMAGE | 원본 프레임 사이에 보간된 프레임이 삽입되어 더 부드러운 시퀀스를 생성한 새 이미지 배치입니다. 총 출력 프레임 수는 `(입력 프레임 수 - 1) * 승수 + 1`입니다. |
