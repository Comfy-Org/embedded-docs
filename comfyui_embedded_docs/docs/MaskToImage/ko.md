...
| 매개변수 | 데이터 타입 | 설명 |
|-----------|-------------|-------------|
| `mask`    | `MASK`      | 마스크 입력은 변환 프로세스에 필수적이며, 이미지 형식으로 변환될 원본 데이터 역할을 합니다. 이 입력은 결과 이미지의 모양과 내용을 결정합니다. |

## 출력

| 매개변수 | 데이터 타입 | 설명 |
|-----------|-------------|-------------|
| `image`   | `IMAGE`     | 출력은 입력 마스크의 이미지 표현으로, 시각적 검사 및 추가 이미지 기반 조작을 가능하게 합니다. |