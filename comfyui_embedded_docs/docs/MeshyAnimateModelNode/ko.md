...
| `action_id` | INT | 예 | 0 ~ 696 | 적용할 애니메이션 액션의 ID 번호입니다. 사용 가능한 값 목록은 <https://docs.meshy.ai/en/api/animation-library>를 참조하십시오. (기본값: 0) |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `GLB` | STRING | 애니메이션된 모델의 문자열 식별자입니다. 이 출력은 이전 버전과의 호환성을 위해서만 제공됩니다. |
| `FBX` | FILE3DGLB | GLB 형식의 애니메이션된 3D 모델 파일입니다. |
