...
| `model` | COMBO | Oui | `gemini-2.5-pro-preview-05-06`<br>`gemini-2.5-flash-preview-04-17`<br>`gemini-2.5-pro`<br>`gemini-2.5-flash`<br>`gemini-3-pro-preview`<br>`gemini-3-1-pro`<br>`gemini-3-1-flash-lite` | Le modèle Gemini à utiliser pour générer des réponses. Valeur par défaut : gemini-3-1-pro. |
...
| `video` | VIDEO | Non | - | Vidéo facultative à utiliser comme contexte pour le modèle. Valeur par défaut : Aucune. |
| `files` | GEMINI_INPUT_FILES | Non | - | Fichier(s) facultatif(s) à utiliser comme contexte pour le modèle. Accepte les entrées du nœud Gemini Generate Content Input Files. Valeur par défaut : Aucun. |
