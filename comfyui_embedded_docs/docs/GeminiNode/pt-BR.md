...
| `seed` | Quando `seed` é fixado em um valor específico, o modelo faz o possível para fornecer a mesma resposta para solicitações repetidas. Saída determinística não é garantida. Além disso, alterar o modelo ou as configurações de parâmetros, como a temperatura, pode causar variações na resposta mesmo quando você usa o mesmo valor de `seed`. Por padrão, um valor de `seed` aleatório é usado. Padrão: 42. | INT | Sim | 0 a 18446744073709551615 |
| `images` | Imagem(ns) opcional(is) para usar como contexto para o modelo. Para incluir várias imagens, você pode usar o nó Batch Images. Padrão: None. | IMAGE | Não | - |
| `audio` | Áudio opcional para usar como contexto para o modelo. Padrão: None. | AUDIO | Não | - |
| `video` | Vídeo opcional para usar como contexto para o modelo. Padrão: None. | VIDEO | Não | - |
| `files` | Arquivo(s) opcional(is) para usar como contexto para o modelo. Aceita entradas do nó Gemini Generate Content Input Files. Padrão: None. | GEMINI_INPUT_FILES | Não | - |
