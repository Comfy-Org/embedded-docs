...
| `intensidade` | FLOAT | Sim | 0.0-1.0 | Define a diferença em relação à imagem original, deve estar em [0, 1], onde 0 significa quase idêntico e 1 significa similaridade mínima (padrão: 0.5) |
| `semente` | INT | Sim | 0-18446744073709551615 | Semente para determinar se o nó deve ser reexecutado; os resultados reais são não determinísticos independentemente da semente (padrão: 0) |
...
| `prompt_negativo` | STRING | Não | - | Uma descrição textual opcional de elementos indesejados na imagem (padrão: "") |
...
|---------------|--------------|-----------|
| `imagem` | IMAGE | A(s) imagem(ns) gerada(s) com base na imagem de entrada e no prompt |
