...
| `invite` | Prompt pour la génération d'image. Valeur par défaut : chaîne vide. Longueur maximale : 1000 caractères. | STRING | Oui | - |
| `n` | Le nombre d'images à générer. Valeur par défaut : 1. | INT | Oui | 1-6 |
| `intensité` | Définit la différence avec l'image originale ; doit se situer dans [0, 1], où 0 signifie presque identique et 1 signifie une similarité très faible. Valeur par défaut : 0.5. | FLOAT | Oui | 0.0-1.0 (pas de 0.01) |
| `graine` | Graine pour déterminer si le nœud doit être réexécuté ; les résultats réels sont non déterministes quelle que soit la graine. Valeur par défaut : 0. | INT | Oui | 0-18446744073709551615 |
...
| `invite_négative` | Description textuelle optionnelle des éléments indésirables sur une image. Valeur par défaut : chaîne vide. Fournie en tant que socket d'entrée. | STRING | Non | - |
