...
|---------|----------|--------|--------|---------|
| `مدل` | MODEL | بله | - | مدلی که پارامترهای ROPE آن تغییر داده خواهد شد. |
| `مقیاس X` | FLOAT | خیر | 0.0 - 100.0 | فاکتور مقیاس‌گذاری برای مؤلفه X از ROPE (پیش‌فرض: 1.0). |
| `جابجایی X` | FLOAT | خیر | 256.0- - 256.0 | مقدار جابجایی برای مؤلفه X از ROPE (پیش‌فرض: 0.0). |
| `مقیاس Y` | FLOAT | خیر | 0.0 - 100.0 | فاکتور مقیاس‌گذاری برای مؤلفه Y از ROPE (پیش‌فرض: 1.0). |
| `جابجایی Y` | FLOAT | خیر | 256.0- - 256.0 | مقدار جابجایی برای مؤلفه Y از ROPE (پیش‌فرض: 0.0). |
| `مقیاس T` | FLOAT | خیر | 0.0 - 100.0 | فاکتور مقیاس‌گذاری برای مؤلفه T (زمان) از ROPE (پیش‌فرض: 1.0). |
| `جابجایی T` | FLOAT | خیر | 256.0- - 256.0 | مقدار جابجایی برای مؤلفه T (زمان) از ROPE (پیش‌فرض: 0.0). |

## خروجی‌ها

| نام خروجی | نوع داده | توضیحات |
|-----------|----------|---------|
| `مدل` | MODEL | مدل با پارامترهای جدید مقیاس‌گذاری و جابجایی ROPE اعمال‌شده. |
