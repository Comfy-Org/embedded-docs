...
|-----------|-----------|----------|-------|-------------|
| `마스크` | MASK | 예 | - | 미리보기 및 이미지로 저장할 마스크 데이터 |
| `filename_prefix` | STRING | 아니요 | - | 출력 파일 이름의 접두사 (기본값: "ComfyUI") |
| `prompt` | PROMPT | 아니요 | - | 메타데이터용 프롬프트 정보 (자동으로 제공됨) |
| `extra_pnginfo` | EXTRA_PNGINFO | 아니요 | - | 메타데이터용 추가 PNG 정보 (자동으로 제공됨) |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `ui` | DICT | UI에 표시할 미리보기 이미지 정보 및 메타데이터를 포함합니다. |
