...
| `effect_scene` | COMBO | 是 | `"chat"`<br>`"dance"`<br>`"hug"`<br>`"kill"`<br>`"kiss"`<br>`"pat"`<br>`"punch"`<br>`"shrug"`<br>`"slap"`<br>`"tickle"` | 应用于视频生成的特效场景类型 |
| `model_name` | COMBO | 否 | `"kling-v1"`<br>`"kling-v1-5"`<br>`"kling-v1-6"` | 用于角色效果的模型（默认值："kling-v1"） |
| `mode` | COMBO | 否 | `"std"`<br>`"pro"` | 视频生成模式（默认值："std"） |
