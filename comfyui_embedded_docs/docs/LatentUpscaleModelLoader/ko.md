...
|-----------|-----------|----------|-------|-------------|
| `model_name` | STRING | 예 | *`latent_upscale_models` 폴더의 모든 파일* | 로드할 잠재 업스케일 모델 파일의 이름입니다. 사용 가능한 옵션은 ComfyUI의 `latent_upscale_models` 디렉터리에 있는 파일에서 동적으로 가져옵니다. |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `model` | LATENT_UPSCALE_MODEL | 로드된 잠재 업스케일 모델로, 구성이 완료되어 사용할 준비가 되었습니다. |
