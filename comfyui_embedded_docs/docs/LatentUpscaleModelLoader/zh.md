...
| 参数 | 数据类型 | 是否必填 | 范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `model_name` | STRING | 是 | `latent_upscale_models` 文件夹中的所有文件 | 要加载的潜在放大模型文件名。可用选项会根据 ComfyUI 的 `latent_upscale_models` 目录中的文件动态填充。 |

## 输出

| 输出名称 | 数据类型 | 描述 |
|-------------|-----------|-------------|
| `model` | LATENT_UPSCALE_MODEL | 已加载并配置好的潜在放大模型，可供使用。 |
