...
Este nó configura a primeira passada de amostragem da geração de formas do pipeline Trellis2. Ele recebe o voxel de estrutura densa produzido por VaeDecodeStructureTrellis2, extrai as coordenadas esparsas dos voxels preenchidos, cria um tensor latente esparso vazio e anexa os metadados de amostragem ao condicionamento para que o modelo possa lê-los durante a amostragem. Para a segunda passada de formas após o upsampling, use Trellis2UpsampleStage, que combina a cascata e a configuração do estágio da segunda passada.

## Entradas

| Parâmetro | Descrição | Tipo de Dados | Obrigatório | Intervalo |
|-----------|-------------|-----------|----------|-------|
| `positive` | O condicionamento positivo a ser preparado para o estágio de formas. Pode ser um condicionamento Trellis2 padrão ou um condicionamento Pixal3D que fornece um pacote de características de projeção; quando houver características de projeção, elas são calculadas para o estágio selecionado e anexadas ao condicionamento de saída. | CONDITIONING | Sim | Qualquer condicionamento Trellis2 ou Pixal3D |
| `negative` | O condicionamento negativo a ser preparado para o estágio de formas. Os mesmos metadados do estágio de formas são anexados a ele, assim como ao condicionamento positivo. | CONDITIONING | Sim | Qualquer condicionamento Trellis2 ou Pixal3D |
...
- A resolução da grade de voxels seleciona o estágio do pipeline: resolução igual ou inferior a 32 usa o modo `shape_generation_512` com o estágio `shape_512`; resolução superior a 32 usa o modo `shape_generation` com o estágio `shape_1024`.
...
|---------------|-----------|---------------|
| `positive` | O condicionamento positivo com os metadados do estágio de formas anexados: modo de geração, coordenadas esparsas, contagens de coordenadas por lote e características de projeção, quando o condicionamento de origem as fornece. | CONDITIONING |
| `negative` | O condicionamento negativo com os mesmos metadados do estágio de formas anexados. | CONDITIONING |
