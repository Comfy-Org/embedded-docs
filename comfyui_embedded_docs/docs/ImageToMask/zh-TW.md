...
| 參數名稱   | 資料類型      | 描述                                                                                                           |
|-------------|-------------|----------------------------------------------------------------------------------------------------------------------|
| `圖片`     | `IMAGE`     | `image` 參數代表輸入圖像，將根據指定的色彩通道從中生成遮罩。它在決定最終遮罩的內容和特徵方面扮演關鍵角色。 |
| `通道`   | COMBO[STRING] | `channel` 參數指定應使用輸入圖像的哪個色彩通道（紅色、綠色、藍色或 Alpha）來生成遮罩。此選擇直接影響遮罩的外觀以及圖像的哪些部分會被突出顯示或遮罩。 |
