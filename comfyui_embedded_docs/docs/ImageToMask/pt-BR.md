...
| Parâmetro | Tipo de Dados | Descrição |
|-----------|---------------|-----------|
| `mask`    | `MASK`        | A saída 'mask' é uma representação binária ou em escala de cinza do canal de cor especificado da imagem de entrada, útil para processamento adicional de imagem ou operações de mascaramento. |