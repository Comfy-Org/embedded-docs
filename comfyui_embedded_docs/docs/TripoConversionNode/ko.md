...
|-----------|-----------|----------|-------|-------------|
| `원본 모델 작업 ID` | MODEL_TASK_ID,RIG_TASK_ID,RETARGET_TASK_ID | 예 | MODEL_TASK_ID<br>RIG_TASK_ID<br>RETARGET_TASK_ID | 이전 Tripo 작업(모델 생성, 리깅 또는 리타겟팅)의 작업 ID |
| `형식` | COMBO | 예 | GLTF<br>USDZ<br>FBX<br>OBJ<br>STL<br>3MF | 변환된 3D 모델의 대상 파일 형식 |
| `쿼드` | BOOLEAN | 아니요 | True/False | 삼각형을 사각형으로 변환할지 여부 (기본값: False) |
| `면 제한` | INT | 아니요 | -1 ~ 2000000 | 출력 모델의 최대 면 수, 제한 없음은 -1 사용 (기본값: -1) |
| `텍스처 크기` | INT | 아니요 | 128 ~ 4096 | 출력 텍스처의 픽셀 크기 (기본값: 4096) |
| `텍스처 형식` | COMBO | 아니요 | BMP<br>DPX<br>HDR<br>JPEG<br>OPEN_EXR<br>PNG<br>TARGA<br>TIFF<br>WEBP | 내보내는 텍스처의 형식 (기본값: JPEG) |
