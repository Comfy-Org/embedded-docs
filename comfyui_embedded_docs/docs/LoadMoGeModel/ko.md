...
|-----------|-----------|----------|-------|-------------|
| `model_name` | STRING | 예 | `geometry_estimation` 폴더에 있는 사용 가능한 모델 파일 목록 | 불러올 MoGe 모델 파일의 이름입니다. ComfyUI 설치 환경에서 사용 가능한 모델 파일 중에서 선택하십시오. |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `MOGE_MODEL` | MOGE_MODEL | 불러온 MoGe 모델 인스턴스로, 기하학 추정 워크플로우에서 사용할 준비가 완료된 상태입니다. |
