...
Le nœud StableCascade_StageC_VAEEncode traite une image d'entrée via un encodeur VAE pour générer des représentations latentes pour le modèle Stable Cascade. Il redimensionne d'abord l'image en fonction d'un facteur de compression et du ratio de sous-échantillonnage du VAE, puis encode l'image redimensionnée. Le nœud produit deux tenseurs latents : un pour l'étage C (le résultat encodé réel) et un pour l'étage B (un espace réservé rempli de zéros).
...
| --- | --- | --- |
| `stage_c` | La représentation latente encodée pour l'étage C du modèle Stable Cascade | LATENT |
| `stage_b` | Une représentation latente d'espace réservé pour l'étage B. Renvoie actuellement un tenseur rempli de zéros dont les dimensions sont calculées à partir de la taille de l'image d'entrée. | LATENT |
