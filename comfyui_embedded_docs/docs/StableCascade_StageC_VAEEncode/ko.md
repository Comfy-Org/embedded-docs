...
|-----------|-----------|----------|-------|-------------|
| `이미지` | IMAGE | 예 | - | 잠재 공간으로 인코딩할 입력 이미지 |
| `vae` | VAE | 예 | - | 이미지 인코딩에 사용되는 VAE 모델 |
| `압축` | INT | 아니요 | 4-128 | 인코딩 전 이미지에 적용되는 압축 계수 (기본값: 42) |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `StageB 잠재 이미지` | LATENT | Stable Cascade 모델의 stage C용으로 인코딩된 잠재 표현 |
| `stage_b` | LATENT | stage B용 플레이스홀더 잠재 표현 (현재는 0을 반환) |
