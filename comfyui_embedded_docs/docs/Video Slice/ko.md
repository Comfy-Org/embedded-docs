...
|-----------|-----------|----------|-------|-------------|
| `video` | VIDEO | 예 | - | 자를 대상이 되는 입력 비디오입니다. |
| `start_time` | FLOAT | 아니요 | -1e5 ~ 1e5 | 시작 시간(초 단위, 기본값: 0.0)입니다. |
| `duration` | FLOAT | 아니요 | 0.0 이상 | 지속 시간(초 단위)이며, 0으로 설정하면 무제한입니다(기본값: 0.0). |
| `strict_duration` | BOOLEAN | 아니요 | - | True로 설정 시, 지정된 지속 시간을 충족할 수 없을 경우 오류가 발생합니다(기본값: False). |
