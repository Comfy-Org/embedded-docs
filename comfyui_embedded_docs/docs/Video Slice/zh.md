...
| 参数 | 数据类型 | 是否必填 | 范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `video` | VIDEO | 是 | - | 待切片的输入视频。 |
| `start_time` | FLOAT | 否 | -1e5 至 1e5 | 开始时间（秒），默认值：0.0。 |
| `duration` | FLOAT | 否 | 0.0 及以上 | 持续时间（秒），0 表示不限时长，默认值：0.0。 |
| `strict_duration` | BOOLEAN | 否 | - | 若为 True，当无法满足指定持续时间时将引发错误，默认值：False。 |
