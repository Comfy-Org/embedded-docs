...
|-----------|-------------|---------------|-------------|-------|
| `model` | Modelo a utilizar para la generación de video. | COMBO | Sí | `"viduq3-pro"`<br>`"viduq3-turbo"` |
| `image` | Imagen que se usará como fotograma inicial del video generado. | IMAGE | Sí | - |
| `prompt` | Prompt de texto opcional para la generación de video (máximo 2000 caracteres) (por defecto: vacío). | STRING | No | - |
| `seed` | Valor de semilla para controlar la aleatoriedad de la generación (por defecto: 1). | INT | No | 0 a 2147483647 |
...
| `model.audio` | Cuando está habilitado, genera video con sonido (incluye diálogos y efectos de sonido) (por defecto: False). | BOOLEAN | Sí | `True`<br>`False` |
