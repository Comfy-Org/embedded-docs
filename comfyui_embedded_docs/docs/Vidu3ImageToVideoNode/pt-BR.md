...
|-----------|-------------|-----------|----------|-------|
| `model` | Modelo a ser usado para a geração de vídeo. | COMBO | Sim | `"viduq3-pro"`<br>`"viduq3-turbo"` |
...
| Parâmetro | Descrição | Tipo de Dados | Obrigatório | Intervalo |
|-----------|-------------|-----------|----------|-------|
| `model.resolution` | Resolução do vídeo de saída. | COMBO | Sim | `"720p"`<br>`"1080p"`<br>`"2K"` |
| `model.duration` | Duração do vídeo de saída em segundos (padrão: 5). | INT | Sim | 1 a 16 |
| `model.audio` | Quando ativado, gera vídeo com som (incluindo diálogo e efeitos sonoros) (padrão: False). | BOOLEAN | Sim | `True`<br>`False` |
