...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `model` | COMBO | 是 | `"viduq3-pro"`<br>`"viduq3-turbo"` | 用於影片生成的模型。 |
| `model.resolution` | COMBO | 是 | `"720p"`<br>`"1080p"`<br>`"2K"` (僅限 viduq3-pro) | 輸出影片的解析度。可用選項取決於所選模型。 |
| `model.duration` | INT | 是 | 1 到 16 | 輸出影片的長度（秒）（預設值：5）。 |
| `model.audio` | BOOLEAN | 是 | `True` / `False` | 啟用時，輸出包含聲音（包括對話和音效）的影片（預設值：False）。 |
| `image` | IMAGE | 是 | - | 用作生成影片起始幀的影像。 |
| `prompt` | STRING | 否 | - | 用於影片生成的可選文字提示（最多 2000 個字元）（預設值：空白）。 |
