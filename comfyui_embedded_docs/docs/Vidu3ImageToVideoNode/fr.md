...
|-----------|-------------|-----------------|--------|-------|
| `model` | Modèle à utiliser pour la génération vidéo. | COMBO | Oui | `"viduq3-pro"`<br>`"viduq3-turbo"` |
| `image` | Une image à utiliser comme image de départ de la vidéo générée. | IMAGE | Oui | - |
| `prompt` | Un prompt texte facultatif pour la génération vidéo (2000 caractères maximum) (par défaut : vide). | STRING | Non | - |
| `seed` | Une valeur de seed pour contrôler le caractère aléatoire de la génération (par défaut : 1). | INT | Non | 0 à 2147483647 |
...
| `model.audio` | Lorsque activé, génère une vidéo avec le son (y compris les dialogues et les effets sonores) (par défaut : False). | BOOLEAN | Oui | `True`<br>`False` |
