...
|-----------|-------------|-------------|
| `images` | `IMAGE` | Изображения для сохранения. Этот параметр является ключевым, так как он непосредственно содержит данные изображения, которые будут обработаны и сохранены на диск. |
| `filename_prefix` | STRING   | Префикс имени файла для изображений, сохраняемых в папку `ComfyUI/output/`. По умолчанию используется `ComfyUI`, но вы можете его изменить. |
