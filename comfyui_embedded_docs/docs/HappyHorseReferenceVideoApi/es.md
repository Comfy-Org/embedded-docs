...
|-----------|---------------|-------------|-------|-------------|
| `model` | COMBO | Sí | `"happyhorse-1.0-r2v"` | El modelo HappyHorse a utilizar para la generación del video. |
...
| `seed` | INT | No | 0 a 2147483647 | Un valor de semilla para generación reproducible (predeterminado: 0). La semilla puede configurarse para cambiar automáticamente después de cada generación. |
| `watermark` | BOOLEAN | No | True o False | Si se debe agregar una marca de agua de IA generada al video resultante (predeterminado: False). |
