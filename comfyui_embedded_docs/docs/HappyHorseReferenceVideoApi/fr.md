...
|-----------|-----------------|--------|-------|-------------|
| `modèle` | COMBO | Oui | `"happyhorse-1.0-r2v"` | Le modèle HappyHorse à utiliser pour la génération vidéo. |
...
| `graine` | INT | Non | 0 à 2147483647 | Une valeur de graine pour une génération reproductible (par défaut : 0). La graine peut être configurée pour changer automatiquement après chaque génération. |
| `filigrane` | BOOLEAN | Non | True ou False | Indique s'il faut ajouter un filigrane de génération IA à la vidéo résultante (par défaut : False). |
