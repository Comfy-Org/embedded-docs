...
|-----------|-------------|-----------|----------|-------|
| `model` | Oluşturma için kullanılacak HappyHorse referanstan videoya modeli. | DYNAMIC_COMBO | Evet | `"happyhorse-1.1-r2v"`<br>`"happyhorse-1.0-r2v"` |
| `tohum` | Oluşturma için kullanılacak tohum (varsayılan: 0). Her oluşturmadan sonra otomatik olarak değişecek şekilde ayarlanabilir. | INT | Evet | 0 ile 2147483647 arası |
| `filigran` | Sonuca yapay zeka tarafından oluşturulmuş bir filigran eklenip eklenmeyeceği (varsayılan: False). | BOOLEAN | Evet | True veya False |
