...
| 參數 | 資料類型 | 說明 |
|--------------|--------------|-------------|
| `config_name` | COMBO[STRING] | 指定要使用的配置文件名稱。這對於決定模型的參數與設定至關重要，會影響模型的行為與效能。 |
| `ckpt_name`  | COMBO[STRING] | 指示要載入的檢查點文件名稱。這直接影響正在初始化的模型狀態，進而影響其初始權重與偏差。 |

## 輸出

| 參數 | 資料類型 | 說明 |
|-----------|-------------|-------------|
| `model`   | MODEL     | 代表從檢查點載入的主要模型，可供進一步操作或推論使用。 |
| `clip`    | CLIP      | 提供從檢查點載入的 CLIP 模型組件（若可用且已請求）。 |
| `vae`     | VAE       | 提供從檢查點載入的 VAE 模型組件（若可用且已請求）。 |