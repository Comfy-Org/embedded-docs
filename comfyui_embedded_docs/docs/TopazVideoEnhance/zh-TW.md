...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `video` | VIDEO | 是 | - | 要增強的輸入影片檔案。 |
| `upscaler_enabled` | BOOLEAN | 是 | - | 啟用或停用影片放大功能（預設值：True）。 |
| `upscaler_model` | COMBO | 是 | `"Starlight (Astra) Fast"`<br>`"Starlight (Astra) Creative"`<br>`"Starlight Precise 2.5"` | 用於放大影片的 AI 模型。 |
| `upscaler_resolution` | COMBO | 是 | `"FullHD (1080p)"`<br>`"4K (2160p)"` | 放大影片的目標解析度。 |
| `upscaler_creativity` | COMBO | 否 | `"low"`<br>`"middle"`<br>`"high"` | 創意程度（僅適用於 Starlight (Astra) Creative）。（預設值："low"） |
| `interpolation_enabled` | BOOLEAN | 否 | - | 啟用或停用影格插值功能（預設值：False）。 |
| `interpolation_model` | COMBO | 否 | `"apo-8"` | 用於影格插值的模型（預設值："apo-8"）。 |
| `interpolation_slowmo` | INT | 否 | 1 到 16 | 套用至輸入影片的慢動作倍數。例如，2 會使輸出速度減半並使時長加倍。（預設值：1） |
| `interpolation_frame_rate` | INT | 否 | 15 到 240 | 輸出影格率。（預設值：60） |
| `interpolation_duplicate` | BOOLEAN | 否 | - | 分析輸入中的重複影格並將其移除。（預設值：False） |
| `interpolation_duplicate_threshold` | FLOAT | 否 | 0.001 到 0.1 | 重複影格的偵測敏感度。（預設值：0.01） |
