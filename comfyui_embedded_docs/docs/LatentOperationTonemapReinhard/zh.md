...
LatentOperationTonemapReinhard 对潜在向量应用 Reinhard 色调映射。该技术对潜在向量进行归一化，并基于幅度均值和标准差，通过统计方法调整其幅度，强度由 `multiplier` 参数控制。

## 输入

| 参数 | 描述 | 数据类型 | 必填 | 范围 |
| --- | --- | --- | --- | --- |
| `multiplier` | 控制色调映射效果的强度（默认值：1.0） | FLOAT | 是 | 0.0 到 100.0 |
