...
OpenAI DALL·E 2 generates images synchronously via OpenAI's DALL·E 2 endpoint. Provide a text prompt to create new images, or provide both an image and a mask to edit an existing image.
...
Note: `image` and `mask` must be provided together. When both are provided, the node switches to image editing mode. If only one of them is provided, an error is raised. The `mask` must be the same size as the `image`.
