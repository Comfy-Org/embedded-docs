...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `正向` | CONDITIONING | 是 | - | 正向條件化輸入。 |
| `負向` | CONDITIONING | 是 | - | 負向條件化輸入。 |
| `vae` | VAE | 是 | - | 用於編碼影像和影片幀的 VAE 模型。 |
| `寬度` | INT | 是 | 32 至 MAX_RESOLUTION | 輸出影片的寬度（像素），預設值：512。必須能被 8 整除。 |
| `高度` | INT | 是 | 32 至 MAX_RESOLUTION | 輸出影片的高度（像素），預設值：896。必須能被 8 整除。 |
| `長度` | INT | 是 | 1 至 MAX_RESOLUTION | 影片的幀數，預設值：81。必須能被 4 整除。 |
| `批次大小` | INT | 是 | 1 至 4096 | 每批次生成的影片數量，預設值：1。 |
| `clip_vision_output` | CLIP_VISION_OUTPUT | 否 | - | 可選的 CLIP 視覺輸出，用於條件化。 |
| `參考圖片` | IMAGE | 否 | - | 可選的參考影像，用於條件化。 |
| `姿勢影片` | IMAGE | 否 | - | 用於姿態條件化的影片。將被縮小至主影片解析度的一半。 |
| `姿勢強度` | FLOAT | 是 | 0.0 至 10.0 | 姿態潛在的強度，預設值：1.0。 |
| `姿勢起始步驟` | FLOAT | 是 | 0.0 至 1.0 | 開始使用姿態條件化的步驟，預設值：0.0。 |
| `姿勢結束步驟` | FLOAT | 是 | 0.0 至 1.0 | 結束使用姿態條件化的步驟，預設值：1.0。 |
...
| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `負向` | CONDITIONING | 修改後的正向條件化，可能包含嵌入的參考影像潛在、CLIP 視覺輸出或姿態影片潛在。 |
| `latent` | CONDITIONING | 修改後的負向條件化，可能包含嵌入的參考影像潛在、CLIP 視覺輸出或姿態影片潛在。 |
