...
| `length` | INT | Oui | 1 à MAX_RESOLUTION | Le nombre de trames dans la vidéo (par défaut : 81). Doit être divisible par 4. |
...
| `pose_video` | IMAGE | Non | - | Vidéo utilisée pour le conditionnement de pose. Sera réduite à la moitié de la résolution de la vidéo principale. |
...
|---------------|-----------------|-------------|
| `positive` | CONDITIONING | Le conditionnement positif modifié, contenant potentiellement des latents d'image de référence intégrés, une sortie CLIP vision ou des latents de vidéo de pose. |
| `negative` | CONDITIONING | Le conditionnement négatif modifié, contenant potentiellement des latents d'image de référence intégrés, une sortie CLIP vision ou des latents de vidéo de pose. |
