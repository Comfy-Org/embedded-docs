...
LatentApplyOperation 節點會對潛在樣本套用指定的潛在操作。它接收潛在資料與一個操作作為輸入，使用提供的操作處理潛在樣本，並回傳修改後的潛在資料。此節點讓您能在工作流程中轉換或操作潛在表示。此節點目前標記為實驗性。
...
| `操作` | 要套用於潛在樣本的操作 | LATENT_OPERATION | 是 | - |

## 輸出

| 輸出名稱 | 說明 | 資料類型 |
| --- | --- | --- |
| `output` | 套用操作後修改過的潛在樣本 | LATENT |
