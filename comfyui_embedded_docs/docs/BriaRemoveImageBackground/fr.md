...
| `moderation` | Paramètres de modération. Lorsqu'il est défini sur `"true"`, des options de modération supplémentaires deviennent disponibles. | COMBO | Non | `"false"`<br>`"true"` |
...
| `seed` | La graine contrôle si le nœud doit s'exécuter à nouveau ; les résultats sont non déterministes quelle que soit la graine. Défaut : `0`. | INT | Non | 0 à 2147483647 |
