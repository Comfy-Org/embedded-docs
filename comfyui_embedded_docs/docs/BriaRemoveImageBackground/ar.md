...
|-----------|-------------|-----------|----------|-------|
| `صورة` | الصورة المدخلة التي ستُزال خلفيتها. | IMAGE | نعم | - |
| `الإشراف` | إعدادات المراقبة. عند ضبطه على `"true"`، تصبح خيارات مراقبة إضافية متاحة. | DYNAMIC_COMBO | نعم | `"false"`<br>`"true"` |
| `البذرة` | يتحكم `seed` في ما إذا كان ينبغي إعادة تشغيل العقدة؛ النتائج غير حتمية بغض النظر عن `seed`. الافتراضي: `0`. | INT | نعم | 0 إلى 2147483647 |
