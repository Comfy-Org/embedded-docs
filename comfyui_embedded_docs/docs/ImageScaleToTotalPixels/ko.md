...
| `확대 방법` | COMBO[STRING] | 이미지 업스케일링에 사용되는 방법입니다. 업스케일링된 이미지의 품질과 특성에 영향을 미칩니다. |
| `메가픽셀수` | `FLOAT` | 메가픽셀 단위의 대상 이미지 크기입니다. 업스케일링된 이미지의 총 픽셀 수를 결정합니다. |

## 출력

| 매개변수 | 데이터 타입 | 설명 |
|---|---|---|
| `이미지` | `IMAGE` | 원본 가로세로 비율을 유지하면서 지정된 총 픽셀 수로 업스케일링된 이미지입니다. |