...
| `méthode_d'agrandissement`| COMBO[STRING]   | La méthode utilisée pour le suréchantillonnage de l'image. Elle affecte la qualité et les caractéristiques de l'image suréchantillonnée. |
| `mégapixels`    | `FLOAT`         | La taille cible de l'image en mégapixels. Cela détermine le nombre total de pixels dans l'image suréchantillonnée. |
