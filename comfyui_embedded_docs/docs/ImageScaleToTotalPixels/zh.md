...
| 参数             | 数据类型        | 描述                                                       |
|-----------------|-----------------|------------------------------------------------------------|
| `image`         | `IMAGE`         | 需要放大至指定总像素数的输入图像。                         |
| `upscale_method`| COMBO[STRING]   | 用于放大图像的方法。该方法会影响放大后图像的质量和特性。 |
| `megapixels`    | `FLOAT`         | 以百万像素为单位的图像目标尺寸。此参数决定放大后图像的总像素数。 |

## 输出

| 参数   | 数据类型 | 描述                                                           |
|--------|----------|----------------------------------------------------------------|
| `image` | `IMAGE`  | 放大后的图像，具有指定的总像素数，并保持原始宽高比。           |