...
Le nœud DisableNoise fournit une configuration de bruit vide qui peut être utilisée pour désactiver la génération de bruit dans les processus d’échantillonnage. Il renvoie un objet de bruit spécial qui ne contient aucune donnée de bruit, permettant à d’autres nœuds d’ignorer les opérations liées au bruit lorsqu’ils sont connectés à cette sortie. Ce nœud est également accessible par l’alias « zero noise ».
