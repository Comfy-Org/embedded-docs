...
DisableNoise 節點提供一個空的雜訊設定，可用於在採樣過程中停用雜訊生成。它會回傳一個不包含任何雜訊資料的特殊雜訊物件，讓其他節點在連接到此輸出時可以跳過與雜訊相關的操作。此節點也可以透過別名「zero noise」搜尋。
...
| 輸出名 | 描述 | 資料類型 |
| --- | --- | --- |
| `NOISE` | 回傳一個空的雜訊設定，可用於在採樣過程中停用雜訊生成。 | NOISE |
