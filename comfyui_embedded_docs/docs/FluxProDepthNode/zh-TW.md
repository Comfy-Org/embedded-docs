...
| `prompt_upsampling` | BOOLEAN | 否 | - | 是否對提示進行上取樣。若啟用，會自動修改提示以產生更具創意的生成結果，但結果不具確定性（相同種子不會產生完全相同的結果）。（預設值：False） |
| `skip_preprocessing` | BOOLEAN | 否 | - | 是否跳過預處理；若 `control_image` 已為深度圖則設為 True，若為原始影像則設為 False。（預設值：False） |
| `guidance` | FLOAT | 否 | 1-100 | 影像生成過程的引導強度（預設值：15） |
| `steps` | INT | 否 | 15-50 | 影像生成過程的步數（預設值：50） |
| `seed` | INT | 否 | 0-18446744073709551615 | 用於建立雜訊的隨機種子。（預設值：0） |

## 輸出

| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `output_image` | IMAGE | 根據深度控制影像和提示生成的影像 |
