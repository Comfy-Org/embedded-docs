...
| `dim` | COMBO | はい | `"x"`<br>`"-x"`<br>`"y"`<br>`"-y"`<br>`"t"`<br>`"-t"` | 潜在サンプルを連結する次元。正の値（x、y、t）は結果内で samples1 を samples2 の前に配置します。負の値（-x、-y、-t）は samples2 を samples1 の前に配置します。次元のマッピングは次のとおりです：x = 幅、y = 高さ、t = 時間/フレーム |
