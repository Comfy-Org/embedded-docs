...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `samples1` | LATENT | 是 | - | 要連接的第一個潛在樣本 |
| `samples2` | LATENT | 是 | - | 要連接的第二個潛在樣本 |
| `dim` | COMBO | 是 | `"x"`<br>`"-x"`<br>`"y"`<br>`"-y"`<br>`"t"`<br>`"-t"` | 用於連接潛在樣本的維度。正值 (x, y, t) 會將 samples1 放在 samples2 之前。負值 (-x, -y, -t) 會將 samples2 放在 samples1 之前。維度對應關係為：x = 寬度，y = 高度，t = 時間/影格 |
...
| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `output` | LATENT | 沿著指定維度組合兩個輸入樣本後所產生的連接潛在樣本 |
