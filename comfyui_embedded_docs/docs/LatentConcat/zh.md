...
|-----------|-----------|----------|-------|-------------|
| `samples1` | LATENT | 是 | - | 要拼接的第一个潜在样本 |
| `samples2` | LATENT | 是 | - | 要拼接的第二个潜在样本 |
| `dim` | COMBO | 是 | `"x"`<br>`"-x"`<br>`"y"`<br>`"-y"`<br>`"t"`<br>`"-t"` | 拼接潜在样本的维度。正值（x、y、t）表示结果中 samples1 排在 samples2 之前。负值（-x、-y、-t）表示 samples2 排在 samples1 之前。维度映射关系为：x = 宽度，y = 高度，t = 时间/帧数 |
