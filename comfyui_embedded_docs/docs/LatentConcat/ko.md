...
|-----------|-----------|----------|-------|-------------|
| `samples1` | LATENT | 예 | - | 연결할 첫 번째 잠재 샘플입니다. |
| `samples2` | LATENT | 예 | - | 연결할 두 번째 잠재 샘플입니다. |
| `dim` | COMBO | 예 | `"x"`<br>`"-x"`<br>`"y"`<br>`"-y"`<br>`"t"`<br>`"-t"` | 잠재 샘플을 연결할 차원입니다. 양수 값(x, y, t)은 결과에서 samples1을 samples2 앞에 배치합니다. 음수 값(-x, -y, -t)은 samples2를 samples1 앞에 배치합니다. 차원 매핑은 다음과 같습니다: x = 너비, y = 높이, t = 시간/프레임 |
...
| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `output` | LATENT | 지정된 차원을 따라 두 입력 샘플을 결합하여 생성된 연결된 잠재 샘플입니다. |
