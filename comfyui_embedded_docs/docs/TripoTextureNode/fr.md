...
| `pbr` | Indique s'il faut générer des matériaux PBR (rendu physiquement réaliste) (par défaut : True) | BOOLEAN | Non | - |
...
| `texture_prompt` | Guidance textuelle facultative pour la texturation. Requise en pratique pour les modèles importés (Tripo : Import Model), qui ne comportent pas d'image source pour en déduire les couleurs. (par défaut : "") | STRING | Non | - |
...
|---------------|-------------|-----------------|
| `fichier_modèle` | Le fichier de modèle généré avec les textures appliquées (pour la rétrocompatibilité uniquement) | STRING |
| `modèle task_id` | L'ID de tâche pour suivre le processus de génération de texture | MODEL_TASK_ID |
