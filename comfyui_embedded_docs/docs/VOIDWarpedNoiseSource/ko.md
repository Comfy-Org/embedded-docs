...
이 노드는 LATENT(VOIDWarpedNoise 노드의 출력 등)를 NOISE 소스로 변환합니다. 이를 통해 SamplerCustomAdvanced 노드와 함께 워프된 노이즈를 사용하여 보다 정밀하게 제어된 이미지를 생성할 수 있습니다.

## 입력

| 매개변수 | 설명 | 데이터 유형 | 필수 여부 | 범위 |
| --- | --- | --- | --- | --- |
| `warped_noise` | VOIDWarpedNoise 노드에서 출력된 워프된 노이즈 LATENT | LATENT | 예 | 해당 없음 |
