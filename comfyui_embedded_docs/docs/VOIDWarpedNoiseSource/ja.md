# VOIDWarpedNoiseSource

## 概要

このノードは、LATENT（例：VOIDWarpedNoise ノードの出力）を NOISE ソースに変換します。これにより、ワープされたノイズを SamplerCustomAdvanced ノードで使用して、より制御された画像生成を行うことができます。

## 入力

| パラメータ | 説明 | データ型 | 必須 | 範囲 |
| --- | --- | --- | --- | --- |
| `warped_noise` | VOIDWarpedNoise から取得したワープ済みノイズの LATENT | LATENT | Yes | N/A |

## 出力

| 出力名 | 説明 | データ型 |
| --- | --- | --- |
| `NOISE` | SamplerCustomAdvanced で使用できるノイズソース | NOISE |

> このドキュメントは AI によって生成されました。エラーを見つけた場合や改善のご提案がある場合は、ぜひ貢献してください！ [GitHub で編集](https://github.com/Comfy-Org/embedded-docs/blob/main/comfyui_embedded_docs/docs/VOIDWarpedNoiseSource/ja.md)

---
**Source fingerprint (SHA-256):** `61d7c82cb8a2acba28f980c4c42c6d4be12788b27676a5d30885799cf9c36185`
