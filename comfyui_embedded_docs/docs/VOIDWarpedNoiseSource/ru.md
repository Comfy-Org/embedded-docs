...
Этот узел преобразует LATENT (например, выходной результат узла VOIDWarpedNoise) в источник NOISE. Это позволяет использовать искаженный шум с узлом SamplerCustomAdvanced для более контролируемой генерации изображений.

## Входы

| Параметр | Описание | Тип данных | Требуется | Диапазон |
| --- | --- | --- | --- | --- |
| `warped_noise` | Искаженный шумовой latent из узла VOIDWarpedNoise | LATENT | Да | N/A |
