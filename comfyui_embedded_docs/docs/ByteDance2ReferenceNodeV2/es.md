...
ByteDance Seedance 2.5 Reference to Video genera, edita o extiende videos usando modelos ByteDance Seedance (Seedance 2.5, 2.5 Draft, 2.0, 2.0 Fast y 2.0 Mini) guiados por un prompt de texto y, opcionalmente, imágenes, videos, audio o recursos de biblioteca previamente subidos como referencias. Sube las referencias, envía una tarea de generación, espera a que finalice y devuelve el archivo de video terminado. Seleccionar `Seedance 2.5 Draft` genera una vista previa rápida en 480p; conecta el `draft_task_id` resultante al nodo ByteDance Seedance 2.5 Draft to Final Video para renderizar el final en 1080p.
...
| `task_type` | Qué hacer con los medios de referencia. Cada valor excepto auto se valida cuando se envía la tarea, por lo que las configuraciones incorrectas fallan antes de que comience la generación.<br>auto: el modelo infiere la tarea a partir del prompt y las entradas, y las configuraciones que entren en conflicto con su interpretación fallan solo después de que haya comenzado la generación.<br>reference: genera un video nuevo guiado por las imágenes, videos y audios de referencia.<br>edit: modifica un video de referencia conectado (agrega, elimina o reemplaza); la salida conserva la duración y la relación de aspecto del clip de origen, y los widgets `duration` y `ratio` se ignoran.<br>extend: continúa un video de referencia conectado hacia adelante o hacia atrás; el prompt debe decir "extend forward", "extend backward" o "continue", la relación de aspecto sigue el clip de origen, y la salida contiene solo el segmento recién generado con la duración que establezcas, no el clip de origen. Predeterminado: auto. | COMBO | Sí | "auto"<br>"reference"<br>"edit"<br>"extend" |
...
| Parámetro | Descripción | Tipo de dato | Obligatorio | Rango |
|-----------|-------------|--------------|-------------|-------|
| `reference_images` | Ranura ampliable: conecta de 1..N imágenes de referencia que guían la salida. El límite de cantidad depende de cada modelo (consulta las secciones de modelos). Las imágenes se validan por relación de aspecto (0.4 a 2.5) y se reducen automáticamente a un lado máximo de 6000 píxeles. | IMAGE | No | 1..9 ranuras (familia Seedance 2.0)<br>1..30 ranuras (Seedance 2.5) |
| `reference_videos` | Ranura ampliable: conecta de 1..N videos de referencia. El límite de cantidad depende de cada modelo (consulta las secciones de modelos). Cada video debe tener al menos 1.8 segundos de duración y debe ajustarse a los límites de píxeles del modelo y la resolución seleccionados. | VIDEO | No | 1..3 ranuras (familia Seedance 2.0)<br>1..10 ranuras (Seedance 2.5) |
| `reference_audios` | Ranura ampliable: conecta de 1..N pistas de audio de referencia. El límite de cantidad depende de cada modelo (consulta las secciones de modelos). Cada audio debe tener al menos 1.8 segundos de duración. | AUDIO | No | 1..3 ranuras (familia Seedance 2.0)<br>1..10 ranuras (Seedance 2.5) |
| `reference_assets` | Ranura ampliable: conecta de 1..N cadenas de ID de recursos para medios ya subidos a la biblioteca virtual de Seedance. Cada recurso debe estar en estado Active. Puedes hacer referencia a un recurso en el prompt con tokens como `asset1` o `asset 1`; el nodo los reemplaza con la etiqueta posicional del recurso (por ejemplo, "Image 2" o "Video 1"). | STRING | No | 1..9 ranuras (familia Seedance 2.0)<br>1..30 ranuras (Seedance 2.5) |
...
| `draft_task_id` | ID de tarea de la ejecución del borrador. Solo el modelo Seedance 2.5 Draft lo produce; conéctalo al nodo ByteDance Seedance 2.5 Draft to Final Video para renderizar el final en 1080p. | STRING |
