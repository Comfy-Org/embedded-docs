...
| `semilla` | Un valor de semilla para la generación de números aleatorios que garantiza resultados reproducibles (valor predeterminado: 1). | INT | Sí | 0 a 2147483647 |
| `resolución` | La resolución del video de salida. | COMBO | Sí | "720p"<br>"1080p" |
| `fotogramas` | Número de transiciones de fotogramas clave (2-9). Seleccionar un valor revela dinámicamente las entradas requeridas para cada fotograma. | DYNAMIC_COMBO | Sí | "2"<br>"3"<br>"4"<br>"5"<br>"6"<br>"7"<br>"8"<br>"9" |

### Entradas de fotogramas (compartidas por todas las opciones de cantidad de fotogramas)

Cuando `frames` se establece en un número, se muestran las siguientes tres entradas para cada fotograma `i` desde 1 hasta ese número. Por ejemplo, al elegir "3" se agregan `prompt1` / `end_image1` / `duration1`, `prompt2` / `end_image2` / `duration2` y `prompt3` / `end_image3` / `duration3`.
...
| `duration{i}` | Duración para el segmento {i} en segundos (valor predeterminado: 4). | INT | Sí | 2 a 7 |
...
- La cantidad de fotogramas puede estar entre 2 y 9, por lo que los índices de fotograma posibles van de 1 a 9.
