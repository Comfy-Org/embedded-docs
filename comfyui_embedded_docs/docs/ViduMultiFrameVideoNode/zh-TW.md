...
| `start_image` | IMAGE | 是 | - | 起始幀影像。長寬比必須介於 1:4 到 4:1 之間。 |
| `seed` | INT | 否 | 0 到 2147483647 | 隨機數生成的種子值，用於確保結果可重現（預設值：1）。 |
| `resolution` | COMBO | 是 | `"720p"`<br>`"1080p"` | 輸出影片的解析度。 |
| `frames` | DYNAMICCOMBO | 是 | `"2"`<br>`"3"`<br>`"4"`<br>`"5"`<br>`"6"`<br>`"7"`<br>`"8"`<br>`"9"` | 關鍵影格轉場數量（2-9）。選取值後會動態顯示每個影格所需的輸入項目。 |
