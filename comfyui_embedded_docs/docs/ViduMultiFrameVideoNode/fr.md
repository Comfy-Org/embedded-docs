...
| `image_de_départ` | L'image de départ. Le rapport d'aspect doit être compris entre 1:4 et 4:1. | IMAGE | Oui | Rapport d'aspect de 1:4 à 4:1 |
| `graine` | Une valeur de seed pour la génération de nombres aléatoires afin d'obtenir des résultats reproductibles (par défaut : 1). | INT | Oui | 0 à 2147483647 |
| `résolution` | La résolution de la vidéo de sortie. | COMBO | Oui | `"720p"`<br>`"1080p"` |
| `images` | Nombre de transitions d'images clés (2-9). La sélection d'une valeur révèle dynamiquement les entrées requises pour chaque image. | DYNAMIC_COMBO | Oui | `"2"`<br>`"3"`<br>`"4"`<br>`"5"`<br>`"6"`<br>`"7"`<br>`"8"`<br>`"9"` |

### Entrées par image (communes à toutes les options de nombre d'images)
...
| Paramètre | Description | Type de données | Requis | Plage |
|-----------|-------------|-----------------|--------|-------|
| `prompt{i}` | Prompt textuel pour la transition de l'image {i}. Champ de texte multiligne. Maximum 2000 caractères. | STRING | Oui | Jusqu'à 2000 caractères |
| `end_image{i}` | Image de fin pour le segment {i}. Le rapport d'aspect doit être compris entre 1:4 et 4:1. | IMAGE | Oui | Rapport d'aspect de 1:4 à 4:1 |
| `duration{i}` | Durée du segment {i} en secondes. | INT | Oui | 2 à 7 (par défaut : 4) |
