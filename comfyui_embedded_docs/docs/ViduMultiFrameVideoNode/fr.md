...
| `start_image` | L'image de départ. Le rapport d'aspect doit être compris entre 1:4 et 4:1. | IMAGE | Oui | Rapport d'aspect de 1:4 à 4:1 |
| `seed` | Une valeur de seed pour la génération de nombres aléatoires afin d'obtenir des résultats reproductibles (par défaut : 1). | INT | Oui | 0 à 2147483647 |
| `resolution` | La résolution de la vidéo de sortie. | COMBO | Oui | `"720p"`<br>`"1080p"` |
| `frames` | Nombre de transitions d'images clés (2-9). La sélection d'une valeur révèle dynamiquement les entrées requises pour chaque image. | DYNAMIC COMBO | Oui | `"2"`<br>`"3"`<br>`"4"`<br>`"5"`<br>`"6"`<br>`"7"`<br>`"8"`<br>`"9"` |
