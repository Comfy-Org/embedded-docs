...
Este nó gera um vídeo a partir de uma descrição de texto usando o modelo mais recente da Kling AI. Ele envia seu prompt para uma API remota e retorna o vídeo gerado. Você pode controlar a duração, o formato e a qualidade do vídeo e, opcionalmente, criar storyboards com vários planos.
...
| `aspect_ratio` | O formato ou as dimensões do vídeo a ser gerado. | COMBO | Sim | `"16:9"`<br>`"9:16"`<br>`"1:1"` |
...
Quando `storyboards` é definido com um valor diferente de `"disabled"`, as seguintes entradas aparecem para cada segmento de storyboard. Nos nomes de parâmetros abaixo, `{i}` é o número do segmento, de 1 até a contagem de storyboards selecionada.

| Parâmetro | Descrição | Tipo de dado | Obrigatório | Intervalo |
|-----------|-------------|-----------|----------|-------|
| `storyboard_{i}_prompt` | Prompt para o segmento de storyboard {i}. Máximo de 512 caracteres. | STRING | Sim | 1 a 512 caracteres |
| `storyboard_{i}_duration` | Duração do segmento de storyboard {i} em segundos (padrão: 4). | INT | Sim | 1 a 15 segundos |

### Restrições e limitações dos parâmetros

- **Limitações específicas do modelo:**
  - O modelo `kling-video-o1` só oferece suporte a durações de **5 ou 10 segundos**.
  - O modelo `kling-video-o1` **não** oferece suporte à geração de áudio.
  - O modelo `kling-video-o1` **não** oferece suporte à resolução 4k.
  - O modelo `kling-video-o1` **não** oferece suporte a storyboards.
- **Restrições de storyboard:**
  - Quando os storyboards estão habilitados, o campo `prompt` é ignorado.
  - Cada storyboard exige seu próprio prompt (1 a 512 caracteres) e duração.
...
|-------------|-------------|-----------|
| `output` | O vídeo gerado com base no prompt de texto e nas configurações fornecidos. | VIDEO |
