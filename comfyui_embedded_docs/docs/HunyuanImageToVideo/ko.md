...
| `width` | INT | 예 | 16 ~ MAX_RESOLUTION | 출력 비디오의 너비 (단위: 픽셀, 기본값: 848, 단계: 16) |
| `height` | INT | 예 | 16 ~ MAX_RESOLUTION | 출력 비디오의 높이 (단위: 픽셀, 기본값: 480, 단계: 16) |
| `length` | INT | 예 | 1 ~ MAX_RESOLUTION | 출력 비디오의 프레임 수 (기본값: 53, 단계: 4) |
| `batch_size` | INT | 예 | 1 ~ 4096 | 동시에 생성할 비디오 수 (기본값: 1) |
| `guidance_type` | COMBO | 예 | "v1 (concat)"<br>"v2 (replace)"<br>"custom" | 시작 이미지를 비디오 생성에 통합하는 방법 |
| `start_image` | IMAGE | 아니오 | - | 비디오 생성을 초기화하는 선택적 시작 이미지 |
...
| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `positive` | CONDITIONING | start_image가 제공될 경우 이미지 가이던스가 적용된 수정된 긍정적 조건 |
