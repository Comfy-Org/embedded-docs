...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `clip` | CLIP | 是 | - | 用於文字分詞和編碼的 CLIP 模型實例，是生成條件的核心元件。 |
| `bert` | STRING | 是 | - | 透過 BERT 分詞器進行編碼的文字輸入。建議使用短語和關鍵詞。支援多行和動態提示詞。 |
| `mt5xl` | STRING | 是 | - | 透過 mT5-XL 分詞器進行編碼的文字輸入。支援多行和動態提示詞（多語言）。可使用完整句子和複雜描述。 |

## 輸出

| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `CONDITIONING` | CONDITIONING | 編碼後的條件輸出，結合了 BERT 和 mT5-XL 分詞器的文字處理結果，用於生成任務的後續處理。 |
