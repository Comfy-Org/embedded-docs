...
|-----------|-----------|----------|-------|-------------|
| `model_3d` | FILE3D | 예 | GLB<br>OBJ<br>FBX | 입력 3D 모델(GLB, OBJ 또는 FBX)입니다. 모델의 면 수는 30,000개 미만이어야 합니다. |
| `seed` | INT | 아니요 | 0 ~ 2147483647 | 시드 값(기본값: 1)입니다. 이 값은 노드 재실행 여부를 제어하지만, 시드 값과 관계없이 결과는 비결정적입니다. |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `OBJ` | FILE3D | OBJ 형식으로 처리된 3D 모델 파일입니다. |
| `FBX` | FILE3D | FBX 형식으로 처리된 3D 모델 파일입니다. |
