...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `model_3d` | FILE3D | 是 | GLB<br>OBJ<br>FBX | 輸入的 3D 模型（GLB、OBJ 或 FBX 格式）。模型面數必須少於 30,000 個。 |
| `seed` | INT | 否 | 0 到 2147483647 | 種子值（預設值：1）。此參數控制節點是否應重新執行，但無論種子值為何，結果皆為非確定性。 |

## 輸出

| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `OBJ` | FILE3D | 以 OBJ 格式處理後的 3D 模型檔案。 |
| `FBX` | FILE3D | 以 FBX 格式處理後的 3D 模型檔案。 |
| `uv_image` | IMAGE | 生成的 UV 紋理影像。 |
