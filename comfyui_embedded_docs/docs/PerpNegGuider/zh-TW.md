...
| `正向` | CONDITIONING | 是 | - | 正面條件化，引導生成朝向期望的內容 |
| `負向` | CONDITIONING | 是 | - | 負面條件化，引導生成遠離不想要的內容 |
| `空條件` | CONDITIONING | 是 | - | 空或中性的條件化，用作基準參考 |
| `cfg` | FLOAT | 是 | 0.0 - 100.0 | 無分類器引導尺度，控制條件化影響生成的程度（預設值：8.0） |
| `負向比例` | FLOAT | 是 | 0.0 - 100.0 | 負面縮放因子，調整負面條件化的強度（預設值：1.0） |
