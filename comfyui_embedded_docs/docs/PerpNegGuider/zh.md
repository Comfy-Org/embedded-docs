...
| 参数 | 数据类型 | 是否必填 | 范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `model` | MODEL | 是 | - | 用于引导生成的模型 |
| `positive` | CONDITIONING | 是 | - | 正向条件，引导生成朝向期望内容 |
| `negative` | CONDITIONING | 是 | - | 负向条件，引导生成远离不期望内容 |
| `empty_conditioning` | CONDITIONING | 是 | - | 空条件或中性条件，用作基线参考 |
| `cfg` | FLOAT | 是 | 0.0 - 100.0 | 无分类器引导尺度，控制条件对生成的影响强度（默认值：8.0） |
| `neg_scale` | FLOAT | 是 | 0.0 - 100.0 | 负向缩放因子，调整负向条件的强度（默认值：1.0） |
