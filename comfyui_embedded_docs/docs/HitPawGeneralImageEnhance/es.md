...
|-----------|--------------|-------------|-------|-------------|
| `modelo` | STRING | Sí | `"generative_portrait"`<br>`"generative"` | El modelo de mejora a utilizar. El modelo `generative_portrait` está optimizado para retratos, mientras que `generative` es un modelo de propósito general. |
| `imagen` | IMAGE | Sí | - | La imagen de entrada que se va a mejorar. |
| `factor de escalado` | INT | Sí | `1`<br>`2`<br>`4` | El factor por el cual se ampliarán las dimensiones de la imagen. Un factor de 1 significa sin ampliación, 2 duplica las dimensiones y 4 las cuadruplica. |
| `reducción automática` | BOOLEAN | No | - | Reduce automáticamente la escala de la imagen de entrada si la salida supera el límite. Cuando está habilitado, el nodo intentará reducir el tamaño de la imagen de entrada para que se ajuste al límite de salida de 4 megapíxeles antes de aplicar el factor de ampliación solicitado. (predeterminado: `False`) |
...
|------------------|--------------|-------------|
| `imagen` | IMAGE | La imagen de salida mejorada y ampliada. |
