...
|-----------|-----------|------------|---------|-------|-------------|
| `prompt` | STRING | required | "" | - | 用于生成的文本提示 |
| `model` | COMBO | required | gemini_2_5_flash_image_preview | `gemini_2_5_flash_image_preview`<br>`gemini_2_5_pro_exp_03_25`<br>`gemini_2_0_flash_exp_image_generation` | 用于生成响应的 Gemini 模型。 |
| `seed` | INT | required | 42 | 0 到 18446744073709551615 | 当种子固定为特定值时，模型会尽力为重复请求提供相同的响应。不保证输出具有确定性。此外，更改模型或参数设置（例如温度）可能会导致响应发生变化，即使使用相同的种子值也是如此。默认情况下，使用随机种子值。 |
| `images` | IMAGE | optional | None | - | 可选图像，用作模型的上下文。要包含多个图像，可以使用“批处理图像”节点。 |
| `files` | GEMINI_INPUT_FILES | optional | None | - | 可选文件，用作模型的上下文。接受来自“Gemini 生成内容输入文件”节点的输入。 |
