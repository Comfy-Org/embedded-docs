...
| 參數 | 資料類型 | 必要 | 範圍 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `steps` | INT | 是 | 1 至 4096 | 要執行的去噪步數。較高的值通常會產生更詳細的結果，但處理時間更長（預設值：20）。 |
| `width` | INT | 是 | 16 至 16384 | 要生成的圖像寬度，單位為像素。此值會影響噪聲排程的計算（預設值：1024）。 |
| `height` | INT | 是 | 16 至 16384 | 要生成的圖像高度，單位為像素。此值會影響噪聲排程的計算（預設值：1024）。 |

## 輸出

| 輸出名稱 | 資料類型 | 描述 |
|-------------|-----------|-------------|
| `sigmas` | SIGMAS | 定義取樣器去噪排程的一系列噪聲級別值（sigmas）。 |
