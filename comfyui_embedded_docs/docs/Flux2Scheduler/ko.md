...
|-----------|-----------|----------|-------|-------------|
| `steps` | INT | 예 | 1 ~ 4096 | 수행할 노이즈 제거 단계 수입니다. 값이 클수록 일반적으로 더 세부적인 결과를 얻을 수 있지만 처리 시간이 더 오래 걸립니다(기본값: 20). |
| `width` | INT | 예 | 16 ~ 16384 | 생성할 이미지의 너비(픽셀 단위)입니다. 이 값은 노이즈 스케줄 계산에 영향을 미칩니다(기본값: 1024). |
| `height` | INT | 예 | 16 ~ 16384 | 생성할 이미지의 높이(픽셀 단위)입니다. 이 값은 노이즈 스케줄 계산에 영향을 미칩니다(기본값: 1024). |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `sigmas` | SIGMAS | 샘플러의 노이즈 제거 스케줄을 정의하는 노이즈 수준 값(sigmas)의 시퀀스입니다. |
