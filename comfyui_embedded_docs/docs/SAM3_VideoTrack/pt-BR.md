...
|-----------|--------------|-------------|-----------|-----------|
| `images` | IMAGE | Sim | Quadros de vídeo em lote | Quadros de vídeo como imagens em lote |
...
| `max_objects` | INT | Não | 0 a 64 (padrão: 0) | Máximo de objetos rastreados. Máscaras iniciais contam para este limite. 0 usa o limite interno de 64. |
| `detect_interval` | INT | Não | 1 a ilimitado (padrão: 1) | Executar detecção a cada N quadros (1 = a cada quadro). Valores maiores economizam processamento. |
