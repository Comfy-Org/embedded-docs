...
| `seuil_de_détection` | Seuil de score pour la détection déclenchée par prompt textuel (par défaut : 0,5) | FLOAT | Non | 0,0 à 1,0 (pas de 0,01) |
| `objets_max` | Nombre maximal d’objets suivis. Les masques initiaux comptent dans cette limite. 0 utilise la limite interne de 64. (par défaut : 4) | INT | Non | 0 à 64 |
| `intervalle_de_détection` | Exécuter la détection toutes les N images (1 = chaque image). Des valeurs plus élevées réduisent le coût de calcul. (par défaut : 1) | INT | Non | 1 ou plus |
