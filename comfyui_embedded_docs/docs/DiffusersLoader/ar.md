...
|---------|---------------|--------|---------|-------|
| `مسار النموذج` | STRING | نعم | تتوفر خيارات متعددة<br>(يتم تعبئتها تلقائيًا من مجلدات diffusers) | مسار دليل نموذج diffusers المراد تحميله. تقوم العقدة بمسح المجلدات المُهيأة تلقائيًا للبحث عن نماذج diffusers الصالحة وتعرض الخيارات المتاحة. |

## المخرجات

| اسم المخرج | نوع البيانات | الوصف |
|-------------|---------------|---------|
| `MODEL` | MODEL | مكون النموذج المُحمّل من تنسيق diffusers |
| `CLIP` | CLIP | مكون نموذج CLIP المُحمّل من تنسيق diffusers |
| `VAE` | VAE | مكون VAE (مُشفر ذاتي متغير) المُحمّل من تنسيق diffusers |
