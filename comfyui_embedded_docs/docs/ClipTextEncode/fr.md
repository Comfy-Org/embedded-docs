...
Considérez-le comme une communication avec un artiste qui parle une autre langue. Le modèle CLIP, entraîné sur de vastes paires image-texte, comble cet écart en convertissant vos descriptions en « instructions » que le modèle d’IA peut suivre.

## Entrées

| Paramètre | Description | Type de données | Requis | Plage |
| --- | --- | --- | --- | --- |
| `text` | Le texte à encoder. Prend en charge les entrées multilignes et les prompts dynamiques. | STRING | Oui | Tout texte |
| `clip` | Le modèle CLIP utilisé pour encoder le texte. | CLIP | Oui | Modèles CLIP chargés |

**Remarque** : l’entrée `clip` doit être un modèle CLIP valide. Si elle est `None`, le nœud génère une erreur. Cela se produit généralement lorsque le checkpoint chargé par un nœud de chargement de checkpoint ne contient pas de modèle CLIP ou d’encodeur de texte valide.
