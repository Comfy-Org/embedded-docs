...
Note: If the `clip` input is None (for example, when it comes from a checkpoint loader whose checkpoint does not contain a valid CLIP or text encoder model), the node raises an error.
