...
**Note**: The `clip` input must be a valid CLIP model. If it is `None`, the node raises an error. This typically happens when the checkpoint loaded by a checkpoint loader node does not contain a valid CLIP or text encoder model.
