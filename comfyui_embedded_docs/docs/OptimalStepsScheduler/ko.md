...
|-----------|-----------|----------|-------|-------------|
| `모델 종류` | COMBO | 예 | "FLUX"<br>"Wan"<br>"Chroma" | 노이즈 수준 계산에 사용할 확산 모델 유형 |
| `스텝` | INT | 예 | 3-1000 | 계산할 총 샘플링 단계 수 (기본값: 20) |
| `디노이즈` | FLOAT | 아니요 | 0.0-1.0 | 노이즈 제거 강도를 제어하며, 유효 단계 수를 조정합니다 (기본값: 1.0) |
...
| 출력 이름 | 데이터 유형 | 설명 |
|-------------|-----------|-------------|
| `sigmas` | SIGMAS | 확산 샘플링을 위한 노이즈 스케줄을 나타내는 시그마 값 시퀀스 |
