...
OptimalStepsScheduler 節點根據所選模型類型和步驟配置，為擴散模型計算噪聲調度 sigma 值。它會根據降噪參數調整總步數，並對噪聲級別進行插值以匹配請求的步數。該節點返回一個 sigma 值序列，用於決定擴散取樣過程中使用的噪聲級別。
...
| `步驟數` | INT | 是 | 3-1000 | 要計算的取樣總步數（預設值：20） |
...
| 輸出名稱 | 資料類型 | 輸出說明 |
|-------------|-----------|-------------|
| `sigmas` | SIGMAS | 代表擴散取樣噪聲調度的 sigma 值序列 |