...
Bria Eraser supprime des objets ou des zones d’une image à l’aide de l’API Bria. Vous fournissez une image et un masque qui délimite les régions à supprimer ; le nœud téléverse les deux vers Bria, exécute la tâche d’effacement, attend qu’elle se termine, puis renvoie l’image modifiée avec les zones masquées effacées.
...
- `visual_input_moderation` — applique la modération du contenu visuel à l’image d’entrée (par défaut : false)
- `visual_output_moderation` — applique la modération du contenu visuel à l’image de sortie (par défaut : false)

Remarque : Le masque doit correspondre au rapport d’aspect de l’image, sinon la requête échoue. Le masque est converti en masque binaire (noir et blanc) avant d’être envoyé à l’API : les zones peintes avec une opacité inférieure à la moitié sont ignorées, et les zones partiellement peintes sont traitées comme blanches et seront effacées. Le masque doit contenir au moins une zone blanche ; un masque vide fait échouer la requête car il n’y a rien à effacer.
