...
|-----------|-----------|----------|-------|-------------|
| `prompt` | STRING | Evet | - | Görüntü oluşturma veya düzenleme için istem (varsayılan: boş) |
| `image` | IMAGE | Hayır | - | Görüntü düzenleme için isteğe bağlı referans görüntüsü |
| `mask` | MASK | Hayır | - | İç boyama için isteğe bağlı maske (beyaz alanlar değiştirilecektir) |
| `aspect_ratio` | COMBO | Hayır | "1:1"<br>"16:9"<br>"9:16"<br>"4:3"<br>"3:4"<br>"3:2"<br>"2:3" | Görüntü oluşturma için en-boy oranı. Çözünürlük Otomatik olarak ayarlanmamışsa dikkate alınmaz (varsayılan: "1:1") |
| `resolution` | COMBO | Hayır | "Auto"<br>"1024x1024"<br>"1152x896"<br>"896x1152"<br>"1216x832"<br>"832x1216"<br>"1344x768"<br>"768x1344"<br>"1536x640"<br>"640x1536" | Görüntü oluşturma için çözünürlük. Otomatik olarak ayarlanmazsa, bu ayar `aspect_ratio` ayarını geçersiz kılar (varsayılan: "Auto") |
...
| `seed` | INT | Hayır | 0-2147483647 | Oluşturma için rastgele tohum (varsayılan: 0) |
| `num_images` | INT | Hayır | 1-8 | Oluşturulacak görüntü sayısı (varsayılan: 1) |
| `rendering_speed` | COMBO | Hayır | "DEFAULT"<br>"TURBO"<br>"QUALITY" | Oluşturma hızı ve kalitesi arasındaki dengeyi kontrol eder (varsayılan: "DEFAULT") |
