...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `提示詞` | STRING | 是 | - | 用於圖像生成或編輯的提示詞（預設：空） |
| `參考影像` | IMAGE | 否 | - | 用於圖像編輯的參考圖像（可選） |
| `遮罩` | MASK | 否 | - | 用於修復的遮罩（白色區域將被替換）（可選） |
| `長寬比` | COMBO | 否 | "1:1"<br>"16:9"<br>"9:16"<br>"4:3"<br>"3:4"<br>"3:2"<br>"2:3" | 圖像生成的長寬比。若解析度未設為自動則忽略此設定（預設："1:1"） |
| `解析度` | COMBO | 否 | "Auto"<br>"1024x1024"<br>"1152x896"<br>"896x1152"<br>"1216x832"<br>"832x1216"<br>"1344x768"<br>"768x1344"<br>"1536x640"<br>"640x1536" | 圖像生成的解析度。若未設為自動，則此設定會覆蓋長寬比設定（預設："Auto"） |
...
| `角色圖片` | IMAGE | 否 | - | 用作角色參考的圖像 |
| `角色遮罩` | MASK | 否 | - | 角色參考圖像的可選遮罩 |
