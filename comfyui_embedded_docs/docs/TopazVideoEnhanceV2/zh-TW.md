...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `影片` | VIDEO | 是 | - | 要處理的輸入影片。必須為 MP4 容器格式。 |
| `升頻模型` | COMBO | 是 | `"Astra 2"`<br>`"Starlight (Astra) Fast"`<br>`"Starlight (Astra) Creative"`<br>`"Starlight Precise 2.5"`<br>`"Disabled"` | 用於提升影片解析度的 AI 模型。選擇 "Disabled" 表示不套用任何升頻處理。 |
...
| `插值模型` | COMBO | 是 | `"Disabled"`<br>`"apo-8"` | 用於影格插值的 AI 模型。選擇 "Disabled" 表示不套用任何插值處理。 |
...
| `動態壓縮等級` | COMBO | 否 | `"Low"`<br>`"Mid"`<br>`"High"` | 影片壓縮的 CQP 等級。預設值："Low"。 |
...
| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `影片` | VIDEO | 套用所選升頻及/或插值濾鏡後的增強影片輸出。 |
