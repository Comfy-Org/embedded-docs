...
| `upscaler_model` | COMBO | Oui | `"Astra 2"`<br>`"Starlight (Astra) Fast"`<br>`"Starlight (Astra) Creative"`<br>`"Starlight Precise 2.5"`<br>`"Disabled"` | Le modèle d'IA utilisé pour suréchantillonner la vidéo. La sélection de "Disabled" signifie qu'aucun suréchantillonnage ne sera appliqué. |
