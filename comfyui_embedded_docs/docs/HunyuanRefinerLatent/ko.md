...
|-----------|-----------|----------|-------|-------------|
| `긍정적` | CONDITIONING | 예 | - | 처리할 양성 컨디셔닝 입력 |
| `부정적` | CONDITIONING | 예 | - | 처리할 음성 컨디셔닝 입력 |
| `잠재` | LATENT | 예 | - | 잠재 표현 입력 |
| `노이즈 증강` | FLOAT | 예 | 0.0 - 1.0 | 적용할 노이즈 증강량 (기본값: 0.10) |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `부정적` | CONDITIONING | 노이즈 증강 및 잠재 이미지 연결이 적용된 처리된 양성 컨디셔닝 |
| `잠재` | CONDITIONING | 노이즈 증강 및 잠재 이미지 연결이 적용된 처리된 음성 컨디셔닝 |
| `잠재` | LATENT | [배치_크기, 32, 높이, 너비, 채널] 차원의 새로운 잠재 출력 |
