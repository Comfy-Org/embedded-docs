...
| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `positive` | CONDITIONING | 노이즈 증강 및 잠재 이미지 연결이 적용된 처리된 양성 컨디셔닝 |
| `negative` | CONDITIONING | 노이즈 증강 및 잠재 이미지 연결이 적용된 처리된 음성 컨디셔닝 |
| `latent` | LATENT | [배치_크기, 32, 높이, 너비, 채널] 차원의 새로운 잠재 출력 |
