...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `正向提示` | CONDITIONING | 是 | - | 待處理的正向條件輸入 |
| `負向提示` | CONDITIONING | 是 | - | 待處理的負向條件輸入 |
| `潛空間` | LATENT | 是 | - | 潛在表示輸入 |
| `雜訊增強` | FLOAT | 是 | 0.0 - 1.0 | 要應用的噪聲增強量（預設值：0.10） |

## 輸出

| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `負向提示` | CONDITIONING | 已處理的正向條件，包含應用的噪聲增強與潛在影像串接 |
| `潛空間` | CONDITIONING | 已處理的負向條件，包含應用的噪聲增強與潛在影像串接 |
| `潛空間` | LATENT | 維度為 [batch_size, 32, height, width, channels] 的新潛在輸出 |
