...
HunyuanRefinerLatent 節點處理用於精細化操作的條件輸入和潛在輸入。它會對正向和負向條件輸入應用噪聲增強，同時結合潛在圖像數據，並生成具有特定維度的新潛在輸出以供後續處理。
...
| 輸出名稱 | 資料類型 | 輸出說明 |
|-------------|-----------|-------------|
| `positive` | CONDITIONING | 經過處理的正向條件輸入，已應用噪聲增強和潛在圖像串接 |
| `negative` | CONDITIONING | 經過處理的負向條件輸入，已應用噪聲增強和潛在圖像串接 |
