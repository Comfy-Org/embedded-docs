...
| `트랙` | TRACKS | 아니요 | - | 객체 경로가 포함된 선택적 모션 트래킹 데이터입니다. |
| `강도` | FLOAT | 아니요 | 0.0 - 100.0 | 트랙 컨디셔닝의 강도입니다. (기본값: 1.0) |
| `너비` | INT | 아니요 | 16 - MAX_RESOLUTION | 출력 비디오의 너비입니다. 16으로 나누어 떨어져야 합니다. (기본값: 832) |
| `높이` | INT | 아니요 | 16 - MAX_RESOLUTION | 출력 비디오의 높이입니다. 16으로 나누어 떨어져야 합니다. (기본값: 480) |
| `길이` | INT | 아니요 | 1 - MAX_RESOLUTION | 비디오 시퀀스의 프레임 수입니다. (기본값: 81) |
| `배치 크기` | INT | 아니요 | 1 - 4096 | 잠재 출력의 배치 크기입니다. (기본값: 1) |
| `시작 이미지` | IMAGE | 예 | - | 인코딩할 시작 이미지 또는 이미지 시퀀스입니다. |
| `clip 비전 출력` | CLIPVISIONOUTPUT | 아니요 | - | 컨디셔닝에 추가할 선택적 CLIP 비전 모델 출력입니다. |
...
| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `negative` | CONDITIONING | 수정된 포지티브 컨디셔닝으로, `concat_latent_image`, `concat_mask` 및 `clip 비전 출력`을 포함할 수 있습니다. |
| `latent` | CONDITIONING | 수정된 네거티브 컨디셔닝으로, `concat_latent_image`, `concat_mask` 및 `clip 비전 출력`을 포함할 수 있습니다. |
| `latent` | LATENT | `배치 크기`, `길이`, `높이` 및 `너비` 입력에 의해 차원이 결정된 빈 잠재 텐서입니다. |
