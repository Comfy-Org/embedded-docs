...
| 输出名称 | 描述 | 数据类型 |
| --- | --- | --- |
| `positive` | 修改后的正向 conditioning，可能包含 `concat_latent_image`、`concat_mask` 和 `CLIP视觉输出`。 | CONDITIONING |
| `negative` | 修改后的负向 conditioning，可能包含 `concat_latent_image`、`concat_mask` 和 `CLIP视觉输出`。 | CONDITIONING |
