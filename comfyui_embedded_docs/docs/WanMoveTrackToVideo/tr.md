...
|-----------|-----------|----------|--------|----------|
| `pozitif` | CONDITIONING | Evet | - | Değiştirilecek pozitif koşullandırma girişi. |
| `negatif` | CONDITIONING | Evet | - | Değiştirilecek negatif koşullandırma girişi. |
| `vae` | VAE | Evet | - | Başlangıç görüntüsünü gizli uzaya kodlamak için kullanılan VAE modeli. |
| `izler` | TRACKS | Hayır | - | Nesne yollarını içeren isteğe bağlı hareket takip verileri. |
| `güç` | FLOAT | Hayır | 0.0 - 100.0 | Takip koşullandırmasının gücü. (varsayılan: 1.0) |
| `genişlik` | INT | Hayır | 16 - MAX_RESOLUTION | Çıktı videosunun genişliği. 16'ya bölünebilir olmalıdır. (varsayılan: 832) |
| `yükseklik` | INT | Hayır | 16 - MAX_RESOLUTION | Çıktı videosunun yüksekliği. 16'ya bölünebilir olmalıdır. (varsayılan: 480) |
| `uzunluk` | INT | Hayır | 1 - MAX_RESOLUTION | Video dizisindeki kare sayısı. (varsayılan: 81) |
| `toplu_boyutu` | INT | Hayır | 1 - 4096 | Gizli çıktı için grup boyutu. (varsayılan: 1) |
| `başlangıç_görseli` | IMAGE | Evet | - | Kodlanacak başlangıç görüntüsü veya görüntü dizisi. |
...
|-----------|-----------|----------|
| `negatif` | CONDITIONING | Potansiyel olarak `concat_latent_image`, `concat_mask` ve `clip_vision_output` içeren değiştirilmiş pozitif koşullandırma. |
| `latent` | CONDITIONING | Potansiyel olarak `concat_latent_image`, `concat_mask` ve `clip_vision_output` içeren değiştirilmiş negatif koşullandırma. |
| `latent` | LATENT | Boyutları `toplu_boyutu`, `uzunluk`, `yükseklik` ve `genişlik` girişleri tarafından şekillendirilen boş bir gizli tensör. |
