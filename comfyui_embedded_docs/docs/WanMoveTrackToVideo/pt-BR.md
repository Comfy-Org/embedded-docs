...
| `strength` | FLOAT | Não | 0,0 - 100,0 | Intensidade do condicionamento do rastro. (padrão: 1,0) |
| `width` | INT | Não | 16 - RESOLUÇÃO_MÁXIMA | A largura do vídeo de saída. Deve ser divisível por 16. (padrão: 832) |
| `height` | INT | Não | 16 - RESOLUÇÃO_MÁXIMA | A altura do vídeo de saída. Deve ser divisível por 16. (padrão: 480) |
| `length` | INT | Não | 1 - RESOLUÇÃO_MÁXIMA | O número de quadros na sequência de vídeo. (padrão: 81) |
| `batch_size` | INT | Não | 1 - 4096 | O tamanho do lote para a saída latente. (padrão: 1) |
| `start_image` | IMAGE | Sim | - | A imagem ou sequência de imagens inicial a ser codificada. |
...
| `latent` | LATENT | Um tensor latente vazio com dimensões definidas pelas entradas `batch_size`, `length`, `height` e `width`. |
