...
Le nœud `ModelMergeFlux1` fusionne deux modèles de diffusion en combinant leurs composants par interpolation pondérée. Il permet un contrôle précis de la manière dont les différentes parties des modèles sont combinées, notamment les blocs de traitement d'image, les couches de plongement temporel, les mécanismes de guidage, les entrées vectorielles, les encodeurs de texte et divers blocs de transformeur. Cela permet de créer des modèles hybrides avec des caractéristiques personnalisées à partir de deux modèles sources.
...
| `time_in.` | Poids d'interpolation du plongement temporel (défaut : 1.0) | FLOAT | Oui | 0.0 à 1.0 |
