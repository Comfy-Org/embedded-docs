...
| 參數名稱       | 資料類型     | 描述 |
|----------------|-------------|------|
| `samples1`     | `LATENT`    | 要進行加法的第一組潛在樣本。它代表其中一個輸入，其特徵將與另一組潛在樣本進行組合。 |
| `samples2`     | `LATENT`    | 要進行加法的第二組潛在樣本。它作為另一個輸入，其特徵透過逐元素加法與第一組潛在樣本進行組合。 |

## 輸出參數

| 參數名稱    | 資料類型     | 描述 |
|------------|-------------|------|
| `latent`   | `LATENT`    | 兩個潛在樣本逐元素加法的結果，代表一個結合了兩個輸入特徵的新潛在樣本集合。 |