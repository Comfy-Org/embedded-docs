...
| 參數         | 資料類型    | 說明 |
|--------------|-------------|-------------|
| `samples1`   | `LATENT`    | 要相加的第一組潛在樣本。代表其中一個輸入，其特徵將與另一組潛在樣本進行組合。 |
| `samples2`   | `LATENT`    | 要相加的第二組潛在樣本。作為另一個輸入，其特徵透過逐元素加法與第一組潛在樣本進行組合。 |

## 輸出

| 參數      | 資料類型    | 說明 |
|-----------|-------------|-------------|
| `latent`  | `LATENT`    | 兩個潛在樣本逐元素加法的結果，代表一組結合了兩個輸入特徵的新潛在樣本。 |