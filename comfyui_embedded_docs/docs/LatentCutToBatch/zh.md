...
| 参数 | 数据类型 | 是否必填 | 范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `samples` | LATENT | 是 | - | 待分割并批处理的潜在表示。 |
| `dim` | COMBO | 是 | `"t"`<br>`"x"`<br>`"y"` | 沿此维度切割潜在样本。`"t"` 指时间维度，`"x"` 指宽度，`"y"` 指高度。 |
| `slice_size` | INT | 是 | 1 至 16384 | 从指定维度切割的每个切片的大小。如果该维度的大小不能被此值整除，则丢弃余数。（默认值：1） |

## 输出

| 输出名称 | 数据类型 | 描述 |
|-------------|-----------|-------------|
| `samples` | LATENT | 生成的潜在批次，包含切割并堆叠后的样本。 |
