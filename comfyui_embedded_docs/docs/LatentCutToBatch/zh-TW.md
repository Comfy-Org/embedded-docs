...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `樣本` | LATENT | 是 | - | 要進行分割與批次處理的潛在表示。 |
| `維度` | COMBO | 是 | `"t"`<br>`"x"`<br>`"y"` | 切割潛在樣本時所依據的維度。`"t"` 代表時間維度，`"x"` 代表寬度，`"y"` 代表高度。 |
| `切片大小` | INT | 是 | 1 至 16384 | 從指定維度切割的每個切片大小。如果該維度的大小無法被此值整除，則會捨棄餘數。（預設值：1） |

## 輸出

| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `樣本` | LATENT | 產生的潛在批次，包含已切片並堆疊的樣本。 |
