...
|-----------|-----------------|--------|-------|-------------|
| `samples` | LATENT | Oui | - | La représentation latente à diviser et à mettre en lot. |
| `dim` | COMBO | Oui | `"t"`<br>`"x"`<br>`"y"` | La dimension selon laquelle couper les échantillons latents. `"t"` fait référence à la dimension temporelle, `"x"` à la largeur et `"y"` à la hauteur. |
