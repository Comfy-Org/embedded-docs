...
ModelSamplingStableCascade 노드는 샘플링 매개변수에 shift 값을 적용하여 모델에 Stable Cascade 샘플링 설정을 적용합니다. 원본 모델은 변경하지 않고, 사용자 지정 Stable Cascade 샘플링 구성이 적용된 입력 모델의 패치된 복사본을 반환합니다.
...
| `시프트` | 샘플링 매개변수에 적용되는 shift 값 (기본값: 2.0) | FLOAT | 예 | 0.0 - 100.0 (단계: 0.01) |
