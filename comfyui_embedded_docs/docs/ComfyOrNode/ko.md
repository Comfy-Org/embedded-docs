...
|-----------|-----------|----------|-------|-------------|
| `value` | ANY | 예 | 여러 값 허용 | 진리값을 평가할 값입니다. 입력을 더 추가하여 여러 값을 제공할 수 있습니다. 이 노드는 이러한 값 중 하나라도 참(true)이면 `true`를 반환합니다. |
...
| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `BOOLEAN` | BOOLEAN | 입력 값 중 하나라도 참(true)이면 `true`를 반환하고, 모든 입력 값이 거짓(false)이면 `false`를 반환합니다. |
