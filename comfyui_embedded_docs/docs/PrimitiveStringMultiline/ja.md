...
PrimitiveStringMultiline ノードは、ワークフローで文字列値を入力および受け渡しするための複数行テキスト入力フィールドを提供します。複数行のテキスト入力を受け入れ、同じ文字列値をそのまま出力します。このノードは、複数行にわたる長いテキストコンテンツや整形されたテキストを入力する必要がある場合に便利です。

## 入力

| パラメータ | 説明 | データ型 | 必須 | 範囲 |
| --- | --- | --- | --- | --- |
| `value` | 複数行にわたることができるテキスト入力値 | STRING | はい | N/A |
