...
| 参数 | 数据类型 | 是否必填 | 范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `duration` | FLOAT | 是 | 0.0 至 1.8446744073709552e+19 | 空音频片段的时长（秒）（默认值：60.0） |
| `sample_rate` | INT | 是 | 1 至 192000 | 空音频片段的采样率（默认值：44100） |
| `channels` | INT | 是 | 1 至 2 | 音频声道数（1 为单声道，2 为立体声）（默认值：2） |
