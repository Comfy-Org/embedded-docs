...
| 参数 | 数据类型 | 是否必填 | 范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `prompt` | STRING | 否 | - | 用于引导视频扩展的正面文本提示 |
| `negative_prompt` | STRING | 否 | - | 用于指定扩展视频中需避免元素的负面文本提示 |
| `cfg_scale` | FLOAT | 否 | 0.0 - 1.0 | 控制提示引导的强度（默认值：0.5） |
| `video_id` | STRING | 是 | - | 待扩展视频的 ID。支持由文本转视频、图像转视频以及先前视频扩展操作生成的视频。扩展后总时长不得超过 3 分钟。 |
