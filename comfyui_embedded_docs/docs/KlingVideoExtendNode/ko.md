...
|-----------|-----------|----------|-------|-------------|
| `prompt` | STRING | 아니요 | - | 비디오 확장을 안내하는 긍정 텍스트 프롬프트 |
| `negative_prompt` | STRING | 아니요 | - | 확장된 비디오에서 제외할 요소에 대한 부정 텍스트 프롬프트 |
| `cfg_scale` | FLOAT | 아니요 | 0.0 - 1.0 | 프롬프트 안내 강도를 제어합니다 (기본값: 0.5) |
| `video_id` | STRING | 예 | - | 확장할 비디오의 ID입니다. 텍스트-비디오, 이미지-비디오 및 이전 비디오 확장 작업으로 생성된 비디오를 지원합니다. 확장 후 총 재생 시간이 3분을 초과할 수 없습니다. |
...
| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `output` | VIDEO | Kling API에 의해 생성된 확장된 비디오 |
| `video_id` | STRING | 확장된 비디오의 고유 식별자 |
| `duration` | STRING | 확장된 비디오의 재생 시간 |
