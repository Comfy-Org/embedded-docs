...
| `final_width` | INT | Sim | 64 - 32768 | A largura da imagem mesclada final em pixels (padrão: 1024). |
| `final_height` | INT | Sim | 64 - 32768 | A altura da imagem mesclada final em pixels (padrão: 1024). |
| `overlap` | INT | Sim | 0 - 4096 | A quantidade de sobreposição entre blocos adjacentes em pixels. Um valor maior que 0 permite um efeito de mesclagem suave nas junções dos blocos (padrão: 128). |
