...
| 參數名稱       | 資料類型    | 描述 |
|----------------|-------------|------|
| `latents`      | `LATENT`    | 此參數代表需要重新批次處理的輸入潛在表示，對於決定輸出批次的結構和內容至關重要。 |
| `批次大小`   | `INT`       | 此參數指定輸出中每個批次所需的樣本數量，直接影響輸入潛在表示在新批次中的分組和劃分方式。 |

## 輸出參數

| 參數名稱 | 資料類型 | 描述 |
|----------|----------|------|
| `latent` | `LATENT` | 輸出是根據指定批次大小調整後重新組織的潛在表示批次，便於進行後續處理或分析。 |