...
| `direction` | الاتجاه Forward يتابع بعد المقطع السابق؛ أما Backward فيُضاف قبل المقطع السابق. يستخدم Forward المقطع المصدر كإطار بداية؛ ويستخدمه Backward كإطار نهاية. عند تحديد "Forward (continue after)" يُضاف خيار `loop`. | DYNAMIC_COMBO | نعم | "Forward (continue after)"<br>"Backward (lead-in before)" |
| `prompt` | موجّه نصي للمحتوى الجديد. الافتراضي: "" (فارغ). يجب أن يتراوح بين 1 و6000 حرف. | STRING | نعم | 1 إلى 6000 characters |
| `resolution` | دقة الإخراج لمقطع الفيديو الممتد. الافتراضي: "720p". | COMBO | نعم | "540p"<br>"720p"<br>"1080p" |
| `seed` | بذرة لتحديد ما إذا كان يجب إعادة تشغيل العقدة؛ النتائج غير حتمية بغض النظر عن البذرة. الافتراضي: 0. | INT | نعم | 0 إلى 0xFFFFFFFFFFFFFFFF (18446744073709551615) |
