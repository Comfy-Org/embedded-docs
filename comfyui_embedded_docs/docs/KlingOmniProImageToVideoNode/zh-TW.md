...
| `提示詞` | STRING | 是 | - | 描述影片內容的文字提示。可包含正面和負面描述。文字會自動標準化，長度必須在 1 到 2500 個字元之間。啟用分鏡時此參數會被忽略。 |
| `長寬比` | COMBO | 是 | `"16:9"`<br>`"9:16"`<br>`"1:1"` | 生成影片的目標長寬比。 |
| `時長` | INT | 是 | 3 到 15 | 影片的長度，單位為秒。可使用滑桿調整數值（預設值：5）。 |
| `參考圖片` | IMAGE | 是 | - | 最多 7 張參考圖片。每張圖片至少需為 300x300 像素，長寬比需介於 1:2.5 與 2.5:1 之間。 |
| `解析度` | COMBO | 否 | `"4k"`<br>`"1080p"`<br>`"720p"` | 影片的輸出解析度。此參數為可選（預設值："1080p"）。 |
| `分鏡腳本` | DYNAMIC_COMBO | 否 | `"disabled"`<br>`"1 storyboard"`<br>`"2 storyboards"`<br>`"3 storyboards"`<br>`"4 storyboards"`<br>`"5 storyboards"`<br>`"6 storyboards"` | 生成一系列帶有各自提示和時長的影片片段。僅 `kling-v3-omni` 支援此功能。啟用時，全域 `提示詞` 會被忽略，且所有分鏡片段的總時長必須等於全域 `時長`。 |
| `產生音訊` | BOOLEAN | 否 | `true`<br>`false` | 為影片生成音訊。僅 `kling-v3-omni` 支援此功能（預設值：false）。 |
