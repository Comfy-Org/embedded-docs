...
此节点使用 Kling AI 模型，根据文本提示和最多七张参考图像生成视频。您可以控制视频的宽高比、时长和分辨率，并可选择使用故事板或生成音频。该节点将请求发送到外部 API，并返回生成的视频。
...
启用 `storyboards` 时，每个选定的故事板片段会出现以下输入。N 的范围从 1 到所选故事板数量。

| 参数 | 描述 | 数据类型 | 必需 | 范围 |
|-----------|-------------|-----------|----------|-------|
| `storyboard_N_prompt` | 故事板片段 N 的提示。最多 512 个字符。 | STRING | 否 | 1 到 512 个字符 |
| `storyboard_N_duration` | 故事板片段 N 的时长（秒）（默认值：4）。 | INT | 否 | 1 到 15 |

**注意：** `reference_images` 输入最多接受 7 张图像。如果提供的数量超过此限制，节点将引发错误。每张图像都会验证最小尺寸和宽高比。
