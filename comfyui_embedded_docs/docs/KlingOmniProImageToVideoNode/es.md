...
| `prompt` | STRING | Sí | - | Un texto descriptivo que describe el contenido del video. Puede incluir descripciones tanto positivas como negativas. El texto se normaliza automáticamente y debe tener entre 1 y 2500 caracteres. Se ignora cuando los guiones gráficos están habilitados. |
| `aspect_ratio` | COMBO | Sí | `"16:9"`<br>`"9:16"`<br>`"1:1"` | La relación de aspecto deseada para el video generado. |
| `duration` | INT | Sí | 3 a 15 | La duración del video en segundos. El valor se puede ajustar con un control deslizante (predeterminado: 5). |
