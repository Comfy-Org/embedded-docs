...
| Parâmetro | Descrição | Tipo de Dados | Obrigatório | Intervalo |
|-----------|-------------|-----------|----------|-------|
| `duração` | Duração do vídeo estendido em segundos (padrão: 4). Essa configuração aparece após selecionar um modelo. | INT | Sim | 1 a 7 |
| `resolução` | Resolução do vídeo de saída. Essa configuração aparece após selecionar um modelo. | COMBO | Sim | `"720p"`<br>`"1080p"` |
