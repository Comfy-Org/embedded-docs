...
|-----------|-------------|-----------|----------|-------|
| `model` | Video uzatma için kullanılacak model. Bir model seçmek, modele özgü süre ve çözünürlük ayarlarını ortaya çıkarır. | COMBO | Evet | `"viduq2-pro"`<br>`"viduq2-turbo"` |
...
| `end_frame` | Uzatma için hedef bitiş karesi olarak kullanılacak isteğe bağlı bir görsel. | IMAGE | Hayır | - |
...
| Parametre | Açıklama | Veri Türü | Gerekli | Aralık |
|-----------|-------------|-----------|----------|-------|
| `model.duration` | Uzatılmış videonun süresi (saniye cinsinden, varsayılan: 4). Bu ayar, bir model seçtikten sonra görünür. | INT | Evet | 1 ile 7 arası |
| `model.resolution` | Çıktı videosunun çözünürlüğü. Bu ayar, bir model seçtikten sonra görünür. | COMBO | Evet | `"720p"`<br>`"1080p"` |
