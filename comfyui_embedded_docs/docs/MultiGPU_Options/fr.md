...
|-----------|-----------------|--------|-------|-------------|
| `device_index` | INT | Oui | 0 à 64 | Le numéro d'index du périphérique GPU à configurer (par défaut : 0) |
| `relative_speed` | FLOAT | Oui | 0.0 à illimité | La vitesse relative de ce GPU par rapport aux autres, utilisée pour la répartition de la charge de travail (par défaut : 1.0, pas : 0.01) |
| `gpu_options` | GPU_OPTIONS | Non | - | Un groupe d'options GPU existant auquel ajouter les options de ce périphérique. Si non fourni, un nouveau groupe est créé |

## Sorties

| Nom de sortie | Type de données | Description |
|---------------|-----------------|-------------|
| `GPU_OPTIONS` | GPU_OPTIONS | Un groupe d'options GPU contenant les paramètres configurés du périphérique, pouvant être transmis à d'autres nœuds pour des opérations multi-GPU |
