...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `device_index` | INT | 是 | 0 至 64 | 要設定的 GPU 裝置索引編號（預設值：0） |
| `relative_speed` | FLOAT | 是 | 0.0 至無上限 | 此 GPU 相對於其他 GPU 的相對速度，用於工作負載分配（預設值：1.0，步進值：0.01） |
| `gpu_options` | GPU_OPTIONS | 否 | - | 要加入此裝置選項的現有 GPU 選項群組。若未提供，則會建立新的群組 |

## 輸出

| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `GPU_OPTIONS` | GPU_OPTIONS | 包含已設定裝置設定的 GPU 選項群組，可傳遞給其他節點進行多 GPU 操作 |
