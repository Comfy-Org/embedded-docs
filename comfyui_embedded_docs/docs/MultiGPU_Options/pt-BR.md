...
|-----------|--------------|-------------|-------|-----------|
| `device_index` | INT | Sim | 0 a 64 | O número do índice do dispositivo GPU a ser configurado (padrão: 0) |
| `relative_speed` | FLOAT | Sim | 0.0 a ilimitado | A velocidade relativa desta GPU em comparação com outras, usada para distribuição de carga de trabalho (padrão: 1.0, incremento: 0.01) |
| `gpu_options` | GPU_OPTIONS | Não | - | Um grupo de opções de GPU existente para adicionar as opções deste dispositivo. Se não for fornecido, um novo grupo é criado |

## Saídas

| Nome da Saída | Tipo de Dado | Descrição |
|---------------|--------------|-----------|
| `GPU_OPTIONS` | GPU_OPTIONS | Um grupo de opções de GPU contendo as configurações do dispositivo configurado, que pode ser passado para outros nós para operações com múltiplas GPUs |
