...
Not 節點對任何輸入值執行邏輯 NOT 操作。如果輸入值被視為假值（例如 0、空字串、None、False），則回傳 True；如果輸入值為真值，則回傳 False。它使用 Python 的標準規則來判斷真值。

## 輸入

| 參數 | 說明 | 資料類型 | 必要 | 範圍 |
| --- | --- | --- | --- | --- |
| `value` | 要反轉的輸入值。接受任何資料類型，並使用 Python 的真值規則進行評估。 | ANY | 是 | 任何值 |
