...
|-----------|-------------|-----------|----------|-------|
| `audio` | Faixa de áudio que orienta o vídeo. Sua duração (2-20 segundos) define a duração do vídeo. | AUDIO | Sim | 2-20 segundos |
| `model` | A versão do modelo LTX 2.5 a ser usada. Selecionar um modelo também revela a subopção `resolution` para esse modelo. | COMBO | Sim | "LTX-2.5 (Fast)"<br>"LTX-2.5 (Pro)" |
...
| `seed` | Um número que controla a aleatoriedade da geração. A mesma seed produz o mesmo resultado (padrão: 42). | INT | Sim | Qualquer inteiro |
| `image` | Primeiro quadro opcional a ser usado para o vídeo. Apenas uma imagem é suportada. | IMAGE | Não | Imagem única |
