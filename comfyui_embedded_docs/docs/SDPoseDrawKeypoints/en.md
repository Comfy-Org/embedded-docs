...
| --- | --- | --- | --- | --- |
| `keypoints` | The pose keypoint data to be drawn. This data typically comes from a pose detection node and can contain one or more frames. | POSE_KEYPOINT | Yes | - |
...
| `stick_width` | The width of the lines used to draw the body and head skeleton (default: 4). | INT | No | 1 to 10 |
...
| `draw_head` | Controls whether the head keypoints (nose, eyes, ears) are drawn (default: True). | BOOLEAN | No | - |

**Note:** If the `keypoints` input is empty or `None`, the node will output a blank 64x64 image.

**Note:** `draw_body` and `draw_head` work independently. When `draw_head` is disabled, the head keypoints are not drawn even if `draw_body` is enabled. When `draw_body` is disabled but `draw_head` is enabled, only the head keypoints and the neck point are drawn. If both are disabled, no body or head keypoints are drawn.

## Outputs

| Output Name | Description | Data Type |
| --- | --- | --- |
| `output` | An image with the drawn pose keypoints. The image dimensions match the `canvas_height` and `canvas_width` specified in the input keypoint data. When the input contains multiple frames, a batch of images is returned. | IMAGE |
