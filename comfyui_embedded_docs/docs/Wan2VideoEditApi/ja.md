...
| `audio_setting` | COMBO | いいえ | `"auto"`<br>`"origin"` | "auto"：モデルがプロンプトに基づいてオーディオを再生成するかどうかを判断します。"origin"：入力動画の元のオーディオを保持します。（デフォルト："auto"） |
| `watermark` | BOOLEAN | いいえ | - | 結果に AI 生成の透かしを追加するかどうかです。（デフォルト：False） |
