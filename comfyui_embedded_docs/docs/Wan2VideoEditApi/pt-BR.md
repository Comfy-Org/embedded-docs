...
|-----------|-----------|----------|-------|-------------|
| `modelo` | COMBO | Sim | `"wan2.7-videoedit"` | O modelo a ser usado para edição de vídeo. |
...
| `vídeo` | VIDEO | Sim | - | O vídeo a ser editado. |
| `semente` | INT | Não | 0 a 2147483647 | A semente a ser usada para a geração. (padrão: 0) |
| `configuração_de_áudio` | COMBO | Não | `"auto"`<br>`"origin"` | 'auto': o modelo decide se deve regenerar o áudio com base no prompt. 'origin': preserva o áudio original do vídeo de entrada. (padrão: "auto") |
