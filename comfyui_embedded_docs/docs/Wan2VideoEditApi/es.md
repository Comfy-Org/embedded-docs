...
| `seed` | INT | No | 0 a 2147483647 | La semilla a utilizar para la generación. (por defecto: 0) |
| `audio_setting` | COMBO | No | `"auto"`<br>`"origin"` | 'auto': el modelo decide si regenerar el audio según el prompt. 'origin': conserva el audio original del video de entrada. (por defecto: "auto") |
| `watermark` | BOOLEAN | No | - | Si se debe agregar una marca de agua generada por IA al resultado. (por defecto: Falso) |
