...
|-----------|-----------|----------|-------|-------------|
| `التكييف` | CONDITIONING | نعم | - | بيانات التكييف المراد تعديلها باستخدام طريقة اللاتينات المرجعية |
| `طريقة المرجع الكامن` | STRING | نعم | `"offset"`<br>`"index"`<br>`"uxo/uno"`<br>`"index_timestep_zero"` | الطريقة المستخدمة لمعالجة اللاتينات المرجعية. إذا تم اختيار "uxo" أو "uso"، فسيتم تحويلها إلى "uxo". تم وضع علامة متقدمة على هذا المعامل. |

## المخرجات

| اسم المخرج | نوع البيانات | الوصف |
|-------------|-----------|-------------|
| `التكييف` | CONDITIONING | بيانات التكييف المعدلة مع تطبيق طريقة اللاتينات المرجعية |
