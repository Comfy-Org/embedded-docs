...
|-----------|-----------|----------|-------|-------------|
| `conditioning` | CONDITIONING | 예 | - | 참조 잠재 변수 방식으로 수정할 컨디셔닝 데이터 |
| `reference_latents_method` | STRING | 예 | `"offset"`<br>`"index"`<br>`"uxo/uno"`<br>`"index_timestep_zero"` | 참조 잠재 변수 처리에 사용할 방식입니다. "uxo" 또는 "uso"를 선택하면 "uxo"로 변환됩니다. 이 매개변수는 고급 설정으로 표시되어 있습니다. |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `conditioning` | CONDITIONING | 참조 잠재 변수 방식이 적용된 수정된 컨디셔닝 데이터 |
