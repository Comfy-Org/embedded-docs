...
| --- | --- | --- | --- | --- |
| `شرط‌گذاری` | داده‌های شرطی‌سازی که باید با روش latentهای مرجع اصلاح شوند | CONDITIONING | بله | - |
| `روش latent مرجع` | روشی که برای پردازش latentهای مرجع استفاده می‌شود. اگر مقداری شامل "uxo" یا "uso" انتخاب شود، پیش از ذخیره‌سازی به "uxo" تبدیل می‌شود. این پارامتر به‌عنوان پیشرفته علامت‌گذاری شده است. | COMBO | بله | `"offset"`<br>`"index"`<br>`"uxo/uno"`<br>`"index_timestep_zero"` |
