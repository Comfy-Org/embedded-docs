...
| `reference_latents_method` | O método a ser usado para o processamento dos latentes de referência. Se "uxo" ou "uso" for selecionado, ele será convertido para "uxo". Este parâmetro é marcado como avançado. | COMBO | Sim | `"offset"`<br>`"index"`<br>`"uxo/uno"`<br>`"index_timestep_zero"` |
