...
BerniniConditioning 節點負責為 Wan2.2-A14B 模型準備影片與影像的條件化資料。它使用提供的 VAE 對來源影片、參考影片及參考影像進行編碼，然後將其作為情境內標記附加到條件化資料中。任務會根據連接的輸入自動推斷。
...
| `參考影片` | 要插入來源影片中的影片（ads2v）。會依長度裁剪並保持寬高比縮放（長邊限制為 ref_max_size）。 | IMAGE | 否 | - |
| `參考圖像` | 作為情境內標記注入的參考影像（r2v, rv2v）。每張影像會依其原始寬高比獨立編碼（長邊限制為 ref_max_size）。最多可提供 8 張影像。 | IMAGE | 否 | 0 至 8 張影像 |
...
上下文流按固定順序附加：`source_video`（來源 ID 1），然後是 `reference_video`（來源 ID 2），再是每張參考影像（來源 ID 3、4...）。包含多張影像的參考影像輸入會為每張影像貢獻一個流。
