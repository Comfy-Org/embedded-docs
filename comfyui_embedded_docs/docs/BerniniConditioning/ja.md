...
BerniniConditioningノードは、Wan2.2-A14Bモデル用の動画および画像の条件付けデータを準備します。提供されたVAEを使用してソース動画、参照動画、参照画像をエンコードし、それらをインコンテキストトークンとして条件付けデータに付加します。タスクは接続された入力から自動的に推論されます。
...
| `リファレンス動画` | ソース動画に挿入する動画（ads2v）。長さに合わせてトリミングされ、アスペクト比を維持したままリサイズされます（長辺は ref_max_size に制限）。 | IMAGE | いいえ | - |
| `リファレンス画像` | インコンテキストトークンとして注入される参照画像（r2v、rv2v）。各画像は独自のネイティブなアスペクト比で個別にエンコードされます（長辺は ref_max_size に制限）。最大8枚の画像を指定できます。 | IMAGE | いいえ | 0～8枚の画像 |
...
コンテキストストリームは固定順序で付加されます：`source_video`（ソースID 1）、次に `reference_video`（ソースID 2）、その後に各参照画像（ソースID 3、4...）。複数の画像を含む参照画像入力は、画像ごとに1つのストリームを提供します。
