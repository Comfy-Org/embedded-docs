...
|-----------|-------------|-----------|----------|-------|
| `prompt` | 视频生成的文本提示。 | STRING | 是 | 1 到 6000 个字符 |
| `resolution` | 生成视频的输出分辨率（默认值："720p"）。 | COMBO | 是 | `"360p"`<br>`"540p"`<br>`"720p"`<br>`"1080p"` |
| `duration` | 生成视频的时长（默认值："5s"）。 | COMBO | 是 | `"5s"`<br>`"10s"` |
| `seed` | 用于控制可复现性的随机数生成种子。 | INT | 是 | 0 到 4294967295 |
| `keyframes` | 来自 Luma Ray 3.2 关键帧节点的关键帧序列（至少 2 个）。 | LUMA_RAY32_KEYFRAME | 是 | 2 到 64 个关键帧 |
