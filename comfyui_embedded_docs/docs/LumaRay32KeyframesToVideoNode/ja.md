...
| パラメータ | 説明 | データ型 | 必須 | 範囲 |
|-----------|-------------|-----------|----------|-------|
| `prompt` | ビデオ生成のためのテキストプロンプト。 | STRING | はい | 1～6000 文字 |
| `resolution` | 生成されるビデオの出力解像度（デフォルト："720p"）。 | COMBO | はい | `"360p"`<br>`"540p"`<br>`"720p"`<br>`"1080p"` |
| `duration` | 生成されるビデオの長さ（デフォルト："5s"）。 | COMBO | はい | `"5s"`<br>`"10s"` |
| `seed` | 再現性を制御するための乱数生成のシード。 | INT | はい | 0～4294967295 |
| `keyframes` | Luma Ray 3.2 キーフレームノードからのキーフレームシーケンス（最低 2 つ）。 | LUMA_RAY32_KEYFRAME | はい | 2～64 キーフレーム |
