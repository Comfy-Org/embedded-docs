...
|-----------|-----------|----------|-------|-------------|
| `model` | MODEL | 예 | - | 스타일 참조 패치를 적용할 기본 모델입니다 |
| `model_patch` | MODEL_PATCH | 예 | - | 스타일 참조 정보를 포함하는 모델 패치입니다 |
| `clip_vision_output` | CLIP_VISION_OUTPUT | 예 | - | CLIP 비전 처리에서 추출된 인코딩된 시각적 특징입니다 |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `model` | MODEL | 스타일 참조 패치가 적용된 수정된 모델입니다 |
