...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `ckpt_name` | STRING | 是 | checkpoints 資料夾中的所有模型檔案 | 要載入的檢查點（模型）名稱。選擇檢查點模型檔案名稱，此名稱決定了後續影像生成所使用的 AI 模型。 |
...
| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `MODEL` | MODEL | 用於對潛在空間進行去噪的模型。這是用於影像生成的核心擴散模型。 |
| `CLIP` | CLIP | 用於編碼文字提示的 CLIP 模型，將文字描述轉換為 AI 可以理解的資訊。 |
| `VAE` | VAE | 用於在影像與潛在空間之間進行編碼和解碼的 VAE 模型。 |
