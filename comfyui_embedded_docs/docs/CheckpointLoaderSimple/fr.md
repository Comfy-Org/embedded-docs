...
|---------------|-----------------|-------------|
| `MODEL` | MODEL | Le modèle utilisé pour le débruitage des latents. Il s'agit du modèle de diffusion principal utilisé pour la génération d'images. |
| `CLIP` | CLIP | Le modèle CLIP utilisé pour encoder les invites textuelles, convertissant les descriptions textuelles en informations compréhensibles par l'IA. |
| `VAE` | VAE | Le modèle VAE utilisé pour encoder et décoder les images vers et depuis l'espace latent. |
