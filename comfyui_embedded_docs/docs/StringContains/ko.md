...
|-----------|-----------|----------|-------|-------------|
| `string` | STRING | 예 | - | 검색 대상이 되는 주 텍스트 문자열입니다 |
| `substring` | STRING | 예 | - | 주 문자열 내에서 검색할 텍스트입니다 |
| `case_sensitive` | BOOLEAN | 아니요 | - | 검색 시 대소문자를 구분할지 여부를 결정합니다 (기본값: true) |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `contains` | BOOLEAN | 하위 문자열이 문자열에서 발견되면 true를, 그렇지 않으면 false를 반환합니다 |
