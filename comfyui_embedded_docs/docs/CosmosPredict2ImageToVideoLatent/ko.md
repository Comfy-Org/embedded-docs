...
| `width` | INT | 아니요 | 16 ~ MAX_RESOLUTION | 출력 비디오의 픽셀 단위 너비 (기본값: 848, 16으로 나누어 떨어져야 함) |
| `height` | INT | 아니요 | 16 ~ MAX_RESOLUTION | 출력 비디오의 픽셀 단위 높이 (기본값: 480, 16으로 나누어 떨어져야 함) |
| `length` | INT | 아니요 | 1 ~ MAX_RESOLUTION | 비디오 시퀀스의 프레임 수 (기본값: 93, 단계: 4) |
| `batch_size` | INT | 아니요 | 1 ~ 4096 | 생성할 비디오 시퀀스의 개수 (기본값: 1) |
...
| `noise_mask` | LATENT | 생성 중 잠재 표현의 어떤 부분을 보존해야 하는지 나타내는 마스크 |
