...
|-----------|-----------|----------|-------|-------------|
| `lora` | LORA_MODEL | 예 | - | 저장할 LoRA 모델입니다. LoRA 레이어가 적용된 모델은 사용하지 마십시오. |
| `prefix` | STRING | 예 | - | 저장된 LoRA 파일에 사용할 접두사입니다(기본값: "loras/ComfyUI_trained_lora"). |
| `steps` | INT | 아니요 | - | 선택 사항: LoRA가 학습된 단계 수로, 저장된 파일 이름을 지정하는 데 사용됩니다. |
