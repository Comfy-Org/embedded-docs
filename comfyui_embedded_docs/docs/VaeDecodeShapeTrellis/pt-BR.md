...
|-----------|-------------|-----------|----------|-------|
| `samples` | As amostras latentes a decodificar, incluindo o tensor de amostras e os dados de coordenadas esparsas. O dicionário latente também pode conter campos opcionais: `coord_counts` para formas em lote, `coord_resolution` para controlar a resolução da malha e `model_frame` para orientação das coordenadas. | LATENT | Sim | None |
| `vae` | O modelo VAE usado para decodificar o latente de forma em uma malha. | VAE | Sim | None |

### Notas sobre `samples`

- O parâmetro `samples` é um dicionário latente que deve conter o tensor `samples` e as coordenadas esparsas `coords`.
...
- Se `coord_resolution` for fornecido, a resolução da malha é calculada como `coord_resolution * 16`. Caso contrário, o buffer de resolução integrado do VAE é usado (valor padrão: 1024).
