...
Bu düğüm, Trellis2 şekil latent temsillerini 3D bir mesh'e dönüştürür. Seyrek şekil latent verilerini mesh geometrisine dönüştürmek için bir VAE kullanır ve ayrıca kod çözme sırasında üretilen şekil alt bölüm verilerini çıktı olarak verir. Düğüm hem tek hem de toplu latent girdileri destekler ve mesh yönünü beklenen koordinat çerçevesine otomatik olarak ayarlar.

## Girişler

| Parametre | Açıklama | Veri Türü | Zorunlu | Aralık |
|-----------|-------------|-----------|----------|-------|
| `samples` | Kod çözülecek latent örnekler; örnek tensörünü ve seyrek koordinat verilerini içerir. Latent sözlüğü ayrıca isteğe bağlı alanlar içerebilir: toplu şekiller için `coord_counts`, mesh çözünürlüğünü kontrol etmek için `coord_resolution` ve koordinat yönü için `model_frame`. | LATENT | Evet | None |
...
- `coord_counts` mevcutsa, negatif olmayan tamsayılardan oluşan 1D bir tensör olmalıdır ve tüm sayıların toplamı toplam koordinat satırı sayısına eşit olmalıdır. Her sayı, kümedeki bir şekli temsil eder.
...
|-------------|-------------|-----------|
| `mesh` | Kod çözülen 3D mesh; köşe konumlarını ve yüzey indekslerini içerir. | MESH |
