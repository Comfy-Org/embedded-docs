...
- Si `coord_counts` est présent, il doit s’agir d’un tenseur 1D d’entiers non négatifs, et la somme de tous les comptes doit être égale au nombre total de lignes de coordonnées. Chaque compte représente une forme du lot.
...
|---------------|-------------|-----------------|
| `maillage` | Le maillage 3D décodé, contenant les positions des sommets et les indices de faces. | MESH |
