...
|-----------|-------------|-----------------|--------|-------|
| `aspect_ratio` | Le rapport d'aspect pour les dimensions de sortie (par défaut : `"1:1 (Square)"`). | COMBO | Oui | `"1:1 (Square)"`<br>`"2:3 (Portrait Photo)"`<br>`"3:2 (Photo)"`<br>`"3:4 (Portrait Standard)"`<br>`"4:3 (Standard)"`<br>`"9:16 (Portrait Widescreen)"`<br>`"16:9 (Widescreen)"`<br>`"21:9 (Ultrawide)"` |
| `megapixels` | Mégapixels totaux cibles. 1.0 MP ≈ 1024x1024 pour un carré (par défaut : 1.0). | FLOAT | Oui | 0.1 - 16.0 (pas : 0.1) |
| `preview` | Aperçu en direct de la résolution de sortie calculée. Ce widget en lecture seule se met à jour automatiquement et n'accepte aucune saisie utilisateur. | RESOLUTION_PREVIEW | Non | N/A |
