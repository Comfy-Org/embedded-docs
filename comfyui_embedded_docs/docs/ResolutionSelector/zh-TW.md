...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `長寬比` | COMBO | 是 | `"SQUARE"`<br>`"PORTRAIT_2_3"`<br>`"PORTRAIT_3_4"`<br>`"PORTRAIT_9_16"`<br>`"LANDSCAPE_3_2"`<br>`"LANDSCAPE_4_3"`<br>`"LANDSCAPE_16_9"` | 輸出尺寸的長寬比（預設值：`"SQUARE"`）。 |
| `百萬像素` | FLOAT | 是 | 0.1 - 16.0 | 目標總百萬像素。對於正方形長寬比，1.0 MP 約等於 1024×1024（預設值：1.0）。 |

## 輸出

| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `高度` | INT | 計算出的像素寬度，為 8 的倍數。 |
