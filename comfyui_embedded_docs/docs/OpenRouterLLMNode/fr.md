...
|-----------|-----------------|--------|-------|-------------|
| `prompt` | STRING | Oui | N/A | Texte d'entrée pour le modèle. |
| `model` | STRING | Oui | Plusieurs options disponibles (voir note ci-dessous) | Le modèle OpenRouter utilisé pour générer la réponse. |
| `seed` | INT | Oui | 0 à 2147483647 | Graine pour l'échantillonnage. Mettre à 0 pour omettre. La plupart des modèles ne traitent cela qu'à titre indicatif. (par défaut : 0) |
| `system_prompt` | STRING | Non | N/A | Instructions fondamentales qui dictent le comportement du modèle. (par défaut : "") |
...
|------------------|-----------------|-------------|
| `output` | STRING | La réponse textuelle générée par le modèle OpenRouter. |
