...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `樣本` | LATENT | 是 | - | 要解碼為影像的潛在表示 |
| `vae` | VAE | 是 | - | 用於解碼潛在樣本的 VAE 模型 |
| `區塊大小` | INT | 是 | 64-4096 (步長: 32) | 每個處理區塊的大小 (預設值: 512) |
| `重疊` | INT | 是 | 0-4096 (步長: 32) | 相鄰區塊之間的重疊量 (預設值: 64) |
| `時間區塊大小` | INT | 是 | 8-4096 (步長: 4) | 僅用於影片 VAE：每次解碼的幀數 (預設值: 64) |
| `時間重疊` | INT | 是 | 4-4096 (步長: 4) | 僅用於影片 VAE：重疊的幀數 (預設值: 8) |
