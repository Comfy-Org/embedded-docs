...
|-----------|-----------|----------|-------|-------------|
| `samples` | LATENT | Да | - | Латентное представление для декодирования в изображения |
| `vae` | VAE | Да | - | Модель VAE, используемая для декодирования латентных сэмплов |
| `tile_size` | INT | Да | 64-4096 (шаг: 32) | Размер каждого фрагмента (тайла) для обработки (по умолчанию: 512) |
| `overlap` | INT | Да | 0-4096 (шаг: 32) | Величина перекрытия между соседними фрагментами (по умолчанию: 64) |
