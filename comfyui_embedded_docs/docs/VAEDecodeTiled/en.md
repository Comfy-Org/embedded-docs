...
**Note:** The node automatically adjusts overlap values if they exceed practical limits. If `tile_size` is less than 4 times the `overlap`, the overlap is reduced to one quarter of the tile size. Similarly, if `temporal_size` is less than twice the `temporal_overlap`, the temporal overlap is halved. The node also accounts for the VAE's internal compression ratios when calculating tile and overlap sizes for both spatial and temporal dimensions. For VAEs without temporal compression (non-video VAEs), the `temporal_size` and `temporal_overlap` parameters are ignored.

## Outputs

| Output Name | Description | Data Type |
| --- | --- | --- |
| `IMAGE` | The decoded image or images generated from the latent representation. When decoding video latents, all decoded frames are combined into a single list of images. | IMAGE |
