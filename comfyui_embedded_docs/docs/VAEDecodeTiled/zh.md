...
| 参数 | 数据类型 | 是否必填 | 范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `samples` | LATENT | 是 | - | 待解码为图像的潜在表示 |
| `vae` | VAE | 是 | - | 用于解码潜在样本的 VAE 模型 |
| `tile_size` | INT | 是 | 64-4096（步长：32） | 每个处理图块的大小（默认值：512） |
| `overlap` | INT | 是 | 0-4096（步长：32） | 相邻图块之间的重叠区域大小（默认值：64） |
| `temporal_size` | INT | 是 | 8-4096（步长：4） | 仅用于视频 VAE：每次解码的帧数（默认值：64） |
| `temporal_overlap` | INT | 是 | 4-4096（步长：4） | 仅用于视频 VAE：帧之间的重叠帧数（默认值：8） |
