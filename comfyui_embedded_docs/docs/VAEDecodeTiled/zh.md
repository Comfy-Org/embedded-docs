...
| `tile_size` | INT | 是 | 64-4096 (步长: 32) | 处理时每个图块的尺寸（默认: 512） |
| `overlap` | INT | 是 | 0-4096 (步长: 32) | 相邻图块之间的重叠量（默认: 64） |
| `temporal_size` | INT | 是 | 8-4096 (步长: 4) | 仅用于视频 VAE：单次解码的帧数（默认: 64） |
| `temporal_overlap` | INT | 是 | 4-4096 (步长: 4) | 仅用于视频 VAE：帧重叠量（默认: 8） |
