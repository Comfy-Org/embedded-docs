...
O nó `EmptyAceStepLatentAudio` cria amostras de áudio latente vazias com uma duração especificada. Ele gera um lote de latentes de áudio silenciosos preenchidos com zeros, onde o comprimento é calculado com base nos segundos de entrada e nos parâmetros de processamento de áudio. Este nó é útil para inicializar fluxos de trabalho de processamento de áudio que exigem representações latentes.

## Entradas

| Parâmetro | Descrição | Tipo de Dados | Obrigatório | Intervalo |
| --- | --- | --- | --- | --- |
| `seconds` | A duração do áudio em segundos (padrão: 120.0) | FLOAT | Sim | 1.0 - 1000.0 (passo 0.1) |
...
Nota: O comprimento latente é derivado do valor `seconds` usando uma taxa de amostragem interna de 44100 Hz, calculado como `int(seconds × 44100 / 512 / 8)` quadros. O tensor latente resultante é preenchido inteiramente com zeros.
