...
|-----------|-----------|----------|-------|-------------|
| `seconds` | FLOAT | 예 | 1.0 - 1000.0 | 오디오 길이(초) (기본값: 120.0) |
| `batch_size` | INT | 예 | 1 - 4096 | 배치 내 잠재 이미지 수 (기본값: 1) |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `output` | LATENT | 0으로 채워진 빈 잠재 오디오 샘플을 반환합니다. 출력에는 `samples` 텐서와 "audio"로 설정된 `type` 필드가 포함됩니다. |
