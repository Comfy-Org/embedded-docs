...
EasyCache 노드는 디퓨전 모델에 네이티브 캐싱 시스템을 추가하여, 매 단계를 다시 계산하는 대신 이전에 계산된 단계의 결과를 재사용함으로써 샘플링 속도를 높입니다. 구성 가능한 샘플링 프로세스의 시작 및 종료 지점 사이에서만 활성화되며, 예상 출력 변화가 사용자 정의 임계값 아래로 유지되면 단계를 건너뜁니다. 이 노드는 고급 디버깅 용도를 위한 실험적 노드입니다.
...
| `verbose` | 상세 정보를 기록할지 여부입니다(기본값: False). | BOOLEAN | 예 | - |
