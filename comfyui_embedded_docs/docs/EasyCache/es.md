...
|-----------|--------------|-------------|-------|-------------|
| `model` | MODEL | Sí | - | El modelo al que se le añadirá EasyCache. |
| `reuse_threshold` | FLOAT | No | 0.0 - 3.0 | El umbral para reutilizar pasos almacenados en caché (predeterminado: 0.2). |
| `start_percent` | FLOAT | No | 0.0 - 1.0 | El paso de muestreo relativo para comenzar a usar EasyCache (predeterminado: 0.15). |
| `end_percent` | FLOAT | No | 0.0 - 1.0 | El paso de muestreo relativo para finalizar el uso de EasyCache (predeterminado: 0.95). |
