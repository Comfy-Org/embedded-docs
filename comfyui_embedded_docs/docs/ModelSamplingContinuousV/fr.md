...
|-----------|-----------------|--------|-------|-------------|
| `modèle` | MODEL | Oui | - | Le modèle d'entrée à modifier avec l'échantillonnage continu par prédiction V |
| `échantillonnage` | STRING | Oui | `"v_prediction"` | La méthode d'échantillonnage à appliquer (actuellement seule la prédiction V est prise en charge) |
...
|---------------|-----------------|-------------|
| `modèle` | MODEL | Le modèle modifié avec l'échantillonnage continu par prédiction V appliqué |
