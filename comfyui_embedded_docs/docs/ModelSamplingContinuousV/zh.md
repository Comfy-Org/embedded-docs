...
| 参数 | 数据类型 | 是否必填 | 范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `model` | MODEL | 是 | - | 要使用连续 V 预测采样进行修改的输入模型 |
| `sampling` | STRING | 是 | `"v_prediction"` | 要应用的采样方法（目前仅支持 V 预测） |
| `sigma_max` | FLOAT | 是 | 0.0 - 1000.0 | 采样的最大 sigma 值（默认值：500.0） |
| `sigma_min` | FLOAT | 是 | 0.0 - 1000.0 | 采样的最小 sigma 值（默认值：0.03） |

## 输出

| 输出名称 | 数据类型 | 描述 |
|-------------|-----------|-------------|
| `model` | MODEL | 已应用连续 V 预测采样的修改后模型 |
