...
|-----------|-----------|----------|-------|-------------|
| `model` | MODEL | 예 | - | 연속적인 V-예측 샘플링으로 수정할 입력 모델 |
| `sampling` | STRING | 예 | `"v_prediction"` | 적용할 샘플링 방법 (현재는 V-예측만 지원) |
| `sigma_max` | FLOAT | 예 | 0.0 - 1000.0 | 샘플링을 위한 최대 시그마 값 (기본값: 500.0) |
| `sigma_min` | FLOAT | 예 | 0.0 - 1000.0 | 샘플링을 위한 최소 시그마 값 (기본값: 0.03) |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `model` | MODEL | 연속적인 V-예측 샘플링이 적용된 수정된 모델 |
