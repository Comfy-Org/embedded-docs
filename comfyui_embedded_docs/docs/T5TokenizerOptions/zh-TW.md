...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `clip` | CLIP | 是 | - | 要為其配置分詞器選項的 CLIP 模型 |
| `min_padding` | INT | 否 | 0 至 10000 | 為所有 T5 模型類型設定的最小填充值（預設值：0） |
| `min_length` | INT | 否 | 0 至 10000 | 為所有 T5 模型類型設定的最小長度值（預設值：0） |

## 輸出

| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `output` | CLIP | 已將更新的分詞器選項套用至所有 T5 變體的修改後 CLIP 模型 |
