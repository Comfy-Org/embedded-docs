...
|-----------|-----------|----------|-------|-------------|
| `audio_*` | AUDIO | 예 | 1~8개 파일 | 음성 복제를 위한 오디오 녹음입니다. 1~8개의 오디오 파일을 제공해야 합니다. |
| `remove_background_noise` | BOOLEAN | 아니요 | True / False | 오디오 분리를 사용하여 음성 샘플에서 배경 소음을 제거합니다. (기본값: False) |
...
| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `voice` | ELEVENLABS_VOICE | 새로 생성된 복제 음성 모델의 고유 식별자입니다. 이 출력은 다른 ElevenLabs 텍스트 음성 변환 노드에 연결할 수 있습니다. |
