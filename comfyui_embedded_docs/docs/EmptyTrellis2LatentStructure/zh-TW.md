...
此節點為 Trellis2 模型建立一個空的潛在結構，其中所有值皆設為零。它會產生一個帶有 32 個通道、解析度為 16×16×16 的空白 3D 潛在張量，其大小符合批次中指定的項目數量。

## 輸入

| 參數 | 說明 | 資料類型 | 必填 | 範圍 |
|-----------|-------------|-----------|----------|-------|
| `batch_size` | 批次中的潛在影像數量（預設值：1）。 | INT | 是 | 1 至 4096 |

## 輸出

| 輸出名稱 | 說明 | 資料類型 |
|-------------|-------------|-----------|
| `LATENT` | 空的 Trellis2 潛在結構。樣本為填零張量，形狀為 (batch_size, 32, 16, 16, 16)，潛在類型設定為 "trellis2"。 | LATENT |
