...
|---------|----------|-------|--------|---------|
| `model_name` | STRING | بله | فهرست فایل‌های موجود در پوشه `models/optical_flow/` | مدل جریان نوری برای بارگذاری. فایل‌ها باید در پوشه `optical_flow` قرار داده شوند. امروزه تنها فایل `raft_large.pth` از کتابخانه torchvision پشتیبانی می‌شود. |

## خروجی‌ها

| نام خروجی | نوع داده | توضیحات |
|-----------|----------|---------|
| `OPTICAL_FLOW` | MODEL | مدل جریان نوری بارگذاری‌شده، که در یک ModelPatcher برای استفاده با سایر گره‌ها پیچیده شده است. |
