...
Carrega um modelo de fluxo óptico da pasta `models/optical_flow/`. Atualmente, apenas o formato RAFT-large do torchvision é suportado, que é o modelo usado pelo nó VOIDWarpedNoise. O ComfyUI não baixa automaticamente os pesos do fluxo óptico; você deve colocar o arquivo de checkpoint manualmente no diretório `models/optical_flow/`.
