...
| 參數 | 資料類型 | 必要 | 範圍 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `模型` | MODEL | 是 | - | 要套用跳層引導的模型 |
| `雙層` | STRING | 否 | - | 要跳過的雙區塊層索引，以逗號分隔（預設："7, 8, 9"） |
| `單層` | STRING | 否 | - | 要跳過的單區塊層索引，以逗號分隔（預設："7, 8, 9"） |
| `起始百分比` | FLOAT | 否 | 0.0 - 1.0 | 跳層引導開始時的去噪過程起始百分比（預設：0.0） |
| `結束百分比` | FLOAT | 否 | 0.0 - 1.0 | 跳層引導停止時的去噪過程結束百分比（預設：1.0） |
