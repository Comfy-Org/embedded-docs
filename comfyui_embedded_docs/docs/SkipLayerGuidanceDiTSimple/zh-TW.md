...
| `double_layers` | STRING | 否 | - | 要跳過的雙區塊層索引，以逗號分隔（預設："7, 8, 9"） |
| `single_layers` | STRING | 否 | - | 要跳過的單區塊層索引，以逗號分隔（預設："7, 8, 9"） |
| `start_percent` | FLOAT | 否 | 0.0 - 1.0 | 跳層引導開始時的去噪過程起始百分比（預設：0.0） |
| `end_percent` | FLOAT | 否 | 0.0 - 1.0 | 跳層引導停止時的去噪過程結束百分比（預設：1.0） |
