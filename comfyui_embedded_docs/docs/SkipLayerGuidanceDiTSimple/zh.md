...
| 参数 | 数据类型 | 必填 | 范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `model` | MODEL | 是 | - | 要应用跳层引导的模型 |
| `double_layers` | STRING | 否 | - | 要跳过的双块层索引列表，以逗号分隔（默认值："7, 8, 9"） |
| `single_layers` | STRING | 否 | - | 要跳过的单块层索引列表，以逗号分隔（默认值："7, 8, 9"） |
| `start_percent` | FLOAT | 否 | 0.0 - 1.0 | 跳层引导开始的去噪过程起始百分比（默认值：0.0） |
| `end_percent` | FLOAT | 否 | 0.0 - 1.0 | 跳层引导结束的去噪过程结束百分比（默认值：1.0） |
