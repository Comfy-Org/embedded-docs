...
|-----------|-----------------|--------|-------|-------------|
| `image` | IMAGE | Oui | | L'image à enregistrer en tant qu'actif personnel. |
| `group_id` | STRING | Non | | Réutiliser un ID de groupe d'actifs Seedance existant pour éviter une vérification humaine répétée pour la même personne. Laissez vide pour exécuter l'authentification réelle dans le navigateur et créer un nouveau groupe (par défaut : vide). |
