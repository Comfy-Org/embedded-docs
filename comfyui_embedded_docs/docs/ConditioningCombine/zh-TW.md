...
| 參數名稱 | 資料類型 | 說明 |
|---------|----------|------|
| `conditioning_1` | `CONDITIONING` | 第一個要合併的條件輸入。在合併過程中與 `conditioning_2` 具有同等重要性。 |
| `conditioning_2` | `CONDITIONING` | 第二個要合併的條件輸入。在合併過程中與 `conditioning_1` 具有同等重要性。 |

## 輸出

| 參數名稱 | 資料類型 | 說明 |
|---------|----------|------|
| `conditioning` | `CONDITIONING` | 合併 `conditioning_1` 和 `conditioning_2` 的結果，封裝了融合後的資訊。 |
