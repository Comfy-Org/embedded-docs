...
| `strength` | FLOAT | 是 | -10.0 至 10.0 | 控制网络影响的强度。正值应用效果，负值可反转效果（默认值：1.0）。 |
| `image` | IMAGE | 否 | - | 可选的引导生成过程的基础图像。 |
| `inpaint_image` | IMAGE | 否 | - | 专门用于修补由遮罩定义区域的可选图像。 |
| `mask` | MASK | 否 | - | 定义图像中应被编辑或修补区域的可选遮罩。 |
...
| 输出名称 | 数据类型 | 描述 |
|-------------|-----------|-------------|
| `model` | MODEL | 应用了控制网络补丁的模型，可用于采样流程。 |
