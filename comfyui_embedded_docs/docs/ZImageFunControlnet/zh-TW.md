...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `模型` | MODEL | 是 | - | 用於生成過程的基礎模型。 |
| `模型補丁` | MODEL_PATCH | 是 | - | 一個專門的修補模型，用於套用控制網路的引導。 |
| `vae` | VAE | 是 | - | 用於編碼和解碼影像的變分自編碼器。 |
| `強度` | FLOAT | 是 | -10.0 至 10.0 | 控制網路影響的強度。正值套用效果，負值則可能反轉效果（預設值：1.0）。 |
| `影像` | IMAGE | 否 | - | 一個可選的基礎影像，用於引導生成過程。 |
| `修補影像` | IMAGE | 否 | - | 一個可選的影像，專門用於修補由遮罩定義的區域。 |
| `遮罩` | MASK | 否 | - | 一個可選的遮罩，用於定義影像中應被編輯或修補的區域。 |
...
| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `模型` | MODEL | 已套用控制網路修補的模型，準備好用於取樣流程。 |
