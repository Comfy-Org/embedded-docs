...
| `model_patch` | MODEL_PATCH | 是 | - | 一個專門的修補模型，用於套用控制網路的引導。 |
| `vae` | VAE | 是 | - | 用於編碼和解碼影像的變分自編碼器。 |
| `strength` | FLOAT | 是 | -10.0 至 10.0 | 控制網路影響的強度。正值套用效果，負值則可能反轉效果（預設值：1.0）。 |
| `image` | IMAGE | 否 | - | 一個可選的基礎影像，用於引導生成過程。 |
| `inpaint_image` | IMAGE | 否 | - | 一個可選的影像，專門用於修補由遮罩定義的區域。 |
| `mask` | MASK | 否 | - | 一個可選的遮罩，用於定義影像中應被編輯或修補的區域。 |
...
| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `model` | MODEL | 已套用控制網路修補的模型，準備好用於取樣流程。 |
| `positive` | CONDITIONING | 正向條件化，可能已由控制網路輸入修改。 |
| `negative` | CONDITIONING | 負向條件化，可能已由控制網路輸入修改。 |
