...
| --- | --- | --- | --- | --- |
| `x` | A coordenada X do canto superior esquerdo da caixa delimitadora (padrão: 0). | INT | Sim | 0 a 16384 |
| `y` | A coordenada Y do canto superior esquerdo da caixa delimitadora (padrão: 0). | INT | Sim | 0 a 16384 |
| `largura` | A largura da caixa delimitadora (padrão: 512). | INT | Sim | 1 a 16384 |
| `altura` | A altura da caixa delimitadora (padrão: 512). | INT | Sim | 1 a 16384 |

Nota: Todos os valores máximos seguem a constante MAX_RESOLUTION do ComfyUI, que define a maior dimensão de imagem que o nó aceita.
