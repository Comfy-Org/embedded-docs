...
|-----------|-----------|----------|-------|-------------|
| `x` | INT | 예 | 0 ~ 8192 | 경계 상자 상단-왼쪽 모서리의 X 좌표입니다 (기본값: 0). |
| `y` | INT | 예 | 0 ~ 8192 | 경계 상자 상단-왼쪽 모서리의 Y 좌표입니다 (기본값: 0). |
| `width` | INT | 예 | 1 ~ 8192 | 경계 상자의 너비입니다 (기본값: 512). |
| `height` | INT | 예 | 1 ~ 8192 | 경계 상자의 높이입니다 (기본값: 512). |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `bounding_box` | BOUNDING_BOX | 정의된 사각형의 `x`, `y`, `width`, `height` 속성을 포함하는 데이터 구조입니다. |
