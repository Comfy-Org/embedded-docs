...
| `oluşturma türü` | Oluşturulacak 3B modelin türü. Mevcut seçenekler ve bunlarla ilişkili parametreler şunlardır:<br>- **Normal**: Standart bir model oluşturur. Bir `pbr` parametresi içerir (varsayılan: `False`).<br>- **LowPoly**: Düşük çokgenli bir model oluşturur. `polygon_type` (`"triangle"` veya `"quadrilateral"`) ve `pbr` (varsayılan: `False`) parametrelerini içerir.<br>- **Geometry**: Yalnızca geometri içeren bir model oluşturur. | DYNAMIC_COMBO | Evet | `"Normal"`<br>`"LowPoly"`<br>`"Geometry"` |
...
| `OBJ` | OBJ dosya formatında oluşturulan 3B model. | FILE3DOBJ |
| `doku_görseli` | Varsa, oluşturulan OBJ dosyasından çıkarılan doku görüntüsü. | IMAGE |
