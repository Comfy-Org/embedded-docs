...
| `tipo_de_geração` | O tipo de modelo 3D a ser gerado. As opções disponíveis e seus parâmetros associados são:<br>- **Normal**: Gera um modelo padrão. Inclui um parâmetro `pbr` (padrão: `False`).<br>- **LowPoly**: Gera um modelo de baixa poligonagem. Inclui os parâmetros `polygon_type` (`"triangle"` ou `"quadrilateral"`) e `pbr` (padrão: `False`).<br>- **Geometry**: Gera um modelo apenas com geometria. | DYNAMIC_COMBO | Sim | `"Normal"`<br>`"LowPoly"`<br>`"Geometry"` |
