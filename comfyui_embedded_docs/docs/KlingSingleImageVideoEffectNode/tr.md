...
|-----------|-----------|----------|-------|-------------|
| `image` | GÖRSEL | Evet | - | Referans Görsel. URL veya Base64 kodlu dize (data:image ön eki olmadan). Dosya boyutu 10MB'ı geçemez, çözünürlük 300x300px'den az olamaz, en boy oranı 1:2,5 ile 2,5:1 arasında olmalıdır |
| `effect_scene` | KOMBO | Evet | `"dizzydizzy"`<br>`"bloombloom"`<br>`"neon"`<br>`"cartoon"`<br>`"sketch"`<br>`"oil"`<br>`"watercolor"`<br>`"3d"` | Video oluşturmaya uygulanacak özel efekt sahnesinin türü. Bazı efektler farklı fiyatlandırmaya sahip olabilir. |
| `model_name` | KOMBO | Evet | `"kling-v1-5"`<br>`"kling-v1-6"` | Video efektini oluşturmak için kullanılacak belirli model sürümü. |
| `duration` | KOMBO | Evet | `"5"`<br>`"10"` | Oluşturulan videonun saniye cinsinden uzunluğu. |
...
|-------------|-----------|-------------|
| `output` | VİDEO | Uygulanan efektlerle oluşturulan video |
| `video_id` | METİN | Oluşturulan video için benzersiz tanımlayıcı |
| `duration` | METİN | Oluşturulan videonun süresi |
