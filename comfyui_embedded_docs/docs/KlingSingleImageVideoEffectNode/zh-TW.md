...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `影像` | IMAGE | 是 | - | 參考圖片。URL 或 Base64 編碼字串（不含 data:image 前綴）。檔案大小不得超過 10MB，解析度不低於 300x300px，長寬比介於 1:2.5 至 2.5:1 之間 |
...
| `時長` | COMBO | 是 | `"5"`<br>`"10"` | 生成影片的長度，以秒為單位。 |
