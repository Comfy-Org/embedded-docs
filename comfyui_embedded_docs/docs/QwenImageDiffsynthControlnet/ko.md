...
|-----------|-----------|----------|-------|-------------|
| `모델` | MODEL | 예 | - | 제어 네트워크로 패치할 기본 모델 |
| `모델 패치` | MODEL_PATCH | 예 | - | 기본 모델에 적용할 제어 네트워크 패치 모델 |
| `VAE` | VAE | 예 | - | 확산 과정에서 사용되는 VAE(변분 오토인코더) |
| `이미지` | IMAGE | 예 | - | 제어 네트워크를 안내하는 데 사용되는 입력 이미지(RGB 채널만 사용됨) |
| `강도` | FLOAT | 예 | -10.0 ~ 10.0 | 제어 네트워크 영향의 강도(기본값: 1.0) |
| `마스크` | MASK | 아니요 | - | 제어 네트워크를 적용할 영역을 정의하는 선택적 마스크(내부적으로 반전됨) |
...
| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `모델` | MODEL | 확산 합성 제어 네트워크 패치가 적용된 수정된 모델 |
