...
|-----------|---------------|-------------|-------|-------------|
| `modelo` | MODEL | Sí | - | El modelo base que se va a parchear con la red de control |
| `parche_del_modelo` | MODEL_PATCH | Sí | - | El modelo de parche de red de control que se aplicará al modelo base |
| `vae` | VAE | Sí | - | El VAE (Autoencoder Variacional) utilizado en el proceso de difusión |
| `imagen` | IMAGE | Sí | - | La imagen de entrada utilizada para guiar la red de control (solo se utilizan los canales RGB) |
| `intensidad` | FLOAT | Sí | -10.0 a 10.0 | La intensidad de la influencia de la red de control (predeterminado: 1.0) |
| `máscara` | MASK | No | - | Máscara opcional que define las áreas donde se debe aplicar la red de control (invertida internamente) |
...
|------------------|---------------|-------------|
| `modelo` | MODEL | El modelo modificado con el parche de red de control de síntesis por difusión aplicado |
