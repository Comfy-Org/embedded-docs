...
|-----------|-----------------|--------|-------|-------------|
| `modèle` | MODEL | Oui | - | Le modèle de base à patcher avec le réseau de contrôle |
| `correctif_modèle` | MODEL_PATCH | Oui | - | Le patch du réseau de contrôle à appliquer au modèle de base |
...
| `intensité` | FLOAT | Oui | -10,0 à 10,0 | La force de l'influence du réseau de contrôle (par défaut : 1,0) |
| `masque` | MASK | Non | - | Masque optionnel qui définit les zones où le réseau de contrôle doit être appliqué (inversé en interne) |
...
|---------------|-----------------|-------------|
| `modèle` | MODEL | Le modèle modifié avec le patch de réseau de contrôle de synthèse par diffusion appliqué |
