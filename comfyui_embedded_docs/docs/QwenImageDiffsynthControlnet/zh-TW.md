...
QwenImageDiffsynthControlnet 節點應用擴散合成控制網路修補程式來修改基礎模型的行為。它使用影像輸入和可選的遮罩，透過可調節的強度來引導模型的生成過程，創建一個融合了控制網路影響的修補模型，以實現更受控的影像合成。

## 輸入參數

| 參數名稱 | 資料類型 | 是否必填 | 數值範圍 | 參數說明 |
|-----------|-----------|----------|-------|-------------|
| `model` | MODEL | 是 | - | 要使用控制網路進行修補的基礎模型 |
...
| `mask` | MASK | 否 | - | 可選的遮罩，定義應應用控制網路的區域（內部會進行反轉） |

**注意：** 當提供遮罩時，它會自動進行反轉（1.0 - mask）並重塑以符合控制網路處理所需的尺寸。
