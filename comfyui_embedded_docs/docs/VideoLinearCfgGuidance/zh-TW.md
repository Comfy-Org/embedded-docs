...
| 參數 | 資料類型 | 說明 |
|-----------|-------------|-------------|
| `model`   | MODEL     | model 參數代表將套用線性 CFG 引導的影片模型。此參數對於定義將透過引導比例進行修改的基礎模型至關重要。 |
| `min_cfg` | `FLOAT`     | min_cfg 參數指定要套用的最小條件引導比例，作為線性比例調整的起點。它在決定引導比例下限方面扮演關鍵角色，進而影響模型的輸出。 |

## 輸出

| 參數 | 資料類型 | 說明 |
|-----------|-------------|-------------|
| `model`   | MODEL     | 輸出為輸入模型的修改版本，已套用線性 CFG 引導比例。此調整後的模型能夠根據指定的引導比例，生成具有不同程度條件影響的輸出。 |