...
VideoLinearCFGGuidance 節點對影片模型施加線性條件引導尺度調整，在指定範圍內動態調節條件化與非條件化組件的影響程度。此機制能實現生成過程的動態控制，讓使用者能根據所需的條件化程度微調模型輸出。
...
| `最小 cfg` | `FLOAT`     | 此參數指定要施加的最小條件引導尺度，作為線性尺度調整的起始點，在決定引導尺度下限方面扮演關鍵角色，會影響模型的輸出結果。 |
