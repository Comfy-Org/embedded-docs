...
| Paramètre | Description | Type de données |
| --- | --- | --- |
| `model` | La sortie est une version modifiée du modèle d'entrée, avec l'échelle de guidage CFG linéaire appliquée. Ce modèle ajusté est capable de générer des sorties avec différents degrés de conditionnement, en fonction de l'échelle de guidage spécifiée. | MODEL |
