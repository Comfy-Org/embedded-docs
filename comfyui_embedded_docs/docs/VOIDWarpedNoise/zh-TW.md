...
為 VOID 影片精煉程序的第二遍處理生成時間相關的雜訊。此節點將 Pass 1 的輸出影片作為輸入，並沿著光流向量扭曲高斯雜訊，產生能與影片內容一致移動的雜訊。此扭曲後的雜訊會用作 Pass 2 的起始潛在變量，以改善最終輸出的時間一致性。
...
| `length` | 像素幀的數量。向下取整以確保 `latent_t` 為偶數（`patch_size_t=2` 的要求），例如 49 → 45（預設值：45）。 | INT | 是 | 1 to MAX_RESOLUTION (step 1) |
| `batch_size` | 要生成的相同雜訊序列數量（預設值：1）。 | INT | 是 | 1 to 64 |

**關於 `length` 參數的說明：**`length` 值會自動向下取整至最接近的有效值，以產生偶數的 `latent_t` 維度。這是 CogVideoX-Fun-V1.5 模型的 `patch_size_t=2` 限制所要求的。當發生取整時，系統會記錄一則警告。
