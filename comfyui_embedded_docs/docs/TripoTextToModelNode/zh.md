...
|-----------|-----------|----------|-------|-------------|
| `prompt` | STRING | 是 | - | 用于生成 3D 模型的文本描述（支持多行输入） |
| `negative_prompt` | STRING | 否 | - | 描述生成模型中应避免的内容的文本（支持多行输入） |
| `model_version` | COMBO | 否 | 提供多个选项 | 用于生成的 Tripo 模型版本（默认值：v2.5-20250123） |
| `style` | COMBO | 否 | 提供多个选项 | 生成模型的样式设置（默认值："None"） |
| `texture` | BOOLEAN | 否 | - | 是否为模型生成纹理（默认值：True） |
| `pbr` | BOOLEAN | 否 | - | 是否生成 PBR（基于物理的渲染）材质（默认值：True） |
| `image_seed` | INT | 否 | - | 图像生成的随机种子（默认值：42） |
...
| `face_limit` | INT | 否 | -1 到 2000000 | 生成模型中的最大面数，-1 表示无限制（默认值：-1） |
| `quad` | BOOLEAN | 否 | - | 是否生成基于四边形的几何体而非三角形（默认值：False） |
