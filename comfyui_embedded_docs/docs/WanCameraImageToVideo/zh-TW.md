...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `正面提示詞` | CONDITIONING | 是 | - | 用於影片生成的正向條件化提示 |
| `負面提示詞` | CONDITIONING | 是 | - | 用於影片生成中要避免的負向條件化提示 |
| `VAE` | VAE | 是 | - | 用於將影像編碼到潛在空間的 VAE 模型 |
| `寬度` | INT | 是 | 16 至 MAX_RESOLUTION | 輸出影片寬度（像素）（預設值：832，步進值：16） |
| `高度` | INT | 是 | 16 至 MAX_RESOLUTION | 輸出影片高度（像素）（預設值：480，步進值：16） |
| `長度` | INT | 是 | 1 至 MAX_RESOLUTION | 影片序列中的影格數（預設值：81，步進值：4） |
| `批次大小` | INT | 是 | 1 至 4096 | 同時生成的影片數量（預設值：1） |
| `CLIP視覺輸出` | CLIP_VISION_OUTPUT | 否 | - | 可選的 CLIP 視覺輸出，用於額外的條件化處理 |
| `起始圖像` | IMAGE | 否 | - | 可選的起始影像，用於初始化影片序列。當提供此參數時，影片的前幾個影格將基於此影像，並套用遮罩來混合起始影格與生成的內容。該影像會調整大小以符合指定的寬度和高度。 |
| `攝影機條件` | WAN_CAMERA_EMBEDDING | 否 | - | 可選的攝影機嵌入條件，用於影片生成。當提供此參數時，這些條件會同時應用於正向和負向條件化處理。 |
...
| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `負面提示詞` | CONDITIONING | 已套用攝影機條件和 CLIP 視覺輸出的修改後正向條件化處理 |
| `潛在空間` | CONDITIONING | 已套用攝影機條件和 CLIP 視覺輸出的修改後負向條件化處理 |
