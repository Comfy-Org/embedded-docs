...
| `batch_size` | Número de vídeos a gerar simultaneamente (padrão: 1) | INT | Sim | 1 a 4096 |
...
| `camera_conditions` | Condições opcionais de embedding de câmera para geração de vídeo. Quando fornecidas, essas condições são aplicadas tanto ao condicionamento positivo quanto ao negativo. | WAN_CAMERA_EMBEDDING | Não | - |
