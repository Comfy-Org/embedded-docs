...
| `matériau` | COMBO | Oui | `"PBR"`<br>`"Matte"` | Le type de matériau pour le modèle 3D généré. |
| `format_fichier_géométrie` | COMBO | Oui | `"glb"`<br>`"obj"`<br>`"stl"`<br>`"usdz"` | Le format de fichier de sortie pour la géométrie du modèle 3D. |
| `mode_texture` | COMBO | Oui | `"Original"`<br>`"Clean"`<br>`"Style"` | Le mode de génération de texture. "Original" préserve les textures d'entrée, "Clean" les supprime et "Style" applique une texture stylisée. |
| `graine` | INT | Oui | 0 à 2147483647 | Une graine aléatoire pour des résultats reproductibles. Utilisez la même graine pour obtenir la même sortie. |
| `TAPose` | BOOLEAN | Oui | Vrai / Faux | Indique si la pose en T doit être appliquée au modèle généré. |
| `texture_hd` | BOOLEAN | Oui | Vrai / Faux | Indique s'il faut générer une carte de texture haute définition. |
...
| `utiliser_alpha_original` | BOOLEAN | Oui | Vrai / Faux | Indique s'il faut utiliser le canal alpha d'origine des images d'entrée. |
...
| `largeur_boîte` | INT | Oui | 1 à 1000 | La largeur de la boîte englobante du modèle généré en centimètres. |
| `hauteur_boîte` | INT | Oui | 1 à 1000 | La hauteur de la boîte englobante du modèle généré en centimètres. |
| `longueur_boîte` | INT | Oui | 1 à 1000 | La longueur de la boîte englobante du modèle généré en centimètres. |
| `hauteur_cm` | INT | Oui | 1 à 300 | La hauteur du modèle généré en centimètres. |
