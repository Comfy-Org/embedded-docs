...
|-----------|-----------|----------|-------|-------------|
| `lora_name` | STRING | 是 | 多个可用选项 | 从 loras 目录加载的 LoRA 文件名 |
| `strength_model` | FLOAT | 是 | -20.0 到 20.0 | 模型调整的强度乘数（默认值：1.0） |
| `strength_clip` | FLOAT | 是 | -20.0 到 20.0 | CLIP 调整的强度乘数（默认值：1.0） |
| `prev_hooks` | HOOKS | 否 | 不适用 | 可选的现有钩子组，用于与新的 LoRA 钩子合并 |
