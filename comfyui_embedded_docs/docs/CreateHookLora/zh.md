...
| 参数名 | 数据类型 | 必填 | 取值范围 | 描述 |
|--------|-----------|------|----------|------|
| `lora_name` | STRING | 是 | 支持多选项 | 从 loras 目录加载的 LoRA 文件名 |
| `strength_model` | FLOAT | 是 | -20.0 至 20.0 | 模型调整的强度乘数（默认值：1.0） |
| `strength_clip` | FLOAT | 是 | -20.0 至 20.0 | CLIP 调整的强度乘数（默认值：1.0） |
| `prev_hooks` | HOOKS | 否 | 不适用 | 可选的现有钩子组，将与新创建的 LoRA 钩子组合 |
