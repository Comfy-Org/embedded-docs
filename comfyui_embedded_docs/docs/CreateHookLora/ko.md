...
|-----------|-----------|----------|-------|-------------|
| `lora_name` | STRING | 예 | 여러 옵션 사용 가능 | loras 디렉토리에서 로드할 LoRA 파일의 이름 |
| `strength_model` | FLOAT | 예 | -20.0 ~ 20.0 | 모델 조정을 위한 강도 배율 (기본값: 1.0) |
| `strength_clip` | FLOAT | 예 | -20.0 ~ 20.0 | CLIP 조정을 위한 강도 배율 (기본값: 1.0) |
| `prev_hooks` | HOOKS | 아니요 | 해당 없음 | 새로운 LoRA 훅과 결합할 선택적 기존 훅 그룹 |
...
| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `HOOKS` | HOOKS | 결합된 LoRA 훅과 이전 훅을 포함하는 훅 그룹 |
