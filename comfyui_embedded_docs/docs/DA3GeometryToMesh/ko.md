...
| `batch_index` | 변환할 배치의 이미지 인덱스입니다. 이미지별 정점 개수가 다르므로 배치를 쌓을 수 없습니다(기본값: 0). 입력 지오메트리의 배치 크기보다 작아야 하며, 그렇지 않으면 오류가 발생합니다 | INT | 예 | 0 ~ 4096 |
...
유한하지 않거나 0이거나 음수인 깊이 값을 가진 픽셀은 항상 메시에서 제외됩니다. 결과 메시가 비어 있으면 오류가 발생하며, 오류 메시지는 `discontinuity_threshold`를 높이거나 `confidence_threshold`를 낮추거나 `use_sky_mask`를 비활성화하도록 제안합니다.
