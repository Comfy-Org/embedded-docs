...
| 字段 | 数据类型 | 描述 |
|---------|-------------|-----------------------------------------------------------------------------------|
| `image` | `IMAGE` | 待量化的输入图像张量。作为执行颜色缩减的主要数据，它会影响节点的执行过程。 |
| `colors`| `INT` | 指定图像要减少到的颜色数量。通过决定调色板大小，直接影响量化过程。 |
| `dither`| COMBO[STRING] | 决定量化过程中应用的抖动技术，影响输出图像的视觉质量和外观。 |

## 输出

| 字段 | 数据类型 | 描述 |
|-------|-------------|-------------------------------------------------------------------------------|
| `image`| `IMAGE` | 输入图像的量化版本，颜色复杂度降低，并可选地应用抖动以保持视觉质量。 |