...
|-----------|-----------|----------|-------|-------------|
| `model` | MODEL | نعم | - | نموذج SDPose المستخدم لاكتشاف النقاط الرئيسية. يجب أن يكون نموذجًا يحتوي على خاصية `heatmap_head`، وتحديدًا من مستودع SDPose. |
...
| `batch_size` | INT | لا | 1 إلى 10000 | عدد الصور التي سيتم معالجتها في دفعة واحدة عند التشغيل في وضع الصورة الكاملة (أي عندما لا يتم توفير `bboxes`). يمكن أن يؤدي ذلك إلى تسريع المعالجة. (القيمة الافتراضية: 16) |
