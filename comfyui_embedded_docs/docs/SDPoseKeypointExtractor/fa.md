...
|---------|----------|--------|--------|---------|
| `مدل` | MODEL | بله | - | مدل SDPose مورد استفاده برای تشخیص نقاط کلیدی. باید مدلی با ویژگی `heatmap_head` باشد، به‌طور خاص از مخزن SDPose. |
| `vae` | VAE | بله | - | مدل VAE مورد استفاده برای رمزگذاری تصاویر ورودی به فضای نهفته برای پردازش. |
| `تصویر` | IMAGE | بله | - | تصویر ورودی یا دسته‌ای از تصاویر که نقاط کلیدی حالت از آن استخراج می‌شود. |
| `اندازه دسته` | INT | خیر | 1 تا 10000 | تعداد تصاویری که در حالت تصویر کامل (یعنی زمانی که `جعبه‌های مرزی` ارائه نشده است) به‌طور همزمان پردازش می‌شوند. این کار می‌تواند سرعت پردازش را افزایش دهد. (پیش‌فرض: 16) |
| `جعبه‌های مرزی` | BOUNDINGBOX | خیر | - | جعبه‌های مرزی اختیاری برای تشخیص‌های دقیق‌تر. برای تشخیص چند نفر الزامی است. در صورت ارائه، گره نقاط کلیدی را از هر ناحیه مشخص‌شده استخراج می‌کند. |
...
|-----------|----------|---------|
| `keypoints` | POSE_KEYPOINT | نقاط کلیدی در قالب فریم OpenPose (عرض بوم، ارتفاع بوم، افراد). خروجی شامل افراد تشخیص‌داده‌شده است که هر کدام دارای آرایه‌ای از مختصات نقاط کلیدی (x, y) و نمرات اطمینان متناظر آن‌ها هستند. |
