...
Adjust Brightness ノードは、入力画像の明るさを変更します。各ピクセルの値に指定された係数を乗算し、その結果の値を有効な範囲内に収まるようにクランプすることで機能します。係数が1.0の場合は画像は変更されず、1.0未満の場合は暗くなり、1.0より大きい場合は明るくなります。
...
| `factor` | 明るさ係数。1.0 = 変更なし、<1.0 = 暗く、>1.0 = 明るく。（デフォルト: 1.0） | FLOAT | いいえ | 0.0 - 2.0 |

## 出力

| 出力名 | 説明 | データ型 |
| --- | --- | --- |
| `image` | 明るさが調整された出力画像。 | IMAGE |
