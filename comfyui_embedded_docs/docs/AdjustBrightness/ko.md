...
Adjust Brightness 노드는 입력 이미지의 밝기를 수정합니다. 각 픽셀의 값에 지정된 계수를 곱한 다음 결과 값을 유효한 범위 내에 유지되도록 클램핑하는 방식으로 작동합니다. 계수가 1.0이면 이미지가 변경되지 않고, 1.0보다 낮으면 더 어두워지며, 1.0보다 높으면 더 밝아집니다.
...
| `factor` | 밝기 계수입니다. 1.0 = 변경 없음, <1.0 = 더 어두움, >1.0 = 더 밝음. (기본값: 1.0) | FLOAT | 아니요 | 0.0 - 2.0 |
