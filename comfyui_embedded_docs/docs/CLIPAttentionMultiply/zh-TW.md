...
| `q` | FLOAT | 是 | 0.0 - 10.0 | 查詢投影權重與偏差的乘法因子（預設值：1.0） |
| `k` | FLOAT | 是 | 0.0 - 10.0 | 鍵投影權重與偏差的乘法因子（預設值：1.0） |
| `v` | FLOAT | 是 | 0.0 - 10.0 | 值投影權重與偏差的乘法因子（預設值：1.0） |
| `out` | FLOAT | 是 | 0.0 - 10.0 | 輸出投影權重與偏差的乘法因子（預設值：1.0） |

## 輸出

| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `CLIP` | CLIP | 回傳一個已套用指定注意力縮放因子的修改後 CLIP 模型 |
