...
| `strength_start` | Control strength at the start of sampling (high sigma). The influence of the image guidance begins at this value. (default: 1.0) | FLOAT | Yes | 0.0 - 10.0 |
| `strength_end` | Control strength at the end of sampling (low sigma). Linearly interpolated from start. The influence of the image guidance ends at this value. (default: 1.0) | FLOAT | Yes | 0.0 - 10.0 |
| `restore_cfg` | Pulls denoised output toward the input latent. Higher = stronger fidelity to input. 0 to disable. (default: 4.0) | FLOAT | Yes | 0.0 - 20.0 |
| `restore_cfg_s_tmin` | Sigma threshold below which `restore_cfg` is disabled. (default: 0.05) | FLOAT | Yes | 0.0 - 1.0 |
...
*Note:* `restore_cfg` only takes effect when set to a value greater than 0. Setting it to 0 disables the restore post-processing entirely. When active, the correction is applied only while the current sigma value is above `restore_cfg_s_tmin`.
