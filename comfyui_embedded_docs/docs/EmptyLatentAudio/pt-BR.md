...
| --- | --- | --- |
| `LATENT` | Retorna um tensor latente vazio para processamento de áudio com a duração e o tamanho de lote especificados. O tensor tem a forma [batch_size, 64, length], em que length é calculado a partir da duração do áudio e da taxa de amostragem. A saída também inclui metadados que indicam que o tipo é "audio" e uma taxa de redução temporal de 2048. | LATENT |
