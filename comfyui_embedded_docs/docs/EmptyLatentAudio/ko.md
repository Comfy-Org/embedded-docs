...
|-----------|-----------|----------|-------|-------------|
| `seconds` | FLOAT | 예 | 1.0 - 1000.0 | 오디오 지속 시간(초) (기본값: 47.6) |
...
| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `LATENT` | LATENT | 지정된 지속 시간과 배치 크기를 가진 오디오 처리를 위한 빈 잠재 텐서를 반환합니다. 텐서의 형태는 [batch_size, 64, length]이며, 여기서 length는 오디오 지속 시간과 샘플 속도로부터 계산됩니다. |
