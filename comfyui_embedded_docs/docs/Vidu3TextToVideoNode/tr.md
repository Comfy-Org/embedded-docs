...
| --- | --- | --- | --- | --- |
| `model` | Video üretimi için kullanılacak model. Bir model seçmek, en-boy oranı, çözünürlük, süre ve ses için ek yapılandırma parametrelerini ortaya çıkarır. | DYNAMIC_COMBO | Evet | `"viduq3-pro"`<br>`"viduq3-turbo"` |
...
| Parametre | Açıklama | Veri Türü | Zorunlu | Aralık |
| --- | --- | --- | --- | --- |
| `en-boy oranı` | Çıktı videosunun en-boy oranı. | COMBO | Evet | `"16:9"`<br>`"9:16"`<br>`"3:4"`<br>`"4:3"`<br>`"1:1"` |
| `çözünürlük` | Çıktı videosunun çözünürlüğü. | COMBO | Evet | `"720p"`<br>`"1080p"` |
| `süre` | Çıktı videosunun saniye cinsinden süresi (varsayılan: 5). | INT | Evet | 1 ila 16 |
| `ses` | Etkinleştirildiğinde, videoyu sesli (diyalog ve ses efektleri dahil) olarak çıkarır (varsayılan: False). | BOOLEAN | Evet | True/False |
