...
| --- | --- | --- | --- | --- |
| `modelo` | Modelo a ser usado para geração de vídeo. Selecionar um modelo revela parâmetros adicionais de configuração para proporção de aspecto, resolução, duração e áudio. | DYNAMIC_COMBO | Sim | `"viduq3-pro"`<br>`"viduq3-turbo"` |
| `prompt` | Descrição textual para geração de vídeo, com comprimento máximo de 2000 caracteres. | STRING | Sim | N/A |
| `semente` | Valor de semente para controlar a aleatoriedade da geração (padrão: 1). | INT | Sim | 0 a 2147483647 |
...
| Parâmetro | Descrição | Tipo de Dados | Obrigatório | Intervalo |
| --- | --- | --- | --- | --- |
| `proporção` | A proporção de aspecto do vídeo de saída. | COMBO | Sim | `"16:9"`<br>`"9:16"`<br>`"3:4"`<br>`"4:3"`<br>`"1:1"` |
...
| `áudio` | Quando habilitado, gera vídeo com som (incluindo diálogo e efeitos sonoros) (padrão: False). | BOOLEAN | Sim | True/False |
