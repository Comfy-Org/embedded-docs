...
| 參數 | 說明 | 資料類型 | 必填 | 範圍 |
| --- | --- | --- | --- | --- |
| `aspect_ratio` | 輸出影片的畫面比例。 | COMBO | 是 | `"16:9"`<br>`"9:16"`<br>`"3:4"`<br>`"4:3"`<br>`"1:1"` |
| `resolution` | 輸出影片的解析度。 | COMBO | 是 | `"720p"`<br>`"1080p"` |
| `duration` | 輸出影片的時長（秒）（預設：5）。 | INT | 是 | 1 至 16 |
| `audio` | 啟用後，輸出影片將包含聲音（包括對白與音效）（預設：False）。 | BOOLEAN | 是 | True/False |
