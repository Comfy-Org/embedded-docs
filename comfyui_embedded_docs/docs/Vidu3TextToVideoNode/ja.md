...
| パラメータ | 説明 | データ型 | 必須 | 範囲 |
| --- | --- | --- | --- | --- |
| `モデル` | ビデオ生成に使用するモデル。モデルを選択すると、アスペクト比、解像度、長さ、オーディオに関する追加の設定パラメータが表示されます。 | DYNAMIC_COMBO | はい | `"viduq3-pro"`<br>`"viduq3-turbo"` |
| `プロンプト` | ビデオ生成のためのテキストによる説明。最大長は2000文字です。 | STRING | はい | N/A |
| `シード` | 生成のランダム性を制御するシード値（デフォルト: 1）。 | INT | はい | 0 から 2147483647 |
...
| パラメータ | 説明 | データ型 | 必須 | 範囲 |
| --- | --- | --- | --- | --- |
| `アスペクト比` | 出力ビデオのアスペクト比。 | COMBO | はい | `"16:9"`<br>`"9:16"`<br>`"3:4"`<br>`"4:3"`<br>`"1:1"` |
| `解像度` | 出力ビデオの解像度。 | COMBO | はい | `"720p"`<br>`"1080p"` |
| `長さ` | 出力ビデオの長さ（秒単位）（デフォルト: 5）。 | INT | はい | 1 から 16 |
| `オーディオ` | 有効にすると、音声付きのビデオ（会話や効果音を含む）を出力します（デフォルト: False）。 | BOOLEAN | はい | True/False |
