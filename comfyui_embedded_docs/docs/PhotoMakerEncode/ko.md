...
|-----------|-----------|----------|-------|-------------|
| `photomaker` | PHOTOMAKER | 예 | - | 이미지 처리 및 임베딩 생성을 위해 사용되는 PhotoMaker 모델 |
| `image` | IMAGE | 예 | - | 조건화를 위한 시각적 특성을 제공하는 참조 이미지 |
| `clip` | CLIP | 예 | - | 텍스트 토큰화 및 인코딩에 사용되는 CLIP 모델 |
| `text` | STRING | 예 | - | 조건화 생성을 위한 텍스트 프롬프트 (기본값: "photograph of photomaker") |
