...
|-----------|-----------|----------|-------|-------------|
| `포토메이커 모델` | PHOTOMAKER | 예 | - | 이미지 처리 및 임베딩 생성을 위한 PhotoMaker 모델 |
| `이미지` | IMAGE | 예 | - | 컨디셔닝을 위한 시각적 특성을 제공하는 참조 이미지 |
| `clip` | CLIP | 예 | - | 텍스트 토큰화 및 인코딩에 사용되는 CLIP 모델 |
| `텍스트` | STRING | 예 | - | 컨디셔닝 생성을 위한 텍스트 프롬프트 (기본값: "photograph of photomaker") |
...
| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `CONDITIONING` | CONDITIONING | 이미지 생성 안내를 위한 이미지 및 텍스트 임베딩을 포함하는 컨디셔닝 데이터 |
