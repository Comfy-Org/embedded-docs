...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `參考圖像` | IMAGE | 是 | - | 影響生成風格的參考影像。 |
| `強度` | FLOAT | 是 | -2.0 到 2.0（步長：0.05） | 參考強度；負值會反轉風格影響（預設值：1.0）。 |
| `style_reference` | STYLE_REF | 否 | - | 可選的傳入風格參考鏈；此節點會再附加一個參考。 |
...
| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `style_reference` | STYLE_REF | 風格參考條目的列表，每個條目包含 URL 和強度值。將此輸出輸入到 Krea 2 影像節點。 |
