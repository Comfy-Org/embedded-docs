...
|-----------|-----------|----------|-------|-------------|
| `substyle` | STRING | 예 | 여러 옵션 사용 가능 (Recraft API에 의해 결정됨) | realistic_image 스타일에 적용할 특정 하위 스타일입니다. "None"으로 설정하면 하위 스타일이 적용되지 않습니다. |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `recraft_style` | STYLEV3 | `realistic_image` 스타일과 선택한 하위 스타일 설정을 포함하는 Recraft 스타일 구성 객체입니다. 이 출력은 스타일 입력을 허용하는 다른 Recraft 노드에 연결할 수 있습니다. |
