...
|-----------|-----------|----------|-------|-------------|
| `圖像` | IMAGE | 是 | - | 要編輯的圖片。 |
| `編輯指令` | STRING | 是 | - | 如何編輯圖片的文字描述。最多 2560 個字元。 |
| `模型` | MODEL | 是 | `"reve-edit@20250915"`<br>`"reve-edit-fast@20251030"` | 用於編輯的模型版本。 |
...
| `放大` | COMBO | 否 | `"disabled"`<br>`"enabled"` | 控制是否對生成的圖片進行放大。 |
| `upscale.upscale_factor` | FLOAT | 否 | - | 啟用放大時，圖片的放大倍數。 |
| `去背` | BOOLEAN | 否 | - | 控制是否移除生成圖片的背景。 |
| `種子` | INT | 否 | 0 到 2147483647 | 種子碼控制節點是否應重新執行；無論種子碼為何，結果皆非確定性。（預設值：0） |
