...
| `relación_de_aspecto` | La relación de aspecto para la imagen editada. Cuando se establece en "auto", la relación de aspecto se determina automáticamente. | COMBO | No | `"auto"`<br>`"16:9"`<br>`"9:16"`<br>`"3:2"`<br>`"2:3"`<br>`"4:3"`<br>`"3:4"`<br>`"1:1"` |
| `escalado_en_tiempo_de_prueba` | Factor de escalado en tiempo de prueba para el modelo. Valores más altos pueden mejorar la calidad pero aumentan el tiempo de procesamiento. | FLOAT | No | - |
