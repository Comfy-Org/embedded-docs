...
|-----------|--------------|-------------|-------|-------------|
| `imagen` | IMAGE | Sí | - | La imagen a editar. |
| `instrucción_de_edición` | STRING | Sí | - | Descripción textual de cómo editar la imagen. Máximo 2560 caracteres. |
| `modelo` | MODEL | Sí | `"reve-edit@20250915"`<br>`"reve-edit-fast@20251030"` | Versión del modelo a utilizar para la edición. |
...
| `escalar` | COMBO | No | `"disabled"`<br>`"enabled"` | Controla si se debe escalar la imagen generada. |
| `upscale.upscale_factor` | FLOAT | No | - | El factor por el cual escalar la imagen cuando el escalado está habilitado. |
| `eliminar_fondo` | BOOLEAN | No | - | Controla si se debe eliminar el fondo de la imagen generada. |
...
|------------------|--------------|-------------|
| `imagen` | IMAGE | La imagen editada generada según la instrucción. |
