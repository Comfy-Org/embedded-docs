...
|----------|------------|--------------|----------|----------|
| `ckpt_name` | STRING | Да | Доступно несколько вариантов | Файл контрольной точки для загрузки весов (выберите из доступных контрольных точек) |
| `strength_model` | FLOAT | Да | от -20.0 до 20.0 | Множитель силы, применяемый к весам модели (по умолчанию: 1.0) |
| `strength_clip` | FLOAT | Да | от -20.0 до 20.0 | Множитель силы, применяемый к весам CLIP (по умолчанию: 1.0) |
| `prev_hooks` | HOOKS | Нет | - | Необязательные предыдущие перехватчики для объединения с вновь созданными перехватчиками LoRA |
