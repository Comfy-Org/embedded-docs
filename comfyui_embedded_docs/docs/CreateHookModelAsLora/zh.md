...
| 参数 | 数据类型 | 必需 | 范围 | 描述 |
|------|-----------|------|------|-------------|
| `ckpt_name` | COMBO | 是 | 多个可用选项 | 用于加载权重的检查点文件（从可用检查点中选择） |
| `strength_model` | FLOAT | 是 | -20.0 到 20.0 | 应用于模型权重的强度乘数（默认：1.0） |
| `strength_clip` | FLOAT | 是 | -20.0 到 20.0 | 应用于 CLIP 权重的强度乘数（默认：1.0） |
| `prev_hooks` | HOOKS | 否 | - | 可选的先前钩子，用于与新创建的 LoRA 钩子组合 |
