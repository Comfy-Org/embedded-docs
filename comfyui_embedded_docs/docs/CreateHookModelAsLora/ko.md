...
|-----------|-----------|----------|-------|-------------|
| `ckpt_name` | STRING | 예 | 여러 옵션 사용 가능 | 가중치를 로드할 체크포인트 파일 (사용 가능한 체크포인트 중에서 선택) |
| `strength_model` | FLOAT | 예 | -20.0 ~ 20.0 | 모델 가중치에 적용되는 강도 배율 (기본값: 1.0) |
| `strength_clip` | FLOAT | 예 | -20.0 ~ 20.0 | CLIP 가중치에 적용되는 강도 배율 (기본값: 1.0) |
| `prev_hooks` | HOOKS | 아니요 | - | 새로 생성된 LoRA 훅과 결합할 선택적 이전 훅 |
...
| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `HOOKS` | HOOKS | 생성된 LoRA 훅으로, 제공된 경우 이전 훅과 결합됩니다. |
