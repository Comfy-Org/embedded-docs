...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `雜訊` | NOISE | 是 | - | 提供取樣過程初始雜訊模式和種子（seed）的雜訊生成器 |
| `引導器` | GUIDER | 是 | - | 引導取樣過程朝向預期輸出結果的引導模型 |
| `取樣器` | SAMPLER | 是 | - | 定義生成過程中潛在空間遍歷方式的取樣演算法 |
| `Sigma 值` | SIGMAS | 是 | - | 控制整個取樣步驟中雜訊程度的 sigma 排程 |
| `latent 影像` | LATENT | 是 | - | 作為取樣起始點的初始潛在表示 |

## 輸出

| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `去雜訊輸出` | LATENT | 完成取樣過程後的最終取樣潛在表示 |
