...
|-----------|-----------|----------|-------|-------------|
| `noise` | NOISE | 예 | - | 샘플링 프로세스에 대한 초기 노이즈 패턴과 시드를 제공하는 노이즈 생성기 |
| `guider` | GUIDER | 예 | - | 샘플링 프로세스를 원하는 출력으로 유도하는 가이던스 모델 |
| `sampler` | SAMPLER | 예 | - | 생성 중 잠재 공간을 탐색하는 방식을 정의하는 샘플링 알고리즘 |
| `sigmas` | SIGMAS | 예 | - | 샘플링 단계 전체에서 노이즈 수준을 제어하는 시그마 스케줄 |
| `latent_image` | LATENT | 예 | - | 샘플링의 시작점 역할을 하는 초기 잠재 표현 |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `output` | LATENT | 샘플링 프로세스를 완료한 후의 최종 샘플링된 잠재 표현 |
