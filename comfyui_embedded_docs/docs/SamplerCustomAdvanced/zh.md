...
| 参数 | 数据类型 | 必填 | 取值范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `noise` | NOISE | 是 | - | 为采样过程提供初始噪声模式和种子的噪声生成器 |
| `guider` | GUIDER | 是 | - | 引导采样过程朝向期望输出的引导模型 |
| `sampler` | SAMPLER | 是 | - | 定义生成过程中潜空间遍历方式的采样算法 |
| `sigmas` | SIGMAS | 是 | - | 控制整个采样步骤中噪声水平的 sigma 调度表 |
| `latent_image` | LATENT | 是 | - | 作为采样起点的初始潜表示 |

## 输出结果

| 输出名称 | 数据类型 | 描述 |
|-------------|-----------|-------------|
| `output` | LATENT | 完成采样过程后的最终采样潜表示 |
