...
| 参数 | 数据类型 | 是否必填 | 范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `noise` | NOISE | 是 | - | 噪声生成器，为采样过程提供初始噪声模式和种子 |
| `guider` | GUIDER | 是 | - | 引导模型，将采样过程导向期望的输出结果 |
| `sampler` | SAMPLER | 是 | - | 采样算法，定义生成过程中潜在空间的遍历方式 |
| `sigmas` | SIGMAS | 是 | - | sigma 调度，控制采样步骤中各个阶段的噪声水平 |
| `latent_image` | LATENT | 是 | - | 初始潜在表示，作为采样的起始点 |

## 输出

| 输出名称 | 数据类型 | 描述 |
|-------------|-----------|-------------|
| `output` | LATENT | 完成采样过程后的最终潜在表示 |
