...
| パラメータ | データ型 | 必須 | 範囲 | 説明 |
|-----------|-----------|----------|-------|-------------|
| `noise` | NOISE | はい | - | サンプリングプロセスの初期ノイズパターンとシードを提供するノイズジェネレータ |
| `guider` | GUIDER | はい | - | サンプリングプロセスを目的の出力に向けて導くガイダンスモデル |
| `sampler` | SAMPLER | はい | - | 生成中に潜在空間をどのように移動するかを定義するサンプリングアルゴリズム |
| `sigmas` | SIGMAS | はい | - | サンプリングステップ全体のノイズレベルを制御するシグマスケジュール |
| `latent_image` | LATENT | はい | - | サンプリングの開始点として機能する初期の潜在表現 |

## 出力

| 出力名 | データ型 | 説明 |
|-------------|-----------|-------------|
| `output` | LATENT | サンプリングプロセス完了後の最終的なサンプリング済み潜在表現 |
