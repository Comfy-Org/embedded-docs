...
| `audio` | AUDIO | 否 | - | 用於驅動影片生成的音訊檔案，例如用於唇形同步或使動作配合節拍。如果未提供，模型將自動生成匹配的背景音樂或音效。音訊長度必須在 1.5 到 60 秒之間。 |
| `seed` | INT | 否 | 0 到 2147483647 | 用於控制生成隨機性的數字，確保結果可重現（預設值：0）。 |
| `prompt_extend` | BOOLEAN | 否 | - | 啟用後，提示詞將透過 AI 輔助進行增強（預設值：True）。 |
| `watermark` | BOOLEAN | 否 | - | 啟用後，AI 生成的水印將添加到結果中（預設值：False）。 |
