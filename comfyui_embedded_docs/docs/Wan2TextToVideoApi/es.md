...
|-----------|--------------|-------------|-------|-------------|
| `modelo` | COMBO | Sí | `"wan2.7-t2v"` | El modelo específico a utilizar para la generación de video. |
...
| `semilla` | INT | No | 0 a 2147483647 | Un número utilizado para controlar la aleatoriedad de la generación, asegurando resultados reproducibles (predeterminado: 0). |
| `extender_prompt` | BOOLEAN | No | - | Cuando está habilitado, el prompt se mejorará con asistencia de IA (predeterminado: True). |
| `marca de agua` | BOOLEAN | No | - | Cuando está habilitado, se agregará una marca de agua generada por IA al resultado (predeterminado: False). |
