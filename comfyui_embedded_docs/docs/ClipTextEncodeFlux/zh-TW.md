...
`CLIPTextEncodeFlux` 是一個專為 Flux 架構設計的文字編碼節點。它會透過不同的編碼器——CLIP-L 與 T5XXL——處理兩個獨立的文字輸入，並結合引導尺度（guidance scale）來產生統一的條件輸出，以供影像生成使用。
...
| `t5xxl` | 由 T5XXL 編碼器處理的文字輸入。適合詳細的自然語言描述，用來表達複雜場景與細節。支援多行輸入與動態提示詞。 | STRING | 是 | - |
| `引導` | 控制文字條件對生成過程的影響程度。數值越高，表示越嚴格遵循文字。預設值：3.5。可以 0.1 為增量調整。 | FLOAT | 是 | 0.0 - 100.0 |

## 輸出

| 輸出名稱 | 描述 | 資料類型 |
| --- | --- | --- |
| `CONDITIONING` | 包含來自兩個編碼器的融合嵌入，以及 `guidance` 參數，用於條件式影像生成。 | CONDITIONING |
