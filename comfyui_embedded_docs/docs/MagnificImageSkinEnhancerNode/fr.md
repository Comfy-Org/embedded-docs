...
|-----------|-----------------|--------|-------|-------------|
| `image` | IMAGE | Oui | - | Le portrait à améliorer. |
| `sharpen` | INT | Non | 0 à 100 | Niveau d'intensité de la netteté (par défaut : 0). |
| `smart_grain` | INT | Non | 0 à 100 | Niveau d'intensité du grain intelligent (par défaut : 2). |
| `mode` | COMBO | Oui | `"creative"`<br>`"faithful"`<br>`"flexible"` | Le mode de traitement à utiliser. `"creative"` est destiné à l'amélioration artistique, `"faithful"` à la préservation de l'aspect original, et `"flexible"` à l'optimisation ciblée. |
...
|------------------|-----------------|-------------|
| `image` | IMAGE | Le portrait amélioré. |
