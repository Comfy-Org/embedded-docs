...
| パラメータ | データ型 | 必須 | 範囲 | 説明 |
|-----------|-----------|----------|-------|-------------|
| `human_image` | IMAGE | はい | - | 試着を行う人物画像 |
| `cloth_image` | IMAGE | はい | - | 人物に試着させる衣服画像 |
| `model_name` | STRING | はい | `"kolors-virtual-try-on-v1"` | 使用するバーチャル試着モデル（デフォルト: "kolors-virtual-try-on-v1"） |
