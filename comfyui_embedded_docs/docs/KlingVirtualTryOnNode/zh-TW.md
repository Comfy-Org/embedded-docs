...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `human_image` | IMAGE | 是 | - | 要進行試穿的人物圖片 |
| `cloth_image` | IMAGE | 是 | - | 要在人物身上試穿的服裝圖片 |
| `model_name` | STRING | 是 | `"kolors-virtual-try-on-v1"` | 要使用的虛擬試穿模型（預設值："kolors-virtual-try-on-v1"） |

## 輸出

| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `output` | IMAGE | 顯示人物穿上該服裝後的結果圖片 |
