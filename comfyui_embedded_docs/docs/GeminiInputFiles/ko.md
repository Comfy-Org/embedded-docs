...
| `GEMINI_INPUT_FILES` | GEMINI_INPUT_FILES | 아니요 | 해당 없음 | 이 노드에서 로드된 파일과 함께 일괄 처리할 선택적 추가 파일입니다. 입력 파일을 연결하여 단일 메시지에 여러 입력 파일을 포함할 수 있습니다. |
...
| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `GEMINI_INPUT_FILES` | GEMINI_INPUT_FILES | Gemini LLM 노드에서 사용할 준비가 된 형식화된 파일 데이터로, 로드된 파일 내용을 적절한 API 형식으로 포함합니다. |
