...
|-----------|-----------|----------|-------|-------------|
| `문자열` | STRING | 예 | - | 일치 항목을 검색할 텍스트 문자열 |
| `정규식 패턴` | STRING | 예 | - | 문자열과 일치시킬 정규 표현식 패턴 |
| `대소문자 구분 안 함` | BOOLEAN | 아니요 | - | 일치 시 대소문자를 무시할지 여부 (기본값: True) |
| `여러 줄` | BOOLEAN | 아니요 | - | 정규 표현식 일치에 여러 줄 모드를 활성화할지 여부 (기본값: False) |
| `모든 문자 포함` | BOOLEAN | 아니요 | - | 정규 표현식 일치에 dotall 모드를 활성화할지 여부 (기본값: False) |
