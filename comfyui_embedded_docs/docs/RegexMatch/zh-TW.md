...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `string` | STRING | 是 | - | 要搜尋比對的文字字串 |
| `regex_pattern` | STRING | 是 | - | 用於比對字串的正規表示式模式 |
| `case_insensitive` | BOOLEAN | 否 | - | 比對時是否忽略大小寫（預設：True） |
| `multiline` | BOOLEAN | 否 | - | 是否啟用正規表示式比對的多行模式（預設：False） |
| `dotall` | BOOLEAN | 否 | - | 是否啟用正規表示式比對的 dotall 模式（預設：False） |
