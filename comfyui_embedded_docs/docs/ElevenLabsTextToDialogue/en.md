...
| `inputs` | Number of dialogue entries. Selecting a number generates that many text and voice input fields. | DYNAMIC_COMBO | Yes | `"1"`<br>`"2"`<br>`"3"`<br>`"4"`<br>`"5"`<br>`"6"`<br>`"7"`<br>`"8"`<br>`"9"`<br>`"10"` |
...
**Note:** The `inputs` parameter is dynamic. When you select a number (e.g., "3"), the node will display three corresponding `text` and `voice` input fields (e.g., `text1`, `voice1`, `text2`, `voice2`, `text3`, `voice3`). Each `text` field must contain at least one character. Each `voice` field accepts a voice connected from the Voice Selector or Instant Voice Clone node.
