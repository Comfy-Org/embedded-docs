...
|-----------|-------------|-----------------|--------|-------|
| `stability` | Stabilité de la voix. Des valeurs plus faibles offrent une gamme émotionnelle plus large, des valeurs plus élevées produisent une parole plus cohérente mais potentiellement monotone. (valeur par défaut : 0.5) | FLOAT | Oui | 0.0 - 1.0 |
...
| `language_code` | Code de langue ISO-639-1 ou ISO-639-3 (par exemple, 'en', 'es', 'fra'). Laisser vide pour la détection automatique. (valeur par défaut : vide) | STRING | Oui | - |
| `seed` | Graine (seed) pour la reproductibilité. (valeur par défaut : 1) | INT | Oui | 0 - 4294967295 |
