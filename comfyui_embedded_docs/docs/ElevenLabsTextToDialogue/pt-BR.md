...
| `model` | COMBO | Não | `"eleven_v3"` | Modelo a ser usado para geração de diálogo. |
| `inputs` | DYNAMICCOMBO | Sim | `"1"`<br>`"2"`<br>`"3"`<br>`"4"`<br>`"5"`<br>`"6"`<br>`"7"`<br>`"8"`<br>`"9"`<br>`"10"` | Número de entradas de diálogo. Selecionar um número gerará essa quantidade de campos de entrada de texto e voz. |
