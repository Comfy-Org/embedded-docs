...
|-----------|-------------|-----------|----------|-------|
| `estabilidade` | Estabilidade da voz. Valores menores proporcionam uma faixa emocional mais ampla; valores maiores produzem uma fala mais consistente, mas potencialmente monótona. (padrão: 0.5) | FLOAT | Sim | 0.0 - 1.0 |
| `aplicar_normalização_texto` | Modo de normalização de texto. 'auto' permite que o sistema decida, 'on' sempre aplica a normalização e 'off' a ignora. | COMBO | Sim | `"auto"`<br>`"on"`<br>`"off"` |
