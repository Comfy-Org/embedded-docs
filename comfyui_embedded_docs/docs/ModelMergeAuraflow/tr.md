...
ModelMergeAuraflow, çeşitli model bileşenleri için belirli karıştırma ağırlıklarını ayarlayarak iki farklı modeli birbirine harmanlamanıza olanak tanır. Modellerin farklı bölümlerinin ilk katmanlardan son çıktılara kadar nasıl birleştirileceği üzerinde ince ayar kontrolü sağlar ve Auraflow tarzı model mimarileriyle kullanılmak üzere tasarlanmıştır. Bu düğüm, birleştirme süreci üzerinde hassas kontrol ile özel model kombinasyonları oluşturmak için özellikle kullanışlıdır.
...
| `init_x_linear.` | İlk doğrusal dönüşüm için karıştırma ağırlığı (varsayılan: 1.0) | FLOAT | Evet | 0.0 - 1.0 |
| `positional_encoding` | Konumsal kodlama bileşenleri için karıştırma ağırlığı (varsayılan: 1.0) | FLOAT | Evet | 0.0 - 1.0 |
| `cond_seq_linear.` | Koşullu dizi doğrusal katmanları için karıştırma ağırlığı (varsayılan: 1.0) | FLOAT | Evet | 0.0 - 1.0 |
| `register_tokens` | Token kayıt bileşenleri için karıştırma ağırlığı (varsayılan: 1.0) | FLOAT | Evet | 0.0 - 1.0 |
...
| --- | --- | --- |
| `model` | Belirtilen karıştırma ağırlıklarına göre her iki giriş modelinin özelliklerini birleştiren birleştirilmiş model | MODEL |
