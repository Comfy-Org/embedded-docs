...
| `model` | COMBO | Oui | Plusieurs options disponibles | Le modèle de génération vidéo à utiliser |
| `aspect_ratio` | COMBO | Oui | Plusieurs options disponibles | Le format d'image pour la vidéo générée (par défaut : 16:9) |
| `resolution` | COMBO | Oui | Plusieurs options disponibles | La résolution de sortie pour la vidéo (par défaut : 540p) |
| `duration` | COMBO | Oui | Plusieurs options disponibles | La durée de la vidéo générée |
| `loop` | BOOLEAN | Oui | - | Indique si la vidéo doit être en boucle (par défaut : Faux) |
