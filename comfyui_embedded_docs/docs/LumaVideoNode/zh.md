...
| `model` | COMBO | 是 | 多个可用选项 | 要使用的视频生成模型 |
| `aspect_ratio` | COMBO | 是 | 多个可用选项 | 生成视频的宽高比（默认：16:9） |
| `resolution` | COMBO | 是 | 多个可用选项 | 视频的输出分辨率（默认：540p） |
| `duration` | COMBO | 是 | 多个可用选项 | 生成视频的时长 |
| `loop` | BOOLEAN | 是 | - | 视频是否应循环播放（默认：False） |
