...
|-----------|-----------|----------|-------|-------------|
| `ckpt_name` | STRING | 예 | 사용 가능한 여러 옵션 | LoRA 모델로 로드할 체크포인트 파일. 사용 가능한 옵션은 체크포인트 폴더 내용에 따라 다릅니다. |
| `strength_model` | FLOAT | 예 | -20.0 ~ 20.0 | 모델 구성 요소에 LoRA를 적용하기 위한 강도 배수 (기본값: 1.0) |
| `prev_hooks` | HOOKS | 아니오 | - | 이 훅과 연결할 선택적 이전 훅들 |
