...
| 参数名 | 数据类型 | 必填 | 取值范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `ckpt_name` | STRING | 是 | 提供多个选项 | 要作为 LoRA 模型加载的检查点文件。可用选项取决于 checkpoints 文件夹中的内容。 |
| `strength_model` | FLOAT | 是 | -20.0 至 20.0 | 将 LoRA 应用于模型组件的强度乘数（默认值：1.0） |
| `prev_hooks` | HOOKS | 否 | - | 可选的先前钩子，用于与此钩子链接 |
