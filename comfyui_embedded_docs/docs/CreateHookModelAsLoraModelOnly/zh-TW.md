...
此節點建立一個掛鉤，用於應用 LoRA（低秩適應）模型來僅修改神經網路的模型組件。它載入一個檢查點檔案，並以指定的強度應用於模型，同時保持 CLIP 組件不變。這是一個實驗性節點，擴展了基礎 CreateHookModelAsLora 類別的功能。
...
| `prev_hooks` | HOOKS | 否 | - | 可選的前置掛鉤，用於與此掛鉤進行鏈接 |
