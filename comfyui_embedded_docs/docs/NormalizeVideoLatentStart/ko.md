...
|-----------|-----------|----------|-------|-------------|
| `latent` | LATENT | 예 | - | 처리할 비디오 잠재 변수 표현입니다. |
| `start_frame_count` | INT | 예 | 1~16384 | 시작부터 계산하여 정규화할 잠재 프레임 수입니다(기본값: 4). |
| `reference_frame_count` | INT | 예 | 1~16384 | 시작 프레임 이후에 참조로 사용할 잠재 프레임 수입니다(기본값: 5). |
...
| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `latent` | LATENT | 시작 프레임이 정규화된 처리된 비디오 잠재 변수입니다. |
