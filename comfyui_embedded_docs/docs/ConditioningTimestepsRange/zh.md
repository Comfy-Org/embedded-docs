...
| 参数 | 数据类型 | 是否必填 | 范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `start_percent` | FLOAT | 是 | 0.0 - 1.0 | 时间步长范围的起始百分比（默认值：0.0） |
| `end_percent` | FLOAT | 是 | 0.0 - 1.0 | 时间步长范围的结束百分比（默认值：1.0） |

## 输出

| 输出名称 | 数据类型 | 描述 |
|-------------|-----------|-------------|
| `TIMESTEPS_RANGE` | TIMESTEPS_RANGE | 由 start_percent 和 end_percent 定义的主时间步长范围 |
| `BEFORE_RANGE` | TIMESTEPS_RANGE | 从 0.0 到 start_percent 的时间步长范围 |
