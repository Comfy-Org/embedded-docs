...
| `sampling_mode` | Controls whether random sampling is used during text generation. When set to "on", additional parameters for controlling sampling become available. The default is "on". | DYNAMIC_COMBO | Yes | "on"<br>"off" |
...
The following sampling parameters are available when `sampling_mode` is set to "on":

| Parameter | Description | Data Type | Required | Range |
| --- | --- | --- | --- | --- |
| `temperature` | Controls the randomness of the output. Lower values make the output more predictable, higher values make it more creative. The default value is 0.7. | FLOAT | No | 0.01 to 2.0 |
| `top_k` | Limits the sampling pool to the top K most likely next tokens. A value of 0 disables this filter. The default value is 64. | INT | No | 0 to 1000 |
| `top_p` | Uses nucleus sampling, limiting choices to tokens whose cumulative probability is less than this value. The default value is 0.95. | FLOAT | No | 0.0 to 1.0 |
| `min_p` | Sets a minimum probability threshold for tokens to be considered. The default value is 0.05. | FLOAT | No | 0.0 to 1.0 |
| `repetition_penalty` | Penalizes tokens that have already been generated to reduce repetition. A value of 1.0 applies no penalty. The default value is 1.05. | FLOAT | No | 0.0 to 5.0 |
...
When `sampling_mode` is set to "off", no additional sampling parameters are available and the node generates text without random sampling.
