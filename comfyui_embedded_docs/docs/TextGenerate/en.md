...
| `system_prompt` | Replaces the system prompt in the model's chat template. Ignored when the default template is not used. Connect a STRING input instead of typing in the node (default: empty). | STRING | No | N/A |
...
**Note:** When the generated text contains a reasoning block (it opens with `<think>` or the prompt ends with it), the reasoning is returned separately: `generated_text` carries the answer and `thinking` carries the reasoning with the opening tag removed. Otherwise `generated_text` carries everything the model produced and `thinking` is empty.

## Outputs

| Output Name | Description | Data Type |
| --- | --- | --- |
| `generated_text` | The text generated by the model based on the input prompt and optional image, video, or audio, with any reasoning block separated out into the `thinking` output. | STRING |
| `thinking` | The reasoning block the model produced, with the opening `<think>` tag removed. Empty when the model produced no reasoning block. | STRING |
