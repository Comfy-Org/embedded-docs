...
| `模型` | COMBO | 是 | 多個選項可用 | 用於圖像生成的模型選擇 |
| `長寬比` | COMBO | 是 | 多個選項可用 | 生成圖像的長寬比（預設：16:9 比例） |
| `種子值` | INT | 是 | 0 到 18446744073709551615 | 決定節點是否應重新執行的種子值；無論種子為何，實際結果都是非確定性的（預設：0） |
| `風格圖權重` | FLOAT | 否 | 0.0 到 1.0 | 風格圖像的權重。如果未提供 style_image 則忽略此參數（預設：1.0） |
| `Luma 參考圖` | LUMA_REF | 否 | - | Luma Reference 節點連接，用輸入圖像影響生成；最多可考慮 4 張圖像 |
