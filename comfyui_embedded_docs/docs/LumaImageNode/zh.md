...
| `model` | COMBO | 是 | 多个可用选项 | 用于图像生成的模型选择 |
| `aspect_ratio` | COMBO | 是 | 多个可用选项 | 生成图像的宽高比（默认：16:9比例） |
| `seed` | INT | 是 | 0 到 18446744073709551615 | 确定节点是否应重新运行的种子；无论种子如何，实际结果都是非确定性的（默认：0） |
| `style_image_weight` | FLOAT | 否 | 0.0 到 1.0 | 风格图像的权重。如果未提供style_image则忽略（默认：1.0） |
| `image_luma_ref` | LUMA_REF | 否 | - | Luma参考节点连接，通过输入图像影响生成；最多可考虑4张图像 |
| `style_image` | IMAGE | 否 | - | 风格参考图像；仅使用1张图像 |
| `character_image` | IMAGE | 否 | - | 角色参考图像；可以是多张图像的批次，最多可考虑4张图像 |
