...
|-----------|-----------|----------|-------|-------------|
| `prompt` | STRING | 是 | - | 图像生成的提示词（默认：空字符串）。长度至少为 3 个字符。 |
| `model` | COMBO | 是 | `photon-flash-1`<br>`photon-1`<br>`photon` | 用于图像生成的模型选择。不同模型成本不同。 |
| `aspect_ratio` | COMBO | 是 | `16:9`<br>`1:1`<br>`4:3`<br>`3:2`<br>`21:9`<br>`9:16`<br>`3:4`<br>`2:3`<br>`9:21` | 生成图像的宽高比（默认：`16:9`） |
| `seed` | INT | 是 | 0 到 18446744073709551615 | 用于判断节点是否应重新运行的种子；实际结果与种子无关，具有非确定性（默认：0） |
| `style_image_weight` | FLOAT | 否 | 0.0 到 1.0 | 风格图像的权重。若未提供 `style_image` 则忽略此参数（默认：1.0） |
| `image_luma_ref` | LUMA_REF | 否 | - | Luma 参考节点连接，用于通过输入图像影响生成结果；最多可考虑 4 张图像。 |
| `style_image` | IMAGE | 否 | - | 风格参考图像；仅使用 1 张图像。 |
| `character_image` | IMAGE | 否 | - | 角色参考图像；可为多张图像的批次，最多可考虑 4 张图像。 |
