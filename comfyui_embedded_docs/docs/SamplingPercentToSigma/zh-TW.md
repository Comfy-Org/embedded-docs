...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `model` | MODEL | 是 | - | 包含用於轉換的取樣參數的模型 |
| `sampling_percent` | FLOAT | 是 | 0.0 到 1.0 | 要轉換為 sigma 的取樣百分比（預設值：0.0） |
| `return_actual_sigma` | BOOLEAN | 是 | - | 返回實際 sigma 值，而非用於區間檢查的值。此選項僅影響 0.0 和 1.0 時的結果。（預設值：False） |

## 輸出

| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `sigma_value` | FLOAT | 對應於輸入取樣百分比的轉換後 sigma 值 |
