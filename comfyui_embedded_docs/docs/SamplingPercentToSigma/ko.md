...
|-----------|-----------|----------|-------|-------------|
| `모델` | MODEL | 예 | - | 변환에 사용되는 샘플링 매개변수를 포함한 모델 |
| `샘플링 백분율` | FLOAT | 예 | 0.0 ~ 1.0 | 시그마로 변환할 샘플링 백분율 (기본값: 0.0) |
| `실제 시그마 값 반환` | BOOLEAN | 예 | - | 구간 확인에 사용되는 값 대신 실제 시그마 값을 반환합니다. 이는 0.0 및 1.0에서의 결과에만 영향을 미칩니다. (기본값: False) |
