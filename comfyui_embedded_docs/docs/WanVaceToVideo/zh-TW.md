...
| `長度` | INT | 是 | 1 至 MAX_RESOLUTION | 影片的幀數（預設值：81，步長：4） |
| `批次大小` | INT | 是 | 1 至 4096 | 同時生成的影片數量（預設值：1） |
| `強度` | FLOAT | 是 | 0.0 至 1000.0 | 影片條件的控制強度（預設值：1.0，步長：0.01） |
| `控制影片` | IMAGE | 否 | - | 用於控制條件的可選輸入影片 |
| `控制遮罩` | MASK | 否 | - | 用於控制影片中哪些部分要修改的可選遮罩 |
| `參考影像` | IMAGE | 否 | - | 用於額外條件的可選參考影像 |
...
| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `負向` | CONDITIONING | 已套用影片控制資料的正向條件 |
| `潛在空間` | CONDITIONING | 已套用影片控制資料的負向條件 |
