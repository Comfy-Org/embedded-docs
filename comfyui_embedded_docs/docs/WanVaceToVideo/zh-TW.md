...
**注意：** 當提供 `control_video` 時，它將被放大以符合指定的寬度和高度。如果提供 `control_masks`，則必須與控制影片的尺寸相符。當提供 `reference_image` 時，它會透過 VAE 進行編碼並前置到潛在序列中。

## 輸出結果

| 輸出名稱 | 資料類型 | 描述 |
|-------------|-----------|-------------|
| `負向` | CONDITIONING | 應用影片控制資料後的正向條件 |
| `潛在空間` | CONDITIONING | 應用影片控制資料後的負向條件 |
