...
| パラメータ | データ型 | 必須 | 範囲 | 説明 |
|-----------|-----------|----------|-------|-------------|
| `positive` | CONDITIONING | はい | - | 生成をガイドするためのポジティブな条件付け入力 |
| `negative` | CONDITIONING | はい | - | 生成をガイドするためのネガティブな条件付け入力 |
...
| `length` | INT | はい | 1 から MAX_RESOLUTION | ビデオのフレーム数（デフォルト: 81, ステップ: 4） |
| `batch_size` | INT | はい | 1 から 4096 | 同時に生成するビデオの数（デフォルト: 1） |
| `strength` | FLOAT | はい | 0.0 から 1000.0 | ビデオ条件付けの制御強度（デフォルト: 1.0, ステップ: 0.01） |
| `control_video` | IMAGE | いいえ | - | 制御条件付けのためのオプションの入力ビデオ |
| `control_masks` | MASK | いいえ | - | ビデオのどの部分を変更するかを制御するためのオプションのマスク |
| `reference_image` | IMAGE | いいえ | - | 追加の条件付けのためのオプションの参照画像 |
...
| 出力名 | データ型 | 説明 |
|-------------|-----------|-------------|
| `positive` | CONDITIONING | ビデオ制御データが適用されたポジティブな条件付け |
| `negative` | CONDITIONING | ビデオ制御データが適用されたネガティブな条件付け |
| `latent` | LATENT | ビデオ生成の準備ができた空の潜在テンソル |
