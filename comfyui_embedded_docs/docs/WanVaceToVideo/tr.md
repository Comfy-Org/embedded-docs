...
| `genişlik` | Çıktı video genişliği piksel cinsinden (varsayılan: 832, adım: 16) | INT | Evet | 16 ile MAX_RESOLUTION |
| `yükseklik` | Çıktı video yüksekliği piksel cinsinden (varsayılan: 480, adım: 16) | INT | Evet | 16 ile MAX_RESOLUTION |
...
| `kontrol_videosu` | Kontrol koşullandırması için isteğe bağlı girdi videosu. Sağlanmazsa otomatik olarak nötr gri bir video oluşturulur. | IMAGE | Hayır | - |
| `kontrol_maskeleri` | Kontrol videosunun hangi bölümlerinin etkin olduğunu belirleyen isteğe bağlı maskeler. Sağlanmazsa tam beyaz bir maske kullanılır. | MASK | Hayır | - |
| `referans_görüntüsü` | Ek koşullandırma için isteğe bağlı referans görüntüsü. Sağlandığında kodlanır ve latent dizisinin başına eklenir. Yalnızca ilk görüntü kullanılır. | IMAGE | Hayır | - |
