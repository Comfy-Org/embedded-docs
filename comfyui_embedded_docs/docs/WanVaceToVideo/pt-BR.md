...
| --- | --- | --- | --- | --- |
| `positive` | Entrada de condicionamento positivo para guiar a geração | CONDITIONING | Sim | - |
| `negative` | Entrada de condicionamento negativo para guiar a geração | CONDITIONING | Sim | - |
...
| `length` | Número de frames no vídeo (padrão: 81, passo: 4) | INT | Sim | 1 a MAX_RESOLUTION |
| `batch_size` | Número de vídeos a serem gerados simultaneamente (padrão: 1) | INT | Sim | 1 a 4096 |
| `strength` | Força do condicionamento para controle VACE (padrão: 1.0, passo: 0.01). Esta não é uma força de LoRA. Os pesos de LoRA são aplicados por meio de nós LoRA separados. | FLOAT | Sim | 0.0 a 1000.0 |
...
| `control_masks` | Máscaras opcionais que determinam quais partes do vídeo de controle estão ativas. Se não forem fornecidas, uma máscara totalmente branca é usada. | MASK | Não | - |
| `reference_image` | Imagem de referência opcional para condicionamento adicional. Quando fornecida, ela é codificada e adicionada no início da sequência latente. Apenas a primeira imagem é usada. | IMAGE | Não | - |
