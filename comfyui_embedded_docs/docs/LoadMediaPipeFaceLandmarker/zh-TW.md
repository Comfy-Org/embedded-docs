...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `model_name` | STRING | 是 | `models/detection/` 目錄中可用的模型清單 | 來自 `models/detection/` 目錄的人臉偵測模型。 |

## 輸出

| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `FACE_DETECTION_MODEL` | FACE_DETECTION_MODEL | 一個已載入的 FaceLandmarker 模型物件，包含兩種偵測變體（短距離/全距離）、臉部拓撲連接集、標準資料以及用於 GPU 管理的模型修補器。 |
