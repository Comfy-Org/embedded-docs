...
|-----------|-----------------|--------|-------|-------------|
| `model_name` | STRING | Oui | Liste des modèles disponibles dans le répertoire `models/detection/` | Modèle de détection faciale provenant de models/detection/. |

## Sorties

| Nom de sortie | Type de données | Description |
|---------------|-----------------|-------------|
| `FACE_DETECTION_MODEL` | FACE_DETECTION_MODEL | Un objet modèle FaceLandmarker chargé contenant les deux variantes de détection (courte/longue portée), les ensembles de connexions pour la topologie faciale, les données canoniques et les correcteurs de modèle pour la gestion du GPU. |
