...
| `duration_seconds` | COMBO | 예 | `dur_2`<br>`dur_5`<br>`dur_10` | 생성된 비디오의 길이(초 단위) |
...
| `seed` | INT | 예 | 0-2147483647 | 비디오 생성을 위한 시드 값 (기본값: 0) |
| `negative_prompt` | STRING | 아니오 | - | 이미지에서 원하지 않는 요소에 대한 선택적 텍스트 설명 |
| `pixverse_template` | CUSTOM | 아니오 | - | 생성 스타일에 영향을 주기 위한 선택적 템플릿으로, PixVerse Template 노드에서 생성됨 |
