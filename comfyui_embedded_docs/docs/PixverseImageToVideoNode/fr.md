...
| `mode_mouvement` | COMBO | Oui | `normal`<br>`fast`<br>`slow`<br>`zoom_in`<br>`zoom_out`<br>`pan_left`<br>`pan_right`<br>`pan_up`<br>`pan_down`<br>`tilt_up`<br>`tilt_down`<br>`roll_clockwise`<br>`roll_counterclockwise` | Style de mouvement appliqué à la génération vidéo |
| `graine` | INT | Oui | 0-2147483647 | Graine pour la génération vidéo (par défaut : 0) |
| `prompt_négatif` | STRING | Non | - | Description textuelle facultative des éléments indésirables sur une image |
| `modèle_pixverse` | CUSTOM | Non | - | Modèle facultatif pour influencer le style de génération, créé par le nœud PixVerse Template |
