...
| `quality` | COMBO | 是 | `res_540p`<br>`res_1080p` | 视频质量设置（默认：res_540p） |
| `duration_seconds` | COMBO | 是 | `dur_2`<br>`dur_5`<br>`dur_10` | 生成视频的持续时间（秒） |
...
| `seed` | INT | 是 | 0-2147483647 | 视频生成的随机种子（默认：0） |
| `negative_prompt` | STRING | 否 | - | 图像中不希望出现的元素的可选文本描述 |
