...
Le nœud AudioEncoderLoader charge un modèle d'encodeur audio à partir d'un fichier stocké dans votre dossier `audio_encoders`. Il prend en entrée le nom de fichier d'un modèle d'encodeur audio et renvoie le modèle chargé, qui peut ensuite être utilisé pour des tâches de traitement audio dans votre workflow. Si le fichier sélectionné ne contient pas un modèle d'encodeur audio valide, le nœud génère une erreur.

## Entrées

| Paramètre | Description | Type de données | Requis | Plage |
| --- | --- | --- | --- | --- |
| `nom_encodeur_audio` | Sélectionne le fichier de modèle d'encodeur audio à charger depuis le dossier des encodeurs audio | COMBO | Oui | Liste des fichiers d'encodeur audio disponibles dans le dossier `audio_encoders` |
