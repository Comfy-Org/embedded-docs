...
| `最大偏移` | FLOAT | 是 | 0.0 - 100.0 | 取樣計算的最大偏移值（預設值：1.15） |
| `基礎偏移` | FLOAT | 是 | 0.0 - 100.0 | 取樣計算的基礎偏移值（預設值：0.5） |
| `寬度` | INT | 是 | 16 - MAX_RESOLUTION | 目標影像的寬度（像素為單位）（預設值：1024） |
| `高度` | INT | 是 | 16 - MAX_RESOLUTION | 目標影像的高度（像素為單位）（預設值：1024） |
