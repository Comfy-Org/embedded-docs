...
| `max_shift` | FLOAT | 是 | 0.0 - 100.0 | 采样计算的最大偏移值（默认值：1.15） |
| `base_shift` | FLOAT | 是 | 0.0 - 100.0 | 采样计算的基准偏移值（默认值：0.5） |
| `width` | INT | 是 | 16 - MAX_RESOLUTION | 目标图像的宽度（像素）（默认值：1024） |
| `height` | INT | 是 | 16 - MAX_RESOLUTION | 目标图像的高度（像素）（默认值：1024） |

## 输出

| 输出名称 | 数据类型 | 描述 |
|-------------|-----------|-------------|
| `model` | MODEL | 已应用 Flux 采样配置的修改后模型 |
