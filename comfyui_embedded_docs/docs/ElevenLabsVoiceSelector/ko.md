...
|-----------|-----------|----------|-------|-------------|
| `voice` | STRING | 예 | `"Adam"`<br>`"Antoni"`<br>`"Arnold"`<br>`"Bella"`<br>`"Domi"`<br>`"Elli"`<br>`"Josh"`<br>`"Rachel"`<br>`"Sam"` | 미리 정의된 ElevenLabs 음성 중에서 음성을 선택합니다. |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `voice` | STRING | 선택한 ElevenLabs 음성의 고유 식별자로, 텍스트 음성 변환 생성을 위해 다른 노드에 전달할 수 있습니다. |
