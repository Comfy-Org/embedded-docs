...
Le nœud ComboOptionTestNode est un nœud logique conçu pour tester et transmettre les sélections de listes déroulantes. Il prend deux entrées de liste déroulante, chacune avec un ensemble d'options prédéfini, et renvoie directement les valeurs sélectionnées sans modification.
...
| `combo2` | La seconde sélection parmi un ensemble différent de trois options de test. | COMBO | Oui | `"option4"`<br>`"option5"`<br>`"option6"` |
...
| `output_2` | Renvoie la valeur sélectionnée dans la seconde liste déroulante (`combo2`). | COMBO |
