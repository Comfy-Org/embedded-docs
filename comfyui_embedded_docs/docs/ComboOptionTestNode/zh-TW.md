...
The ComboOptionTestNode 是一個邏輯節點，用於測試並傳遞下拉式選單的選取結果。它接收兩個下拉式選單輸入，每個輸入都具有預先定義的選項集合，並直接輸出選取的值而不作任何修改。

## 輸入

| 參數 | 描述 | 資料類型 | 必要 | 範圍 |
| --- | --- | --- | --- | --- |
| `combo` | 從三個測試選項中做出的第一個選擇。 | COMBO | 是 | `"option1"`<br>`"option2"`<br>`"option3"` |
| `combo2` | 從另一組三個測試選項中做出的第二個選擇。 | COMBO | 是 | `"option4"`<br>`"option5"`<br>`"option6"` |
