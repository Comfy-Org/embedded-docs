...
| `luminance` | ITU-R BT.709標準輝度計算式を用いて計算された、画像の知覚的な明るさのヒストグラムです。 | HISTOGRAM |
| `red` | 赤色チャンネルにおけるピクセル強度の分布を示すヒストグラムです。 | HISTOGRAM |
| `green` | 緑色チャンネルにおけるピクセル強度の分布を示すヒストグラムです。 | HISTOGRAM |
