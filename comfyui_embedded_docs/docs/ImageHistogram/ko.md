...
|-----------|-----------|----------|-------|-------------|
| `이미지` | IMAGE | 예 | 해당 없음 | 분석할 입력 이미지입니다. 노드는 배치(batch) 내 첫 번째 이미지를 처리합니다. |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `휘도` | HISTOGRAM | 빨간색, 녹색, 파란색 채널의 평균 픽셀 강도를 나타내는 복합 히스토그램입니다. |
| `레드` | HISTOGRAM | ITU-R BT.709 표준 휘도 공식을 사용하여 계산된 이미지의 인지적 밝기에 대한 히스토그램입니다. |
| `그린` | HISTOGRAM | 빨간색 채널의 픽셀 강도 분포를 보여주는 히스토그램입니다. |
| `블루` | HISTOGRAM | 녹색 채널의 픽셀 강도 분포를 보여주는 히스토그램입니다. |
| `blue` | HISTOGRAM | 파란색 채널의 픽셀 강도 분포를 보여주는 히스토그램입니다. |
