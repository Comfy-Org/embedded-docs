...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `影像` | IMAGE | 是 | 不適用 | 要分析的輸入圖像。此節點會處理批次中的第一張圖像。 |

## 輸出

| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `亮度` | HISTOGRAM | 一個複合直方圖，代表紅色、綠色和藍色通道的平均像素強度。 |
| `紅色` | HISTOGRAM | 圖像感知亮度的直方圖，使用 ITU-R BT.709 標準亮度公式計算。 |
| `綠色` | HISTOGRAM | 顯示紅色色彩通道中像素強度分佈的直方圖。 |
| `藍色` | HISTOGRAM | 顯示綠色色彩通道中像素強度分佈的直方圖。 |
