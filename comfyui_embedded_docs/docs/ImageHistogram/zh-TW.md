...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `image` | IMAGE | 是 | 不適用 | 要分析的輸入圖像。此節點會處理批次中的第一張圖像。 |
...
| `luminance` | HISTOGRAM | 圖像感知亮度的直方圖，使用 ITU-R BT.709 標準亮度公式計算。 |
| `red` | HISTOGRAM | 顯示紅色色彩通道中像素強度分佈的直方圖。 |
| `green` | HISTOGRAM | 顯示綠色色彩通道中像素強度分佈的直方圖。 |
| `blue` | HISTOGRAM | 顯示藍色色彩通道中像素強度分佈的直方圖。 |
