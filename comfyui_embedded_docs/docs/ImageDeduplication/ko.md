...
|-----------|-----------|----------|-------|-------------|
| `이미지` | IMAGE | 예 | - | 중복 제거를 처리할 이미지 배치입니다. |
| `유사도 임계값` | FLOAT | 아니요 | 0.0 - 1.0 | 유사도 임계값(0-1)입니다. 값이 높을수록 더 유사함을 의미합니다. 이 임계값 이상인 이미지는 중복으로 간주됩니다. (기본값: 0.95) |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `이미지` | IMAGE | 중복이 제거된 필터링된 이미지 목록입니다. |
