...
| `positive` | CONDITIONING | 是 | - | 正向条件，引导生成过程趋向期望内容 |
| `negative` | CONDITIONING | 是 | - | 负向条件，引导生成过程远离不想要的内容 |
| `cfg` | FLOAT | 是 | 0.0 至 100.0 | 无分类器引导尺度，控制条件对生成过程的影响强度（默认值：8.0） |

## 输出

| 输出名称 | 数据类型 | 描述 |
|-------------|-----------|-------------|
| `GUIDER` | GUIDER | 一个引导器对象，可传递给采样节点以控制生成过程 |
