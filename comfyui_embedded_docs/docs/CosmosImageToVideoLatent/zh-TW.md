...
| `寬度` | INT | 是 | 16 至 MAX_RESOLUTION | 輸出影片的寬度（像素），預設值：1280 |
| `高度` | INT | 是 | 16 至 MAX_RESOLUTION | 輸出影片的高度（像素），預設值：704 |
| `長度` | INT | 是 | 1 至 MAX_RESOLUTION | 影片序列中的影格數量，預設值：121 |
| `批次大小` | INT | 是 | 1 至 4096 | 要生成的潛在批次數量，預設值：1 |
| `起始影像` | IMAGE | 否 | - | 可選，編碼至影片序列開頭的圖像 |
| `結束圖片` | IMAGE | 否 | - | 可選，編碼至影片序列結尾的圖像 |
