...
| `width` | INT | 否 | 16 至 MAX_RESOLUTION | 輸出影片的寬度（單位：像素，預設值：1280） |
| `height` | INT | 否 | 16 至 MAX_RESOLUTION | 輸出影片的高度（單位：像素，預設值：704） |
| `length` | INT | 否 | 1 至 MAX_RESOLUTION | 影片序列中的幀數（預設值：121） |
| `batch_size` | INT | 否 | 1 至 4096 | 要生成的潛在批次數量（預設值：1） |
| `start_image` | IMAGE | 否 | - | 可選的圖像，用於編碼到影片序列的開頭 |
| `end_image` | IMAGE | 否 | - | 可選的圖像，用於編碼到影片序列的結尾 |
