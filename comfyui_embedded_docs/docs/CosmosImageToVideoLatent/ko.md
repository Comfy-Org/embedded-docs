...
| `width` | INT | 아니오 | 16 ~ MAX_RESOLUTION | 출력 비디오의 너비 (픽셀 단위, 기본값: 1280) |
| `height` | INT | 아니오 | 16 ~ MAX_RESOLUTION | 출력 비디오의 높이 (픽셀 단위, 기본값: 704) |
| `length` | INT | 아니오 | 1 ~ MAX_RESOLUTION | 비디오 시퀀스의 프레임 수 (기본값: 121) |
| `batch_size` | INT | 아니오 | 1 ~ 4096 | 생성할 잠재 배치의 수 (기본값: 1) |
| `start_image` | IMAGE | 아니오 | - | 비디오 시퀀스의 시작 부분에 인코딩할 선택적 이미지 |
| `end_image` | IMAGE | 아니오 | - | 비디오 시퀀스의 끝 부분에 인코딩할 선택적 이미지 |
