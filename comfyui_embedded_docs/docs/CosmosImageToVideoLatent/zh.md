...
| `width` | INT | 否 | 16 至 MAX_RESOLUTION | 输出视频的宽度（像素）（默认值：1280） |
| `height` | INT | 否 | 16 至 MAX_RESOLUTION | 输出视频的高度（像素）（默认值：704） |
| `length` | INT | 否 | 1 至 MAX_RESOLUTION | 视频序列的帧数（默认值：121） |
| `batch_size` | INT | 否 | 1 至 4096 | 要生成的潜空间批次数量（默认值：1） |
| `start_image` | IMAGE | 否 | - | 可选，在视频序列开头编码的图像 |
| `end_image` | IMAGE | 否 | - | 可选，在视频序列末尾编码的图像 |
