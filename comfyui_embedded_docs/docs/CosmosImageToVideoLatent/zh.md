...
| `width` | INT | 是 | 16 至 MAX_RESOLUTION | 输出视频的宽度（像素），默认值：1280 |
| `height` | INT | 是 | 16 至 MAX_RESOLUTION | 输出视频的高度（像素），默认值：704 |
| `length` | INT | 是 | 1 至 MAX_RESOLUTION | 视频序列中的帧数，默认值：121 |
| `batch_size` | INT | 是 | 1 至 4096 | 要生成的潜在批次数量，默认值：1 |
| `start_image` | IMAGE | 否 | - | 可选，编码到视频序列起始位置的图像 |
| `end_image` | IMAGE | 否 | - | 可选，编码到视频序列结束位置的图像 |
