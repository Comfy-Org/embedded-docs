...
ByteDance Seedream 4.5 & 5.0 génère des images à partir d’un prompt texte (text-to-image) ou génère/modifie des images guidées par des images de référence facultatives, en utilisant les modèles ByteDance Seedream 4.0, 4.5 et 5.0 à une résolution allant jusqu’à 4K. Le nœud envoie le prompt et les éventuelles images de référence à l’API ByteDance, attend la fin de la tâche de génération et renvoie le tenseur d’image ou les tenseurs résultants.
...
| Paramètre | Description | Type de données | Obligatoire | Plage |
|-----------|-------------|-----------------|-------------|-------|
| `size_preset` | Choisissez une taille recommandée. Sélectionnez Custom pour utiliser la largeur et la hauteur ci-dessous. Par défaut : premier préréglage recommandé pour ce modèle. | COMBO | Non | Préréglages de taille recommandés propres au modèle<br>"Custom" |
| `width` | Largeur personnalisée de l’image. La valeur n’est utilisée que si `size_preset` est défini sur `Custom`. Par défaut : 2048. | INT | Non | 1024 à 3136 (pas 2) |
| `height` | Hauteur personnalisée de l’image. La valeur n’est utilisée que si `size_preset` est défini sur `Custom`. Par défaut : 2048. | INT | Non | 1024 à 2496 (pas 2) |
...
|-----------|-------------|-----------------|-------------|-------|
| `size_preset` | Choisissez une taille recommandée. Sélectionnez Custom pour utiliser la largeur et la hauteur ci-dessous. Par défaut : premier préréglage recommandé pour ce modèle. | COMBO | Non | Préréglages de taille recommandés propres au modèle<br>"Custom" |
| `width` | Largeur personnalisée de l’image. La valeur n’est utilisée que si `size_preset` est défini sur `Custom`. Par défaut : 2048. | INT | Non | 1024 à 6240 (pas 2) |
| `height` | Hauteur personnalisée de l’image. La valeur n’est utilisée que si `size_preset` est défini sur `Custom`. Par défaut : 2048. | INT | Non | 1024 à 4992 (pas 2) |
| `max_images` | Nombre maximal d’images à générer. Avec 1, exactement une image est produite. Avec >1, le modèle génère entre 1 et `max_images` images liées (p. ex., scènes d’histoire, variations de personnage). Le nombre total d’images (entrée + générées) ne peut pas dépasser 15. Par défaut : 1. | INT | Non | 1 à 14 |
| `fail_on_partial` | Si activé, interrompt l’exécution si des images demandées sont manquantes ou renvoient une erreur. Par défaut : false. | BOOLEAN | Non | true / false |
| `seed` | Graine à utiliser pour la génération. Par défaut : 42. | INT | Non | 0 à 2147483647 |
| `watermark` | Indique s’il faut ajouter un filigrane « AI generated » à l’image. Par défaut : false. | BOOLEAN | Non | true / false |
| `thinking` | Active le raisonnement d’optimisation du prompt du modèle (« thinking ») pour une meilleure conformité. Peut augmenter sensiblement le temps de génération — notamment sur Seedream 5.0 Pro. Ne peut être désactivé que pour la génération text-to-image (pas lorsque des images de référence sont fournies). Par défaut : true. | BOOLEAN | Non | true / false |
...
| `width` | Largeur personnalisée de l’image. La valeur n’est utilisée que si `size_preset` est défini sur `Custom`. Par défaut : 2048. | INT | Non | 1024 à 6240 (pas 2) |
| `height` | Hauteur personnalisée de l’image. La valeur n’est utilisée que si `size_preset` est défini sur `Custom`. Par défaut : 2048. | INT | Non | 1024 à 4992 (pas 2) |
| `max_images` | Nombre maximal d’images à générer. Avec 1, exactement une image est produite. Avec >1, le modèle génère entre 1 et `max_images` images liées (p. ex., scènes d’histoire, variations de personnage). Le nombre total d’images (entrée + générées) ne peut pas dépasser 15. Par défaut : 1. | INT | Non | 1 à 10 |
| `fail_on_partial` | Si activé, interrompt l’exécution si des images demandées sont manquantes ou renvoient une erreur. Par défaut : false. | BOOLEAN | Non | true / false |
| `seed` | Graine à utiliser pour la génération. Par défaut : 42. | INT | Non | 0 à 2147483647 |
| `watermark` | Indique s’il faut ajouter un filigrane « AI generated » à l’image. Par défaut : false. | BOOLEAN | Non | true / false |
| `thinking` | Active le raisonnement d’optimisation du prompt du modèle (« thinking ») pour une meilleure conformité. Peut augmenter sensiblement le temps de génération — notamment sur Seedream 5.0 Pro. Ne peut être désactivé que pour la génération text-to-image (pas lorsque des images de référence sont fournies). Par défaut : true. | BOOLEAN | Non | true / false |
...
| Paramètre | Description | Type de données | Obligatoire | Plage |
|-----------|-------------|-----------------|-------------|-------|
| `images` | Emplacement extensible : image(s) de référence facultative(s) pour la génération image-to-image ou multi-référence. Connectez 1..N images (p. ex., `image_1`, `image_2`, ...) ; la limite de nombre est propre à chaque modèle (voir notes ci-dessous). Si une image connectée contient un lot d’images, chaque image du lot est comptée dans la limite. | IMAGE | Non | 0 à 10 (Seedream 5.0 Pro, Seedream 4.5, Seedream 4.0)<br>0 à 14 (Seedream 5.0 Lite) |
...
- Exigences de résolution (largeur x hauteur) :
  ...
  - Tous les modèles non-Pro : au plus 16,78 MP (16 777 216 pixels).
