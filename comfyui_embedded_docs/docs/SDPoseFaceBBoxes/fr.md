...
|-----------|-----------------|--------|-------|-------------|
| `keypoints` | POSE_KEYPOINT | Oui | - | Données de points clés de pose contenant des informations sur les personnes détectées et leurs repères corporels/du visage par image. |
...
|---------------|-----------------|-------------|
| `bboxes` | BOUNDINGBOX | Une liste de boîtes englobantes de visages pour chaque image. Chaque boîte englobante est définie par ses coordonnées du coin supérieur gauche (`x`, `y`), sa `largeur` et sa `hauteur`. Cette sortie est compatible avec l'entrée `bboxes` du nœud SDPoseKeypointExtractor. |
