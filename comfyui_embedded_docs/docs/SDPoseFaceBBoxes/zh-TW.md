...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `keypoints` | POSE_KEYPOINT | 是 | - | 包含每幀畫面中偵測到的人物及其身體/臉部關鍵點資訊的姿態關鍵點資料。 |
| `scale` | FLOAT | 否 | 1.0 - 10.0 | 每個偵測到的人臉周圍邊界框面積的倍數。數值越大，生成的框越大。（預設值：1.5） |
| `force_square` | BOOLEAN | 否 | - | 擴展較短的邊界框軸，使裁切區域始終為正方形。（預設值：True） |
...
| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `bboxes` | BOUNDINGBOX | 每幀畫面的人臉邊界框列表。每個邊界框由其左上角座標（`x`、`y`）、`width`（寬度）和 `height`（高度）定義。此輸出與 SDPoseKeypointExtractor 節點的 `bboxes` 輸入相容。 |
