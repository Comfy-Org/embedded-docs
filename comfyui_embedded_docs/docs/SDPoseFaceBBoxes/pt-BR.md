...
|-------------|-----------|-------------|
| `bboxes` | BOUNDINGBOX | Uma lista de caixas delimitadoras faciais para cada quadro. Cada caixa delimitadora é definida por suas coordenadas do canto superior esquerdo (`x`, `y`), `width` e `height`. Esta saída é compatível com a entrada `bboxes` do nó SDPoseKeypointExtractor. |
