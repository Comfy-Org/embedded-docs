...
| 參數 | 說明 | 資料類型 | 必填 | 範圍 |
|-----------|-------------|-----------|----------|-------|
| `圖片` | 包含要替換物件的影像。影像上傳前會移除 alpha 通道。 | IMAGE | 是 | - |
| `指令` | 要將什麼替換成什麼，例如「Replace the red apple with a green pear」。長度必須至少為 1 個字元。 | STRING | 是 | 多行文字；預設：""（空） |
| `種子` | Bria 在此不使用 `seed`，且每次呼叫都會重新構思編輯，因此重複執行可能產生不同結果。此值永遠不會被傳送：它只會改變此節點的快取鍵，因此一個在其他方面相同的圖形會再次執行編輯，而不是傳回快取結果。 | INT | 是 | 0 至 2147483647，步長 1；預設：42；啟用產生後控制 |

### 審核輸入

| 參數 | 說明 | 資料類型 | 必填 | 範圍 |
|-----------|-------------|-----------|----------|-------|
| `內容審核` | 審核設定。選取 "true" 會顯示下方的審核子選項；否則不會顯示。 | DYNAMIC_COMBO | 是 | `"false"`<br>`"true"` |
