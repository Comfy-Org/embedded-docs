...
| `圖片` | IMAGE | 是 | - | 要編碼並用於條件化影片潛在表示的輸入影像。 |
| `latent` | LATENT | 是 | - | 要修改的目標影片潛在表示。 |
| `強度` | FLOAT | 否 | 0.0 - 1.0 | 控制編碼影像融入潛在表示的混合強度。值為 1.0 時完全取代初始影格，較低的值則進行混合。（預設值：1.0） |
| `繞過` | BOOLEAN | 否 | - | 繞過條件化處理。啟用時，節點會直接回傳未修改的輸入潛在表示。（預設值：False） |
