...
| `強度` | 控制編碼影像對 latent 初始幀的條件化強度。值為 1.0 時，初始幀會完全由編碼影像條件化；數值越低，條件化強度越弱。初始幀的雜訊遮罩會設為 `1.0 - strength`。（預設值：1.0） | FLOAT | 是 | 0.0 - 1.0 |
| `繞過` | 繞過條件化。啟用時，節點會原樣返回輸入的 latent。（預設值：False） | BOOLEAN | 是 | True or False |
