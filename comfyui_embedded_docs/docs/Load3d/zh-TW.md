...
Load3D 節點是用於載入和處理 3D 模型檔案的核心節點。載入節點時，會自動從 `ComfyUI/input/3d/` 獲取可用的 3D 資源。您也可以使用上傳功能上傳支援的 3D 檔案進行預覽。
...
| 參數名稱       | 類型           | 描述                                                     | 預設值 | 範圍          |
|---------------|----------------|----------------------------------------------------------|---------|--------------|
| model_file    | 檔案選擇       | 3D 模型檔案路徑，支援上傳，預設從 `ComfyUI/input/3d/` 讀取模型檔案 | - | 支援格式      |
...
![Load 3D 節點介面](./asset/load3d_ui.jpg)
...
1. **重設檢視比例**：點擊按鈕後，檢視會根據設定的寬度和高度調整畫布渲染區域比例
2. **影片錄製**：可將目前的 3D 檢視操作錄製為影片，允許匯入，並可作為 `recording_video` 輸出到後續節點