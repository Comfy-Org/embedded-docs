...
VoxelToMeshBasic 節點可將 3D 體素資料轉換為網格幾何體。它透過應用閾值來處理體素體積，以確定體積的哪些部分在生成的網格中成為實體表面。該節點輸出包含頂點和面的完整網格結構，可用於 3D 渲染和建模。
...
| `臨界值` | FLOAT | 是 | -1.0 至 1.0 | 用於確定哪些體素成為網格表面部分的閾值（預設值：0.6） |
