...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `model` | MODEL | 是 | - | 將要套用 TSR 函數修補的擴散模型。 |
| `tsr_k` | FLOAT | 否 | 0.01 - 100.0 | 控制重新縮放的強度。較低的 k 值會產生更詳細的結果；較高的 k 值在圖像生成中會產生更平滑的結果。設定 k = 1 會停用重新縮放。（預設值：0.95） |
| `tsr_sigma` | FLOAT | 否 | 0.01 - 100.0 | 控制重新縮放生效的時機。數值越大，生效時間越早。（預設值：1.0） |

## 輸出

| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `patched_model` | MODEL | 輸入模型，現已套用時間分數重新縮放函數至其採樣過程。 |
