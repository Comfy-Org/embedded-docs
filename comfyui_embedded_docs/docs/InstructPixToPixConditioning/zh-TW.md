...
InstructPixToPixConditioning 節點透過將正向與反向文字提示詞與影像資料結合，為 InstructPix2Pix 影像編輯準備條件資料。它透過 VAE 編碼器處理輸入影像以創建潛在表徵，並將這些潛在表徵附加到正向與反向條件資料中。此節點會自動將影像尺寸裁剪至 8 像素的倍數，以確保與 VAE 編碼過程的相容性。
