...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `正向` | CONDITIONING | 是 | - | 包含文字提示與所需影像特徵設定的正向條件化資料 |
| `負向` | CONDITIONING | 是 | - | 包含文字提示與非所需影像特徵設定的負向條件化資料 |
| `vae` | VAE | 是 | - | 用於將輸入影像編碼為潛在表示的 VAE 模型 |
| `像素` | IMAGE | 是 | - | 待處理並編碼至潛在空間的輸入影像 |
...
| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `負向` | CONDITIONING | 附加了潛在影像表示的正向條件化資料 |
| `潛在空間` | CONDITIONING | 附加了潛在影像表示的負向條件化資料 |
