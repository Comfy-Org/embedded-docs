...
| 参数名 | 数据类型 | 必填 | 取值范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `positive` | CONDITIONING | 是 | - | 包含期望图像特征的文本提示和设置的正向条件数据 |
| `negative` | CONDITIONING | 是 | - | 包含不期望图像特征的文本提示和设置的负向条件数据 |
| `vae` | VAE | 是 | - | 用于将输入图像编码为潜在表示的 VAE 模型 |
| `pixels` | IMAGE | 是 | - | 待处理并编码到潜在空间的输入图像 |
...
| 输出名称 | 数据类型 | 描述 |
|-------------|-----------|-------------|
| `positive` | CONDITIONING | 附带潜在图像表示的正向条件数据 |
| `negative` | CONDITIONING | 附带潜在图像表示的负向条件数据 |
