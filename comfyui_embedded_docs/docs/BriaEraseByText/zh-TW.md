...
| 參數 | 說明 | 資料類型 | 必填 | 範圍 |
|-----------|-------------|-----------|----------|-------|
| `image` | 應從中移除指定物件的影像。 | IMAGE | 是 | - |
| `object_name` | 要移除的物件名稱，例如 'the lamp'。可以一次指定多個物件，例如 'the phone and the pencils'。即使指定的物件不在圖片中，仍會回傳並計費一張重新渲染的影像。長度必須至少為 1 個字元（預設：空）。 | STRING | 是 | - |
| `moderation` | 審核設定。選擇是否顯示選用的審核控制項。 | DYNAMIC_COMBO | 是 | `"false"`<br>`"true"` |
