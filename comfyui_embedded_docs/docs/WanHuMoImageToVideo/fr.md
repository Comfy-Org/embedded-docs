...
| --- | --- | --- | --- | --- |
| `positif` | Entrée de conditionnement positive qui guide la génération vidéo vers le contenu souhaité. | CONDITIONING | Oui | - |
| `négatif` | Entrée de conditionnement négative qui éloigne la génération vidéo du contenu indésirable. | CONDITIONING | Oui | - |
| `vae` | Modèle VAE utilisé pour encoder les images de référence dans l'espace latent. | VAE | Oui | - |
| `largeur` | Largeur des images vidéo de sortie en pixels. Par défaut : 832. | INT | Oui | 16 à MAX_RESOLUTION, pas de 16 |
| `hauteur` | Hauteur des images vidéo de sortie en pixels. Par défaut : 480. | INT | Oui | 16 à MAX_RESOLUTION, pas de 16 |
| `longueur` | Nombre d'images dans la séquence vidéo générée. Par défaut : 97. | INT | Oui | 1 à MAX_RESOLUTION, pas de 4 |
| `taille_du_lot` | Nombre de séquences vidéo à générer simultanément. Par défaut : 1. | INT | Oui | 1 à 4096 |
| `sortie_encodeur_audio` | Données d'encodage audio facultatives pouvant influencer la génération vidéo en fonction du contenu audio. | AUDIOENCODEROUTPUT | Non | - |
| `image_référence` | Image de référence facultative utilisée pour guider le style et le contenu de la génération vidéo. Seule la première image du lot est utilisée. | IMAGE | Non | - |
