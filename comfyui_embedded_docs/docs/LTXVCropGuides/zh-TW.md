...
| `負向` | CONDITIONING | 是 | - | 包含生成時應避免內容引導資訊的負向條件輸入 |
| `潛在空間` | LATENT | 是 | - | 包含影像樣本與雜訊遮罩資料的潛在表示 |

## 輸出

| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `負向` | CONDITIONING | 已清除關鍵幀索引與引導注意力條目的處理後正向條件 |
| `潛在空間` | CONDITIONING | 已清除關鍵幀索引與引導注意力條目的處理後負向條件 |
| `潛在空間` | LATENT | 已裁切的潛在表示，包含調整後的樣本與雜訊遮罩，其中關鍵幀部分已被移除 |
