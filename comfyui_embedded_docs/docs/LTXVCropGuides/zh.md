...
| 输出名称 | 描述 | 数据类型 |
| --- | --- | --- |
| `positive` | 已处理的正向 conditioning，关键帧索引和引导注意力条目已被清除 | CONDITIONING |
| `negative` | 已处理的负向 conditioning，关键帧索引和引导注意力条目已被清除 | CONDITIONING |
| `latent` | 裁剪后的潜在表示，包含调整后的样本和噪声掩码，关键帧部分已被移除 | LATENT |
