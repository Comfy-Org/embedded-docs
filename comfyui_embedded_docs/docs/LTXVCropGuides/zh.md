...
| 参数 | 数据类型 | 是否必填 | 范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `positive` | CONDITIONING | 是 | - | 包含生成引导信息的正向 conditioning 输入 |
| `negative` | CONDITIONING | 是 | - | 包含生成中需避免内容的引导信息的负向 conditioning 输入 |
| `latent` | LATENT | 是 | - | 包含图像样本和噪声掩码数据的潜在表示 |

## 输出

| 输出名称 | 数据类型 | 描述 |
|-------------|-----------|-------------|
| `positive` | CONDITIONING | 已处理的正向 conditioning，关键帧索引和引导注意力条目已被清除 |
| `negative` | CONDITIONING | 已处理的负向 conditioning，关键帧索引和引导注意力条目已被清除 |
| `latent` | LATENT | 裁剪后的潜在表示，包含调整后的样本和噪声掩码，关键帧部分已被移除 |
