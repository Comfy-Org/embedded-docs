...
|-----------|-----------|----------|-------|-------------|
| `positive` | CONDITIONING | 예 | - | 생성을 위한 가이드 정보를 포함하는 양성 컨디셔닝 입력 |
| `negative` | CONDITIONING | 예 | - | 생성에서 피해야 할 사항에 대한 가이드 정보를 포함하는 음성 컨디셔닝 입력 |
| `latent` | LATENT | 예 | - | 이미지 샘플과 노이즈 마스크 데이터를 포함하는 잠재 표현 |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `positive` | CONDITIONING | 키프레임 인덱스와 가이드 어텐션 항목이 제거된 처리된 양성 컨디셔닝 |
| `negative` | CONDITIONING | 키프레임 인덱스와 가이드 어텐션 항목이 제거된 처리된 음성 컨디셔닝 |
| `latent` | LATENT | 키프레임 섹션이 제거되고 샘플과 노이즈 마스크가 조정된 잘라낸 잠재 표현 |
