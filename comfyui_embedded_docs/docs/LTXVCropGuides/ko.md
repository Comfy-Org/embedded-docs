...
|-----------|-----------|----------|-------|-------------|
| `positive` | CONDITIONING | 예 | - | 생성에 대한 안내 정보를 포함하는 양극 조건화 입력 |
| `negative` | CONDITIONING | 예 | - | 생성 과정에서 피해야 할 내용에 대한 안내 정보를 포함하는 음극 조건화 입력 |
...
| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `positive` | CONDITIONING | 키프레임 인덱스가 지워진 처리된 양극 조건화 |
| `negative` | CONDITIONING | 키프레임 인덱스가 지워진 처리된 음극 조건화 |
| `latent` | LATENT | 조정된 샘플과 노이즈 마스크를 포함한 잘려진 잠재 표현 |