...
| `프롬프트` | STRING | 예 | - | 출력 이미지에서 보고 싶은 내용. 요소, 색상 및 대상을 명확히 정의하는 강력하고 설명적인 프롬프트가 더 나은 결과를 이끌어냅니다. (기본값: 빈 문자열) |
| `창의성` | FLOAT | 예 | 0.2-0.5 | 초기 이미지에 크게 조건화되지 않은 추가 세부 정보를 생성할 가능성을 제어합니다. (기본값: 0.35) |
