...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `提示詞` | STRING | 是 | 不適用 | 影像的文字提示。 |
| `模型` | DICT | 是 | 見下方 | Krea 2 Medium 最適合表現力豐富的插圖；Krea 2 Large 最適合表現力豐富的照片級寫實風格。 |
| `隨機種子` | INT | 是 | 0 到 2147483647 | 用於再現結果的隨機種子（預設值：0）。 |

`model` 參數是一個字典，包含以下子參數：

| 子參數 | 資料類型 | 必要 | 範圍 | 說明 |
|---------------|-----------|----------|-------|-------------|
| `模型` | STRING | 是 | `"krea 2 medium"`<br>`"krea 2 large"` | 選擇 Krea 2 模型變體。 |
| `aspect_ratio` | STRING | 是 | 不適用 | 生成影像的長寬比。 |
| `resolution` | STRING | 是 | 不適用 | 生成影像的解析度。 |
| `creativity` | FLOAT | 是 | 不適用 | 控制生成過程的創意程度。 |
| `moodboard_id` | STRING | 否 | 不適用 | 用於影響影像的 Krea 情緒板 UUID。必須是有效的 UUID。 |
| `moodboard_strength` | FLOAT | 否 | 不適用 | 情緒板的影響強度（預設值：0.35）。 |
| `style_reference` | LIST | 否 | 0 到 10 個項目 | 影像風格參考列表。每個參考必須包含 `url`（STRING）和 `strength`（FLOAT）。 |
