...
| `output_mode` | El modo de salida para el modelo generado. `"geometry"` produce una malla sin procesar, `"textured"` añade una textura estándar, y `"detailed"` crea un modelo texturizado de alto detalle (predeterminado: `"textured"`). | COMBO | Sí | `"geometry"`<br>`"textured"`<br>`"detailed"` |
| `face_limit` | Número máximo de caras para la malla de salida. Establecer en -1 para sin límite (predeterminado: -1). | INT | No | -1 a 100000 |
| `model_seed` | Semilla para generación reproducible del modelo. Establecer en 0 para aleatorio (predeterminado: 0). | INT | No | 0 a 2147483647 |
| `auto_size` | Ajustar automáticamente el tamaño del modelo para que quepa dentro de un cuadro delimitador estándar (predeterminado: False). | BOOLEAN | No | True / False |
| `export_uv` | Exportar coordenadas UV con el modelo (predeterminado: True). | BOOLEAN | No | True / False |
| `compress_geometry` | Comprimir los datos de geometría para reducir el tamaño del archivo (predeterminado: False). | BOOLEAN | No | True / False |
