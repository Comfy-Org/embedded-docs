...
|-----------|-----------|----------|-------|-------------|
| `이미지` | IMAGE | 예 | - | 3D 모델로 변환할 입력 이미지입니다. 여러 이미지를 제공할 수 있습니다. |
| `시드` | INT | 아니요 | 0-65535 | 생성을 위한 무작위 시드 값입니다(기본값: 0). 0으로 설정하면 무작위 시드가 사용됩니다. |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `GLB` | STRING | 생성된 3D 모델의 파일 경로입니다(하위 호환성 전용). |
