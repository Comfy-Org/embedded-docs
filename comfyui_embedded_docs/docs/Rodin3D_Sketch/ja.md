...
| パラメータ | データ型 | 必須 | 範囲 | 説明 |
|-----------|-----------|----------|-------|-------------|
| `Images` | IMAGE | はい | - | 3Dモデルに変換する入力画像です。複数の画像を指定できます。 |
| `Seed` | INT | いいえ | 0-65535 | 生成用のランダムシード値です（デフォルト：0）。0に設定するとランダムシードになります。 |

## 出力

| 出力名 | データ型 | 説明 |
|-------------|-----------|-------------|
| `3D Model Path` | STRING | 生成された3Dモデルのファイルパス（下位互換性のため） |
