...
|-----------|-----------|----------|-------|-------------|
| `3D 모델` | FILE3D | 예 | - | 입력 3D 모델(GLB 또는 OBJ). 파일은 GLB 또는 OBJ 형식이어야 하며 200MB를 초과할 수 없습니다. |
| `폴리곤 유형` | STRING | 예 | `"triangle"`<br>`"quadrilateral"` | 표면 구성 유형입니다. |
| `면 수준` | STRING | 예 | `"medium"`<br>`"high"`<br>`"low"` | 폴리곤 축소 수준입니다. |
| `시드` | INT | 아니요 | 0 ~ 2147483647 | 시드는 노드 재실행 여부를 제어하며, 시드와 관계없이 결과는 비결정적입니다. (기본값: 0) |
