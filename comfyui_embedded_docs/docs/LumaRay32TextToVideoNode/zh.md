...
|-----------|-------------|-----------|----------|-------|
| `prompt` | 用于视频生成的文本提示。 | STRING | 是 | 1-6000 个字符 |
| `aspect_ratio` | 生成视频的宽高比。 | STRING | 是 | `"16:9"`<br>`"9:16"`<br>`"1:1"`<br>`"4:3"`<br>`"3:4"`<br>`"21:9"` |
| `resolution` | 视频的输出分辨率（默认值："720p"）。 | STRING | 是 | `"360p"`<br>`"540p"`<br>`"720p"`<br>`"1080p"` |
| `duration` | 生成视频的时长。 | STRING | 是 | `"5s"`<br>`"10s"` |
| `loop` | 使视频无缝循环播放。仅当时长为 5s 时可用。 | BOOLEAN | 否 | True/False（默认值：False） |
| `seed` | 用于可复现生成的随机种子。 | INT | 否 | 0 至 2147483647 |
