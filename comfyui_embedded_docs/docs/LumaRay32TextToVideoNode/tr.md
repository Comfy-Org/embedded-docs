...
| `aspect_ratio` | Oluşturulan videonun en-boy oranı. | STRING | Evet | `"16:9"`<br>`"9:16"`<br>`"1:1"`<br>`"4:3"`<br>`"3:4"`<br>`"21:9"` |
| `resolution` | Videonun çıktı çözünürlüğü (varsayılan: "720p"). | STRING | Evet | `"360p"`<br>`"540p"`<br>`"720p"`<br>`"1080p"` |
| `duration` | Oluşturulan videonun süresi. | STRING | Evet | `"5s"`<br>`"10s"` |
| `loop` | Videonun kesintisiz döngüye girmesini sağlar. Yalnızca 5 saniyelik süre ile kullanılabilir. | BOOLEAN | Hayır | Doğru/Yanlış (varsayılan: Yanlış) |
