...
| `aspect_ratio` | 생성된 비디오의 화면 비율입니다. | STRING | 예 | `"16:9"`<br>`"9:16"`<br>`"1:1"`<br>`"4:3"`<br>`"3:4"`<br>`"21:9"` |
| `resolution` | 비디오의 출력 해상도입니다(기본값: "720p"). | STRING | 예 | `"360p"`<br>`"540p"`<br>`"720p"`<br>`"1080p"` |
| `duration` | 생성된 비디오의 길이입니다. | STRING | 예 | `"5s"`<br>`"10s"` |
| `loop` | 비디오를 끊김 없이 반복 재생되도록 합니다. 5초 길이에서만 사용 가능합니다. | BOOLEAN | 아니요 | True/False (기본값: False) |
| `seed` | 재현 가능한 생성을 위한 시드 값입니다. | INT | 아니요 | 0 ~ 2147483647 |
