...
|-----------|-------------|-----------------|--------|-------|
| `prompt` | Invite textuelle pour la génération vidéo. | STRING | Oui | 1 à 6000 caractères |
...
**Remarque :** Le paramètre `loop` ne peut être activé que lorsque `duration` est défini sur « 5s ». Si vous définissez `duration` sur « 10s » et activez `loop`, le nœud renvoie une erreur.
...
| `identifiant_génération` | L'identifiant unique de la demande de génération vidéo. | STRING |
