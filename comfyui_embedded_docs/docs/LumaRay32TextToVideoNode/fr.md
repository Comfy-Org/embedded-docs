...
| `aspect_ratio` | Le rapport hauteur/largeur de la vidéo générée. | STRING | Oui | `"16:9"`<br>`"9:16"`<br>`"1:1"`<br>`"4:3"`<br>`"3:4"`<br>`"21:9"` |
| `resolution` | La résolution de sortie de la vidéo (par défaut : "720p"). | STRING | Oui | `"360p"`<br>`"540p"`<br>`"720p"`<br>`"1080p"` |
| `duration` | La durée de la vidéo générée. | STRING | Oui | `"5s"`<br>`"10s"` |
| `loop` | Permet de boucler la vidéo de manière fluide. Disponible uniquement avec une durée de 5s. | BOOLEAN | Non | True/False (par défaut : False) |
