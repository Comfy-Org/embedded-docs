...
| `aspect_ratio` | 生成される動画のアスペクト比。 | STRING | はい | `"16:9"`<br>`"9:16"`<br>`"1:1"`<br>`"4:3"`<br>`"3:4"`<br>`"21:9"` |
| `resolution` | 動画の出力解像度（デフォルト："720p"）。 | STRING | はい | `"360p"`<br>`"540p"`<br>`"720p"`<br>`"1080p"` |
| `duration` | 生成される動画の長さ。 | STRING | はい | `"5s"`<br>`"10s"` |
| `loop` | 動画をシームレスにループさせます。5秒の長さでのみ利用可能です。 | BOOLEAN | いいえ | True/False（デフォルト：False） |
| `seed` | 再現可能な生成のためのシード値。 | INT | いいえ | 0～2147483647 |
