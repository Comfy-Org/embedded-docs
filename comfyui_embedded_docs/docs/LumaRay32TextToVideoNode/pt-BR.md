...
|-----------|-------------|-----------|----------|-------|
| `prompt` | Prompt de texto para a geração de vídeo. | STRING | Sim | 1 a 6000 caracteres |
...
**Observação:** O parâmetro `loop` só pode ser ativado quando `duration` está definido como "5s". Se você definir `duration` como "10s" e ativar `loop`, o nó retorna um erro.
