...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `image` | IMAGE | 是 | - | 要裁剪的輸入影像批次。 |
| `bboxes` | BOUNDINGBOX | 是 | - | 定義裁剪區域的邊界框列表。此輸入為強制性，必須連接。 |
| `output_width` | INT | 否 | 64 - 4096 | 每個裁剪區域調整後的寬度（預設值：512）。 |
| `output_height` | INT | 否 | 64 - 4096 | 每個裁剪區域調整後的高度（預設值：512）。 |
| `padding` | INT | 否 | 0 - 1024 | 裁剪前在邊界框每側添加的額外像素填充（預設值：0）。 |
| `keep_aspect` | COMBO | 否 | `"stretch"`<br>`"pad"` | 選擇是否拉伸裁剪區域以符合輸出尺寸，或以黑色像素填充以保留其長寬比（預設值："stretch"）。 |
...
| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `image` | IMAGE | 所有裁剪並調整大小後的區域，堆疊成單一影像批次。 |
