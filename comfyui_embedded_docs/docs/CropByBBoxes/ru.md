...
| `bboxes` | BOUNDINGBOX | Да | - | Список ограничивающих рамок, определяющих области для кадрирования. Этот вход является обязательным, то есть должен быть подключён. |
| `output_width` | INT | Нет | 64 - 4096 | Ширина, до которой масштабируется каждое кадрирование (по умолчанию: 512). |
| `output_height` | INT | Нет | 64 - 4096 | Высота, до которой масштабируется каждое кадрирование (по умолчанию: 512). |
...
| `keep_aspect` | COMBO | Нет | `"stretch"`<br>`"pad"` | Определяет, следует ли растянуть область кадрирования до выходного размера или дополнить её чёрными пикселями для сохранения соотношения сторон (по умолчанию: "stretch"). |
...
|------------|------------|----------|
| `image` | IMAGE | Все обрезанные и масштабированные области, объединённые в единый пакет изображений. |
