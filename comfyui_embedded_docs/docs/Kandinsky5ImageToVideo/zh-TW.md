...
| `寬度` | INT | 否 | 16 至 8192（步長 16） | 輸出影片的寬度（像素），預設值：768。 |
| `高度` | INT | 否 | 16 至 8192（步長 16） | 輸出影片的高度（像素），預設值：512。 |
| `長度` | INT | 否 | 1 至 8192（步長 4） | 影片的幀數，預設值：121。 |
| `批次大小` | INT | 否 | 1 至 4096 | 同時生成的影片序列數量，預設值：1。 |
| `起始圖片` | IMAGE | 否 | 不適用 | 可選的起始影像。若提供，則會對其進行編碼，並用於取代模型輸出潛在空間中的雜訊起始部分。 |
...
| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `negative` | CONDITIONING | 修改後的正向條件設定，可能已更新編碼後的起始影像資料。 |
| `latent` | CONDITIONING | 修改後的負向條件設定，可能已更新編碼後的起始影像資料。 |
| `cond_latent` | LATENT | 一個充滿零值的空影片潛在張量，其形狀符合指定的尺寸。 |
