...
| --- | --- | --- | --- | --- |
| `positif` | Les invites de conditionnement positives pour guider la génération vidéo. | CONDITIONING | Oui | N/A |
| `négatif` | Les invites de conditionnement négatives pour éloigner la génération vidéo de certains concepts. | CONDITIONING | Oui | N/A |
| `vae` | Le modèle VAE utilisé pour encoder l'image de départ facultative dans l'espace latent. | VAE | Oui | N/A |
| `largeur` | La largeur de la vidéo de sortie en pixels (par défaut : 768). | INT | Oui | 16 à 16384 (pas de 16) |
| `hauteur` | La hauteur de la vidéo de sortie en pixels (par défaut : 512). | INT | Oui | 16 à 16384 (pas de 16) |
| `longueur` | Le nombre d'images dans la vidéo (par défaut : 121). | INT | Oui | 1 à 16384 (pas de 4) |
| `taille_du_lot` | Le nombre de séquences vidéo à générer simultanément (par défaut : 1). | INT | Oui | 1 à 4096 |
| `image_de_départ` | Une image de départ facultative ou un lot d'images. Si elle est fournie, elle est encodée et utilisée pour remplacer le début bruité des latents de sortie du modèle. | IMAGE | Non | N/A |
