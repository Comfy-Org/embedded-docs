...
|-----------|--------------|-------------|-------|-------------|
| `positivo` | CONDITIONING | Sí | N/D | Los mensajes de condicionamiento positivo para guiar la generación del video. |
| `negativo` | CONDITIONING | Sí | N/D | Los mensajes de condicionamiento negativo para alejar la generación del video de ciertos conceptos. |
| `vae` | VAE | Sí | N/D | El modelo VAE utilizado para codificar la imagen de inicio opcional en el espacio latente. |
| `ancho` | INT | No | 16 a 8192 (paso 16) | El ancho del video de salida en píxeles (predeterminado: 768). |
| `alto` | INT | No | 16 a 8192 (paso 16) | La altura del video de salida en píxeles (predeterminado: 512). |
| `duración` | INT | No | 1 a 8192 (paso 4) | El número de fotogramas en el video (predeterminado: 121). |
| `tamaño_lote` | INT | No | 1 a 4096 | El número de secuencias de video a generar simultáneamente (predeterminado: 1). |
| `imagen_inicial` | IMAGE | No | N/D | Una imagen de inicio opcional. Si se proporciona, se codifica y se utiliza para reemplazar el inicio ruidoso de los latentes de salida del modelo. |
...
|------------------|--------------|-------------|
| `negativo` | CONDITIONING | El condicionamiento positivo modificado, potencialmente actualizado con los datos de la imagen de inicio codificada. |
| `latente` | CONDITIONING | El condicionamiento negativo modificado, potencialmente actualizado con los datos de la imagen de inicio codificada. |
| `latente_cond` | LATENT | Un tensor latente de video vacío con ceros, con forma para las dimensiones especificadas. |
