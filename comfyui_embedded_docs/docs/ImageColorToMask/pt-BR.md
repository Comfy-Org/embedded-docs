...
| Parâmetro | Tipo de Dados | Descrição |
|-----------|---------------|-----------|
| `mask`    | `MASK`        | A saída é uma máscara que destaca as áreas da imagem de entrada que correspondem à cor especificada. Essa máscara pode ser usada para tarefas adicionais de processamento de imagem, como segmentação ou isolamento de objetos. |