...
PhotoMakerLoader 節點會從可用的模型檔案中載入 PhotoMaker 模型。它會讀取指定的模型檔案，並準備好 PhotoMaker ID 編碼器，以供基於身分的影像生成任務使用。此節點標記為實驗性，僅供測試用途。
...
| 輸出名稱 | 資料類型 | 輸出說明 |
|-------------|-----------|-------------|
| `photomaker_model` | PHOTOMAKER | 已載入的 PhotoMaker 模型，包含 ID 編碼器，準備好用於身分編碼操作。 |