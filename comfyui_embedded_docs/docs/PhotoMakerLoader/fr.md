...
| --- | --- | --- | --- | --- |
| `photomaker_model_name` | Le nom du fichier de modèle PhotoMaker à charger. Les options disponibles sont déterminées par les fichiers de modèle présents dans le dossier `photomaker`. | COMBO | Oui | Plusieurs options disponibles |

Remarque : Le fichier de modèle sélectionné doit exister dans le dossier `photomaker`. Le nœud génère une erreur si le fichier spécifié est introuvable.
