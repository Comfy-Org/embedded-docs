...
Este nodo genera objetos o escenas dentro de una región enmascarada de una imagen mediante la API de Bria. Carga la imagen y la máscara, envía el prompt al servicio de relleno generativo de Bria, espera a que la operación se complete y devuelve la imagen editada. Esta es una operación de API de pago (US$0.0429 por solicitud).
...
| `refine_prompt` | Ajustar automáticamente el prompt para obtener mejores resultados; deshabilite para usar el prompt exactamente como está escrito. (predeterminado: true) | BOOLEAN | Sí | true<br>false |
...
Nota: El `prompt` no debe estar vacío, y la `mask` debe tener la misma relación de aspecto que la `image`.
