...
|-----------|-------------|-----------|----------|-------|
| `影像` | 要編輯的輸入影像。 | IMAGE | 是 | - |
| `遮罩` | 白色區域會被生成的內容填滿，黑色區域則保留。遮罩在傳送前會進行二值化處理，因此部分塗繪的區域會被視為白色。必須與影像具有相同的長寬比。 | MASK | 是 | - |
| `提示詞` | 描述要在遮罩區域內生成內容的文字。至少需包含 1 個字元。（預設值：""） | STRING | 是 | - |
| `負面提示詞` | 描述應避免出現在生成結果中的內容的提示詞。若留空，則不會傳送至 API。（預設值：""） | STRING | 是 | - |
| `最佳化提示詞` | 自動調整提示詞以獲得更好的結果；停用則完全按照原樣使用提示詞。（預設值：true） | BOOLEAN | 是 | true<br>false |
