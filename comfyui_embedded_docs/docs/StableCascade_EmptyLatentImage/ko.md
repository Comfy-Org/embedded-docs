...
|-----------|-----------|----------|-------|-------------|
| `width` | INT | 예 | 256 ~ MAX_RESOLUTION | 출력 이미지의 픽셀 단위 너비 (기본값: 1024, 증가 단위: 8) |
| `height` | INT | 예 | 256 ~ MAX_RESOLUTION | 출력 이미지의 픽셀 단위 높이 (기본값: 1024, 증가 단위: 8) |
| `compression` | INT | 예 | 4 ~ 128 | 스테이지 C의 잠재 차원을 결정하는 압축 계수 (기본값: 42, 증가 단위: 1) |
| `batch_size` | INT | 아니요 | 1 ~ 4096 | 배치로 생성할 잠재 샘플의 개수 (기본값: 1) |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `stage_c` | LATENT | 차원이 [batch_size, 16, height//compression, width//compression]인 스테이지 C 잠재 텐서 |
