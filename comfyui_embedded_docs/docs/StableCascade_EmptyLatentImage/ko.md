...
|-----------|-----------|----------|-------|-------------|
| `너비` | INT | 예 | 256 ~ MAX_RESOLUTION | 출력 이미지의 픽셀 단위 너비 (기본값: 1024, 증가 단위: 8) |
| `높이` | INT | 예 | 256 ~ MAX_RESOLUTION | 출력 이미지의 픽셀 단위 높이 (기본값: 1024, 증가 단위: 8) |
| `압축` | INT | 예 | 4 ~ 128 | 스테이지 C의 잠재 차원을 결정하는 압축 계수 (기본값: 42, 증가 단위: 1) |
| `배치 크기` | INT | 아니요 | 1 ~ 4096 | 배치로 생성할 잠재 샘플의 개수 (기본값: 1) |
