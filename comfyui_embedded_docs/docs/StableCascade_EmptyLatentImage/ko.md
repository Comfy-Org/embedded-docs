...
|-----------|-----------|----------|-------|-------------|
| `width` | INT | 예 | 256 ~ MAX_RESOLUTION | 출력 이미지의 너비 (픽셀 단위) (기본값: 1024, 단계: 8) |
| `height` | INT | 예 | 256 ~ MAX_RESOLUTION | 출력 이미지의 높이 (픽셀 단위) (기본값: 1024, 단계: 8) |
| `compression` | INT | 예 | 4 ~ 128 | 스테이지 C의 잠재 차원을 결정하는 압축 계수 (기본값: 42, 단계: 1) |
| `batch_size` | INT | 아니오 | 1 ~ 4096 | 배치에서 생성할 잠재 샘플의 수 (기본값: 1) |
