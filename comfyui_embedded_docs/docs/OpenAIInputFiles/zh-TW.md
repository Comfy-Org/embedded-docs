...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `file` | COMBO | 是 | 提供多個選項（輸入目錄中所有小於 32MB 的 .txt 與 .pdf 檔案） | 作為模型上下文的輸入檔案。目前僅接受文字檔（.txt）與 PDF 檔（.pdf）。檔案大小必須小於 32MB。 |
| `OPENAI_INPUT_FILES` | OPENAI_INPUT_FILES | 否 | 不適用 | 可選的附加檔案，可與此節點載入的檔案批次處理。允許串聯輸入檔案，使單一訊息能夠包含多個輸入檔案。 |
...
| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `OPENAI_INPUT_FILES` | OPENAI_INPUT_FILES | 已格式化的輸入檔案，準備好作為 OpenAI API 呼叫的上下文使用。 |
