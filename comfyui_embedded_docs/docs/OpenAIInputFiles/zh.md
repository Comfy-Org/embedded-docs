...
| 参数 | 数据类型 | 是否必填 | 范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `file` | COMBO | 是 | 提供多个选项（输入目录中所有小于 32MB 的 .txt 和 .pdf 文件） | 作为模型上下文的输入文件。目前仅接受文本（.txt）和 PDF（.pdf）文件。文件大小必须小于 32MB。 |
| `OPENAI_INPUT_FILES` | OPENAI_INPUT_FILES | 否 | 不适用 | 可选的附加文件，与此节点加载的文件合并处理。允许链接输入文件，以便单条消息可以包含多个输入文件。 |
...
| 输出名称 | 数据类型 | 描述 |
|-------------|-----------|-------------|
| `OPENAI_INPUT_FILES` | OPENAI_INPUT_FILES | 格式化后的输入文件，准备用作 OpenAI API 调用的上下文。 |
