...
| パラメータ | データ型 | 必須 | 範囲 | 説明 |
|-----------|-----------|----------|-------|-------------|
| `file` | COMBO | はい | 複数のオプションが利用可能（入力ディレクトリ内の32MB未満のすべての .txt および .pdf ファイル） | モデルのコンテキストとして含める入力ファイル。現時点ではテキスト（.txt）ファイルとPDF（.pdf）ファイルのみを受け付けます。ファイルは32MB未満である必要があります。 |
