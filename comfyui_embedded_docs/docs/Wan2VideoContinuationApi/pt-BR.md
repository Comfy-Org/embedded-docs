...
| :--- | :--- | :--- | :--- | :--- |
| `modelo` | COMBO | Sim | `"wan2.7-i2v"` | O modelo de geração de vídeo a ser usado. |
...
| `model.negative_prompt` | STRING | Sim | - | Prompt negativo descrevendo o que evitar. (padrão: string vazia) |
...
| `primeiro_clip` | VIDEO | Sim | - | Vídeo de entrada a ser continuado. Duração: 2s-10s. A proporção de aspecto da saída é derivada deste vídeo. |
| `último_quadro` | IMAGE | Não | - | Imagem do último quadro. A continuação fará a transição em direção a este quadro. |
| `semente` | INT | Sim | 0 a 2147483647 | Semente a ser usada para a geração. (padrão: 0) |
| `estender_prompt` | BOOLEAN | Sim | - | Se deve aprimorar o prompt com assistência de IA. (padrão: True) |
| `marca_d'água` | BOOLEAN | Sim | - | Se deve adicionar uma marca d'água gerada por IA ao resultado. (padrão: False) |
