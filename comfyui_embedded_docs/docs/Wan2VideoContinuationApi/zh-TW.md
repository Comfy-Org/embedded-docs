...
| 參數 | 描述 | 資料類型 | 必填 | 範圍 |
|-----------|-------------|-----------|----------|-------|
| `prompt` | 描述元素和視覺特徵的提示詞。支援英文和中文。(預設值：空字串) | STRING | 是 | - |
| `negative_prompt` | 描述應避免內容的負面提示詞。(預設值：空字串) | STRING | 是 | - |
| `resolution` | 輸出影片的解析度。 | COMBO | 是 | `"720P"`<br>`"1080P"` |
| `duration` | 輸出總持續時間（秒）。模型會生成接續內容，以填補輸入影片剪輯之後的剩餘時間。(預設值：5) | INT | 是 | 2 至 15 |
