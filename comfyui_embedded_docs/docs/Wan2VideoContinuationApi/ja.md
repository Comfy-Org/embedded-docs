...
| `最初のクリップ` | 継続元となる入力ビデオ。長さ: 2秒〜10秒。出力のアスペクト比はこのビデオから取得されます。 | VIDEO | はい | 2秒〜10秒 |
| `ラストフレーム` | 最後のフレーム画像。継続部分はこのフレームに向かって遷移します。 | IMAGE | いいえ | - |
| `シード値` | 生成に使用するシード値。(デフォルト: 0) | INT | はい | 0 〜 2147483647 |
| `プロンプト拡張` | AI支援でプロンプトを拡張するかどうか。(デフォルト: True) | BOOLEAN | はい | - |
...
| Parameter | Description | Data Type | Required | Range |
|-----------|-------------|-----------|----------|-------|
| `prompt` | 要素と視覚的特徴を説明するプロンプト。英語と中国語に対応しています。(デフォルト: 空文字列) | STRING | はい | - |
| `negative_prompt` | 避けるべき内容を説明するネガティブプロンプト。(デフォルト: 空文字列) | STRING | はい | - |
| `resolution` | 出力ビデオの解像度。 | COMBO | はい | `"720P"`<br>`"1080P"` |
| `duration` | 出力全体の長さ（秒）。モデルは、入力クリップ後の残り時間を埋める継続部分を生成します。(デフォルト: 5) | INT | はい | 2 〜 15 |
