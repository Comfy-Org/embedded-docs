...
| `primer_clip` | Video de entrada a partir del cual continuar. Duración: de 2s a 10s. La relación de aspecto de la salida se deriva de este video. | VIDEO | Sí | 2 a 10 s |
...
| `prompt_negativo` | Prompt negativo que describe lo que se debe evitar. (por defecto: cadena vacía) | STRING | Sí | - |
| `resolución` | La resolución del video de salida. | COMBO | Sí | `"720P"`<br>`"1080P"` |
| `duración` | Duración total de la salida en segundos. El modelo genera la continuación para completar el tiempo restante después del clip de entrada. (por defecto: 5) | INT | Sí | 2 a 15 |
