...
|----------------------|--------------|-------------|
| `clip_vision`         | `CLIP_VISION` | Os embeddings de visão do CLIP que fornecem contexto visual para o processo de condicionamento. |
...
| `elevation`           | `FLOAT`      | O ângulo de elevação para condicionamento de modelos 3D, afetando a perspectiva da imagem gerada. |
| `azimuth`             | `FLOAT`      | O ângulo azimutal para condicionamento de modelos 3D, afetando a orientação da imagem gerada. |
| `elevation_batch_increment` | `FLOAT` | A variação incremental no ângulo de elevação ao longo do lote, permitindo perspectivas variadas. |
| `azimuth_batch_increment` | `FLOAT` | A variação incremental no ângulo azimutal ao longo do lote, permitindo orientações variadas. |
...
| `latent`      | `LATENT`     | A representação latente derivada do processo de condicionamento, pronta para etapas adicionais de processamento ou geração. |