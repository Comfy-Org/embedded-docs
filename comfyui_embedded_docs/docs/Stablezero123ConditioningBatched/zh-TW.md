...
此節點專為 StableZero123 模型設計，以批次方式處理條件化資訊。它專注於同時高效處理多組條件化數據，針對需要批次處理的場景優化工作流程。
...
| `vae`                   | `VAE`          | 在條件化過程中用於編碼和解碼圖像的變分自編碼器。 |
| `width`                 | `INT`          | 輸出圖像的寬度。 |
| `height`                | `INT`          | 輸出圖像的高度。 |
| `batch_size`            | `INT`          | 單一批次中要處理的條件化集合數量。 |
| `elevation`             | `FLOAT`        | 用於 3D 模型條件化的仰角，影響生成圖像的視角。 |
| `azimuth`               | `FLOAT`        | 用於 3D 模型條件化的方位角，影響生成圖像的方向。 |
| `elevation_batch_increment` | `FLOAT`    | 在批次中各項目間仰角的增量變化，允許產生多樣化的視角。 |
| `azimuth_batch_increment` | `FLOAT`    | 在批次中各項目間方位角的增量變化，允許產生多樣化的方向。 |

## 輸出參數

| 參數名稱     | 資料類型        | 描述 |
|------------|-----------------|------|
| `positive` | `CONDITIONING`  | 正向條件化輸出，專門用於在生成內容中增強特定特徵或面向。 |
| `negative` | `CONDITIONING`  | 負向條件化輸出，專門用於在生成內容中抑制特定特徵或面向。 |
