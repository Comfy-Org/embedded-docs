...
此節點使用 Moonvalley API 將參考圖片轉換為影片。它接收輸入圖片和文字提示，以生成具有指定解析度、品質設定和創意控制的影片。該節點處理從圖片上傳到影片生成和下載的整個過程。

## 輸入參數

| 參數名稱 | 資料類型 | 必填 | 數值範圍 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `image` | IMAGE | 是 | - | 用於生成影片的參考圖片 |
...
- 輸入圖片的尺寸必須在 300x300 像素與允許的最大高度/寬度之間
