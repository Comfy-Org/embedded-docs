...
```markdown
Ce nœud prend le latent de forme en résolution 512 produit par la première passe d'échantillonnage de l'étape de forme, le suréchantillonne à une résolution cible plus élevée et prépare le conditionnement et le latent nécessaires à la seconde passe d'échantillonnage de l'étape de forme. Il attache des métadonnées par étape au conditionnement afin que le modèle puisse les utiliser pendant la génération.

## Entrées

| Paramètre | Description | Type de données | Requis | Plage |
|-----------|-------------|-----------------|--------|-------|
| `positive` | Le conditionnement positif auquel les métadonnées de forme de l'étape de suréchantillonnage sont attachées. | CONDITIONING | Oui | |
| `negative` | Le conditionnement négatif auquel les métadonnées de forme de l'étape de suréchantillonnage sont attachées. | CONDITIONING | Oui | |
...
| `target_resolution` | Résolution de voxels de la forme suréchantillonnée. Plus élevée = plus de détails, plus de VRAM. Défaut : 1024. | INT | Oui | 1024 - 2048 (step 128) |
