...
此節點使用 Recraft API 服務來移除影像的背景。它會處理輸入批次中的每個影像，並返回具有透明背景的處理後影像，以及標示已移除背景區域的對應 Alpha 遮罩。

## 輸入參數

| 參數名稱 | 資料類型 | 必填 | 數值範圍 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `圖片` | IMAGE | 是 | - | 要進行背景移除處理的輸入影像 |
| `auth_token` | STRING | 否 | - | 用於存取 Recraft API 的驗證令牌 |
