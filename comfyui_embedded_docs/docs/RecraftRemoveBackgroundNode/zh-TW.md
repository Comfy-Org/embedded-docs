...
此節點使用 Recraft API 服務移除影像的背景。它會個別處理輸入批次中的每張影像，並同時傳回具有透明背景的處理後影像，以及對應的 alpha 遮罩，用來表示已移除的背景區域。

## 輸入

| 參數 | 描述 | 資料類型 | 必填 | 範圍 |
|-----------|-------------|-----------|----------|-------|
| `圖片` | 要進行背景移除處理的輸入影像。批次中的每張影像會個別處理。 | IMAGE | 是 | - |
...
| `mask` | 表示已移除背景區域的 Alpha 通道遮罩，格式為 B,H,W | MASK |
