...
이 노드는 Recraft API 서비스를 사용하여 이미지에서 배경을 제거합니다. 입력 배치의 각 이미지를 처리하고, 투명한 배경을 가진 처리된 이미지와 제거된 배경 영역을 나타내는 해당 알파 마스크를 모두 반환합니다.

## 입력

| 매개변수 | 설명 | 데이터 유형 | 필수 | 범위 |
|-----------|-------------|-----------|----------|-------|
| `image` | 배경 제거를 위해 처리할 입력 이미지 | IMAGE | 예 | - |

## 출력

| 출력 이름 | 설명 | 데이터 유형 |
|-------------|-------------|-----------|
| image | 투명한 배경을 가진 처리된 이미지 | IMAGE |
