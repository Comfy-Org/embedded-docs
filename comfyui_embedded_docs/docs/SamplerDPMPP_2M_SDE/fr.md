...
|-----------|-----------|----------|-------|-------------|
| `solver_type` | STRING | Oui | `"midpoint"`<br>`"heun"` | Le type de solveur d'équation différentielle à utiliser pour le processus d'échantillonnage |
...
| `noise_device` | STRING | Oui | `"gpu"`<br>`"cpu"` | Le périphérique où les calculs de bruit sont effectués |
