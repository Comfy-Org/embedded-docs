...
|-----------|-----------|----------|-------|-------------|
| `audio` | AUDIO | 예 | - | 자를 오디오 입력 |
| `start_index` | FLOAT | 예 | -0xffffffffffffffff ~ 0xffffffffffffffff | 시작 시간(초)이며, 음수 값을 사용하면 끝에서부터 계산합니다(초 미만 단위 지원). 기본값: 0.0 |
| `duration` | FLOAT | 예 | 0.0 ~ 0xffffffffffffffff | 지속 시간(초). 기본값: 60.0 |
...
| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `audio` | AUDIO | 지정된 시작 시간과 지속 시간으로 잘린 오디오 세그먼트 |
