...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `音訊` | AUDIO | 是 | - | 要進行裁剪的音訊輸入 |
| `起始索引` | FLOAT | 是 | -0xffffffffffffffff 至 0xffffffffffffffff | 起始時間（秒），可為負數以從結尾倒數（支援小數秒）。預設值：0.0 |
| `持續時間` | FLOAT | 是 | 0.0 至 0xffffffffffffffff | 持續時間（秒）。預設值：60.0 |
...
| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `音訊` | AUDIO | 已裁剪的音訊片段，具有指定的起始時間與持續時間 |
