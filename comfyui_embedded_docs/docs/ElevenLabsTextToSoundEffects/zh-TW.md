...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `text` | STRING | 是 | 不適用 | 要生成音效的文字描述。此為必填欄位。 |
| `model` | COMBO | 是 | `"eleven_sfx_v2"` | 用於音效生成的模型。選擇此模型會顯示額外參數：`duration`（預設值：5.0，範圍：0.5 至 30.0 秒）、`loop`（預設值：False）和 `prompt_influence`（預設值：0.3，範圍：0.0 至 1.0）。 |
| `output_format` | COMBO | 是 | `"mp3_44100_192"`<br>`"opus_48000_192"` | 音頻輸出格式。 |
