...
| `watermark` | 是否為結果添加 AI 生成的浮水印。（進階選項；預設值：False） | BOOLEAN | 是 | True / False |
...
| 參數 | 描述 | 資料類型 | 必填 | 範圍 |
|-----------|-------------|-----------|----------|-------|
| `prompt` | 描述元素和視覺特徵的提示詞。支援英文和中文。（預設值：""） | STRING | 否 | N/A |
| `resolution` | 輸出影片解析度。（預設值：`"720P"`） | COMBO | 是 | `"720P"`<br>`"1080P"` |
| `duration` | 生成影片的時長（秒）。（預設值：5） | INT | 是 | 3 至 15 |
