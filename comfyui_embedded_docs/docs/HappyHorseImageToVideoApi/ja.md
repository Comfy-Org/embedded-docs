...
| パラメータ | 説明 | データ型 | 必須 | 範囲 |
|-----------|-------------|-----------|----------|-------|
| `モデル` | 動画生成に使用するHappyHorseモデル。 | DYNAMIC_COMBO | はい | `"happyhorse-1.1-i2v"`<br>`"happyhorse-1.0-i2v"` |
| `最初のフレーム` | 最初のフレーム画像。出力のアスペクト比はこの画像から導出されます。 | IMAGE | はい | 300×300 px 最小; 1:2.5 から 2.5:1 |
| `シード` | 生成に使用するシード。(デフォルト: 0) | INT | はい | 0 から 2147483647 |
| `ウォーターマーク` | 結果にAI生成の透かしを追加するかどうか。(詳細オプション、デフォルト: False) | BOOLEAN | はい | True / False |
...
| パラメータ | 説明 | データ型 | 必須 | 範囲 |
|-----------|-------------|-----------|----------|-------|
| `prompt` | 要素と視覚的特徴を説明するプロンプト。英語と中国語に対応。(デフォルト: "") | STRING | いいえ | N/A |
| `resolution` | 出力ビデオの解像度。(デフォルト: `"720P"`) | COMBO | はい | `"720P"`<br>`"1080P"` |
| `duration` | 生成される動画の長さ（秒）。(デフォルト: 5) | INT | はい | 3 から 15 |
