...
| `first_frame` | 首帧图像。输出的宽高比由此图像决定。 | IMAGE | 是 | N/A |
...
| 参数 | 描述 | 数据类型 | 必需 | 范围 |
|-----------|-------------|-----------|----------|-------|
| `model.prompt` | 描述元素和视觉特征的提示词。支持英文和中文。（默认值：""） | STRING | 否 | N/A |
| `model.resolution` | 输出视频分辨率。（默认值："720P"） | COMBO | 是 | `"720P"`<br>`"1080P"` |
| `model.duration` | 生成视频的时长（秒）。（默认值：5） | INT | 是 | 3 到 15 |
