...
|-----------|-----------|----------|-------|-------------|
| `이미지` | IMAGE | 예 | - | 3D 모델을 생성하는 데 사용되는 입력 이미지 |
| `모델 버전` | COMBO | 아니요 | 여러 옵션 사용 가능 | 생성에 사용할 Tripo 모델 버전 |
...
| `모델 시드` | INT | 아니요 | - | 모델 생성을 위한 난수 시드 (기본값: 42) |
| `방향` | COMBO | 아니요 | 여러 옵션 사용 가능 | 생성된 모델의 방향 설정 |
| `텍스처 시드` | INT | 아니요 | - | 텍스처 생성을 위한 난수 시드 (기본값: 42) |
| `텍스처 품질` | COMBO | 아니요 | "standard"<br>"detailed" | 텍스처 생성 품질 수준 (기본값: "standard") |
| `텍스처 정렬` | COMBO | 아니요 | "original_image"<br>"geometry" | 텍스처 매핑 정렬 방법 (기본값: "original_image") |
| `얼굴 제한` | INT | 아니요 | -1 ~ 500000 | 생성된 모델의 최대 면 수, -1은 제한 없음 (기본값: -1) |
| `쿼드` | BOOLEAN | 아니요 | - | 삼각형 대신 사각형 면 사용 여부 (기본값: False) |
...
| `GLB` | MODEL_TASK_ID | 모델 생성 과정 추적을 위한 작업 ID |
