...
|-----------|-----------------|--------|-------|-------------|
| `destination` | LATENT | Oui | - | Le latent de destination dans lequel les images seront remplacées. |
| `source` | LATENT | Non | - | Le latent source fournissant les images à insérer dans le latent de destination. S'il n'est pas fourni, le latent de destination est retourné inchangé. |
| `index` | INT | Oui | -MAX_RESOLUTION à MAX_RESOLUTION | L'index de l'image latente de départ dans le latent de destination où les images du latent source seront placées. Les valeurs négatives comptent à partir de la fin (par défaut : 0). |
...
|------------------|-----------------|-------------|
| `output` | LATENT | Le latent vidéo résultant après l'opération de remplacement d'images. |
