...
| 参数 | 数据类型 | 是否必填 | 范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `positive` | CONDITIONING | 是 | - | 正面条件提示，用于引导生成视频中应出现的内容 |
| `negative` | CONDITIONING | 是 | - | 负面条件提示，用于指定生成视频中应避免的内容 |
...
| `ref_image` | IMAGE | 否 | - | 可选的参考图像，为视频内容提供视觉引导 |
| `control_video` | IMAGE | 否 | - | 可选的控制视频，用于引导生成视频的运动和结构 |
| `ref_motion` | IMAGE | 否 | - | 可选的运动参考，为视频中的运动模式提供引导 |

## 输出

| 输出名称 | 数据类型 | 描述 |
|-------------|-----------|-------------|
| `positive` | CONDITIONING | 经过修改以适应视频生成的正面条件处理结果 |
| `negative` | CONDITIONING | 经过修改以适应视频生成的负面条件处理结果 |
