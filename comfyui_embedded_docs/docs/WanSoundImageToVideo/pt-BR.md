...
| `audio_encoder_output` | AUDIOENCODEROUTPUT | Não | - | Codificação de áudio opcional que pode influenciar a geração de vídeo com base nas características sonoras |
...
| `ref_motion` | IMAGE | Não | - | Referência de movimento opcional que fornece orientação para padrões de movimento no vídeo |
