...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `正面提示` | CONDITIONING | 是 | - | 正向條件提示，引導生成影片中應出現的內容 |
| `負面提示` | CONDITIONING | 是 | - | 負向條件提示，指定生成影片中應避免的內容 |
| `VAE` | VAE | 是 | - | 用於編碼和解碼影片潛在表示的 VAE 模型 |
| `寬度` | INT | 是 | 16 至 MAX_RESOLUTION | 輸出影片的寬度（像素），預設值：832，必須為 16 的倍數 |
| `高度` | INT | 是 | 16 至 MAX_RESOLUTION | 輸出影片的高度（像素），預設值：480，必須為 16 的倍數 |
| `長度` | INT | 是 | 1 至 MAX_RESOLUTION | 生成影片的影格數，預設值：77，必須為 4 的倍數 |
| `批次大小` | INT | 是 | 1 至 4096 | 同時生成的影片數量，預設值：1 |
| `音訊編碼器輸出` | AUDIOENCODEROUTPUT | 否 | - | 可選的音訊編碼，可根據聲音特徵影響影片生成 |
| `參考圖片` | IMAGE | 否 | - | 可選的參考影像，為影片內容提供視覺引導 |
| `控制影片` | IMAGE | 否 | - | 可選的控制影片，引導生成影片的動作和結構 |
| `參考動作` | IMAGE | 否 | - | 可選的動作參考，為影片中的運動模式提供引導 |

## 輸出

| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `負面提示` | CONDITIONING | 已處理的正向條件，已針對影片生成進行修改 |
| `潛在空間` | CONDITIONING | 已處理的負向條件，已針對影片生成進行修改 |
