...
| `negative_prompt` | STRING | 否 | - | 负面文本提示，用于指导视频中应避免的内容（默认：空） |
| `duration_seconds` | INT | 否 | 5-8 | 输出视频的时长，单位为秒（默认：5） |
| `enhance_prompt` | BOOLEAN | 否 | - | 是否使用 AI 辅助增强提示词（默认：True）。此为高级参数。 |
| `person_generation` | COMBO | 否 | "ALLOW"<br>"BLOCK" | 是否允许在视频中生成人物（默认："ALLOW"）。此为高级参数。 |
| `seed` | INT | 否 | 0-4294967295 | 视频生成的随机种子（0 表示随机）（默认：0）。此为高级参数。 |
