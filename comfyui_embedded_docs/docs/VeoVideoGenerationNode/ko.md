...
|-----------|-----------|----------|-------|-------------|
| `프롬프트` | STRING | 예 | - | 비디오에 대한 텍스트 설명 (기본값: 비어 있음) |
| `종횡비` | COMBO | 예 | "16:9"<br>"9:16" | 출력 비디오의 화면 비율 (기본값: "16:9") |
| `부정 프롬프트` | STRING | 아니요 | - | 비디오에서 제외할 내용을 안내하는 부정 텍스트 프롬프트 (기본값: 비어 있음) |
| `duration_seconds` | INT | 아니요 | 5-8 | 출력 비디오의 재생 시간(초) (기본값: 5) |
| `프롬프트 개선` | BOOLEAN | 아니요 | - | AI 지원으로 프롬프트를 향상할지 여부 (기본값: True). 고급 매개변수입니다. |
| `사람 생성` | COMBO | 아니요 | "ALLOW"<br>"BLOCK" | 비디오에 사람 생성을 허용할지 여부 (기본값: "ALLOW"). 고급 매개변수입니다. |
| `시드` | INT | 아니요 | 0-4294967295 | 비디오 생성을 위한 시드 (0은 무작위) (기본값: 0). 고급 매개변수입니다. |
| `이미지` | IMAGE | 아니요 | - | 비디오 생성을 안내하는 선택적 참조 이미지 |
| `모델` | COMBO | 아니요 | "veo-2.0-generate-001" | 비디오 생성에 사용할 Veo 2 모델 (기본값: "veo-2.0-generate-001") |
