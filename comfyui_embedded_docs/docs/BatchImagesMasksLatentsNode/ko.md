...
|-----------|-----------|----------|-------|-------------|
| `inputs` | IMAGE, MASK 또는 LATENT | 예 | 1~50개 입력 | 배치로 결합할 동적 입력 목록입니다. 1~50개 항목을 추가할 수 있습니다. 모든 항목은 동일한 유형(모두 이미지, 모두 마스크, 또는 모두 잠재 표현)이어야 합니다. |
...
| 출력 이름 | 데이터 유형 | 설명 |
|-------------|-----------|-------------|
| `output` | IMAGE, MASK 또는 LATENT | 단일 배치 출력입니다. 데이터 유형은 입력 유형(배치 IMAGE, 배치 MASK 또는 배치 LATENT)과 일치합니다. |
