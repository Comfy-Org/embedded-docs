...
| `sde_start_percent` | FLOAT | No | 0.0 - 1.0 | El porcentaje inicial para el muestreo SDE (predeterminado: 0.2) |
| `sde_end_percent` | FLOAT | No | 0.0 - 1.0 | El porcentaje final para el muestreo SDE (predeterminado: 0.8) |
| `s_noise` | FLOAT | No | 0.0 - 100.0 | Controla la cantidad de ruido añadido durante el muestreo (predeterminado: 1.0) |
| `predictor_order` | INT | No | 1 - 6 | El orden del componente predictor en el solucionador (predeterminado: 3) |
| `corrector_order` | INT | No | 0 - 6 | El orden del componente corrector en el solucionador (predeterminado: 4) |
| `use_pece` | BOOLEAN | No | - | Activa o desactiva el método PECE (Predecir-Evaluar-Corregir-Evaluar) |
| `simple_order_2` | BOOLEAN | No | - | Activa o desactiva los cálculos simplificados de segundo orden |
