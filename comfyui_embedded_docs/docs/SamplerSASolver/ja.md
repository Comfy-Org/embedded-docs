# SamplerSASolver

SamplerSASolver ノードは、拡散モデル用のカスタムサンプリングアルゴリズムを実装しています。このノードは、設定可能な次数設定と確率微分方程式（SDE）パラメータを備えた予測子・修正子（predictor-corrector）アプローチを使用して、入力モデルからサンプルを生成します。

## 入力

| パラメータ | 説明 | データ型 | 必須 | 範囲 |
|-----------|-------------|-----------|----------|-------|
| `model` | サンプリングに使用する拡散モデル。 | MODEL | はい | - |
| `eta` | ステップサイズのスケーリング係数を制御します（デフォルト: 1.0）。 | FLOAT | いいえ | 0.0 - 10.0 |
| `sde_start_percent` | SDE サンプリングが開始されるノイズ除去プロセスの開始パーセンテージ。モデルのサンプリングスケジュールを使用してシグマ値に変換されます（デフォルト: 0.2）。 | FLOAT | いいえ | 0.0 - 1.0 |
| `sde_end_percent` | SDE サンプリングが終了するノイズ除去プロセスの終了パーセンテージ。モデルのサンプリングスケジュールを使用してシグマ値に変換されます（デフォルト: 0.8）。 | FLOAT | いいえ | 0.0 - 1.0 |
| `s_noise` | サンプリング中に追加されるノイズの量を制御します（デフォルト: 1.0）。 | FLOAT | いいえ | 0.0 - 100.0 |
| `predictor_order` | ソルバーにおける予測子コンポーネントの次数（デフォルト: 3）。 | INT | いいえ | 1 - 6 |
| `corrector_order` | ソルバーにおける修正子コンポーネントの次数（デフォルト: 4）。 | INT | いいえ | 0 - 6 |
| `use_pece` | PECE（Predict-Evaluate-Correct-Evaluate）メソッドを有効または無効にします。 | BOOLEAN | いいえ | - |
| `simple_order_2` | 簡略化された二次計算を有効または無効にします。 | BOOLEAN | いいえ | - |

注：`model` を除くすべての入力は高度なパラメータであり、ノードのインターフェースではデフォルトで非表示になっています。

## 出力

| 出力名 | 説明 | データ型 |
|-------------|-------------|-----------|
| `sampler` | 拡散モデルで使用できる設定済みのサンプラーオブジェクト。 | SAMPLER |

> このドキュメントは AI によって生成されました。エラーを見つけた場合や改善のご提案がある場合は、ぜひ貢献してください！ [GitHub で編集](https://github.com/Comfy-Org/embedded-docs/blob/main/comfyui_embedded_docs/docs/SamplerSASolver/ja.md)

---
**Source fingerprint (SHA-256):** `31da2d436665bf533c28b32248f632edab8f6d92372402904702ae954230f98d`
