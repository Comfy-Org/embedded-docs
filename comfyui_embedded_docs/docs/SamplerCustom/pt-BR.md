...
|-----------|---------------|-------------|
| `model` | `MODEL` | A entrada 'model' especifica o modelo a ser usado para amostragem, desempenhando um papel crucial na determinação do comportamento e do resultado da amostragem. |
| `add_noise` | `BOOLEAN` | A entrada 'add_noise' permite que os usuários especifiquem se ruído deve ser adicionado ao processo de amostragem, influenciando a diversidade e as características das amostras geradas. |
...
| `positive` | `CONDITIONING` | A entrada 'positive' representa informações de condicionamento positivo, guiando o processo de amostragem para gerar amostras que estejam alinhadas com atributos positivos especificados. |
