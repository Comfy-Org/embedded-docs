...
El nodo Save Video guarda el video de entrada en tu directorio de salida de ComfyUI. Puedes elegir el prefijo del nombre de archivo, el formato de contenedor, el códec de video y opciones de codificación como la calidad. El nodo genera automáticamente un nombre de archivo único usando un contador y puede incrustar metadatos del flujo de trabajo en el archivo guardado.
...
| Parámetro | Descripción | Tipo de datos | Requerido | Rango |
| --- | --- | --- | --- | --- |
| `encoding` | `auto` conserva flujos H.264 compatibles. `re-encode` aplica opciones de codificación personalizadas. | DYNAMIC_COMBO | No | `"auto"`<br>`"re-encode"` |
...
| Parámetro | Descripción | Tipo de datos | Requerido | Rango |
| --- | --- | --- | --- | --- |
| `encoding` | `auto` conserva flujos AV1 compatibles. `re-encode` aplica opciones de codificación personalizadas. | DYNAMIC_COMBO | No | `"auto"`<br>`"re-encode"` |
...
Nota: Cuando `format` es `"auto"`, el contenedor guardado se elige automáticamente: `av1` produce WebM, mientras que `auto` y `h264` producen MP4. El formato `webm` solo permite los códecs `auto` y `av1`. Cuando `codec` es `"auto"`, el flujo de video de origen se conserva en lugar de recodificarse. El archivo guardado usa un sufijo de contador para evitar sobrescribir archivos existentes. La extensión del archivo se determina por el contenedor resuelto. Los metadatos del flujo de trabajo (el prompt y la información adicional de los nodos) se incrustan en el archivo guardado a menos que el guardado de metadatos esté deshabilitado al iniciar.
...
| `ui` | Una vista previa del archivo de video guardado, incluida la ruta del archivo y la información de la subcarpeta para mostrarla en la UI. | PREVIEW_VIDEO |
