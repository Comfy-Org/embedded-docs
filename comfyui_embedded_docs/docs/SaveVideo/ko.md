...
|-----------|-----------|----------|-------|-------------|
| `video` | VIDEO | 예 | - | 저장할 비디오입니다. |
| `filename_prefix` | STRING | 아니요 | - | 저장할 파일의 접두사입니다. %date:yyyy-MM-dd% 또는 %Empty Latent Image.width%와 같은 형식 정보를 포함하여 노드의 값을 사용할 수 있습니다 (기본값: "video/ComfyUI"). |
| `format` | COMBO | 아니요 | `"auto"`<br>`"mp4"`<br>`"webm"`<br>`"mkv"`<br>`"gif"` | 비디오를 저장할 형식입니다 (기본값: "auto"). |
| `codec` | COMBO | 아니요 | `"auto"`<br>`"h264"`<br>`"h265"`<br>`"vp9"`<br>`"av1"`<br>`"prores"` | 비디오에 사용할 코덱입니다 (기본값: "auto"). |
