...
| `weight_dtype` | モデルの重みの数値精度（データ型）です。"default" はモデルの元の精度を使用し、"fp16" は16ビット浮動小数点数、"bf16" は bfloat16、"fp32" は32ビット浮動小数点数を使用します。（デフォルト: "default"） | STRING | いいえ | `"default"`<br>`"fp16"`<br>`"bf16"`<br>`"fp32"` |
