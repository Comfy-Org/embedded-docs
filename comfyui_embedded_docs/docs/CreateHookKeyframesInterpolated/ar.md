...
| `الاستيفاء` | COMBO | نعم | `LINEAR`<br>`EASE_IN`<br>`EASE_OUT`<br>`EASE_IN_OUT`<br>`EASE_OUT_IN`<br>`SINE`<br>`CUBIC`<br>`QUARTIC`<br>`QUINTIC`<br>`EXPO`<br>`CIRC`<br>`BACK`<br>`BOUNCE`<br>`ELASTIC` | طريقة الاستيفاء المستخدمة للانتقال بين قيم القوة (القيمة الافتراضية: LINEAR) |
...
| `عدد الإطارات المفتاحية` | INT | نعم | 2 - 100 | عدد إطارات المفاتيح المراد توليدها في تسلسل الاستيفاء (القيمة الافتراضية: 5) |
| `طباعة الإطارات المفتاحية` | BOOLEAN | نعم | True/False | ما إذا كان سيتم طباعة معلومات إطارات المفاتيح المُولَّدة في السجل (القيمة الافتراضية: False) |
| `الإطار المفتاحي السابق` | HOOK_KEYFRAMES | لا | - | مجموعة إطارات مفاتيح خطافية سابقة اختيارية للإلحاق بها |
