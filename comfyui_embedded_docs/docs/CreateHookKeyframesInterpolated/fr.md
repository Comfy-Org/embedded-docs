...
| `strength_end` | FLOAT | Oui | 0.0 - 10.0 | La valeur de force finale pour la séquence d'interpolation (par défaut : 1.0) |
...
| `start_percent` | FLOAT | Oui | 0.0 - 1.0 | La position de pourcentage de départ dans le processus de génération (par défaut : 0.0) |
| `end_percent` | FLOAT | Oui | 0.0 - 1.0 | La position de pourcentage finale dans le processus de génération (par défaut : 1.0) |
| `keyframes_count` | INT | Oui | 2 - 100 | Le nombre d'images clés à générer dans la séquence d'interpolation (par défaut : 5) |
| `print_keyframes` | BOOLEAN | Oui | Vrai/Faux | Indique s'il faut afficher les informations des images clés générées dans le journal (par défaut : Faux) |
| `prev_hook_kf` | HOOK_KEYFRAMES | Non | - | Groupe d'images clés de hook précédent optionnel auquel ajouter la séquence |
