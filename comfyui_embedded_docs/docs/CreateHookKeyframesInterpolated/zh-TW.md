...
建立一個具有在起始點和結束點之間插值強度值的掛鉤關鍵影格序列。此節點會生成多個關鍵影格，在生成過程的指定百分比範圍內平滑過渡強度參數，並使用各種插值方法來控制過渡曲線。
...
| `插值方式` | COMBO | 是 | 多種選項可用 | 用於在強度值之間過渡的插值方法 |
...
| `前一個 Hook 關鍵影格` | HOOK_KEYFRAMES | 否 | - | 可選的前一個掛鉤關鍵影格群組，用於附加到其後 |
