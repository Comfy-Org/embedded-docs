...
| `interpolation` | COMBO | 是 | `LINEAR`<br>`EASE_IN`<br>`EASE_OUT`<br>`EASE_IN_OUT`<br>`EASE_OUT_IN`<br>`SINE`<br>`CUBIC`<br>`QUARTIC`<br>`QUINTIC`<br>`EXPO`<br>`CIRC`<br>`BACK`<br>`BOUNCE`<br>`ELASTIC` | 用於在強度值之間過渡的插值方法（預設值：LINEAR） |
| `start_percent` | FLOAT | 是 | 0.0 - 1.0 | 生成過程中的起始百分比位置（預設值：0.0） |
| `end_percent` | FLOAT | 是 | 0.0 - 1.0 | 生成過程中的結束百分比位置（預設值：1.0） |
| `keyframes_count` | INT | 是 | 2 - 100 | 在插值序列中生成的關鍵影格數量（預設值：5） |
| `print_keyframes` | BOOLEAN | 是 | True/False | 是否將生成的關鍵影格資訊輸出到日誌（預設值：False） |
| `prev_hook_kf` | HOOK_KEYFRAMES | 否 | - | 可選的先前鉤子關鍵影格群組，用於附加 |
