...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `prompt` | STRING | 是 | - | 提供給 DALL·E 的文字提示（預設值：""） |
| `seed` | INT | 否 | 0 至 2147483647 | 後端尚未實作此功能（預設值：0） |
| `quality` | COMBO | 否 | "standard"<br>"hd" | 圖像品質（預設值："standard"） |
| `style` | COMBO | 否 | "natural"<br>"vivid" | Vivid 會使模型傾向於生成超寫實且戲劇化的圖像。Natural 則使模型產生更自然、較不超寫實的圖像。（預設值："natural"） |
| `size` | COMBO | 否 | "1024x1024"<br>"1024x1792"<br>"1792x1024" | 圖像尺寸（預設值："1024x1024"） |
