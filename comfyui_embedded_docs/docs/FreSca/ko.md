...
| `scale_low` | FLOAT | 아니요 | 0 - 10 | 저주파 성분에 대한 스케일링 계수 (기본값: 1.0) |
| `scale_high` | FLOAT | 아니요 | 0 - 10 | 고주파 성분에 대한 스케일링 계수 (기본값: 1.25) |
| `freq_cutoff` | INT | 아니요 | 1 - 10000 | 저주파로 간주할 중심 주변 주파수 인덱스 개수 (기본값: 20) |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `model` | MODEL | 안내 함수에 주파수 종속 스케일링이 적용된 수정된 모델 |
