...
|----------|-----------|------|----------|------|
| `positive_NEW` | CONDITIONING | 是 | - | 待修改的正向条件输入 |
| `negative_NEW` | CONDITIONING | 是 | - | 待修改的负向条件输入 |
| `strength` | FLOAT | 是 | 0.0 至 10.0 | 应用于条件数据的强度乘数（默认值：1.0） |
| `set_cond_area` | STRING | 是 | "default"<br>"mask bounds" | 决定条件区域的计算方式 |
| `mask` | MASK | 否 | - | 用于约束条件区域的可选遮罩 |
| `hooks` | HOOKS | 否 | - | 用于高级条件修改的可选钩子组 |
| `timesteps` | TIMESTEPS_RANGE | 否 | - | 用于限制条件应用时间的可选时间步范围 |

## 输出结果

| 输出名称 | 数据类型 | 描述 |
|----------|-----------|------|
| `positive` | CONDITIONING | 应用属性修改后的正向条件数据 |
