...
|-----------|-----------|----------|-------|-------------|
| `새 긍정 조건` | CONDITIONING | 예 | - | 수정할 긍정 컨디셔닝 입력 |
| `새 부정 조건` | CONDITIONING | 예 | - | 수정할 부정 컨디셔닝 입력 |
| `강도` | FLOAT | 예 | 0.0 ~ 10.0 | 컨디셔닝에 적용되는 강도 승수 (기본값: 1.0) |
| `조건 영역 설정` | STRING | 예 | "default"<br>"mask bounds" | 컨디셔닝 영역 계산 방식을 결정합니다 (기본값: "default") |
| `마스크` | MASK | 아니요 | - | 컨디셔닝 영역을 제한하는 선택적 마스크 |
| `후크` | HOOKS | 아니요 | - | 고급 컨디셔닝 수정을 위한 선택적 후크 그룹 |
| `타임스텝 범위` | TIMESTEPS_RANGE | 아니요 | - | 컨디셔닝 적용 시점을 제한하는 선택적 타임스텝 범위 |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `부정 조건` | CONDITIONING | 속성이 적용된 수정된 긍정 컨디셔닝 |
