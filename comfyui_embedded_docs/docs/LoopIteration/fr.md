...
Ce nœud fournit des métadonnées d'itération pour les flux de travail de type boucle. Il reçoit des entrées basées sur des listes et transmet le premier élément de chaque liste, afin qu'une boucle puisse suivre l'index courant, si elle se trouve à la première ou à la dernière étape, l'élément de liste courant et toute valeur d'itération en cours. Le comportement du cache est contrôlé par l'indicateur `reuse_cache`. Ce nœud est marqué comme réservé aux développeurs.
...
| `current_iteration_value` | La valeur transportée par l'itération courante. Facultatif ; lorsqu'elle est fournie, la valeur complète est transmise à la sortie, sinon None est renvoyé. | ANY | Non | - |
...
Remarque : Ce nœud prend en charge les entrées de type liste, ce qui signifie que chaque entrée est attendue sous forme de liste et que le nœud ne produit que le premier élément de chaque liste (sauf `current_iteration_value`, qui est transmis sous forme de liste). Il accepte également des entrées supplémentaires au-delà de celles listées ici.
...
| `current_iteration_value` | La valeur transportée par l'itération courante, ou None lorsqu'aucune valeur n'a été fournie. Cette sortie est renvoyée sous forme de liste. | ANY |
