...
| `current_iteration_value` | The value carried by the current iteration. Optional; the first element is passed through to the output when provided, otherwise None is returned. | ANY | No | - |
...
Note: This node is list-input enabled, meaning each input is expected to arrive as a list and the node outputs only the first element of each list. It also accepts additional inputs beyond those listed here.
...
| `current_iteration_value` | The value carried by the current iteration, or None when no value was provided. | ANY |
