...
|-----------|-----------|----------|-------|-------------|
| `eta` | FLOAT | 아니요 | 0.0 - 100.0 | 샘플링 과정의 단계 크기와 확률적 특성을 제어합니다(기본값: 1.0). 이는 고급 매개변수입니다. |
| `s_noise` | FLOAT | 아니요 | 0.0 - 100.0 | 샘플링 중 추가되는 노이즈의 양을 제어합니다(기본값: 1.0). 이는 고급 매개변수입니다. |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `sampler` | SAMPLER | 샘플링 파이프라인에서 사용할 수 있는 구성된 오일러 조상 샘플러를 반환합니다. |
