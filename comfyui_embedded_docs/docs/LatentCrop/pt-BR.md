...
|-----------|---------------|-----------|
| `amostras` | `LATENT` | O parâmetro `amostras` representa as representações latentes a serem cortadas. É essencial para definir os dados sobre os quais a operação de corte será realizada. |
| `largura` | `INT` | Especifica a largura da área de corte. Influencia diretamente as dimensões da representação latente de saída. |
| `altura` | `INT` | Especifica a altura da área de corte, afetando o tamanho da representação latente resultante após o corte. |
