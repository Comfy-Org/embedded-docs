...
|-----------|-----------|----------|-------|-------------|
| `model` | MODEL | 예 | - | 건너뛴 레이어 가이던스를 적용할 모델 |
| `layers` | STRING | 예 | - | 건너뛸 레이어 인덱스의 쉼표로 구분된 목록 (기본값: "7, 8, 9") |
| `scale` | FLOAT | 예 | 0.0 - 10.0 | 건너뛴 레이어 가이던스 효과의 강도 (기본값: 3.0) |
| `start_percent` | FLOAT | 예 | 0.0 - 1.0 | 전체 단계 대비 가이던스 적용 시작 지점의 백분율 (기본값: 0.01) |
| `end_percent` | FLOAT | 예 | 0.0 - 1.0 | 전체 단계 대비 가이던스 적용 종료 지점의 백분율 (기본값: 0.15) |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `model` | MODEL | 건너뛴 레이어 가이던스가 적용된 수정된 모델 |
