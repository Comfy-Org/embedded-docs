...
| `layers` | STRING | 是 | - | 要跳过的层索引逗号分隔列表（默认："7, 8, 9"） |
| `scale` | FLOAT | 是 | 0.0 - 10.0 | 跳层引导效果的强度（默认：3.0） |
| `start_percent` | FLOAT | 是 | 0.0 - 1.0 | 引导应用的起始点，以总步数的百分比表示（默认：0.01） |
| `end_percent` | FLOAT | 是 | 0.0 - 1.0 | 引导应用的结束点，以总步数的百分比表示（默认：0.15） |

## 输出结果

| 输出名称 | 数据类型 | 描述 |
|----------|-----------|------|
| `model` | MODEL | 应用了跳层引导的修改后模型 |