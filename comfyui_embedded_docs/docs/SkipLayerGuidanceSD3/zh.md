...
| `layers` | STRING | 是 | - | 要跳过的层索引列表，以逗号分隔（默认值："7, 8, 9"） |
| `scale` | FLOAT | 是 | 0.0 - 10.0 | 跳过层引导效果的强度（默认值：3.0） |
| `start_percent` | FLOAT | 是 | 0.0 - 1.0 | 引导应用起始点，占总步数的百分比（默认值：0.01） |
| `end_percent` | FLOAT | 是 | 0.0 - 1.0 | 引导应用结束点，占总步数的百分比（默认值：0.15） |

## 输出

| 输出名称 | 数据类型 | 描述 |
|-------------|-----------|-------------|
| `model` | MODEL | 已应用跳过层引导的修改后模型 |
