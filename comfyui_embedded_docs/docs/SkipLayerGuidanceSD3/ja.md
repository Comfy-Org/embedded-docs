...
| `scale` | FLOAT | はい | 0.0 - 10.0 | スキップレイヤーガイダンス効果の強さ（デフォルト: 3.0） |
| `start_percent` | FLOAT | はい | 0.0 - 1.0 | ガイダンス適用の開始位置（全ステップに対する割合）（デフォルト: 0.01） |
| `end_percent` | FLOAT | はい | 0.0 - 1.0 | ガイダンス適用の終了位置（全ステップに対する割合）（デフォルト: 0.15） |
