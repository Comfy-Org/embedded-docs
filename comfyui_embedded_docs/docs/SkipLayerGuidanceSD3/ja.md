...
| `layers` | STRING | はい | - | スキップするレイヤーインデックスのカンマ区切りリスト（デフォルト: "7, 8, 9"） |
| `scale` | FLOAT | はい | 0.0 - 10.0 | スキップレイヤーガイダンス効果の強度（デフォルト: 3.0） |
| `start_percent` | FLOAT | はい | 0.0 - 1.0 | ガイダンス適用の開始ポイントを総ステップ数のパーセンテージで指定（デフォルト: 0.01） |
| `end_percent` | FLOAT | はい | 0.0 - 1.0 | ガイダンス適用の終了ポイントを総ステップ数のパーセンテージで指定（デフォルト: 0.15） |
