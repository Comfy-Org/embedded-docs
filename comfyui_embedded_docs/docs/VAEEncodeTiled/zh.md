...
| 参数 | 数据类型 | 是否必填 | 范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `pixels` | IMAGE | 是 | - | 待编码的输入图像数据 |
| `vae` | VAE | 是 | - | 用于编码的变分自编码器模型 |
| `tile_size` | INT | 是 | 64-4096（步长：64） | 空间处理中每个图块的大小（默认值：512） |
| `overlap` | INT | 是 | 0-4096（步长：32） | 相邻图块之间的重叠量（默认值：64） |
| `temporal_size` | INT | 是 | 8-4096（步长：4） | 仅用于视频 VAE：每次编码的帧数（默认值：64） |
| `temporal_overlap` | INT | 是 | 4-4096（步长：4） | 仅用于视频 VAE：重叠的帧数（默认值：8） |
