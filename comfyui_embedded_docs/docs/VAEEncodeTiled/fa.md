...
|-----------|-----------|----------|-------|-------------|
| `pixels` | IMAGE | بله | - | داده‌های تصویر ورودی برای رمزگذاری |
| `vae` | VAE | بله | - | مدل رمزگذار خودکار متغیر مورد استفاده برای رمزگذاری |
| `tile_size` | INT | بله | 64-4096 (گام: 64) | اندازه هر کاشی برای پردازش مکانی (پیش‌فرض: 512) |
| `overlap` | INT | بله | 0-4096 (گام: 32) | میزان هم‌پوشانی بین کاشی‌های مجاور (پیش‌فرض: 64) |
| `temporal_size` | INT | بله | 8-4096 (گام: 4) | فقط برای VAEهای ویدیویی استفاده می‌شود: تعداد فریم‌هایی که در یک زمان رمزگذاری می‌شوند (پیش‌فرض: 64) |
| `temporal_overlap` | INT | بله | 4-4096 (گام: 4) | فقط برای VAEهای ویدیویی استفاده می‌شود: تعداد فریم‌های هم‌پوشانی (پیش‌فرض: 8) |
