...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `模型` | COMBO | 是 | "wan2.5-i2i-preview" | 要使用的模型（預設值："wan2.5-i2i-preview"）。 |
| `圖像` | IMAGE | 是 | - | 單張圖像編輯或多張圖像融合，最多 2 張圖像。 |
| `提示詞` | STRING | 是 | - | 描述元素和視覺特徵的提示詞。支援英文和中文（預設值：空白）。 |
| `負面提示詞` | STRING | 否 | - | 描述要避免內容的負面提示詞（預設值：空白）。 |
| `種子值` | INT | 否 | 0 至 2147483647 | 用於生成的隨機種子（預設值：0）。 |
