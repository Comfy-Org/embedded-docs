...
| `prompt` | STRING | 예 | - | 요소와 시각적 특징을 설명하는 프롬프트. 영어와 중국어 지원 (기본값: 비어 있음). |
| `negative_prompt` | STRING | 아니요 | - | 피해야 할 사항을 설명하는 네거티브 프롬프트 (기본값: 비어 있음). |
| `seed` | INT | 아니요 | 0 ~ 2147483647 | 생성에 사용할 시드 (기본값: 0). |
| `watermark` | BOOLEAN | 아니요 | - | 결과물에 AI 생성 워터마크를 추가할지 여부 (기본값: false). |
...
| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `image` | IMAGE | 입력 이미지와 텍스트 프롬프트를 기반으로 생성된 이미지. |
