...
| `instrucciones` | STRING | No | N/A | Orientación adicional sobre estilo o formato. Este es un parámetro opcional y avanzado. |
| `imágenes_de_referencia` | IMAGE | No | 0 a 4 imágenes | Hasta 4 imágenes de referencia para guiar la generación. Esta es una entrada opcional. |
| `modelo` | COMBO | Sí | `"Quiver SVG v1"`<br>`"Quiver SVG v1 Max"`<br>`"Quiver SVG v1 Preview"` | Modelo a utilizar para la generación de SVG. Las opciones disponibles están determinadas por la API de Quiver. |
| `semilla` | INT | Sí | 0 a 2147483647 | Semilla para determinar si el nodo debe re-ejecutarse; los resultados reales son no deterministas independientemente de la semilla. Valor predeterminado: 0. |
