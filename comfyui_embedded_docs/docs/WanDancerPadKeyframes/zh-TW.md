...
| `segment_index` | INT | 是 | 0 至 100 | 此片段為第幾個片段（0 表示第一個，1 表示第二個，依此類推，預設值：0）。 |
| `audio` | AUDIO | 是 | 音訊資料 | 用於計算總輸出影格數並提取片段音訊的音軌。 |
...
| `keyframes_mask` | MASK | 遮罩，標示有效影格（1 表示關鍵影格位置，0 表示填充位置）。 |
