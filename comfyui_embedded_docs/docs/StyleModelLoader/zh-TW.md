...
| 參數名稱 | Comfy 資料型別 | Python 資料型別 | 說明 |
|---------------------|-----------------|--------------|---------------------------------------------------------------------------------------------------|
| `style_model_name` | COMBO[STRING] | `str` | 指定要載入的風格模型名稱。此名稱用於在預定義的目錄結構中定位模型檔案，允許根據使用者輸入或應用程式需求動態載入不同的風格模型。 |

## 輸出

| 參數名稱 | Comfy 資料型別 | Python 資料型別 | 說明 |
|-----------------|---------------|--------------|---------------------------------------------------------------------------------------------------|
| `style_model` | `STYLE_MODEL` | `StyleModel` | 傳回已載入的風格模型，準備好用於將風格套用至影像。這使得能夠透過套用不同的藝術風格來動態自訂視覺輸出。 |