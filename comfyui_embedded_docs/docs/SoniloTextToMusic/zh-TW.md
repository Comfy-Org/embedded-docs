...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `prompt` | STRING | 是 | 不適用 | 描述要生成音樂的文字提示詞。此為必填欄位。 |
| `duration` | INT | 否 | 0 到 360 | 目標時長（秒）。設為 0 可讓模型根據提示詞推斷時長。最長：6 分鐘（360 秒）。預設值：0。 |
| `seed` | INT | 否 | 0 到 18446744073709551615 | 用於再現結果的種子值。目前 Sonilo 服務會忽略此參數，但保留以維持圖表一致性。預設值：0。 |
