...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `模型` | COMBO | 是 | `"Seedance 2.0"`<br>`"Seedance 2.0 Fast"` | 要使用的 AI 模型。Seedance 2.0 提供最高品質，而 Seedance 2.0 Fast 則針對速度進行最佳化。選擇模型後會顯示 `prompt`、`resolution`、`duration`、`ratio`、`generate_audio` 的額外必要輸入，以及 `reference_images`、`reference_videos`、`reference_audios`、`reference_assets` 和 `auto_downscale` 的選用輸入。 |
| `種子` | INT | 否 | 0 到 2147483647 | 用於控制節點是否應重新執行的數值。無論種子值為何，結果都非確定性（預設值：0）。 |
| `浮水印` | BOOLEAN | 否 | `True` / `False` | 是否在生成的影片中添加浮水印（預設值：False）。 |
