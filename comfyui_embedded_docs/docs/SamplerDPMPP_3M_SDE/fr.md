...
| `s_bruit` | Contrôle la quantité de bruit ajoutée lors de l'échantillonnage (défaut : 1.0) | FLOAT | Oui | 0.0 - 100.0 |
| `appareil_bruit` | Sélectionne le périphérique pour les calculs de bruit, soit GPU, soit CPU (défaut : "gpu") | COMBO | Oui | "gpu"<br>"cpu" |

Remarque : Les trois entrées sont des paramètres avancés.

Lorsque `noise_device` est défini sur "cpu", l'échantillonneur standard `dpmpp_3m_sde` est créé ; lorsqu'il est défini sur "gpu", l'échantillonneur accéléré par GPU `dpmpp_3m_sde_gpu` est créé.
