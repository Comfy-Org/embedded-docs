...
| `quality` | COMBO | 是 | `"360p"`<br>`"540p"`<br>`"720p"`<br>`"1080p"` | 视频质量设置（默认：`"540p"`） |
| `duration_seconds` | COMBO | 是 | `5`<br>`8` | 视频时长（秒） |
| `motion_mode` | COMBO | 是 | `"normal"`<br>`"fast"` | 过渡的运动风格（默认：`"normal"`） |
| `seed` | INT | 是 | 0 到 2147483647 | 视频生成的种子（默认：0） |
