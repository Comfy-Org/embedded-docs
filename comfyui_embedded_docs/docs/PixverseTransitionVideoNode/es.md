...
|-----------|-----------|----------|-------|-------------|
| `first_frame` | IMAGE | Sí | - | La imagen inicial para la transición de video |
| `last_frame` | IMAGE | Sí | - | La imagen final para la transición de video |
| `prompt` | STRING | Sí | - | Prompt para la generación del video (valor por defecto: cadena vacía) |
| `quality` | COMBO | Sí | Opciones de calidad disponibles del enum PixverseQuality<br>Por defecto: res_540p | Configuración de calidad de video |
| `duration_seconds` | COMBO | Sí | Opciones de duración disponibles del enum PixverseDuration | Duración del video en segundos |
| `motion_mode` | COMBO | Sí | Opciones de modo de movimiento disponibles del enum PixverseMotionMode | Estilo de movimiento para la transición |
| `seed` | INT | Sí | 0 a 2147483647 | Semilla para la generación de video (valor por defecto: 0) |
| `negative_prompt` | STRING | No | - | Una descripción textual opcional de elementos no deseados en una imagen (valor por defecto: cadena vacía) |
