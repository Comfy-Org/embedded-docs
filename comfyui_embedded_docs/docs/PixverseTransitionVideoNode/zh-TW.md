...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `第一幀` | IMAGE | 是 | - | 影片轉場的起始圖片 |
| `最後一幀` | IMAGE | 是 | - | 影片轉場的結束圖片 |
| `提示詞` | STRING | 是 | - | 影片生成的提示詞（預設值：空字串） |
| `品質` | COMBO | 是 | `"360p"`<br>`"540p"`<br>`"720p"`<br>`"1080p"` | 影片品質設定（預設值：`"540p"`） |
| `持續秒數` | COMBO | 是 | `5`<br>`8` | 影片長度（秒） |
| `動作模式` | COMBO | 是 | `"normal"`<br>`"fast"` | 轉場的動態風格（預設值：`"normal"`） |
| `種子值` | INT | 是 | 0 至 2147483647 | 影片生成的隨機種子（預設值：0） |
| `負向提示詞` | STRING | 否 | - | 可選的文字描述，用於指定圖片中不希望出現的元素（預設值：空字串） |
