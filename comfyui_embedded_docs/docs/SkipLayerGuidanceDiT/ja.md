...
| `double_layers` | STRING | はい | - | スキップするダブルブロックのレイヤー番号をカンマ区切りで指定（デフォルト: "7, 8, 9"） |
| `single_layers` | STRING | はい | - | スキップするシングルブロックのレイヤー番号をカンマ区切りで指定（デフォルト: "7, 8, 9"） |
...
| `rescaling_scale` | FLOAT | はい | 0.0 - 10.0 | リスケーリングのスケール係数（デフォルト: 0.0） |
...
| 出力名 | データ型 | 説明 |
|-------------|-----------|-------------|
| `model` | MODEL | スキップレイヤーガイダンスが適用された修正済みモデル |