...
| `雙層` | STRING | 是 | - | 要跳過的雙區塊層號，以逗號分隔（預設："7, 8, 9"） |
| `單層` | STRING | 是 | - | 要跳過的單區塊層號，以逗號分隔（預設："7, 8, 9"） |
| `縮放` | FLOAT | 是 | 0.0 - 10.0 | 引導縮放因子（預設：3.0） |
| `起始百分比` | FLOAT | 是 | 0.0 - 1.0 | 開始應用引導的百分比（預設：0.01） |
| `結束百分比` | FLOAT | 是 | 0.0 - 1.0 | 結束應用引導的百分比（預設：0.15） |
| `重新縮放比例` | FLOAT | 是 | 0.0 - 10.0 | 重新縮放因子，用於調整輸出幅度（預設：0.0，表示不重新縮放） |
