...
| `double_layers` | STRING | 是 | - | 双块中要跳过的层号，以逗号分隔（默认值："7, 8, 9"） |
| `single_layers` | STRING | 是 | - | 单块中要跳过的层号，以逗号分隔（默认值："7, 8, 9"） |
| `scale` | FLOAT | 是 | 0.0 - 10.0 | 引导缩放因子（默认值：3.0） |
| `start_percent` | FLOAT | 是 | 0.0 - 1.0 | 引导应用的起始百分比（默认值：0.01） |
| `end_percent` | FLOAT | 是 | 0.0 - 1.0 | 引导应用的结束百分比（默认值：0.15） |
| `rescaling_scale` | FLOAT | 是 | 0.0 - 10.0 | 用于调整输出幅度的重新缩放因子（默认值：0.0，表示不重新缩放） |
