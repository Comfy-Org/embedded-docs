...
|-----------|-----------|----------|-------|-------------|
| `model` | MODEL | 예 | - | 레이어 건너뛰기 가이던스를 적용할 모델 |
| `double_layers` | STRING | 예 | - | 건너뛸 이중 블록의 레이어 번호를 쉼표로 구분한 목록 (기본값: "7, 8, 9") |
| `single_layers` | STRING | 예 | - | 건너뛸 단일 블록의 레이어 번호를 쉼표로 구분한 목록 (기본값: "7, 8, 9") |
| `scale` | FLOAT | 예 | 0.0 - 10.0 | 가이던스 스케일 계수 (기본값: 3.0) |
| `start_percent` | FLOAT | 예 | 0.0 - 1.0 | 가이던스 적용 시작 비율 (기본값: 0.01) |
| `end_percent` | FLOAT | 예 | 0.0 - 1.0 | 가이던스 적용 종료 비율 (기본값: 0.15) |
| `rescaling_scale` | FLOAT | 예 | 0.0 - 10.0 | 출력 크기를 조정하기 위한 재조정 스케일 계수 (기본값: 0.0, 재조정 없음을 의미) |
...
| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `model` | MODEL | 레이어 건너뛰기 가이던스가 적용된 수정된 모델 |
