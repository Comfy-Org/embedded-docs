...
| `width`   | `FLOAT`     | 以總圖像寬度的百分比指定區域的寬度，影響條件化在水平方向上對圖像的作用範圍。 |
| `height`  | `FLOAT`     | 以總圖像高度的百分比確定區域的高度，影響條件化在垂直方向上的作用範圍。 |
| `x`       | `FLOAT`     | 以總圖像寬度的百分比表示區域的水平起始點，用於定位條件化效果。 |
| `y`       | `FLOAT`     | 以總圖像高度的百分比指定區域的垂直起始點，用於定位條件化效果。 |
| `strength`| `FLOAT`     | 控制指定區域內條件化效果的強度，允許對其影響進行微調。 |
