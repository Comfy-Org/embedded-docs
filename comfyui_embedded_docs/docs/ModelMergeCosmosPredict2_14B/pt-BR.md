...
O nó ModelMergeCosmosPredict2_14B combina dois modelos de IA em um só, mesclando seus componentes internos correspondentes. Ele permite controlar quanto do segundo modelo é misturado ao primeiro ajustando um peso separado para cada embedder, bloco e camada final.
...
| `pos_embedder.` | Peso de mesclagem do embedder de posição (padrão: 1.0) | FLOAT | Sim | 0.0 - 1.0 |
| `x_embedder.` | Peso de mesclagem do embedder de entrada (padrão: 1.0) | FLOAT | Sim | 0.0 - 1.0 |
| `t_embedder.` | Peso de mesclagem do embedder de tempo (padrão: 1.0) | FLOAT | Sim | 0.0 - 1.0 |
...
**Nota:** Todos os parâmetros de peso de mesclagem aceitam valores entre 0.0 e 1.0 em incrementos de 0.01, onde 0.0 significa nenhuma contribuição do model2 e 1.0 significa contribuição total do model2 para aquele componente específico.
