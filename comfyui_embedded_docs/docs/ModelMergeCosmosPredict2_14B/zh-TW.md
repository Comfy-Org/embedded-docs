...
ModelMergeCosmosPredict2_14B 節點允許您透過調整不同模型元件的影響力來合併兩個 AI 模型。它提供了對第二個模型的每個部分對最終合併模型貢獻程度的細粒度控制，使用特定模型層和元件的混合權重。
...
**注意：** 所有混合權重參數接受 0.0 到 1.0 之間的值，其中 0.0 表示該特定元件不從 model2 貢獻，1.0 表示該特定元件完全從 model2 貢獻。
