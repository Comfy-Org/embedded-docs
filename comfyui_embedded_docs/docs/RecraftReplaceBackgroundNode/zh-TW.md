...
| `種子值` | INT | 是 | 0-18446744073709551615 | 決定節點是否應重新執行的種子值；由於外部 API 的特性，實際結果無論種子值為何都是非確定性的（預設：0） |
| `Recraft 風格` | STYLEV3 | 否 | - | 生成背景的可選樣式選擇 |
| `負向提示詞` | STRING | 否 | - | 圖像上不希望出現元素的選項文字描述（預設：空） |
