...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `提示詞` | STRING | 是 | 無 | 影片的文字描述（預設：空字串）。 |
| `反向提示詞` | STRING | 否 | 無 | 負面文字提示，用於引導影片中應避免的內容（預設：空字串）。 |
| `解析度` | COMBO | 是 | `"720p"`<br>`"1080p"`<br>`"4k"` | 輸出影片的解析度。 |
| `長寬比` | COMBO | 否 | `"16:9"`<br>`"9:16"` | 輸出影片的長寬比（預設："16:9"）。 |
| `時長` | INT | 否 | 4 到 8 | 輸出影片的時長（秒）（預設：8）。 |
| `種子` | INT | 否 | 0 到 4294967295 | 影片生成的種子（預設：0）。 |
| `起始影格` | IMAGE | 是 | 無 | 影片的起始幀。 |
| `結束影格` | IMAGE | 是 | 無 | 影片的結束幀。 |
| `模型` | COMBO | 否 | `"veo-3.1-generate"`<br>`"veo-3.1-fast-generate"`<br>`"veo-3.1-lite"` | 用於生成的特定 Veo 3 模型（預設："veo-3.1-generate"）。 |
| `生成音訊` | BOOLEAN | 否 | 無 | 為影片生成音訊（預設：True）。 |
