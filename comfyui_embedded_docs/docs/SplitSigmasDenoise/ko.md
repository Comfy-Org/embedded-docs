...
|-----------|-----------|----------|-------|-------------|
| `시그마 배열` | SIGMAS | 예 | - | 노이즈 스케줄을 나타내는 시그마 값의 입력 시퀀스 |
| `노이즈 제거양` | FLOAT | 예 | 0.0 - 1.0 | 시그마 시퀀스를 분할할 위치를 결정하는 디노이징 강도 계수 (기본값: 1.0) |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `로우 시그마` | SIGMAS | 더 높은 시그마 값을 포함하는 시그마 시퀀스의 첫 번째 부분 |
