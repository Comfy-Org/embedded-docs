...
|-----------|-----------|----------|-------|-------------|
| `sigmas` | SIGMAS | 예 | - | 노이즈 스케줄을 나타내는 입력 시그마 값 시퀀스 |
| `denoise` | FLOAT | 예 | 0.0 - 1.0 | 시그마 시퀀스를 어디에서 분할할지 결정하는 디노이징 강도 계수 (기본값: 1.0) |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `high_sigmas` | SIGMAS | 더 높은 시그마 값을 포함하는 시그마 시퀀스의 첫 번째 부분 |
