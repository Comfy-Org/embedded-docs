...
|-----------|-----------|----------|-------|-------------|
| `prompt_text` | STRING | 예 | - | 비디오로 생성하고자 하는 내용을 설명하는 주요 텍스트 설명입니다 |
| `negative_prompt` | STRING | 예 | - | 생성된 비디오에 나타나지 않길 원하는 내용을 설명하는 텍스트입니다 |
| `seed` | INT | 예 | - | 재현 가능한 결과를 위해 생성 과정의 무작위성을 제어하는 숫자입니다 |
| `resolution` | STRING | 예 | - | 출력 비디오의 해상도 설정입니다 |
| `duration` | INT | 예 | - | 비디오의 길이(초 단위)입니다 |
| `aspect_ratio` | FLOAT | 아니요 | 0.4 - 2.5 | 화면 비율(너비 / 높이)입니다 (기본값: 1.7777777777777777) |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `output` | VIDEO | Pika API로부터 반환된 생성된 비디오 파일입니다 |
