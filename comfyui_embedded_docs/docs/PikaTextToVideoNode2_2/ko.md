...
|-----------|-----------|----------|-------|-------------|
| `prompt_text` | STRING | 예 | - | 생성하려는 비디오 내용을 설명하는 주요 텍스트 설명 |
| `negative_prompt` | STRING | 예 | - | 생성된 비디오에 나타나지 않기를 바라는 내용을 설명하는 텍스트 |
| `seed` | INT | 예 | - | 재현 가능한 결과를 위해 생성의 무작위성을 제어하는 숫자 |
| `resolution` | STRING | 예 | - | 출력 비디오의 해상도 설정 |
| `duration` | INT | 예 | - | 비디오의 길이(초 단위) |
| `aspect_ratio` | FLOAT | 아니오 | 0.4 - 2.5 | 화면비(너비 / 높이) (기본값: 1.7777777777777777) |
