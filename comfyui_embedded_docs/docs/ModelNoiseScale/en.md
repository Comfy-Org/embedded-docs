...
This node adjusts the noise scale used during model sampling. It allows you to set a specific noise scale value, which controls the amount of noise applied to the model's sampling process. The node clones the model and updates its sampling configuration with the new noise scale while keeping the existing shift and multiplier settings.
