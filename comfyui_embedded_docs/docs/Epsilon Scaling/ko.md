...
| `scaling_factor` | FLOAT | 아니요 | 0.5 - 1.5 | 예측된 노이즈가 스케일링되는 비율입니다. 1.0보다 큰 값은 노이즈를 줄이고, 1.0보다 작은 값은 노이즈를 증가시킵니다(기본값: 1.005). |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `model` | MODEL | 샘플링 과정에 엡실론 스케일링 함수가 적용된 입력 모델의 패치 버전입니다. |
