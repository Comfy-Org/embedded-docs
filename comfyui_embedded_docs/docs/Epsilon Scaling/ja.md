...
| パラメータ | データ型 | 必須 | 範囲 | 説明 |
|-----------|-----------|----------|-------|-------------|
| `model` | MODEL | はい | - | イプシロンスケーリングパッチが適用されるモデルです。 |
| `scaling_factor` | FLOAT | いいえ | 0.5 - 1.5 | 予測ノイズをスケーリングする係数です。1.0より大きい値はノイズを低減し、1.0より小さい値はノイズを増加させます（デフォルト: 1.005）。 |

## 出力

| 出力名 | データ型 | 説明 |
|-------------|-----------|-------------|
| `model` | MODEL | 入力モデルにイプシロンスケーリング関数を適用したパッチ版です。サンプリングプロセスに変更が加えられています。 |
