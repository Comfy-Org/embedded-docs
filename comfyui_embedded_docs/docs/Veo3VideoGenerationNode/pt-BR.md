...
| `person_generation` | COMBO | Não | "ALLOW"<br>"BLOCK" | Permite ou bloqueia a geração de pessoas no vídeo (padrão: "ALLOW") |
| `seed` | INT | Não | 0-4294967295 | Semente para geração do vídeo (0 para aleatório) (padrão: 0) |
| `image` | IMAGE | Não | - | Imagem de referência opcional para orientar a geração do vídeo |
| `model` | COMBO | Não | "veo-3.1-generate"<br>"veo-3.1-fast-generate"<br>"veo-3.1-lite"<br>"veo-3.0-generate-001"<br>"veo-3.0-fast-generate-001" | Modelo Veo 3 a ser usado para geração do vídeo (padrão: "veo-3.0-generate-001") |
| `generate_audio` | BOOLEAN | Não | - | Gera áudio para o vídeo. Suportado por todos os modelos Veo 3. (padrão: False) |
