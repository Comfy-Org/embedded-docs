...
|-----------|-----------|------------|---------|-------|-------------|
| `cond_NEW` | CONDITIONING | 必填 | - | - | 要修改的条件数据 |
| `strength` | FLOAT | 必填 | 1.0 | 0.0-10.0 | 控制条件调节效果的强度 |
| `set_cond_area` | STRING | 必填 | default | ["default", "mask bounds"] | 决定条件区域的应用方式 |
| `mask` | MASK | 可选 | - | - | 用于限制条件调节应用区域的可选遮罩 |
| `hooks` | HOOKS | 可选 | - | - | 用于自定义处理的可选钩子函数 |
| `timesteps` | TIMESTEPS_RANGE | 可选 | - | - | 用于限制条件调节生效时间范围的可选时间步范围 |
