...
| `strength` | FLOAT | 必填 | 1.0 | 0.0-10.0 | 控制條件化效果的強度 |
| `set_cond_area` | STRING | 必填 | default | ["default", "mask bounds"] | 決定條件化區域的應用方式 |
| `mask` | MASK | 可選 | - | - | 用於限制條件化應用區域的可選遮罩 |
| `hooks` | HOOKS | 可選 | - | - | 用於自定義處理的可選掛鉤函數 |
| `timesteps` | TIMESTEPS_RANGE | 可選 | - | - | 用於限制條件化生效時間的可選時間步範圍 |
