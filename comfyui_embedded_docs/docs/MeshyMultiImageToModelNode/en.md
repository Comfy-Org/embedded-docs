...
| --- | --- | --- | --- | --- |
| `model` | Specifies the AI model version to use. | COMBO | Yes | `"meshy-7.1"`<br>`"meshy-7"`<br>`"meshy-6"`<br>`"latest"` |
...
| `ultra_mode` | Run an extra refinement pass at 2048³ for higher-fidelity geometry with finer surface detail. Requires the `"meshy-7.1"`, `"meshy-7"` or `"latest"` model (default: `False`). | BOOLEAN | No | `True`<br>`False` |
...
* `ultra_mode` requires the `"meshy-7.1"`, `"meshy-7"` or `"latest"` model; any other model raises an error.
