...
| `ultra_mode` | 執行額外的細化處理，以 2048³ 獲得更精細表面細節的高保真幾何。需要 `"meshy-7.1"`、`"meshy-7"` 或 `"latest"` 模型（預設：`False`）。 | BOOLEAN | 否 | `True`<br>`False` |
...
* `ultra_mode` 需要 `"meshy-7.1"`、`"meshy-7"` 或 `"latest"` 模型；使用其他模型會引發錯誤。
