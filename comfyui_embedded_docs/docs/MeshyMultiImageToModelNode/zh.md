...
| 参数 | 描述 | 数据类型 | 必填 | 范围 |
| --- | --- | --- | --- | --- |
| `model` | 指定要使用的 AI 模型版本。 | COMBO | 是 | `"meshy-7.1"`<br>`"meshy-7"`<br>`"meshy-6"`<br>`"latest"` |
...
| `ultra_mode` | 运行额外的细化处理，以 2048³ 获得具有更精细表面细节的高保真几何体。需要 `"meshy-7.1"`、`"meshy-7"` 或 `"latest"` 模型（默认：`False`）。 | BOOLEAN | 否 | `True`<br>`False` |
...
* `ultra_mode` 需要 `"meshy-7.1"`、`"meshy-7"` 或 `"latest"` 模型；使用其他模型会引发错误。
