...
| `正向` | CONDITIONING | 是 | - | 引導生成朝向期望內容的正向條件資料 |
| `負向` | CONDITIONING | 是 | - | 引導生成遠離不必要內容的負向條件資料 |
| `縮放比例` | FLOAT | 否 | 0.0 - 10.0 | 套用於輸入影像的縮放倍率（預設值：4.0） |
| `雜訊增強` | FLOAT | 否 | 0.0 - 1.0 | 在放大過程中添加的雜訊量（預設值：0.0） |

## 輸出

| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `負向` | CONDITIONING | 已套用放大資訊的修改後正向條件資料 |
| `潛在空間` | CONDITIONING | 已套用放大資訊的修改後負向條件資料 |
