...
|-----------|-----------|----------|-------|-------------|
| `images` | IMAGE | 是 | - | 需要放大的輸入圖像 |
| `正向` | CONDITIONING | 是 | - | 正向條件數據，引導生成過程朝向期望的內容 |
| `負向` | CONDITIONING | 是 | - | 負向條件數據，使生成過程遠離不需要的內容 |
| `縮放比例` | FLOAT | 否 | 0.0 - 10.0 | 應用於輸入圖像的縮放比例因子（預設值：4.0） |
...
| 輸出名稱 | 資料類型 | 描述 |
|-------------|-----------|-------------|
| `負向` | CONDITIONING | 應用放大信息後的修改正向條件數據 |
| `潛在空間` | CONDITIONING | 應用放大信息後的修改負向條件數據 |
