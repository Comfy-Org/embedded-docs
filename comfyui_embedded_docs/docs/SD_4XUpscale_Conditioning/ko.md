...
| `positive` | CONDITIONING | 예 | - | 원하는 콘텐츠 생성을 안내하는 양성 컨디셔닝 데이터 |
| `negative` | CONDITIONING | 예 | - | 원하지 않는 콘텐츠 생성을 억제하는 음성 컨디셔닝 데이터 |
| `scale_ratio` | FLOAT | 아니요 | 0.0 - 10.0 | 입력 이미지에 적용되는 스케일링 비율 (기본값: 4.0) |
| `noise_augmentation` | FLOAT | 아니요 | 0.0 - 1.0 | 업스케일링 과정에서 추가할 노이즈의 양 (기본값: 0.0) |
...
| `latent` | LATENT | 업스케일된 차원에 맞는 빈 잠재 표현 |
