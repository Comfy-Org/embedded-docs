...
| `positive` | CONDITIONING | 예 | - | 원하는 콘텐츠를 향한 생성을 안내하는 양극 조건 데이터 |
| `negative` | CONDITIONING | 예 | - | 원하지 않는 콘텐츠에서 생성을 멀어지게 하는 부정 조건 데이터 |
| `scale_ratio` | FLOAT | 아니오 | 0.0 - 10.0 | 입력 이미지에 적용되는 스케일링 계수 (기본값: 4.0) |
| `noise_augmentation` | FLOAT | 아니오 | 0.0 - 1.0 | 업스케일링 과정에서 추가할 노이즈 양 (기본값: 0.0) |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `positive` | CONDITIONING | 업스케일링 정보가 적용된 수정된 양극 조건 |
| `negative` | CONDITIONING | 업스케일링 정보가 적용된 수정된 부정 조건 |
