...
| `긍정 조건` | CONDITIONING | 예 | - | 원하는 콘텐츠 생성을 안내하는 양성 컨디셔닝 데이터 |
| `부정 조건` | CONDITIONING | 예 | - | 원하지 않는 콘텐츠 생성을 억제하는 음성 컨디셔닝 데이터 |
| `확대율` | FLOAT | 아니요 | 0.0 - 10.0 | 입력 이미지에 적용되는 스케일링 비율 (기본값: 4.0) |
| `노이즈 증강` | FLOAT | 아니요 | 0.0 - 1.0 | 업스케일링 과정에서 추가할 노이즈의 양 (기본값: 0.0) |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `부정 조건` | CONDITIONING | 업스케일링 정보가 적용된 수정된 양성 컨디셔닝 |
| `잠재 이미지` | CONDITIONING | 업스케일링 정보가 적용된 수정된 음성 컨디셔닝 |
