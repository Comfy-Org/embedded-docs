...
| 参数 | 数据类型 | 必需 | 取值范围 | 描述 |
|------|-----------|------|----------|------|
| `images` | IMAGE | 是 | - | 需要进行超分辨率处理的输入图像 |
| `positive` | CONDITIONING | 是 | - | 正向条件数据，引导生成过程朝向期望内容 |
| `negative` | CONDITIONING | 是 | - | 负向条件数据，使生成过程远离不需要的内容 |
| `scale_ratio` | FLOAT | 否 | 0.0 - 10.0 | 应用于输入图像的缩放因子（默认值：4.0） |
| `noise_augmentation` | FLOAT | 否 | 0.0 - 1.0 | 超分辨率过程中添加的噪声量（默认值：0.0） |

## 输出结果

| 输出名称 | 数据类型 | 描述 |
|----------|-----------|------|
| `positive` | CONDITIONING | 应用了超分辨率信息后的修改正向条件数据 |
| `negative` | CONDITIONING | 应用了超分辨率信息后的修改负向条件数据 |
