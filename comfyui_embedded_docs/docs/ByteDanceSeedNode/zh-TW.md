...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `提示詞` | STRING | 是 | 不適用 | 輸入至模型的文字。 |
| `模型` | COMBO | 是 | `"Seed 2.0 Pro"`<br>`"Seed 2.0 Lite"`<br>`"Seed 2.0 Mini"` | 用於生成回應的 Seed 模型。 |
| `隨機種子` | INT | 是 | 0 至 2147483647 | Seed 控制節點是否應重新執行；無論 seed 為何，結果皆非確定性。（預設值：0） |
| `系統提示` | STRING | 否 | 不適用 | 決定模型行為的基礎指令。（預設值：""） |
...
| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `output` | STRING | 從 Seed 模型生成的文字回應。 |
