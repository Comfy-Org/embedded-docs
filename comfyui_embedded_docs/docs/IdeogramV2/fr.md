...
| `turbo` | BOOLEAN | Non | - | Indique s'il faut utiliser le mode turbo (génération plus rapide, qualité potentiellement inférieure) (par défaut : False) |
...
| `negative_prompt` | STRING | Non | - | Description de ce qui doit être exclu de l'image (par défaut : chaîne vide) |
...
|---------------|-----------------|-------------|
| `output` | IMAGE | La ou les images générées par le modèle Ideogram V2 |
