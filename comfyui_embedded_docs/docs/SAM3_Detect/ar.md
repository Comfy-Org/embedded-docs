...
| --- | --- | --- |
| `masks` | أقنعة التجزئة. عندما يكون `أقنعة فردية` خطأ (الافتراضي)، يعيد قناعًا واحدًا مدمجًا لكل إطار. عندما يكون صحيحًا، يعيد أقنعة فردية لكل كائن مكتشف | MASK |
| `bboxes` | المربعات المحيطة المكتشفة مع الإحداثيات ودرجات الثقة. يتضمن كل مربع قيم `x` و `y` و `width` و `height` و `score` | BOUNDING_BOX |
