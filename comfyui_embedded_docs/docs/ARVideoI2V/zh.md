...
| `width` | INT | 是 | 16 至 8192（步长：16） | 生成视频帧的宽度（默认值：832）。 |
| `height` | INT | 是 | 16 至 8192（步长：16） | 生成视频帧的高度（默认值：480）。 |
| `length` | INT | 是 | 1 至 1024（步长：4） | 生成视频的总帧数（默认值：81）。 |
| `batch_size` | INT | 是 | 1 至 64 | 单批次生成的视频序列数量（默认值：1）。 |
