...
| `max_stage` | INT | 否 | 1-3 | 取樣過程的最大階段數（預設值：3）。控制計算複雜度和品質。 |
| `eta` | FLOAT | 否 | 0.0-100.0 | 反向時間 SDE 的隨機強度（預設值：1.0）。當 eta=0 時，會簡化為確定性 ODE。此設定不適用於 ER-SDE 求解器類型。 |
| `s_noise` | FLOAT | 否 | 0.0-100.0 | 取樣過程的噪聲縮放因子（預設值：1.0）。控制取樣過程中應用的噪聲量。 |
...
| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `sampler` | SAMPLER | 一個已配置的取樣器物件，可用於取樣流程中，並帶有指定的求解器設定。 |
