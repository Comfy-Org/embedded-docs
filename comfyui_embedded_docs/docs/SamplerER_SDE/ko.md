...
| `최대 단계` | INT | 아니요 | 1-3 | 샘플링 과정의 최대 단계 수입니다(기본값: 3). 계산 복잡성과 품질을 제어합니다. |
| `ETA` | FLOAT | 아니요 | 0.0-100.0 | 역시간 SDE의 확률적 강도입니다(기본값: 1.0). eta=0일 경우 결정론적 ODE로 축소됩니다. 이 설정은 ER-SDE 솔버 유형에는 적용되지 않습니다. |
| `S 노이즈` | FLOAT | 아니요 | 0.0-100.0 | 샘플링 과정의 노이즈 스케일링 계수입니다(기본값: 1.0). 샘플링 중 적용되는 노이즈 양을 제어합니다. |
