...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `width` | INT | 是 | 64 至 MAX_RESOLUTION | 生成的潛在影像寬度（像素）（預設值：2048，步長：32） |
| `height` | INT | 是 | 64 至 MAX_RESOLUTION | 生成的潛在影像高度（像素）（預設值：2048，步長：32） |
| `batch_size` | INT | 是 | 1 至 4096 | 批次中生成的潛在樣本數量（預設值：1） |
