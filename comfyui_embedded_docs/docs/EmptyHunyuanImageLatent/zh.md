...
|-----------|-----------|----------|-------|-------------|
| `width` | INT | 是 | 64 至 MAX_RESOLUTION | 生成的潜空间图像宽度（像素），默认值：2048，步长：32 |
| `height` | INT | 是 | 64 至 MAX_RESOLUTION | 生成的潜空间图像高度（像素），默认值：2048，步长：32 |
| `batch_size` | INT | 是 | 1 至 4096 | 每批次生成的潜空间样本数量，默认值：1 |
