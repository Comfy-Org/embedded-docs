...
| `图层` | 要将画面分成多少层。始终会为完整画面预留一张额外的纸，因此你得到的是 `layers + 1` 张图像，而不是 `layers` 张。设为 2，你会得到完整画面加上 2 个图层。设为 0，你只会得到完整画面。（默认值：3） | INT | 是 | 0 至 MAX_RESOLUTION |
...
- **保持纸张顺序。** 这叠纸是记录哪一层在哪个之上唯一的依据。纸张本身没有标注它们的位置，因此重新排序或丢弃图像就意味着重新排序或丢失图层。
