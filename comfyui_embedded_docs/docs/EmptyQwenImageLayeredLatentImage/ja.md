...
| `レイヤー` | 画像をいくつのレイヤーに分けるかを指定します。全体画像用に紙が常に1枚余分に確保されるため、返ってくるのは `layers` 枚ではなく `layers + 1` 枚の画像です。2に設定すると、全体画像と2つのレイヤーが得られます。0に設定すると、全体画像のみが得られます。（デフォルト：3） | INT | はい | 0～MAX_RESOLUTION |
