...
| `레이어` | 그림을 몇 개의 레이어로 나눌지 지정합니다. 전체 그림을 위한 종이가 항상 한 장 더 확보되므로, 반환되는 이미지는 `layers`장이 아니라 `layers + 1`장입니다. 2로 설정하면 전체 그림과 레이어 2개가 생성됩니다. 0으로 설정하면 전체 그림만 생성됩니다. (기본값: 3) | INT | 예 | 0 ~ MAX_RESOLUTION |
...
- **종이의 순서를 유지하세요.** 종이 더미는 어떤 레이어가 어떤 위에 있는지를 기록하는 유일한 수단입니다. 종이 자체에는 위치를 알려 주는 정보가 없으므로 이미지를 재정렬하거나 삭제하면 레이어가 재정렬되거나 손실됩니다.
