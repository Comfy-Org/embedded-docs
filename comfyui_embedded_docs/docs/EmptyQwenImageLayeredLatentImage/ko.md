...
|-----------|-----------|----------|-------|-------------|
| `너비` | INT | 예 | 16 ~ MAX_RESOLUTION | 생성할 잠재 이미지의 너비입니다. 값은 16으로 나누어 떨어져야 합니다. (기본값: 640) |
| `높이` | INT | 예 | 16 ~ MAX_RESOLUTION | 생성할 잠재 이미지의 높이입니다. 값은 16으로 나누어 떨어져야 합니다. (기본값: 640) |
| `레이어` | INT | 예 | 0 ~ MAX_RESOLUTION | 잠재 구조에 추가할 추가 계층 수입니다. 이는 잠재 표현의 깊이를 정의합니다. (기본값: 3) |
| `배치 크기` | INT | 아니요 | 1 ~ 4096 | 한 배치에서 생성할 잠재 샘플 수입니다. (기본값: 1) |
