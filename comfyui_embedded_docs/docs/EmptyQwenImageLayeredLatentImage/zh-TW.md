...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `width` | INT | 是 | 16 至 MAX_RESOLUTION | 要建立的潛在影像寬度。此值必須能被 16 整除。（預設值：640） |
| `height` | INT | 是 | 16 至 MAX_RESOLUTION | 要建立的潛在影像高度。此值必須能被 16 整除。（預設值：640） |
| `layers` | INT | 是 | 0 至 MAX_RESOLUTION | 要添加到潛在結構中的額外層數。這定義了潛在表示的深度。（預設值：3） |
...
| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `samples` | LATENT | 一個填充為零的潛在張量。其形狀為 `[batch_size, 16, layers + 1, height // 8, width // 8]`。 |
