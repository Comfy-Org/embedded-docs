...
`EmptyHunyuanLatentVideo` 節點與 `EmptyLatentImage` 節點類似。您可以將其視為影片生成的空白畫布，其中寬度、高度和長度定義了畫布的屬性，而批次大小則決定了要創建的畫布數量。此節點會創建空的畫布，為後續的影片生成任務做好準備。
...
| 參數名稱 | Comfy 類型 | 描述                                                                               |
| --------- | ---------- | ----------------------------------------------------------------------------------------- |
| `samples` | `LATENT`   | 生成的潛在影片樣本，包含零張量，已準備好進行處理和生成任務。 |