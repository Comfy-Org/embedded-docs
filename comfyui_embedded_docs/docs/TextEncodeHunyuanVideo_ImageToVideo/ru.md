...
| `выход clip_vision` | CLIP_VISION_OUTPUT | Да | - | Визуальные эмбеддинги из модели CLIP vision, обеспечивающие контекст изображения |
| `подсказка` | STRING | Да | - | Текстовое описание для управления генерацией видео, поддерживает многострочный ввод и динамические подсказки |
| `перемежение изображения` | INT | Да | 1-512 | Определяет степень влияния изображения по сравнению с текстовой подсказкой. Чем выше значение, тем больше влияние текстовой подсказки. (по умолчанию: 2) |
