...
| `프롬프트` | STRING | 예 | - | 비디오 생성을 안내하는 텍스트 설명으로, 여러 줄 입력 및 동적 프롬프트를 지원합니다 |
| `이미지 인터리브` | INT | 예 | 1-512 | 텍스트 프롬프트 대비 이미지가 결과에 미치는 영향도를 조절합니다. 값이 높을수록 텍스트 프롬프트의 영향력이 커집니다. (기본값: 2) |
