...
|-----------|-----------|----------|-------|-------------|
| `steps` | INT | 예 | 1 ~ 10000 | 스케줄의 샘플링 단계 수입니다 (기본값: 20) |
| `sigma_max` | FLOAT | 예 | 0.0 ~ 5000.0 | 스케줄 시작 시 최대 시그마 값입니다 (기본값: 14.614642) |
| `sigma_min` | FLOAT | 예 | 0.0 ~ 5000.0 | 스케줄 종료 시 최소 시그마 값입니다 (기본값: 0.0291675) |
| `mu` | FLOAT | 예 | -10.0 ~ 10.0 | 라플라스 분포의 평균 매개변수입니다 (기본값: 0.0) |
| `beta` | FLOAT | 예 | 0.0 ~ 10.0 | 라플라스 분포의 스케일 매개변수입니다 (기본값: 0.5) |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `SIGMAS` | SIGMAS | 라플라스 분포 스케줄을 따르는 시그마 값 시퀀스입니다 |
