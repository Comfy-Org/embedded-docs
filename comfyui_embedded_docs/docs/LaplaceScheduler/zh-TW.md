...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `steps` | INT | 是 | 1 到 10000 | 排程中的取樣步數（預設值：20） |
| `sigma_max` | FLOAT | 是 | 0.0 到 5000.0 | 排程開始時的最大 sigma 值（預設值：14.614642） |
| `sigma_min` | FLOAT | 是 | 0.0 到 5000.0 | 排程結束時的最小 sigma 值（預設值：0.0291675） |
| `mu` | FLOAT | 是 | -10.0 到 10.0 | 拉普拉斯分佈的平均數參數（預設值：0.0） |
| `beta` | FLOAT | 是 | 0.0 到 10.0 | 拉普拉斯分佈的尺度參數（預設值：0.5） |
