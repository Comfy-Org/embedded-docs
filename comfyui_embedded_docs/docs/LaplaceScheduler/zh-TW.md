...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `步驟數` | INT | 是 | 1 到 10000 | 排程中的取樣步數（預設值：20） |
| `最大 sigma` | FLOAT | 是 | 0.0 到 5000.0 | 排程開始時的最大 sigma 值（預設值：14.614642） |
| `最小 sigma` | FLOAT | 是 | 0.0 到 5000.0 | 排程結束時的最小 sigma 值（預設值：0.0291675） |
