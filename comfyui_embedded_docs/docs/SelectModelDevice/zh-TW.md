...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `model` | MODEL | 是 | | 要放置在特定裝置上的擴散模型。 |
| `device` | COMBO | 是 | `"default"`<br>`"cpu"`<br>`"gpu:0"`<br>`"gpu:1"`<br>`"gpu:2"`<br>`"gpu:3"`<br>`"gpu:4"`<br>`"gpu:5"`<br>`"gpu:6"`<br>`"gpu:7"` | 模型的目標裝置。選項會根據可用的 GPU 動態生成。（預設值："default"） |
...
| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `model` | MODEL | 擴散模型，現在已放置在所選裝置上。如果裝置無效或不可用，模型將保持不變直接傳遞。 |
