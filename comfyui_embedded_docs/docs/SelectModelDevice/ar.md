...
|-----------|-----------|----------|-------|-------------|
| `model` | MODEL | نعم | | نموذج الانتشار الذي سيتم وضعه على جهاز محدد. |
| `device` | COMBO | نعم | `"default"`<br>`"cpu"`<br>`"gpu:0"`<br>`"gpu:1"`<br>`"gpu:2"`<br>`"gpu:3"`<br>`"gpu:4"`<br>`"gpu:5"`<br>`"gpu:6"`<br>`"gpu:7"` | الجهاز المستهدف للنموذج. يتم إنشاء الخيارات ديناميكيًا بناءً على وحدات معالجة الرسوم المتاحة. (الافتراضي: "default") |
...
|-------------|-----------|-------------|
| `model` | MODEL | نموذج الانتشار، وقد تم وضعه الآن على الجهاز المحدد. إذا كان الجهاز غير صالح أو غير متاح، يتم تمرير النموذج كما هو دون تغيير. |
