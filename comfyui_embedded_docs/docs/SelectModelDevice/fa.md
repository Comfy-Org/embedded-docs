...
|---------|----------|--------|--------|----------|
| `model` | MODEL | بله | | مدل انتشار که باید روی یک دستگاه خاص قرار گیرد. |
| `device` | COMBO | بله | `"default"`<br>`"cpu"`<br>`"gpu:0"`<br>`"gpu:1"`<br>`"gpu:2"`<br>`"gpu:3"`<br>`"gpu:4"`<br>`"gpu:5"`<br>`"gpu:6"`<br>`"gpu:7"` | دستگاه هدف برای مدل. گزینه‌ها بر اساس GPUهای موجود به‌صورت پویا تولید می‌شوند. (پیش‌فرض: "default") |
...
|-----------|----------|----------|
| `model` | MODEL | مدل انتشار که اکنون روی دستگاه انتخاب شده قرار گرفته است. اگر دستگاه نامعتبر یا در دسترس نبود، مدل بدون تغییر عبور داده می‌شود. |
