...
Este nó gera um vídeo a partir de uma imagem de primeiro quadro obrigatória e uma imagem de último quadro opcional usando modelos ByteDance Seedance. Você descreve o vídeo com um prompt de texto; o primeiro quadro orienta o início do vídeo e o último quadro orienta o fim. Ele oferece suporte ao Seedance 2.5 e à família Seedance 2.0 (Seedance 2.0, Seedance 2.0 Fast e Seedance 2.0 Mini). Selecionar o modelo `Seedance 2.5 Draft` renderiza uma prévia rápida em 480p; conecte o `draft_task_id` resultante ao nó ByteDance Seedance 2.5 Draft to Final Video para renderizar o final em 1080p.
...
| `first_frame_asset_id` | asset_id do Seedance a ser usado como primeiro quadro. Mutuamente exclusivo com a entrada de imagem `first_frame`. O padrão é uma string vazia. | STRING | Não | - |
| `last_frame_asset_id` | asset_id do Seedance a ser usado como último quadro. Mutuamente exclusivo com a entrada de imagem `last_frame`. O padrão é uma string vazia. | STRING | Não | - |
...
| `marca_d'água` | Indica se deve adicionar uma marca d'água ao vídeo. O padrão é False. | BOOLEAN | Sim | False<br>True |

### Entradas do Seedance 2.5

Estas entradas aparecem quando `Seedance 2.5` está selecionado.
...
| `generate_audio` | Ativa a geração de áudio para o vídeo de saída. O padrão é True. | BOOLEAN | Sim | False<br>True |
| `output_format` | Formato de contêiner do vídeo de saída. O padrão é mp4. | COMBO | Sim | `"mp4"` |

### Entradas do Seedance 2.5 Draft

Estas entradas aparecem quando `Seedance 2.5 Draft` está selecionado. O conjunto de parâmetros corresponde ao Seedance 2.5 acima, exceto que `resolution` oferece apenas `"480p"` (padrão `"480p"`).

### Entradas do Seedance 2.0

Estas entradas aparecem quando `Seedance 2.0` está selecionado.
...
- Você deve fornecer o primeiro quadro como uma imagem `first_frame` ou como um `first_frame_asset_id`. Fornecer ambos gera um erro; não fornecer nenhum também gera um erro.
- As entradas `last_frame` e `last_frame_asset_id` são opcionais, mas você não pode fornecer ambas para o mesmo quadro.
- Os IDs de asset devem referenciar assets Seedance Image existentes e ativos.
- A entrada `prompt` é obrigatória e não pode estar vazia.
- A saída `draft_task_id` só é produzida por `Seedance 2.5 Draft`; com qualquer outro modelo, ela deve ser deixada desconectada, caso contrário a execução falha.
...
- Com modelos da família Seedance 2.0 e imagens de quadro locais, as imagens são recortadas no centro e redimensionadas para a resolução e proporção de saída de destino antes da geração. Quando `ratio` é `adaptive`, a proporção suportada mais próxima da imagem de entrada é usada.
- Imagens de quadro locais são validadas quanto à proporção de aspecto e às dimensões suportadas; imagens em tamanho excessivo são reduzidas.
...
| `draft_task_id` | ID da tarefa da execução de draft. Apenas o modelo Seedance 2.5 Draft o produz; conecte-o ao nó ByteDance Seedance 2.5 Draft to Final Video para renderizar o final em 1080p. | STRING |
