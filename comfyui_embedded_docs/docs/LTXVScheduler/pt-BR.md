...
| `max_shift` | FLOAT | Sim | 0.0-100.0 | Valor máximo de deslocamento para cálculo do sigma (padrão: 2.05) |
| `base_shift` | FLOAT | Sim | 0.0-100.0 | Valor base de deslocamento para cálculo do sigma (padrão: 0.95) |
| `stretch` | BOOLEAN | Sim | Verdadeiro/Falso | Estica os sigmas para ficarem no intervalo [terminal, 1] (padrão: Verdadeiro) |
| `terminal` | FLOAT | Sim | 0.0-0.99 | O valor terminal dos sigmas após o esticamento (padrão: 0.1) |
| `latent` | LATENT | Não | - | Entrada latent opcional usada para calcular a contagem de tokens para ajuste do sigma |
