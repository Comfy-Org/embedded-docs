...
| 参数 | 数据类型 | 是否必填 | 范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `steps` | INT | 是 | 1-10000 | 采样步数（默认值：20） |
| `max_shift` | FLOAT | 是 | 0.0-100.0 | sigma 计算的最大偏移值（默认值：2.05） |
| `base_shift` | FLOAT | 是 | 0.0-100.0 | sigma 计算的基础偏移值（默认值：0.95） |
| `stretch` | BOOLEAN | 是 | True/False | 将 sigma 值拉伸至 [terminal, 1] 范围内（默认值：True） |
| `terminal` | FLOAT | 是 | 0.0-0.99 | 拉伸后 sigma 值的终止值（默认值：0.1） |
