...
| 参数 | 数据类型 | 是否必填 | 范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `image` | WEBCAM | 是 | - | 用于捕获图像的摄像头输入源 |
| `width` | INT | 是 | 0 至 MAX_RESOLUTION | 捕获图像的期望宽度（默认值：0，使用摄像头原始分辨率） |
| `height` | INT | 是 | 0 至 MAX_RESOLUTION | 捕获图像的期望高度（默认值：0，使用摄像头原始分辨率） |
| `capture_on_queue` | BOOLEAN | 是 | - | 启用后，每次处理工作流队列时都会捕获新图像（默认值：True） |
