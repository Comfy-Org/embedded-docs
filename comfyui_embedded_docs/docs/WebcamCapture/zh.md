...
| 参数名 | 数据类型 | 必填 | 取值范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `image` | WEBCAM | 是 | - | 用于捕获图像的网络摄像头输入源 |
| `width` | INT | 否 | 0 至 MAX_RESOLUTION | 捕获图像的期望宽度（默认：0，使用网络摄像头原生分辨率） |
| `height` | INT | 否 | 0 至 MAX_RESOLUTION | 捕获图像的期望高度（默认：0，使用网络摄像头原生分辨率） |
| `capture_on_queue` | BOOLEAN | 否 | - | 启用后，每次处理工作流队列时都会捕获新图像（默认：True） |
