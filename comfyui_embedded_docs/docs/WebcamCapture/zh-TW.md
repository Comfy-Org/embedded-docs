...
| `寬度` | INT | 是 | 0 至 MAX_RESOLUTION | 擷取影像的所需寬度（預設值：0，使用網路攝影機的原始解析度） |
| `高度` | INT | 是 | 0 至 MAX_RESOLUTION | 擷取影像的所需高度（預設值：0，使用網路攝影機的原始解析度） |
| `排隊時擷取` | BOOLEAN | 是 | - | 啟用後，每次處理工作流程佇列時都會擷取新影像（預設值：True） |
