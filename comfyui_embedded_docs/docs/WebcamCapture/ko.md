...
| `width` | INT | 예 | 0 ~ MAX_RESOLUTION | 캡처된 이미지의 원하는 너비 (기본값: 0, 웹캠의 기본 해상도 사용) |
| `height` | INT | 예 | 0 ~ MAX_RESOLUTION | 캡처된 이미지의 원하는 높이 (기본값: 0, 웹캠의 기본 해상도 사용) |
| `capture_on_queue` | BOOLEAN | 예 | - | 활성화되면 워크플로 큐가 처리될 때마다 새 이미지를 캡처합니다 (기본값: True) |
