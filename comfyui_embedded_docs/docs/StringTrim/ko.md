...
|-----------|-----------|----------|-------|-------------|
| `문자열` | STRING | 예 | - | 처리할 텍스트 문자열입니다. 여러 줄 입력을 지원합니다. |
| `모드` | COMBO | 예 | "Both"<br>"Left"<br>"Right" | 문자열의 어느 쪽을 트리밍할지 지정합니다. "Both"는 양쪽 끝에서 공백을 제거하고, "Left"는 시작 부분에서만, "Right"는 끝 부분에서만 제거합니다. |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `output` | STRING | 선택한 모드에 따라 공백이 제거된 트리밍된 텍스트 문자열입니다. |
