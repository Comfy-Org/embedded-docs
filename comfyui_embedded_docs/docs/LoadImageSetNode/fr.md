...
|-----------|-----------------|--------|-------|-------------|
| `images` | IMAGE | Oui | Plusieurs fichiers image | Sélectionnez plusieurs images dans le répertoire d'entrée. Prend en charge les formats PNG, JPG, JPEG, WEBP, BMP, GIF, JPE, APNG, TIF et TIFF. Permet la sélection par lots d'images. |
| `resize_method` | STRING | Non | "None"<br>"Stretch"<br>"Crop"<br>"Pad" | Méthode facultative pour redimensionner les images chargées (par défaut : "None"). Choisissez "None" pour conserver les tailles d'origine, "Stretch" pour forcer le redimensionnement, "Crop" pour conserver le rapport hauteur/largeur en recadrant, ou "Pad" pour conserver le rapport hauteur/largeur en ajoutant des marges. |
