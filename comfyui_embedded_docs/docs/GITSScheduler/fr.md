...
| `étapes` | Nombre total d’étapes d’échantillonnage pour lesquelles générer des sigmas (par défaut : 10). | INT | Oui | 2 - 1000 |
| `débruitage` | Facteur de débruitage qui réduit le nombre d’étapes utilisées (par défaut : 1.0). | FLOAT | Oui | 0.0 - 1.0 (pas 0.01) |
