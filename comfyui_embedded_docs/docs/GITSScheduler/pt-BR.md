...
|-----------|-------------|-----------|----------|-------|
| `coeff` | Parâmetro avançado. O valor do coeficiente que controla a curva do cronograma de ruído (padrão: 1.20). O valor é arredondado para duas casas decimais e seleciona qual tabela de níveis de ruído predefinida é utilizada. | FLOAT | Sim | 0.80 - 1.50 (passo 0.05) |
| `steps` | O número total de etapas de amostragem para as quais gerar sigmas (padrão: 10). | INT | Sim | 2 - 1000 |
| `denoise` | Fator de redução de ruído que diminui o número de etapas utilizadas (padrão: 1.0). | FLOAT | Sim | 0.0 - 1.0 (passo 0.01) |
