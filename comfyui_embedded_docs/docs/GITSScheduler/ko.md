...
|-----------|-----------|----------|-------|-------------|
| `coeff` | FLOAT | 예 | 0.80 - 1.50 | 노이즈 스케줄 곡선을 제어하는 계수 값입니다 (기본값: 1.20) |
| `steps` | INT | 예 | 2 - 1000 | 시그마를 생성할 총 샘플링 단계 수입니다 (기본값: 10) |
| `denoise` | FLOAT | 예 | 0.0 - 1.0 | 사용되는 단계 수를 줄이는 디노이징 요소입니다 (기본값: 1.0) |
...
| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `sigmas` | SIGMAS | 노이즈 스케줄에 대해 생성된 시그마 값입니다 |
