...
儲存的檔案使用 `filename_prefix` 並附加 5 位計數器寫入輸出目錄（例如 `3d/ComfyUI_00001.ply`）。當未連接選用的 `camera_info` 輸入時，相機資訊取自 `viewport_state`；同樣，當未連接 `model_3d_info` 時，模型資訊取自 `viewport_state`（若無可用資訊則為空列表）。
