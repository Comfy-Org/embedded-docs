...
保存的文件使用 `filename_prefix` 并附加 5 位计数器写入输出目录（例如 `3d/ComfyUI_00001.ply`）。当未连接可选的 `camera_info` 输入时，相机信息取自 `viewport_state`；同样，当未连接 `model_3d_info` 时，模型信息取自 `viewport_state`（若无可用信息则为空列表）。
