...
| `negative_prompt` | O prompt negativo para evitar. Padrão: string vazia (usa um espaço internamente quando em branco). | STRING | Não | multiline, dynamic prompts enabled |
...
| `images` | Uma ou mais imagens de referência a serem editadas. Todas as imagens são redimensionadas para a resolução de saída antes da codificação. | IMAGE (autogrow) | Não | Até 16 imagens (nomeadas `image_1`…`image_16`), no mínimo 0 |
