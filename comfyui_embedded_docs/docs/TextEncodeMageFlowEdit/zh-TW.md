...
| `negative_prompt` | 用於引導避免的負向提示詞。預設值：空字串（若為空白則內部使用一個空格）。 | STRING | 否 | 多行，啟用動態提示詞 |
| `vae` | 用於將參考影像編碼為潛在空間的 VAE 模型。若未提供，則 conditioning 輸出中不會加入參考潛在變數。 | VAE | 否 | |
| `images` | 要編輯的一張或多張參考影像。所有影像在編碼前會調整為輸出解析度。 | IMAGE (自動增長) | 否 | 最多 16 張影像（命名為 `image_1`…`image_16`），至少 0 張 |
