...
Le nœud Empty Ace Step 1.5 Latent Audio crée un tenseur latent vide conçu pour le traitement audio. Il génère un latent audio silencieux d'une durée et d'une taille de lot spécifiées, qui peut être utilisé comme point de départ pour les flux de travail de génération audio dans ComfyUI. Le nœud calcule la longueur du latent en fonction des secondes d'entrée et d'un taux d'échantillonnage fixe.
