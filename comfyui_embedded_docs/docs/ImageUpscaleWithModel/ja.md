...
|---|---|---|
| `upscale_model` | `UPSCALE_MODEL` | 画像のアップスケールに使用するアップスケールモデルです。アップスケールアルゴリズムとそのパラメータを定義するために重要です。 |
| `image` | `IMAGE` | アップスケールする画像です。この入力は、アップスケール処理の対象となるソースコンテンツを決定するために不可欠です。 |

## 出力

| パラメータ | データ型 | 説明 |
|---|---|---|
| `image` | `IMAGE` | アップスケールモデルによって処理された、アップスケール後の画像です。この出力はアップスケール操作の結果であり、解像度や品質が向上した状態を示します。 |