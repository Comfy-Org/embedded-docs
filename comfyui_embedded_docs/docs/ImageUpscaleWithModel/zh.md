...
| 参数 | Comfy 数据类型 | 描述 |
|---|---|---|
| `upscale_model` | `UPSCALE_MODEL` | 用于放大图像的放大模型。该参数对于定义放大算法及其参数至关重要。 |
| `image` | `IMAGE` | 待放大的图像。此输入对于确定将经历放大过程的源内容至关重要。 |

## 输出

| 参数 | 数据类型 | 描述 |
|---|---|---|
| `image` | `IMAGE` | 经放大模型处理后的放大图像。此输出是放大操作的结果，展示了增强后的分辨率或质量。 |