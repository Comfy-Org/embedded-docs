...
RenormCFG 노드는 조건부 스케일링과 정규화를 적용하여 확산 모델의 classifier-free guidance(CFG) 프로세스를 수정합니다. 지정된 타임스텝 임계값과 재정규화 계수를 기반으로 노이즈 제거 과정을 조정하여 이미지 생성 중 조건부 예측과 무조건부 예측의 영향을 제어합니다.

## 입력

| 매개변수 | 설명 | 데이터 타입 | 필수 | 범위 |
| --- | --- | --- | --- | --- |
| `model` | 재정규화된 CFG를 적용할 확산 모델 | MODEL | 예 | - |
...
| `renorm_cfg` | 원래 조건부 예측 대비 CFG 스케일링된 예측의 최대 노름(norm)을 제한하는 재정규화 계수입니다. 값이 0.0이면 재정규화가 비활성화됩니다(기본값: 1.0) | FLOAT | 아니요 | 0.0 - 100.0 |
