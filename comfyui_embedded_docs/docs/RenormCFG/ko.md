...
| `cfg_trunc` | FLOAT | 아니요 | 0.0 - 100.0 | CFG 스케일링을 적용하기 위한 타임스텝 임계값입니다. 현재 타임스텝이 이 값보다 낮으면 CFG 스케일링이 적용되고, 그렇지 않으면 조건부 예측만 사용됩니다(기본값: 100.0) |
| `renorm_cfg` | FLOAT | 아니요 | 0.0 - 100.0 | 원래 조건부 예측 대비 CFG 스케일링된 예측의 최대 노름을 제한하는 재정규화 계수입니다. 값이 0.0이면 재정규화가 비활성화됩니다(기본값: 1.0) |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `model` | MODEL | 재정규화된 CFG 함수가 적용된 수정된 모델 |
