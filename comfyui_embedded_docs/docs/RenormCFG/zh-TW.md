...
RenormCFG 節點透過應用條件縮放與正規化，來修改擴散模型中的分類器自由引導（CFG）過程。它根據指定的時間步長閾值和重新正規化因子來調整去噪過程，以控制在圖像生成過程中條件預測與無條件預測的影響力。
...
| `cfg_trunc` | FLOAT | 否 | 0.0 - 100.0 | 應用 CFG 縮放的時間步長閾值（預設值：100.0） |
