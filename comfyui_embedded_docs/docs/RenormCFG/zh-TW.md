...
| `cfg_trunc` | FLOAT | 否 | 0.0 - 100.0 | 應用 CFG 縮放的時間步長閾值。當目前時間步長低於此值時，會應用 CFG 縮放；否則僅使用條件預測（預設值：100.0） |
| `renorm_cfg` | FLOAT | 否 | 0.0 - 100.0 | 重新正規化因子，限制 CFG 縮放預測相對於原始條件預測的最大範數。值為 0.0 時停用重新正規化（預設值：1.0） |
