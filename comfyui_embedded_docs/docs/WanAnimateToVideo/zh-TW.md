...
| 輸出名稱 | 說明 | 資料類型 |
| --- | --- | --- |
| `正面提示` | 修改後的正向條件，包含額外的影片上下文，包括 CLIP 視覺輸出、姿勢影片潛在空間、面部影片像素、串聯的潛在圖像和串聯的遮罩 | CONDITIONING |
| `負面提示` | 修改後的負向條件，包含額外的影片上下文，包括 CLIP 視覺輸出、姿勢影片潛在空間、面部影片像素（反轉）、串聯的潛在圖像和串聯的遮罩 | CONDITIONING |
| `潛在空間` | 以潛在空間格式生成的影片內容，形狀為 [batch_size, 16, latent_length + trim_latent, latent_height, latent_width] | LATENT |
| `修剪潛在空間` | 潛在空間修剪資訊，指示從開頭修剪的潛在幀數（對應於參考圖像潛在幀） | INT |
| `修剪圖像` | 參考運動幀的圖像空間修剪資訊，指示從開頭修剪的圖像幀數 | INT |
