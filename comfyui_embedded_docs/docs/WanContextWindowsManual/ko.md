...
|-----------|-----------|----------|-------|-------------|
| `모델` | MODEL | 예 | - | 샘플링 중 컨텍스트 윈도우를 적용할 모델입니다. |
| `컨텍스트 길이` | INT | 예 | 1 ~ 1048576 | 컨텍스트 윈도우의 길이입니다(기본값: 81). |
| `컨텍스트 오버랩` | INT | 예 | 0 ~ 1048576 | 컨텍스트 윈도우의 중첩입니다(기본값: 30). |
| `컨텍스트 스케줄` | COMBO | 예 | `"static_standard"`<br>`"uniform_standard"`<br>`"uniform_looped"`<br>`"batched"` | 컨텍스트 윈도우의 보폭입니다. |
| `컨텍스트 스트라이드` | INT | 예 | 1 ~ 1048576 | 컨텍스트 윈도우의 보폭입니다. 균일 스케줄에만 적용됩니다(기본값: 1). |
| `폐쇄 루프` | BOOLEAN | 예 | - | 컨텍스트 윈도우 루프를 닫을지 여부입니다. 반복 스케줄에만 적용됩니다(기본값: False). |
| `퓨즈 방법` | COMBO | 예 | `"pyramid"`<br>`"gaussian"`<br>`"average"`<br>`"overlap"` | 컨텍스트 윈도우를 융합하는 데 사용할 방법입니다(기본값: "pyramid"). |
...
| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `모델` | MODEL | 컨텍스트 윈도우 구성이 적용된 모델입니다. |
