...
| `fuse_method` | COMBO | はい | `"pyramid"`<br>`"gaussian"`<br>`"average"`<br>`"overlap"` | コンテキストウィンドウをフュージョンする方法（デフォルト: "pyramid"）。 |
| `freenoise` | BOOLEAN | はい | - | FreeNoiseノイズシャッフリングを適用するかどうか。ウィンドウのブレンドを改善します（デフォルト: False）。 |
...
| 出力名 | データ型 | 説明 |
|-------------|-----------|-------------|
| `model` | MODEL | コンテキストウィンドウ設定が適用されたモデル。 |
