...
|---------|----------|--------|--------|---------|
| `clip` | CLIP | بله | - | مدل CLIP مورد استفاده برای توکن‌سازی و رمزگذاری متن |
| `conditioning` | CONDITIONING | بله | - | داده‌های شرطی موجود که با پارامترهای کنترل‌نت بهبود می‌یابند |
| `text` | STRING | بله | - | ورودی متنی که توسط مدل CLIP پردازش می‌شود. از متن چندخطی و پرامپت‌های پویا پشتیبانی می‌کند |
...
|-----------|----------|---------|
| `CONDITIONING` | CONDITIONING | داده‌های شرطی بهبودیافته با پارامترهای cross-attention کنترل‌نت اضافه‌شده (`cross_attn_controlnet` و `pooled_output_controlnet`) که از رمزگذاری متن CLIP استخراج شده‌اند |
