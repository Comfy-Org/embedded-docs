...
| パラメータ | データ型 | 必須 | 範囲 | 説明 |
|-----------|-----------|----------|-------|-------------|
| `clip` | CLIP | はい | - | テキストのトークン化とエンコードに使用される CLIP モデル |
| `conditioning` | CONDITIONING | はい | - | ControlNet パラメータで拡張される既存のコンディショニングデータ |
| `text` | STRING | はい | - | CLIP モデルで処理されるテキスト入力。複数行テキストと動的プロンプトに対応 |
