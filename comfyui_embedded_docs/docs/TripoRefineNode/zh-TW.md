...
TripoRefineNode 專門用於精煉由 v1.4 Tripo 模型創建的 3D 草稿模型。它接收一個模型任務 ID，並透過 Tripo API 進行處理，以生成模型的改進版本。此節點專為處理 Tripo v1.4 模型產生的草稿模型而設計。
...
| `unique_id` | UNIQUE_ID | 否 | - | 用於此操作的唯一識別碼 |
