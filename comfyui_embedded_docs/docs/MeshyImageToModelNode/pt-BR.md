...
| `refazer_malha` | Determina se a malha gerada deve ser processada. Quando definido como `"false"`, o nó retorna uma malha triangular não processada. | DYNAMIC_COMBO | Sim | `"true"`<br>`"false"` |
...
| `gerar_textura` | Determina se as texturas são geradas para o modelo. Definir como `"false"` pula a fase de texturização e retorna uma malha sem texturas. | DYNAMIC_COMBO | Sim | `"true"`<br>`"false"` |
| `enable_pbr` | Quando `gerar_textura` é `"true"`, esta opção gera mapas PBR (metálico, rugosidade, normal) além da cor base. O valor padrão é `False`. | BOOLEAN | Não* | - |
