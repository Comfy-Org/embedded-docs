...
| 参数 | 描述 | 数据类型 | 必需 | 范围 |
| --- | --- | --- | --- | --- |
| `model` | 指定用于生成的 AI 模型版本。 | COMBO | 是 | `"meshy-7.1"`<br>`"meshy-7"`<br>`"meshy-6"`<br>`"latest"` |
...
| `ultra_resolution` | 超精细处理的分辨率：`"2k"` 以 2048³ 运行，`"4k"` 以 4096³ 运行，以获得最精细的表面细节。`"4k"` 需要 `"meshy-7.1"` 或 `"latest"` 模型。仅在启用 `ultra_mode` 时使用（默认：`"2k"`）。 | COMBO | 否 | `"2k"`<br>`"4k"` |
...
* 当启用 `ultra_mode` 时，`model` 参数必须设置为 `"meshy-7.1"`、`"meshy-7"` 或 `"latest"`；选择任何其他模型都会引发错误。使用 `"meshy-7"` 时，超精细处理始终以 2048³ 运行，而 `"4k"` 的超精细分辨率需要 `"meshy-7.1"` 或 `"latest"`。
