...
| `model` | COMBO | 是 | 多个可用选项 | 从可用的 Luma 模型中选择视频生成模型 |
| `resolution` | COMBO | 是 | 多个可用选项 | 生成视频的输出分辨率（默认：540p） |
| `duration` | COMBO | 是 | 多个可用选项 | 生成视频的时长 |
| `loop` | BOOLEAN | 是 | - | 生成的视频是否应循环播放（默认：False） |
| `seed` | INT | 是 | 0 到 18446744073709551615 | 用于确定节点是否应重新运行的种子值；无论种子值如何，实际结果都是非确定性的（默认：0） |
| `first_image` | IMAGE | 否 | - | 生成视频的首帧图像（可选） |
| `last_image` | IMAGE | 否 | - | 生成视频的末帧图像（可选） |
