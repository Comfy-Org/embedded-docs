...
| `model` | COMBO | 是 | 提供多種選項 | 從可用的 Luma 模型中選擇影片生成模型 |
| `resolution` | COMBO | 是 | `"540p"`<br>`"720p"`<br>`"1080p"`<br>`"4k"` | 生成影片的輸出解析度（預設值："540p"）。使用 `ray-1-6` 模型時此參數會被忽略。 |
| `duration` | COMBO | 是 | `"5s"`<br>`"9s"` | 生成影片的持續時間。使用 `ray-1-6` 模型時此參數會被忽略。 |
| `loop` | BOOLEAN | 是 | - | 生成的影片是否應循環播放（預設值：False） |
...
| `luma_concepts` | CUSTOM | 否 | - | 可選的攝影機概念，透過 Luma Concepts 節點指定攝影機運動。（可選） |
