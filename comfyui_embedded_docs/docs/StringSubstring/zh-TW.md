...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `字串` | STRING | 是 | - | 要從中擷取文字的輸入字串。支援多行文字。 |
| `開始` | INT | 是 | - | 子字串的起始位置索引。第一個字元的索引為 0。 |
| `結束` | INT | 是 | - | 子字串的結束位置索引。此索引處的字元不會包含在結果中。 |

## 輸出

| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `output` | STRING | 從輸入文字中擷取的子字串，包含從 `開始` 位置開始到（但不包含）`結束` 位置之間的所有字元。 |
