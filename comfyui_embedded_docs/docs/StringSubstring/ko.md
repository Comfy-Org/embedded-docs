...
|-----------|-----------|----------|-------|-------------|
| `문자열` | STRING | 예 | - | 추출할 입력 텍스트 문자열입니다. 여러 줄 텍스트를 지원합니다. |
| `시작` | INT | 예 | - | 부분 문자열의 시작 위치 인덱스입니다. 첫 번째 문자는 인덱스 0에 있습니다. |
| `끝` | INT | 예 | - | 부분 문자열의 끝 위치 인덱스입니다. 이 인덱스에 있는 문자는 결과에 포함되지 않습니다. |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `output` | STRING | 입력 텍스트에서 추출된 부분 문자열로, `시작` 위치부터 `끝` 위치 바로 앞까지의 모든 문자를 포함합니다. |
