...
| `start` | INT | Sim | - | O índice da posição inicial para a substring. O primeiro caractere está no índice 0. |
| `end` | INT | Sim | - | O índice da posição final para a substring. O caractere neste índice não é incluído no resultado. |

## Saídas

| Nome da Saída | Tipo de Dado | Descrição |
|---------------|--------------|-----------|
| `output` | STRING | A substring extraída do texto de entrada, contendo todos os caracteres da posição `start` até (mas não incluindo) a posição `end`. |
