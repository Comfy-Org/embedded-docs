...
|-----------|-----------|----------|-------|-------------|
| `model` | MODEL | 예 | - | 자기 주의 유도를 적용할 확산 모델 |
| `scale` | FLOAT | 아니요 | -2.0 ~ 5.0 | 자기 주의 유도 효과의 강도 (기본값: 0.5) |
| `blur_sigma` | FLOAT | 아니요 | 0.0 ~ 10.0 | 유도 맵을 생성하는 데 적용되는 흐림 정도 (기본값: 2.0) |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `model` | MODEL | 자기 주의 유도가 적용된 수정된 모델 |
