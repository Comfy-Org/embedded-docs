...
|---------|----------|-------|--------|---------|
| `model` | MODEL | بله | - | مدل انتشار که راهنمایی توجه به خود (self-attention guidance) روی آن اعمال می‌شود |
| `scale` | FLOAT | خیر | 2.0- تا 5.0 | قدرت اثر راهنمایی توجه به خود (پیش‌فرض: 0.5) |
| `blur_sigma` | FLOAT | خیر | 0.0 تا 10.0 | میزان تاری اعمال‌شده برای ایجاد نقشه راهنما (پیش‌فرض: 2.0) |

## خروجی‌ها

| نام خروجی | نوع داده | توضیحات |
|-----------|----------|---------|
| `model` | MODEL | مدل اصلاح‌شده با راهنمایی توجه به خود اعمال‌شده |
