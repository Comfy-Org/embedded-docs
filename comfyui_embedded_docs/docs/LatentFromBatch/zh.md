...
| 参数 | 数据类型 | 描述 |
|---------------|-------------|-------------|
| `samples` | `LATENT` | 从中提取子集的潜在样本集合。此参数对于确定待处理样本的源批次至关重要。 |
| `batch_index` | `INT` | 指定批次中开始提取样本子集的起始索引。此参数支持从批次中的特定位置进行针对性提取。 |
| `length` | `INT` | 定义从指定起始索引开始提取的样本数量。此参数控制待处理子集的大小，允许灵活操作批次片段。 |
