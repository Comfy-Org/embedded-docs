...
|-----------|-----------------|--------|-------|-------------|
| `latents` | LATENT | Oui | N/A | Liste de dictionnaires latents provenant de MakeTrainingDataset. |
| `conditioning` | CONDITIONING | Oui | N/A | Liste de listes de conditionnement provenant de MakeTrainingDataset. |
| `folder_name` | STRING | Non | N/A | Nom du dossier pour enregistrer l'ensemble de données (dans le répertoire de sortie). (par défaut : "training_dataset") |
