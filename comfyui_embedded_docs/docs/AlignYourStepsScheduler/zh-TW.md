...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `model_type` | STRING | 是 | `"SD1"`<br>`"SDXL"`<br>`"SVD"` | 指定用於 sigma 計算的模型類型（預設值："SD1"） |
| `steps` | INT | 是 | 1 到 10000 | 要生成的總取樣步驟數（預設值：10） |
| `denoise` | FLOAT | 是 | 0.0 到 1.0 | 控制圖像的去噪程度，1.0 使用所有步驟，較低的值使用較少的步驟（預設值：1.0） |

## 輸出

| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `sigmas` | SIGMAS | 返回計算出的去噪過程 sigma 值 |
