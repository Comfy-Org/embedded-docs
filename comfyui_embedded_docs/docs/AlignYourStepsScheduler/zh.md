...
| 参数 | 数据类型 | 是否必填 | 范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `model_type` | STRING | 是 | `"SD1"`<br>`"SDXL"`<br>`"SVD"` | 指定用于sigma计算的模型类型（默认值："SD1"） |
| `steps` | INT | 是 | 1 到 10000 | 要生成的采样总步数（默认值：10） |
| `denoise` | FLOAT | 是 | 0.0 到 1.0 | 控制图像去噪程度，1.0表示使用所有步数，较低值使用较少步数（默认值：1.0） |
