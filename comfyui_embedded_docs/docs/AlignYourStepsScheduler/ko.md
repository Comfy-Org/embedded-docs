...
|-----------|-----------|----------|-------|-------------|
| `model_type` | STRING | 예 | `"SD1"`<br>`"SDXL"`<br>`"SVD"` | 시그마 계산에 사용할 모델 유형을 지정합니다 (기본값: "SD1") |
| `steps` | INT | 예 | 1 ~ 10000 | 생성할 총 샘플링 단계 수입니다 (기본값: 10) |
| `denoise` | FLOAT | 예 | 0.0 ~ 1.0 | 이미지 노이즈 제거 정도를 제어합니다. 1.0은 모든 단계를 사용하고, 낮은 값은 더 적은 단계를 사용합니다 (기본값: 1.0) |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `sigmas` | SIGMAS | 노이즈 제거 프로세스를 위해 계산된 시그마 값을 반환합니다 |
