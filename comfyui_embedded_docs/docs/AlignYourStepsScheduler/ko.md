...
|-----------|-----------|----------|-------|-------------|
| `모델 유형` | STRING | 예 | `"SD1"`<br>`"SDXL"`<br>`"SVD"` | 시그마 계산에 사용할 모델 유형을 지정합니다 (기본값: "SD1") |
| `스텝 수` | INT | 예 | 1 ~ 10000 | 생성할 총 샘플링 단계 수입니다 (기본값: 10) |
| `노이즈 제거양` | FLOAT | 예 | 0.0 ~ 1.0 | 이미지 노이즈 제거 정도를 제어합니다. 1.0은 모든 단계를 사용하고, 낮은 값은 더 적은 단계를 사용합니다 (기본값: 1.0) |
