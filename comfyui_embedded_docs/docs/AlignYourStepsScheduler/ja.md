...
|-----------|-----------|------------|---------|-------|-------------|
| `model_type` | STRING | COMBO | - | SD1, SDXL, SVD | シグマ計算に使用するモデルのタイプを指定します |
| `steps` | INT | INT | 10 | 1-10000 | 生成するサンプリングステップの総数 |
| `denoise` | FLOAT | FLOAT | 1.0 | 0.0-1.0 | 画像をどの程度デノイズするかを制御します。1.0はすべてのステップを使用し、低い値では少ないステップを使用します |
