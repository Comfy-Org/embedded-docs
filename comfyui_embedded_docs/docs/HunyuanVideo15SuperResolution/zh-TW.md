...
| `vae` | VAE | 否 | N/A | 用於編碼可選 `start_image` 的 VAE。若提供 `start_image` 則為必要。 |
| `start_image` | IMAGE | 否 | N/A | 可選的起始影像，用於引導超解析度過程。若提供，將被放大並編碼到條件化潛在表示中。 |
| `clip_vision_output` | CLIP_VISION_OUTPUT | 否 | N/A | 可選的 CLIP 視覺嵌入，將添加到條件化資料中。 |
| `latent` | LATENT | 是 | N/A | 將納入條件化資料的輸入潛在影片表示。 |
| `noise_augmentation` | FLOAT | 否 | 0.0 - 1.0 | 應用於條件化資料的雜訊增強強度（預設值：0.70）。 |
...
| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `positive` | CONDITIONING | 修改後的正向條件化資料，現包含串聯的潛在表示、雜訊增強及可選的 CLIP 視覺資料。 |
| `negative` | CONDITIONING | 修改後的負向條件化資料，現包含串聯的潛在表示、雜訊增強及可選的 CLIP 視覺資料。 |
