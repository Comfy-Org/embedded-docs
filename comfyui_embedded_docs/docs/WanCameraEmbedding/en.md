...
Note: `fx`, `fy`, `cx`, and `cy` are advanced parameters. The `length` parameter uses a step of 4 because the first camera frame is repeated internally, so the effective processed sequence length becomes `length + 3`.
