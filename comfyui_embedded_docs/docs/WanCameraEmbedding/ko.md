...
|-----------|-----------|----------|-------|-------------|
| `카메라 포즈` | COMBO | 예 | "Static"<br>"Pan Up"<br>"Pan Down"<br>"Pan Left"<br>"Pan Right"<br>"Zoom In"<br>"Zoom Out"<br>"Anti Clockwise (ACW)"<br>"ClockWise (CW)" | 시뮬레이션할 카메라 움직임 유형 (기본값: "Static") |
| `너비` | INT | 예 | 16 ~ MAX_RESOLUTION | 출력 이미지의 가로 크기 (픽셀 단위, 기본값: 832, 증가 단위: 16) |
| `높이` | INT | 예 | 16 ~ MAX_RESOLUTION | 출력 이미지의 세로 크기 (픽셀 단위, 기본값: 480, 증가 단위: 16) |
| `길이` | INT | 예 | 1 ~ MAX_RESOLUTION | 카메라 궤적 시퀀스의 길이 (기본값: 81, 증가 단위: 4) |
| `속도` | FLOAT | 아니요 | 0.0 ~ 10.0 | 카메라 움직임의 속도 (기본값: 1.0, 증가 단위: 0.1) |
...
| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `너비` | TENSOR | 궤적 시퀀스를 포함하는 생성된 카메라 임베딩 텐서 |
| `높이` | INT | 처리에 사용된 가로 크기 값 |
| `길이` | INT | 처리에 사용된 세로 크기 값 |
| `길이` | INT | 처리에 사용된 길이 값 |
