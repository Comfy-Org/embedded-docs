...
| `length` | INT | はい | 1 ～ MAX_RESOLUTION | カメラ軌跡シーケンスの長さ（デフォルト: 81、ステップ: 4） |
| `speed` | FLOAT | いいえ | 0.0 ～ 10.0 | カメラの動きの速度（デフォルト: 1.0、ステップ: 0.1） |
...
| 出力名 | データ型 | 説明 |
|-------------|-----------|-------------|
| `camera_embedding` | TENSOR | 軌跡シーケンスを含む、生成されたカメラ埋め込みテンソル |
