...
O nó WanDancerVideo prepara dados de condicionamento e um tensor latente vazio para geração de vídeo com o modelo WanDancer. Ele combina condicionamentos positivo e negativo com entradas opcionais, como imagem inicial, máscara, embeddings de visão do CLIP e recursos de áudio para controlar o vídeo gerado.
...
| `length` | INT | Sim | 1 a MAX_RESOLUTION (passo: 4) | O número de quadros no vídeo gerado. Deve permanecer 149 para o WanDancer (padrão: 149). |
| `clip_vision_output` | CLIP_VISION_OUTPUT | Não | | Os embeddings de visão do CLIP para o primeiro quadro. |
| `clip_vision_output_ref` | CLIP_VISION_OUTPUT | Não | | Os embeddings de visão do CLIP para a imagem de referência. |
| `start_image` | IMAGE | Não | | A(s) imagem(ns) inicial(is) a ser(em) codificada(s). Pode ser qualquer número de quadros, até o `length` especificado. |
...
|---------------|--------------|-----------|
| `positive` | CONDITIONING | O condicionamento positivo com quaisquer dados adicionais (latente concatenado, visão do CLIP, áudio) anexados. |
| `negative` | CONDITIONING | O condicionamento negativo com quaisquer dados adicionais (latente concatenado, visão do CLIP, áudio) anexados. |
