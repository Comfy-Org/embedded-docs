...
| `너비` | INT | 예 | 16 ~ MAX_RESOLUTION (단위: 16) | 생성된 비디오의 픽셀 단위 너비입니다 (기본값: 480). |
| `높이` | INT | 예 | 16 ~ MAX_RESOLUTION (단위: 16) | 생성된 비디오의 픽셀 단위 높이입니다 (기본값: 832). |
| `길이` | INT | 예 | 1 ~ MAX_RESOLUTION (단위: 4) | 생성된 비디오의 프레임 수입니다. WanDancer의 경우 149로 유지해야 합니다 (기본값: 149). |
...
| `시작 이미지` | IMAGE | 아니요 | | 인코딩할 초기 이미지입니다. 지정된 `길이`까지 여러 프레임이 될 수 있습니다. |
| `마스크` | MASK | 아니요 | | 시작 이미지에 대한 이미지 조건화 마스크입니다. 흰색 영역은 유지되고 검은색 영역은 생성됩니다. 로컬 생성에 사용됩니다. |
...
| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `negative` | CONDITIONING | 추가 데이터(연결된 잠재 텐서, CLIP 비전, 오디오)가 첨부된 긍정 조건화입니다. |
| `latent` | CONDITIONING | 추가 데이터(연결된 잠재 텐서, CLIP 비전, 오디오)가 첨부된 부정 조건화입니다. |
