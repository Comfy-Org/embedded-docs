...
| Paramètre | Description | Type de données |
| --- | --- | --- |
| `sigmas` | Le nœud produit deux séquences de valeurs sigma, chacune représentant une partie de la séquence originale divisée au pas spécifié. Ces sorties sont cruciales pour les opérations ultérieures nécessitant un traitement différencié des valeurs sigma. | `SIGMAS` |
