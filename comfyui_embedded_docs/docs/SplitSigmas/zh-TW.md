...
| `步驟`    | `INT`       | 'step' 參數指定了分割 sigma 序列的索引位置。它在定義兩個結果 sigma 序列之間的邊界方面扮演關鍵角色，影響節點的功能與輸出的特性。 |

## 輸出

| 參數 | 資料類型 | 說明 |
|-----------|-------------|-------------|
| `低 sigma`  | `SIGMAS`    | 此節點輸出兩個 sigma 值序列，每個序列代表原始序列在指定步長處分割後的一部分。這些輸出對於後續需要對 sigma 值進行區分處理的操作至關重要。 |