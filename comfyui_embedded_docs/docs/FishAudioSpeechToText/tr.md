...
|-----------|-------------|-----------|----------|-------|
| `audio` | Transkribe edilecek ses. | AUDIO | Evet | — |
| `language` | ISO 639-1 dil ipucu (örn. 'en', 'zh'). Dil, bu ipucundan bağımsız olarak otomatik algılanır. Varsayılan: "" (boş dize). | STRING | Hayır | Herhangi bir ISO 639-1 dil kodu, örn. `en`, `zh`; otomatik algılama için boş dize |
| `precise_timestamps` | Kelime düzeyinde zaman damgalı bölümler döndürür. Varsayılan: false. | BOOLEAN | Hayır | true veya false |

Not: `language` parametresi yalnızca bir ipucudur — dil her zaman sesten otomatik olarak algılanır. `precise_timestamps` false olduğunda (varsayılan), kelime düzeyinde zaman damgaları döndürülmez; true olduğunda, çıktı bölümleri kelime düzeyinde zaman damgalarını içerir.

## Çıktılar

| Çıktı Adı | Açıklama | Veri Türü |
|-------------|-------------|-----------|
| `text` | Transkribe edilen metin. | STRING |
