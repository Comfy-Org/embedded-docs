...
| `modèle2`  | `MODEL`         | Le second modèle à partir duquel les correctifs sont extraits et appliqués au premier modèle, en fonction des ratios de mélange spécifiés. |
| `entrée`   | `FLOAT`         | Spécifie le ratio de mélange pour la couche d'entrée des modèles. Il détermine la proportion de la couche d'entrée du second modèle fusionnée dans le premier modèle. |
...
| Paramètre | Type de données | Description |
|-----------|-----------------|-------------|
| `model`   | MODEL           | Le modèle fusionné résultant, qui est un hybride des deux modèles d'entrée avec des correctifs appliqués selon les ratios de mélange spécifiés. |