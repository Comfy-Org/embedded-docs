...
| `model2`  | `MODEL`         | Le second modèle à partir duquel les correctifs sont extraits et appliqués au premier modèle, en fonction des ratios de mélange spécifiés. |
| `input`   | `FLOAT`         | Spécifie le ratio de mélange pour la couche d'entrée des modèles. Il détermine la proportion de la couche d'entrée du second modèle fusionnée dans le premier modèle. |
