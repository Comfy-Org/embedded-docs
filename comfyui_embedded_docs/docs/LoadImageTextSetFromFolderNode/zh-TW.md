...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `folder` | STRING | 是 | - | 要載入影像的資料夾。 |
| `clip` | CLIP | 是 | - | 用於編碼文字的 CLIP 模型。 |
| `resize_method` | COMBO | 否 | "None"<br>"Stretch"<br>"Crop"<br>"Pad" | 調整影像大小的方法（預設值："None"）。 |
| `width` | INT | 否 | -1 至 10000 | 調整影像的目標寬度。設為 -1 表示使用原始寬度（預設值：-1）。 |
| `height` | INT | 否 | -1 至 10000 | 調整影像的目標高度。設為 -1 表示使用原始高度（預設值：-1）。 |
...
| `CONDITIONING` | CONDITIONING | 從文字標題編碼而來的條件資料。 |
