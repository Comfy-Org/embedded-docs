...
| `control_net` | `CONTROL_NET` | 적용할 ControlNet 모델로, 일반적으로 ControlNet 로더에서 입력됩니다. |
| `image` | `IMAGE` | ControlNet 적용을 위한 이미지로, 전처리기로 처리해야 합니다. |
| `vae` | `VAE` | VAE 모델 입력입니다. |
| `strength` | `FLOAT` | 네트워크 조정 강도를 제어하며, 값 범위는 0~10입니다. 권장 값은 0.5~1.5 사이가 적절합니다. 값이 낮을수록 모델에 더 많은 자유도를 부여하고, 값이 높을수록 더 엄격한 제약을 적용합니다. 값이 너무 높으면 이상한 이미지가 생성될 수 있습니다. 이 값을 테스트하고 조정하여 제어 네트워크의 영향을 미세 조정할 수 있습니다. |
