...
|-----------|--------------|-------------|-------|-----------|
| `model` | COMBO | Sim | `"viduq2"` | O modelo de IA a ser usado para geração de vídeo. |
| `subjects` | AUTOGROW | Sim | N/A | Para cada tema, forneça até 3 imagens de referência (7 imagens no total entre todos os temas). Referencie-os nos prompts via `@subject{subject_id}`. |
| `prompt` | STRING | Sim | N/A | A descrição textual usada para guiar a geração do vídeo. Quando o parâmetro `audio` está ativado, o vídeo incluirá fala gerada e música de fundo com base neste prompt. |
| `audio` | BOOLEAN | Não | N/A | Quando ativado, o vídeo conterá fala gerada e música de fundo com base no prompt (padrão: `False`). |
...
| `movement_amplitude` | COMBO | Não | `"auto"`<br>`"small"`<br>`"medium"`<br>`"large"` | Controla a amplitude de movimento dos objetos no quadro. |
