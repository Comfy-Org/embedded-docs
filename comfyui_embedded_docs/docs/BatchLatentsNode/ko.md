...
|-----------|-----------|----------|-------|-------------|
| `latents` | LATENT | 예 | 2~50개 입력 | 단일 배치로 결합할 잠재 샘플 집합입니다. 최소 2개 이상의 잠재를 제공해야 하며, 최대 50개까지 추가할 수 있습니다. 더 많은 잠재를 연결하면 노드가 자동으로 입력 슬롯을 생성합니다. |
...
| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `output` | LATENT | 모든 입력 잠재가 하나의 배치로 결합된 단일 잠재 출력입니다. |
