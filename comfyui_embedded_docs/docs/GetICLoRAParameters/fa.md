...
|---------|----------|--------|--------|---------|
| `iclora_model` | MODEL | بله | N/A | خروجی مستقیم از یک بارگذار LoRA برای IC-LoRA خاصی که می‌خواهید فراداده آن استخراج شود. |

## خروجی‌ها

| نام خروجی | نوع داده | توضیحات |
|-----------|----------|---------|
| `iclora_parameters` | IC_LORA_PARAMETERS | پارامترهای IC-LoRA استخراج‌شده از فراداده LoRA (مانند reference_downscale_factor). در صورت نیاز LoRA به مدیریت ویژه راهنماها، به گره `LTXVAddGuide` متصل کنید. |
