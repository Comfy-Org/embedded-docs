...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `iclora_model` | MODEL | 是 | 無 | 直接從 LoRA 載入器輸出，用於指定要提取元資料的特定 IC-LoRA。 |

## 輸出

| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `iclora_parameters` | IC_LORA_PARAMETERS | 從 LoRA 元資料中提取的 IC-LoRA 參數（例如：reference_downscale_factor）。如果該 LoRA 需要對引導進行特殊處理，請連接到 LTXVAddGuide。 |
