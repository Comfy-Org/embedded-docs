...
| 파라미터 | 데이터 타입 | 필수 여부 | 범위 | 설명 |
|-----------|-----------|----------|-------|-------------|
| `iclora_model` | MODEL | 예 | 해당 없음 | 메타데이터를 추출할 특정 IC-LoRA용 LoRA 로더의 직접 출력입니다. |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `iclora_parameters` | IC_LORA_PARAMETERS | LoRA 메타데이터에서 추출된 IC-LoRA 파라미터(예: reference_downscale_factor)입니다. LoRA에 가이드 특수 처리가 필요한 경우 LTXVAddGuide에 연결하십시오. |
