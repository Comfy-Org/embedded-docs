...
Gira una malla 3D alrededor de los ejes globales usando ángulos Euler XYZ (en grados) o un cuaternión. En el modo Euler XYZ, las rotaciones se aplican como X, luego Y y luego Z; en el modo cuaternión, el cuaternión (w, x, y, z) se normaliza automáticamente. La rotación se aplica a los vértices de la malla, y las normales y tangentes también se rotan para que la iluminación y el sombreado sigan siendo correctos.
...
|-----------|-------------|---------------|-------------|-------|
| `mode` | El modo de rotación a usar. `"euler_xyz"` aplica la rotación como ángulos X, luego Y y luego Z alrededor de los ejes globales (en grados). `"quaternion"` usa un cuaternión (w, x, y, z) que se normaliza automáticamente. | DYNAMIC_COMBO | Sí | `"euler_xyz"`<br>`"quaternion"` |
...
**Nota:** Cuando `mode` es `"euler_xyz"` y los tres ángulos son 0.0, o cuando `mode` es `"quaternion"` y el cuaternión es la identidad (1, 0, 0, 0), la malla se devuelve sin cambios. El cuaternión se normaliza automáticamente antes de usarse; si su magnitud es inferior a 1e-8, el nodo genera un error.

## Salidas

| Nombre de salida | Descripción | Tipo de datos |
|------------------|-------------|---------------|
| `mesh` | La malla rotada. Los vértices se rotan, y las normales se rotan como direcciones. Las tangentes tienen sus componentes X, Y, Z rotados mientras que el componente W (lateralidad) se mantiene sin cambios. | MESH |
