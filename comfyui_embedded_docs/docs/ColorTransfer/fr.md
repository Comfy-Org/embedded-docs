...
|-----------|-----------------|--------|-------|-------------|
| `image_target` | IMAGE | Oui | - | Image(s) à laquelle/auxquelles appliquer la transformation de couleurs. |
| `image_ref` | IMAGE | Oui | - | Image(s) de référence dont les couleurs doivent être reproduites. |
| `method` | COMBO | Oui | `"reinhard_lab"`<br>`"mkl_lab"`<br>`"histogram"` | L'algorithme de transfert de couleurs à utiliser. |
| `source_stats` | DYNAMICCOMBO | Oui | `"per_frame"`<br>`"uniform"`<br>`"target_frame"` | Détermine comment les statistiques de couleurs sont calculées à partir de la ou des images source (cible). |
| `strength` | FLOAT | Oui | 0.0 à 10.0 | L'intensité de l'effet de transfert de couleurs. Une valeur de 1.0 applique la transformation complète, tandis que 0.0 renvoie l'image originale. Par défaut : 1.0 |
...
|------------------|-----------------|-------------|
| `image` | IMAGE | La ou les images résultantes après l'application du transfert de couleurs. |
