...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `model_3d` | FILE3D | 是 | FBX, Any | 要處理的3D模型。模型應為FBX格式，且面數少於30000個。 |
| `seed` | INT | 否 | 0 到 2147483647 | 用於控制節點是否應重新執行的種子值。無論種子值為何，結果皆為非確定性。（預設值：0） |
...
| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `FBX` | FILE3DFBX | 處理後的3D模型，以FBX檔案形式回傳。 |
