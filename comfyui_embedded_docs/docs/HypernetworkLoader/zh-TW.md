...
| 欄位                  | Comfy 資料類型    | 說明                                                                                      |
|-----------------------|-------------------|------------------------------------------------------------------------------------------|
| `model`               | `MODEL`           | 將套用超網路的基礎模型，決定要增強或修改的架構。 |
| `hypernetwork_name`  | `COMBO[STRING]`   | 要載入並套用至模型的超網路名稱，影響模型修改後的行為或效能。 |
| `strength`            | `FLOAT`           | 調整超網路對模型影響強度的純量，允許微調修改的程度。 |

## 輸出

| 欄位   | 資料類型 | 說明                                                              |
|---------|-------------|------------------------------------------------------------------|
| `model` | `MODEL`     | 套用超網路後的修改模型，展示超網路對原始模型的影響。 |