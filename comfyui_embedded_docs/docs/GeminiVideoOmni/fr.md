...
| `seed` | La graine (seed) contrôle si le nœud doit s’exécuter à nouveau ; les résultats ne sont pas déterministes quelle que soit la graine (défaut : 42). | INT | Oui | 0 à 2147483647 |
...
| `temperature` | Contrôle le caractère aléatoire. Plus bas est plus ciblé/déterministe, plus haut est plus varié (défaut : 1.0). | FLOAT | Non | 0.0 à 2.0 |
