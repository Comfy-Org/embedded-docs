...
Genera un video con audio a partir de un prompt de texto utilizando el modelo Gemini Omni Flash de Google. Opcionalmente, proporciona imágenes o videos de referencia para guiar o editar el resultado. Describe la duración deseada (3-10 s) y la relación de aspecto (16:9 o 9:16) directamente en el prompt.
...
| `seed` | La semilla controla si el nodo debe volver a ejecutarse; los resultados son no deterministas independientemente de la semilla (por defecto: 42). | INT | Sí | 0 a 2147483647 |

### Entradas de Omni Flash

| Parámetro | Descripción | Tipo de dato | Requerido | Rango |
|-----------|-------------|-----------|----------|-------|
| `prompt` | Describe el video a generar. Especifica la duración y la relación de aspecto directamente en el prompt, p. ej., «un clip de 6 segundos en 16:9». La duración puede ser de 3 a 10 segundos; la relación de aspecto debe ser 16:9 (horizontal) o 9:16 (vertical). La salida es de 720p, 24 FPS, con audio. | STRING | Sí | Mínimo 1 carácter después de eliminar espacios en blanco |
...
| `videos` | Ranura ampliable: conecta uno o más videos de referencia (`video_1`...`video_3`) para guiar o editar. Hasta 3 videos, cada uno de hasta 10 segundos de duración. | VIDEO | No | 0 a 3 videos, cada uno máx. 10 segundos |
| `temperature` | Controla la aleatoriedad. Un valor más bajo da resultados más centrados/deterministas, un valor más alto da resultados más variados (por defecto: 1.0). | FLOAT | No | 0.0 a 2.0 |
| `top_p` | Muestreo de núcleo: muestrea del conjunto de tokens más pequeño cuya probabilidad acumulada alcanza top_p (por defecto: 0.95). | FLOAT | No | 0.0 a 1.0 |
...
|-------------|-------------|-----------|
| `VIDEO` | El video generado con audio a partir del modelo Gemini. | VIDEO |
