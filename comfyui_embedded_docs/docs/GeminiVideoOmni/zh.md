...
使用 Google 的 Gemini Omni Flash 模型，根据文本提示生成带音频的视频。可选择提供参考图像和/或视频来引导或编辑生成结果。直接在提示中描述所需的时长（3-10 秒）和宽高比（16:9 或 9:16）。
...
| `prompt` | 描述要生成视频的文本提示。去除首尾空白字符后，必须至少包含一个非空白字符。 | STRING | 是 | 去除空白后至少 1 个字符 |
| `images` | 可选的参考图像，用于引导视频生成。最多 14 张图像。 | IMAGE | 否 | 允许多张图像（最多 14 张） |
| `videos` | 可选的参考视频，用于引导或编辑视频生成。最多 3 个视频，每个最长 10 秒。 | VIDEO | 否 | 允许多个视频（最多 3 个，每个最长 10 秒） |
