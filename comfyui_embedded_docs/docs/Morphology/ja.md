...
| `operation` | STRING | はい | `"erode"`<br>`"dilate"`<br>`"open"`<br>`"close"`<br>`"gradient"`<br>`"bottom_hat"`<br>`"top_hat"` | 適用する形態変換処理（デフォルト："erode"） |
| `kernel_size` | INT | はい | 3-999 | 構造要素カーネルのサイズ（デフォルト：3）。奇数である必要があります。 |

## 出力

| 出力名 | データ型 | 説明 |
|-------------|-----------|-------------|
| `image` | IMAGE | 形態変換処理を適用した後の処理済み画像 |
