...
| `operation` | STRING | はい | `"erode"`<br>`"dilate"`<br>`"open"`<br>`"close"`<br>`"gradient"`<br>`"bottom_hat"`<br>`"top_hat"` | 適用する形態学的操作 |
| `kernel_size` | INT | いいえ | 3-999 | 構造化要素カーネルのサイズ（デフォルト: 3） |

## 出力

| 出力名 | データ型 | 説明 |
|-------------|-----------|-------------|
| `image` | IMAGE | 形態学的操作を適用した後の処理済み画像 |