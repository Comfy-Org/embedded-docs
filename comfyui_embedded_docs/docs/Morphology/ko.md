...
| `operation` | STRING | 예 | `"erode"`<br>`"dilate"`<br>`"open"`<br>`"close"`<br>`"gradient"`<br>`"bottom_hat"`<br>`"top_hat"` | 적용할 형태학적 연산입니다 (기본값: "erode") |
| `kernel_size` | INT | 예 | 3-999 | 구조 요소 커널의 크기입니다 (기본값: 3). 홀수여야 합니다. |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `image` | IMAGE | 형태학적 연산 적용 후 처리된 이미지 |
