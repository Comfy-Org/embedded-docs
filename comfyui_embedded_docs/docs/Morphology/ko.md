...
| `operation` | STRING | 예 | `"erode"`<br>`"dilate"`<br>`"open"`<br>`"close"`<br>`"gradient"`<br>`"bottom_hat"`<br>`"top_hat"` | 적용할 모폴로지 연산 |
| `kernel_size` | INT | 아니오 | 3-999 | 구조 요소 커널의 크기 (기본값: 3) |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `image` | IMAGE | 모폴로지 연산을 적용한 후 처리된 이미지 |