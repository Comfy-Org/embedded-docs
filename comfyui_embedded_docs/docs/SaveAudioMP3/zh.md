...
SaveAudioMP3 节点将音频数据保存为 MP3 文件。它接收音频输入，并使用可自定义的文件名和质量设置将其导出到输出目录，自动处理文件命名和 MP3 格式转换。**此节点已弃用，并可能在未来的版本中移除。**
...
**注意：** 如果输入 `audio` 为 None（例如，当源视频没有音轨时），节点会引发 ValueError。
