...
Le nœud SaveAudioMP3 enregistre des données audio sous forme de fichier MP3. Il prend une entrée audio et l'exporte vers le répertoire de sortie avec des paramètres de nom de fichier et de qualité personnalisables, gérant automatiquement la dénomination des fichiers et la conversion au format MP3. **Ce nœud est obsolète et pourrait être supprimé dans les versions futures.**
...
| `préfixe_nom_fichier` | Le préfixe du nom de fichier de sortie (par défaut : "audio/ComfyUI") | STRING | Non | - |
| `qualité` | Le réglage de qualité audio pour le fichier MP3 (par défaut : "V0") | COMBO | Non | `"V0"`<br>`"128k"`<br>`"320k"` |
...
**Note :** Si l'entrée `audio` est None (par exemple, lorsque la vidéo source n'a pas de piste audio), le nœud lève une ValueError.

## Sorties

| Nom de sortie | Description | Type de données |
|-------------|-------------|-----------|
| `audio` | Les données audio qui ont été enregistrées dans un fichier MP3 | AUDIO |
