...
|---------|----------|-----------|---------|--------|---------|
| `model` | MODEL | اجباری | - | - | مدل انتشار که نرمال‌سازی CFG روی آن اعمال می‌شود |
| `strength` | FLOAT | اجباری | 1.0 | 0.0 - 100.0 | شدت اثر نرمال‌سازی اعمال‌شده بر مقیاس‌گذاری CFG را کنترل می‌کند |

## خروجی‌ها

| نام خروجی | نوع داده | توضیحات |
|-----------|----------|---------|
| `patched_model` | MODEL | مدل اصلاح‌شده با نرمال‌سازی CFG اعمال‌شده بر فرآیند نمونه‌برداری آن را برمی‌گرداند |
