...
این گره یک ورودی بولی (true/false) دریافت می‌کند و مقدار مخالف آن را خروجی می‌دهد. این عملیات یک NOT منطقی است که `true` را به `false` و `false` را به `true` تبدیل می‌کند.
