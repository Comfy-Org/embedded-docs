...
| `model_name` | COMBO | 是 | 多個選項可用 | 影片生成的模型選擇（預設值："kling-v2-master"）。 |
| `cfg_scale` | FLOAT | 是 | 0.0-1.0 | 配置縮放參數（預設值：0.8）。 |
| `mode` | COMBO | 是 | 多個選項可用 | 影片生成模式選擇（預設值：std）。 |
| `aspect_ratio` | COMBO | 是 | 多個選項可用 | 生成影片的長寬比（預設值：field_16_9）。 |
| `時長` | COMBO | 是 | 多個選項可用 | 生成影片的持續時間（預設值：field_5）。 |
