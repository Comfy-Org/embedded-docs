...
|-----------|-----------|----------|-------|-------------|
| `start_frame` | IMAGE | Evet | - | Videoyu oluşturmak için kullanılan referans görüntüsü. |
| `prompt` | STRING | Evet | - | Pozitif metin istemi. |
| `negative_prompt` | STRING | Evet | - | Negatif metin istemi. |
| `model_name` | COMBO | Evet | `"kling-v2-master"`<br>`"kling-v2-1-master"`<br>`"kling-v2-5-turbo"`<br>`"kling-v2-1"`<br>`"kling-v1-6"`<br>`"kling-v1-5"`<br>`"kling-v1-4"`<br>`"kling-v1-0"` | Video oluşturma için kullanılan model (varsayılan: `"kling-v2-master"`). |
| `cfg_scale` | FLOAT | Evet | 0.0 ile 1.0 | Videonun istemi ne kadar yakından takip edeceğini kontrol eder. Daha yüksek değerler daha güçlü bağlılık anlamına gelir (varsayılan: 0.8). |
| `mode` | COMBO | Evet | `"std"`<br>`"pro"` | Oluşturma modu. `"std"` standart kalite, `"pro"` daha yüksek kalitedir (varsayılan: `"std"`). |
| `aspect_ratio` | COMBO | Evet | `"16:9"`<br>`"9:16"`<br>`"1:1"` | Oluşturulan videonun en-boy oranı (varsayılan: `"16:9"`). |
| `duration` | COMBO | Evet | `"5"`<br>`"10"` | Oluşturulan videonun saniye cinsinden süresi (varsayılan: `"5"`). |
