...
**Note:** When a VAE is provided, the node generates reference latents from all input images. The node can process up to three images simultaneously. Images are automatically scaled to a target area of roughly 384×384 pixels (aspect ratio preserved) for vision-language processing, and to dimensions divisible by 8 with a target area of roughly 1024×1024 pixels for VAE encoding.
