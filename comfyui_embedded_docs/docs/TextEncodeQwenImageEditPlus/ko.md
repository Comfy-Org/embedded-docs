...
| `프롬프트` | STRING | 예 | - | 원하는 이미지 수정을 설명하는 텍스트 명령(여러 줄 입력 및 동적 프롬프트 지원) |
| `vae` | VAE | 아니요 | - | 입력 이미지에서 참조 잠재값을 생성하기 위한 선택적 VAE 모델 |
| `이미지1` | IMAGE | 아니요 | - | 분석 및 수정을 위한 첫 번째 선택적 입력 이미지 |
| `이미지2` | IMAGE | 아니요 | - | 분석 및 수정을 위한 두 번째 선택적 입력 이미지 |
| `이미지3` | IMAGE | 아니요 | - | 분석 및 수정을 위한 세 번째 선택적 입력 이미지 |
