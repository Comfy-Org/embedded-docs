...
| `prompt` | STRING | 예 | - | 원하는 이미지 수정을 설명하는 텍스트 명령(여러 줄 입력 및 동적 프롬프트 지원) |
| `vae` | VAE | 아니요 | - | 입력 이미지에서 참조 잠재값을 생성하기 위한 선택적 VAE 모델 |
| `image1` | IMAGE | 아니요 | - | 분석 및 수정을 위한 첫 번째 선택적 입력 이미지 |
| `image2` | IMAGE | 아니요 | - | 분석 및 수정을 위한 두 번째 선택적 입력 이미지 |
| `image3` | IMAGE | 아니요 | - | 분석 및 수정을 위한 세 번째 선택적 입력 이미지 |
...
| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `CONDITIONING` | CONDITIONING | 텍스트 토큰과 이미지 생성을 위한 선택적 참조 잠재값을 포함하는 인코딩된 컨디셔닝 데이터 |
