...
| 参数 | 数据类型 | 是否必填 | 范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `image` | IMAGE | 是 | - | 待缩放的输入图像 |
| `upscale_method` | STRING | 是 | "area"<br>"lanczos"<br>"bilinear"<br>"nearest-exact"<br>"bilinear"<br>"bicubic" | 用于缩放图像的插值方法（默认值："area"） |
| `largest_size` | INT | 是 | 0 至 16384 | 缩放后图像的最大尺寸（默认值：512） |

## 输出

| 输出名称 | 数据类型 | 描述 |
|-------------|-----------|-------------|
| `image` | IMAGE | 缩放后的图像，其最大尺寸与指定大小一致 |
