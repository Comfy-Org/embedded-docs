...
| `upscale_method` | STRING | 예 | "area"<br>"lanczos"<br>"bilinear"<br>"nearest-exact"<br>"bilinear"<br>"bicubic" | 이미지 크기 조정에 사용되는 보간 방법 (기본값: "area") |
| `largest_size` | INT | 예 | 0 ~ 16384 | 크기 조정된 이미지의 최대 크기 (기본값: 512) |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `image` | IMAGE | 가장 큰 크기가 지정된 크기와 일치하도록 조정된 이미지 |
