...
| パラメータ | データ型 | 必須 | 範囲 | 説明 |
|-----------|-----------|----------|-------|-------------|
| `model` | MODEL | はい | - | LoRAが適用される拡散モデルです。 |
| `lora` | LORA_MODEL | はい | - | 拡散モデルに適用するLoRAモデルです。 |
| `strength_model` | FLOAT | はい | -100.0 ～ 100.0 | 拡散モデルをどの程度変更するかを指定します。負の値も設定可能です（デフォルト：1.0）。 |
| `bypass` | BOOLEAN | はい | True または False | 有効にすると、ベースモデルの重みを変更せずにバイパスモードでLoRAを適用します。トレーニング時やモデル重みがオフロードされている場合に便利です（デフォルト：False）。 |
...
| 出力名 | データ型 | 説明 |
|-------------|-----------|-------------|
| `model` | MODEL | LoRA重みが適用された変更後の拡散モデルです。 |
