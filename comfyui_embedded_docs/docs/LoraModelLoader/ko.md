...
|-----------|-----------|----------|-------|-------------|
| `모델` | MODEL | 예 | - | LoRA가 적용될 확산 모델입니다. |
| `LoRA` | LORA_MODEL | 예 | - | 확산 모델에 적용할 LoRA 모델입니다. |
| `모델 강도` | FLOAT | 예 | -100.0 ~ 100.0 | 확산 모델을 수정하는 강도입니다. 음수 값도 설정할 수 있습니다 (기본값: 1.0). |
| `bypass` | BOOLEAN | 예 | True 또는 False | 활성화하면 기본 모델 가중치를 수정하지 않고 LoRA를 우회 모드로 적용합니다. 학습 중이거나 모델 가중치가 오프로드된 경우 유용합니다 (기본값: False). |
...
| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `모델` | MODEL | LoRA 가중치가 적용된 수정된 확산 모델입니다. |
