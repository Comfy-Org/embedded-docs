...
| `n` | 要產生的影像數量，以批次形式回傳（預設：1）。 | INT | 否 | 1 至 6 |
| `seed` | 用於生成的種子（預設：42）。可設定為在每次生成後自動更新。 | INT | 否 | 0 至 2147483647 |
...
| `width` | 總像素面積必須介於 512x512 與 2560x2560 之間；寬高比必須介於 1:8 與 8:1 之間。（預設：1024） | INT | 否 | 256 至 2560 (step 16) |
| `height` | 總像素面積必須介於 512x512 與 2560x2560 之間；寬高比必須介於 1:8 與 8:1 之間。（預設：1024） | INT | 否 | 256 至 2560 (step 16) |
