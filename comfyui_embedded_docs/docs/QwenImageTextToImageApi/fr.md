...
|-----------|-------------|-----------------|--------|-------|
| `model` | Modèle à utiliser (par défaut : "qwen-image-3.0-pro"). Ce sélecteur composite fournit également le prompt, la largeur d’image, la hauteur d’image et le prompt négatif facultatif. | MODEL | Oui | "qwen-image-3.0-pro"<br>"qwen-image-3.0" |
