...
|-----------|-------------|-----------------|--------|-------|
| `modèle` | Modèle à utiliser (par défaut : "qwen-image-3.0-pro"). Ce sélecteur composite fournit également le prompt, la largeur d’image, la hauteur d’image et le prompt négatif facultatif. | MODEL | Oui | "qwen-image-3.0-pro"<br>"qwen-image-3.0" |
| `n` | Nombre d’images à générer, renvoyées sous forme de lot (par défaut : 1). | INT | Non | 1 à 6 |
| `graine` | Graine à utiliser pour la génération (par défaut : 42). Peut être configurée pour se mettre à jour automatiquement après chaque génération. | INT | Non | 0 à 2147483647 |
| `extension de prompt` | Indique s’il faut enrichir le prompt avec l’assistance de l’IA (par défaut : true). Option avancée. | BOOLEAN | Non | true<br>false |
| `filigrane` | Indique s’il faut ajouter un filigrane généré par l’IA au résultat (par défaut : false). Option avancée. | BOOLEAN | Non | true<br>false |

### Entrées pour qwen-image-3.0-pro et qwen-image-3.0
