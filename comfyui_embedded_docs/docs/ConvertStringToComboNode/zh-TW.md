...
Convert String to Combo 節點接受文字字串作為輸入，並將其轉換為 Combo 數據類型。這讓您可以將文字值用作其他需要 Combo 輸入之節點的選項。此節點僅原封不動地傳遞字串值，但變更其數據類型。

## 輸入

| 參數 | 說明 | 數據類型 | 必要 | 範圍 |
| --- | --- | --- | --- | --- |
| `string` | 要轉換為 Combo 類型的文字字串。 | STRING | 是 | N/A |

## 輸出

| 輸出名稱 | 說明 | 數據類型 |
| --- | --- | --- |
| `output` | 輸入字串，現已格式化為 Combo 數據類型。 | COMBO |
