...
Convert String to Combo 节点接收一个文本字符串作为输入，并将其转换为 Combo 数据类型。这使您可以将文本值用作需要 Combo 输入的其他节点的选择。它只是将字符串值原样传递，但更改其数据类型。

## 输入
| 参数 | 描述 | 数据类型 | 必填 | 范围 |
| --- | --- | --- | --- | --- |
| `string` | 要转换为 Combo 类型的文本字符串。 | STRING | 是 | N/A |

## 输出
| 输出名称 | 描述 | 数据类型 |
| --- | --- | --- |
| `output` | 输入字符串，现已格式化为 Combo 数据类型。 | COMBO |
