...
Convert String to Combo 노드는 텍스트 문자열을 입력으로 받아 Combo 데이터 타입으로 변환합니다. 이를 통해 Combo 입력이 필요한 다른 노드에서 선택 항목으로 텍스트 값을 사용할 수 있습니다. 이 노드는 문자열 값을 변경하지 않고 그대로 전달하지만 데이터 타입만 변경합니다.
...
| 출력 이름 | 설명 | 데이터 타입 |
| --- | --- | --- |
| `output` | 입력 문자열을 Combo 데이터 타입으로 변환한 결과입니다. | COMBO |
