...
|-----------|-----------|----------|-------|-------------|
| `width` | INT | 예 | 0 ~ MAX_RESOLUTION | 해상도 조건 설정을 위한 너비 값 (기본값: 1024) |
| `height` | INT | 예 | 0 ~ MAX_RESOLUTION | 해상도 조건 설정을 위한 높이 값 (기본값: 1024) |
| `text` | STRING | 예 | - | 인코딩할 텍스트 입력으로, 여러 줄 입력 및 동적 프롬프트를 지원합니다 |
| `clip` | CLIP | 예 | - | 토큰화 및 인코딩에 사용되는 CLIP 모델입니다 |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `CONDITIONING` | CONDITIONING | 텍스트 토큰과 해상도 정보가 포함된 인코딩된 조건 데이터입니다 |
