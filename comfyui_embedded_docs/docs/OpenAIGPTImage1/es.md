...
Genera imágenes de forma sincrónica a través del endpoint GPT Image de OpenAI. Puede crear imágenes nuevas a partir de un prompt de texto o editar imágenes existentes cuando se proporcionan una imagen de entrada y una máscara opcional. El nodo admite los modelos `gpt-image-1`, `gpt-image-1.5` y `gpt-image-2`, y está marcado como obsoleto.
...
| `model` | Modelo GPT Image a usar (predeterminado: "gpt-image-2") | COMBO | No | "gpt-image-1"<br>"gpt-image-1.5"<br>"gpt-image-2" |
| `custom_width` | Se usa solo cuando `size` es "Custom". Debe ser múltiplo de 16 (solo GPT Image 2) (predeterminado: 1024) | INT | No | 1024 a 3840, paso 16 |
| `custom_height` | Se usa solo cuando `size` es "Custom". Debe ser múltiplo de 16 (solo GPT Image 2) (predeterminado: 1024) | INT | No | 1024 a 3840, paso 16 |
...
|------------------|-------------|---------------|
| `IMAGE` | Imagen(es) generada(s) o editada(s). Las imágenes múltiples se devuelven como un lote; si las imágenes devueltas tienen dimensiones diferentes, se redimensionan para coincidir con la primera imagen. | IMAGE |
