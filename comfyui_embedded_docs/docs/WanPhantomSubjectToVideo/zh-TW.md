...
| `negative` | CONDITIONING | 是 | - | 用於避免特定特徵的負向條件化輸入 |
| `vae` | VAE | 是 | - | 用於在提供影像時進行編碼的 VAE 模型 |
| `width` | INT | 是 | 16 至 MAX_RESOLUTION | 輸出影片的寬度（像素）（預設值：832，必須能被 16 整除） |
| `height` | INT | 是 | 16 至 MAX_RESOLUTION | 輸出影片的高度（像素）（預設值：480，必須能被 16 整除） |
| `length` | INT | 是 | 1 至 MAX_RESOLUTION | 生成影片的幀數（預設值：81，必須能被 4 整除） |
| `batch_size` | INT | 是 | 1 至 4096 | 同時生成的影片數量（預設值：1） |
| `images` | IMAGE | 否 | - | 用於時間維度條件化的可選參考影像 |
...
| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `positive` | CONDITIONING | 當提供影像時，經過時間維度串接修改後的正向條件化資料 |
| `negative_text` | CONDITIONING | 當提供影像時，經過時間維度串接修改後的負向條件化資料 |
| `negative_img_text` | CONDITIONING | 當提供影像時，具有歸零時間維度串接的負向條件化資料 |
