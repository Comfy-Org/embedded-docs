...
| 参数 | 数据类型 | 是否必填 | 范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `positive` | CONDITIONING | 是 | - | 用于引导视频生成的正面条件化输入 |
| `negative` | CONDITIONING | 是 | - | 用于避免特定特征的负面条件化输入 |
...
| `length` | INT | 是 | 1 至 MAX_RESOLUTION | 生成视频的帧数（默认值：81，必须能被 4 整除） |
| `batch_size` | INT | 是 | 1 至 4096 | 同时生成的视频数量（默认值：1） |
| `images` | IMAGE | 否 | - | 用于时间维度条件化的可选参考图像 |
...
| 输出名称 | 数据类型 | 描述 |
|-------------|-----------|-------------|
| `positive` | CONDITIONING | 当提供图像时，经过时间维度拼接修改后的正面条件化数据 |
| `negative_text` | CONDITIONING | 当提供图像时，经过时间维度拼接修改后的负面条件化数据 |
| `negative_img_text` | CONDITIONING | 当提供图像时，使用零值时间维度拼接的负面条件化数据 |
