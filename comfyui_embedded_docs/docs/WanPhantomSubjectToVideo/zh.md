...
| 输出名称 | 描述 | 数据类型 |
| --- | --- | --- |
| `positive` | 当提供图像时，经过时间维度拼接修改后的正面条件化数据 | CONDITIONING |
| `negative_text` | 当提供图像时，经过时间维度拼接修改后的负面条件化数据 | CONDITIONING |
| `negative_img_text` | 当提供图像时，使用零值时间维度拼接的负面条件化数据 | CONDITIONING |
