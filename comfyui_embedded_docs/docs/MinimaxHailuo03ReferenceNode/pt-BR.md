...
|-----------|-------------|-----------|----------|-------|
| `modelo` | Modelo a ser usado para geração de vídeo (padrão: "MiniMax H3"). A seleção de "MiniMax H3" fornece as configurações de `prompt`, `resolution`, `ratio`, `duration`, `reference_images`, `reference_videos` e `reference_audios` abaixo. | DYNAMIC_COMBO | Sim | "MiniMax H3" |
