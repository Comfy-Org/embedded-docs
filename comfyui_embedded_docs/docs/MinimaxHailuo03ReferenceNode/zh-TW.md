...
| 參數 | 描述 | 資料類型 | 必填 | 範圍 |
|-----------|-------------|-----------|----------|-------|
| `模型` | 用於影片生成的模型（預設值："MiniMax H3"）。選擇「MiniMax H3」會提供以下設定：`prompt`、`resolution`、`ratio`、`duration`、`reference_images`、`reference_videos` 和 `reference_audios`。 | DYNAMIC_COMBO | 是 | "MiniMax H3" |
