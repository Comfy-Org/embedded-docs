...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `寬度` | INT | 是 | 64 至 MAX_RESOLUTION | 潛在影片張量的寬度（預設值：768，步長：32） |
| `高度` | INT | 是 | 64 至 MAX_RESOLUTION | 潛在影片張量的高度（預設值：512，步長：32） |
| `長度` | INT | 是 | 1 至 MAX_RESOLUTION | 潛在影片中的影格數量（預設值：97，步長：8） |
