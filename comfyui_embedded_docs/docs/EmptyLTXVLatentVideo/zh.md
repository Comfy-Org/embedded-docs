...
| 参数 | 数据类型 | 必填 | 取值范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `width` | INT | 是 | 64 至 MAX_RESOLUTION | 潜在视频张量的宽度（默认值：768，步长：32） |
| `height` | INT | 是 | 64 至 MAX_RESOLUTION | 潜在视频张量的高度（默认值：512，步长：32） |
| `length` | INT | 是 | 1 至 MAX_RESOLUTION | 潜在视频的帧数（默认值：97，步长：8） |
| `batch_size` | INT | 否 | 1 至 4096 | 批次中生成的潜在视频数量（默认值：1） |
