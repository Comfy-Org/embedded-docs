...
|-----------|-----------|----------|-------|-------------|
| `pozitif` | CONDITIONING | Evet | - | Videonun ne içermesi gerektiğini açıklayan pozitif koşullandırma istemleri. |
| `negatif` | CONDITIONING | Evet | - | Videonun ne içermemesi gerektiğini açıklayan negatif koşullandırma istemleri. |
| `vae` | VAE | Evet | - | Başlangıç görüntüsünü gizli uzaya kodlamak için kullanılan VAE (Değişken Otomatik Kodlayıcı) modeli. |
| `genişlik` | INT | Hayır | 16 ila MAKS_ÇÖZÜNÜRLÜK | Çıktı video karelerinin piksel cinsinden genişliği. 16'ya bölünebilir olmalıdır. (varsayılan: 848) |
| `yükseklik` | INT | Hayır | 16 ila MAKS_ÇÖZÜNÜRLÜK | Çıktı video karelerinin piksel cinsinden yüksekliği. 16'ya bölünebilir olmalıdır. (varsayılan: 480) |
| `uzunluk` | INT | Hayır | 1 ila MAKS_ÇÖZÜNÜRLÜK | Video dizisindeki toplam kare sayısı. 4'ün katı olmalıdır. (varsayılan: 33) |
| `toplu_boyutu` | INT | Hayır | 1 ila 4096 | Tek bir grupta oluşturulacak video dizisi sayısı. (varsayılan: 1) |
| `başlangıç_görseli` | IMAGE | Hayır | - | Video oluşturmayı başlatmak için isteğe bağlı bir başlangıç görüntüsü. Sağlanırsa, kodlanır ve ilk kareleri koşullandırmak için kullanılır. Görüntünün yalnızca ilk `uzunluk` karesi kullanılır. |
...
|-------------|-----------|-------------|
| `negatif` | CONDITIONING | Artık kodlanmış başlangıç görüntüsünü veya CLIP görüş çıktısını içerebilen değiştirilmiş pozitif koşullandırma. |
| `latent` | CONDITIONING | Artık kodlanmış başlangıç görüntüsünü veya CLIP görüş çıktısını içerebilen değiştirilmiş negatif koşullandırma. |
