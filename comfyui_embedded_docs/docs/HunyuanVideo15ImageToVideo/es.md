...
|-----------|-----------|----------|-------|-------------|
| `positivo` | CONDITIONING | Sí | - | Los mensajes de condicionamiento positivo que describen lo que debe contener el video. |
| `negativo` | CONDITIONING | Sí | - | Los mensajes de condicionamiento negativo que describen lo que debe evitar el video. |
| `vae` | VAE | Sí | - | El modelo VAE (Autoencoder Variacional) utilizado para codificar la imagen de inicio en el espacio latente. |
| `ancho` | INT | No | 16 a RESOLUCIÓN_MÁXIMA | El ancho de los fotogramas del video de salida en píxeles. Debe ser divisible por 16. (predeterminado: 848) |
| `alto` | INT | No | 16 a RESOLUCIÓN_MÁXIMA | La altura de los fotogramas del video de salida en píxeles. Debe ser divisible por 16. (predeterminado: 480) |
| `longitud` | INT | No | 1 a RESOLUCIÓN_MÁXIMA | El número total de fotogramas en la secuencia de video. Debe ser múltiplo de 4. (predeterminado: 33) |
| `tamaño_de_lote` | INT | No | 1 a 4096 | La cantidad de secuencias de video a generar en un solo lote. (predeterminado: 1) |
| `imagen_inicial` | IMAGE | No | - | Una imagen de inicio opcional para inicializar la generación del video. Si se proporciona, se codifica y se utiliza para condicionar los primeros fotogramas. Solo se utilizan los primeros `longitud` fotogramas de la imagen. |
...
|-------------|-----------|-------------|
| `negativo` | CONDITIONING | El condicionamiento positivo modificado, que ahora puede incluir la imagen de inicio codificada o la salida de CLIP vision. |
| `latente` | CONDITIONING | El condicionamiento negativo modificado, que ahora puede incluir la imagen de inicio codificada o la salida de CLIP vision. |
