...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `text_encoder` | STRING | 是 | 提供多個選項 | 要載入的 LTXV 文字編碼器模型檔案名稱。可用的選項從 `text_encoders` 資料夾中載入。 |
| `ckpt_name` | STRING | 是 | 提供多個選項 | 要載入的檢查點檔案名稱。可用的選項從 `checkpoints` 資料夾中載入。 |
| `device` | STRING | 否 | `"default"`<br>`"cpu"` | 指定要載入模型的裝置。使用 `"cpu"` 強制載入到 CPU。預設行為（`"default"`）使用系統的自動裝置配置。 |
...
| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `clip` | CLIP | 已載入的 LTXV CLIP 模型，可用於編碼音訊生成的文字提示。 |
