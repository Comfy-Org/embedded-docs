...
This node loads a specialized text encoder for the LTXV audio model. It combines a text encoder file with a checkpoint file to create a CLIP model that can be used for audio-related text conditioning tasks. According to the node's recipe description, the LTXV audio text encoder should be a Gemma 3 12B model or a matching Gemma 4 model.
...
| `device` | Specifies the device to load the model onto. Use `"cpu"` to force loading onto the CPU. The default behavior (`"default"`) uses the system's automatic device placement (default: `"default"`). | COMBO | No | `"default"`<br>`"cpu"` |
