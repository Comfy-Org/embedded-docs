...
| 매개변수 | 데이터 타입 | 설명 |
|-----------|-------------|-------------|
| `samples` | `LATENT`    | 'samples' 매개변수는 이미지로 디코딩될 잠재 표현을 나타냅니다. 이미지가 재구성되는 압축 데이터를 제공하므로 디코딩 과정에서 핵심적인 역할을 합니다. |
| `vae`     | VAE       | 'vae' 매개변수는 잠재 표현을 이미지로 디코딩하는 데 사용할 Variational Autoencoder 모델을 지정합니다. 디코딩 메커니즘과 재구성된 이미지의 품질을 결정하는 데 필수적입니다. |

## 출력

| 매개변수 | 데이터 타입 | 설명 |
|-----------|-------------|-------------|
| `image`   | `IMAGE`     | 출력은 지정된 VAE 모델을 사용하여 제공된 잠재 표현으로부터 재구성된 이미지입니다. |