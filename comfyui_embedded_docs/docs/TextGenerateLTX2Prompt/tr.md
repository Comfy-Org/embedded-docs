...
| `istem` | STRING | Evet | | Kullanıcıdan alınan ve geliştirilecek veya tamamlanacak ham metin girdisi. |
| `maksimum_uzunluk` | INT | Evet | | Dil modelinin üretmesine izin verilen maksimum token sayısı. |
| `örnekleme_modu` | COMBO | Evet | `"greedy"`<br>`"top_k"`<br>`"top_p"`<br>`"temperature"` | Metin üretimi sırasında bir sonraki tokeni seçmek için kullanılan örnekleme stratejisi. |
| `görsel` | IMAGE | Hayır | | İsteğe bağlı bir girdi görseli. Sağlandığında, düğüm görsel bağlamı için bir yer tutucu içeren farklı bir sistem yönergesi kullanır. |
| `düşünme` | BOOLEAN | Hayır | | Etkinleştirildiğinde, model nihai yanıttan önce akıl yürütme sürecini çıktı olarak verir. |
...
| `ses` | AUDIO | Hayır | | Üretim için ek bağlam olarak kullanılabilecek isteğe bağlı bir ses girdisi. |
