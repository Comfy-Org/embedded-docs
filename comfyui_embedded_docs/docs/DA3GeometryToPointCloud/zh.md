...
**注意：**
- `batch_index` 必须小于输入几何体的批次大小；否则节点将引发错误。
- 如果过滤移除所有像素，节点将引发错误。降低 `confidence_threshold` 或禁用 `use_sky_mask` 可以避免此问题。
- `confidence_threshold` 仅在几何体包含置信度图时生效（Small/Base 模型）。
- `use_sky_mask` 仅在几何体包含天空图时生效（Mono/Metric 模型）。
- 当 `downsample` 大于 1 时，相机内参会相应缩放，以确保投影点保持正确。

## 输出

| 输出名称 | 描述 | 数据类型 |
|-------------|-------------|-----------|
| `point_cloud` | 包含过滤后的 3D 点、可选颜色和可选置信度值的点云 | DA3_POINT_CLOUD |
