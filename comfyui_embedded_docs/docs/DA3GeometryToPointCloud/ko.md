...
**참고:**
- `batch_index`는 입력 지오메트리의 배치 크기보다 작아야 합니다. 그렇지 않으면 노드에서 오류가 발생합니다.
- 필터링으로 모든 픽셀이 제거되면 노드에서 오류가 발생합니다. `confidence_threshold`를 낮추거나 `use_sky_mask`를 비활성화하면 방지할 수 있습니다.
- `confidence_threshold`는 지오메트리에 신뢰도 맵이 포함된 경우에만 적용됩니다(Small/Base 모델).
- `use_sky_mask`는 지오메트리에 하늘 맵이 포함된 경우에만 적용됩니다(Mono/Metric 모델).
- `downsample`이 1보다 크면 카메라 내부 파라미터가 그에 따라 조정되어 투영된 점이 올바르게 유지됩니다.

## 출력

| 출력 이름 | 설명 | 데이터 타입 |
|-------------|-------------|-----------|
| `point_cloud` | 필터링된 3D 포인트, 선택적 색상 및 선택적 신뢰도 값을 포함하는 포인트 클라우드 | DA3_POINT_CLOUD |
