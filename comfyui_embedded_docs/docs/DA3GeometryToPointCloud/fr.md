...
**Remarques :**
- `batch_index` doit être inférieur à la taille du lot de la géométrie d'entrée ; sinon, le nœud génère une erreur.
- Si le filtrage supprime tous les pixels, le nœud génère une erreur. Réduire `confidence_threshold` ou désactiver `use_sky_mask` peut l'éviter.
- `confidence_threshold` ne s'applique que lorsque la géométrie inclut une carte de confiance (modèles Small/Base).
- `use_sky_mask` ne s'applique que lorsque la géométrie inclut une carte de ciel (modèles Mono/Metric).
- Lorsque `downsample` est supérieur à 1, les intrinsèques de la caméra sont mises à l'échelle en conséquence afin que les points projetés restent corrects.

## Sorties

| Nom de sortie | Description | Type de données |
|---------------|-------------|-----------------|
| `point_cloud` | Un nuage de points contenant les points 3D filtrés, les couleurs optionnelles et les valeurs de confiance optionnelles | DA3_POINT_CLOUD |
