...
|-----------|-----------------|--------|-------|-------------|
| `prompt_text` | STRING | Oui | - | Description textuelle pour guider la génération vidéo. |
| `seed` | INT | Non | 0 à 18446744073709551615 | La graine aléatoire utilisée pour créer le bruit (par défaut : 0). |
| `first_frame_image` | IMAGE | Non | - | Image facultative à utiliser comme première image pour générer une vidéo. |
| `prompt_optimizer` | BOOLEAN | Non | - | Optimiser la description pour améliorer la qualité de génération si nécessaire (par défaut : True). |
| `duration` | COMBO | Non | `6`<br>`10` | La durée de la vidéo de sortie en secondes (par défaut : 6). |
| `resolution` | COMBO | Non | `"768P"`<br>`"1080P"` | Les dimensions de l'affichage vidéo. 1080p correspond à 1920x1080, 768p à 1366x768 (par défaut : "768P"). |
