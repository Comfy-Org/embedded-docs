...
| `seed` | INT | 否 | 0 到 18446744073709551615 | 用于生成噪声的随机种子（默认值：0）。 |
| `first_frame_image` | IMAGE | 否 | - | 可选参数，用作视频第一帧的图像。 |
| `prompt_optimizer` | BOOLEAN | 否 | - | 优化提示词以在需要时提升生成质量（默认值：True）。 |
| `duration` | COMBO | 否 | `6`<br>`10` | 输出视频的时长（秒）（默认值：6）。 |
| `resolution` | COMBO | 否 | `"768P"`<br>`"1080P"` | 视频显示的分辨率。1080P 为 1920x1080，768P 为 1366x768（默认值："768P"）。 |
