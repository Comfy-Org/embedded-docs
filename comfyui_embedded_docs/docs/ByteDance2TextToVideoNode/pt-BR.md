...
Este nó gera um vídeo a partir de um prompt de texto usando os modelos Seedance 2.5 ou 2.0 da ByteDance. Ele envia o prompt ao modelo selecionado, aguarda o processamento do vídeo terminar e retorna o arquivo de vídeo resultante. Selecionar o modelo `Seedance 2.5 Draft` renderiza uma prévia rápida em 480p; conecte o `draft_task_id` resultante ao nó ByteDance Seedance 2.5 Draft to Final Video para renderizar o final em 1080p.
...
Estas entradas aparecem quando `Seedance 2.5 Draft` é selecionado. O conjunto de parâmetros corresponde ao do Seedance 2.5 acima, exceto que `resolution` oferece apenas `"480p"` (padrão `"480p"`).
...
Compartilhadas por Seedance 2.0 Fast e Seedance 2.0 Mini; ambos os modelos expõem os mesmos parâmetros.
...
**Nota:** O seletor `model` é dinâmico; as entradas exibidas em cada seção de modelo aparecem quando esse modelo é selecionado. O prompt deve ter pelo menos 1 caractere após a remoção de espaços em branco. Os limites de resolução e duração dependem do modelo selecionado: o Seedance 2.5 oferece suporte a 480p/720p/1080p e 4 a 30 segundos; o Seedance 2.0 oferece suporte a 480p/720p/1080p/4k e 4 a 15 segundos; e o Seedance 2.0 Fast e o Seedance 2.0 Mini oferecem suporte apenas a 480p/720p e 4 a 15 segundos; o Seedance 2.5 Draft oferece suporte apenas a 480p e 4 a 30 segundos. A saída `draft_task_id` é produzida apenas pelo Seedance 2.5 Draft, portanto, com qualquer outro modelo, ela deve ser deixada desconectada, caso contrário, a execução falha. O valor de `seed` controla apenas se o nó será reexecutado; ele não torna os resultados determinísticos.
...
| `draft_task_id` | ID da tarefa da execução de rascunho. Apenas o modelo Seedance 2.5 Draft o produz; conecte-o ao nó ByteDance Seedance 2.5 Draft to Final Video para renderizar o final em 1080p. | STRING |
