...
|-----------|-------------|-----------|----------|-------|
| `modelo` | El modelo Seedance que se usará para la generación de video. Seedance 2.5 es el modelo más reciente, admite videos de hasta 30 segundos y salida mp4/mov; Seedance 2.5 Draft renderiza una vista previa rápida en 480p cuya salida `draft_task_id` genera el final en 1080p en el nodo ByteDance Seedance 2.5 Draft to Final Video; Seedance 2.0 es para máxima calidad y 4k; Seedance 2.0 Fast está optimizado para velocidad; Seedance 2.0 Mini es para la generación más rápida y de menor costo. Seleccionar un modelo muestra entradas adicionales para el prompt, la resolución, la relación de aspecto, la duración y la generación de audio. | DYNAMIC_COMBO | Sí | `"Seedance 2.5"`<br>`"Seedance 2.5 Draft"`<br>`"Seedance 2.0"`<br>`"Seedance 2.0 Fast"`<br>`"Seedance 2.0 Mini"` |
...
| Parámetro | Descripción | Tipo de datos | Requerido | Rango |
|-----------|-------------|-----------|----------|-------|
| `prompt` | Prompt de texto para la generación de video. Coloca las líneas habladas entre comillas dobles para dirigir el diálogo generado. | STRING | Sí | — |
...
| `generate_audio` | Habilitar la generación de audio para el video de salida. (predeterminado: True) | BOOLEAN | Sí | True / False |
| `output_format` | Formato de contenedor del video de salida. (predeterminado: `"mp4"`) | COMBO | Sí | `"mp4"` |

### Entradas de Seedance 2.5 Draft

Estas entradas aparecen cuando se selecciona `Seedance 2.5 Draft`. El conjunto de parámetros coincide con el de Seedance 2.5 anterior, excepto que `resolution` solo ofrece `"480p"` (predeterminado `"480p"`).
...
**Nota:** El selector `model` es dinámico; las entradas que se muestran bajo cada sección de modelo aparecen cuando se selecciona ese modelo. El prompt debe tener al menos 1 carácter después de eliminar los espacios en blanco. Los límites de resolución y duración dependen del modelo seleccionado: Seedance 2.5 admite 480p/720p/1080p y de 4 a 30 segundos, Seedance 2.0 admite 480p/720p/1080p/4k y de 4 a 15 segundos, y Seedance 2.0 Fast y Seedance 2.0 Mini solo admiten 480p/720p y de 4 a 15 segundos; Seedance 2.5 Draft solo admite 480p y de 4 a 30 segundos. La salida `draft_task_id` solo la produce Seedance 2.5 Draft, por lo que con cualquier otro modelo debe dejarse desconectada; de lo contrario, la ejecución falla. El valor de `seed` solo controla si el nodo se vuelve a ejecutar; no hace que los resultados sean deterministas.
...
| `draft_task_id` | ID de tarea de la ejecución de borrador. Solo el modelo Seedance 2.5 Draft lo produce; conéctalo al nodo ByteDance Seedance 2.5 Draft to Final Video para renderizar el final en 1080p. | STRING |
