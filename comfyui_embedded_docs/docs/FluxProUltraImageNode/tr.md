...
| `prompt_upsampling` | BOOLEAN | Hayır | - | Prompt üzerinde yukarı örnekleme yapılıp yapılmayacağı. Aktif olduğunda, prompt'u daha yaratıcı oluşturum için otomatik olarak değiştirir, ancak sonuçlar belirsizdir (aynı seed tam olarak aynı sonucu üretmez). (varsayılan: False) |
| `seed` | INT | Hayır | 0 ile 18446744073709551615 | Gürültü oluşturmak için kullanılan rastgele seed. (varsayılan: 0) |
| `aspect_ratio` | STRING | Hayır | - | Görüntünün en-boy oranı; 1:4 ile 4:1 arasında olmalıdır. (varsayılan: "16:9") |
| `raw` | BOOLEAN | Hayır | - | True olduğunda, daha az işlenmiş, daha doğal görünümlü görüntüler oluşturur. (varsayılan: False) |
| `image_prompt` | IMAGE | Hayır | - | Oluşturumu yönlendirmek için isteğe bağlı referans görüntü |
| `image_prompt_strength` | FLOAT | Hayır | 0.0 ile 1.0 | Prompt ile görüntü prompt'u arasındaki karışım oranı. (varsayılan: 0.1) |
