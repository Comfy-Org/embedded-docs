...
| `prompt_upsampling` | BOOLEAN | 否 | - | 是否对提示词执行上采样。如果启用，会自动修改提示词以获得更具创造性的生成效果，但结果具有非确定性（相同的种子不会产生完全相同的结果）。（默认：False） |
| `seed` | INT | 否 | 0 到 18446744073709551615 | 用于创建噪声的随机种子。（默认：0） |
| `aspect_ratio` | STRING | 否 | - | 图像的宽高比；必须在 1:4 到 4:1 之间。（默认："16:9"） |
| `raw` | BOOLEAN | 否 | - | 当为 True 时，生成处理较少、更自然的图像。（默认：False） |
| `image_prompt` | IMAGE | 否 | - | 可选的参考图像，用于引导生成 |
| `image_prompt_strength` | FLOAT | 否 | 0.0 到 1.0 | 提示词与图像提示之间的混合程度。（默认：0.1） |
