...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `提示詞` | STRING | 是 | - | 影像生成的提示詞（預設：空字串） |
| `提示詞向上採樣` | BOOLEAN | 否 | - | 是否對提示詞進行上取樣。若啟用，將自動修改提示詞以產生更具創意的生成結果，但結果不具確定性（相同的種子不會產生完全相同的結果）。（預設：False） |
| `種子` | INT | 否 | 0 到 18446744073709551615 | 用於建立雜訊的隨機種子。（預設：0） |
| `長寬比` | STRING | 否 | - | 影像的長寬比；必須介於 1:4 和 4:1 之間。（預設："16:9"） |
| `原始` | BOOLEAN | 否 | - | 設為 True 時，生成較少處理、更自然的影像。（預設：False） |
| `影像提示` | IMAGE | 否 | - | 可選的參考影像，用於引導生成 |
| `影像提示強度` | FLOAT | 否 | 0.0 到 1.0 | 提示詞與影像提示詞之間的混合程度。（預設：0.1） |
