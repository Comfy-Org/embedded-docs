...
|-------------------|----------|----------|--------|------------|----------------------------------------------|
| `image`           | IMAGE    | 输入     | -      | -          | 需要进行边缘提取的原始照片                   |
| `low_threshold`   | FLOAT    | 控件     | 0.4    | 0.01-0.99  | 低阈值，决定忽略多弱的边缘。值越低保留更多细节，但可能产生噪点 |
| `high_threshold`  | FLOAT    | 控件     | 0.8    | 0.01-0.99  | 高阈值，决定保留多强的边缘。值越高只保留最明显的轮廓线 |

## 输出

| 输出名    | 数据类型 | 说明                                       |
|-----------|----------|--------------------------------------------|
| `image`   | IMAGE    | 黑白边缘图像，白色线条为检测到的边缘，黑色区域为无边缘部分 |
