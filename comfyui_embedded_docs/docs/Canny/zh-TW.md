...
|-------------------|-----------|------------|--------|------------|----------|
| `image`           | IMAGE     | 輸入       | -      | -          | 需要提取邊緣的原始照片 |
| `low_threshold`   | FLOAT     | 控件       | 0.4    | 0.01-0.99  | 低閾值，決定忽略多微弱的邊緣。較低值保留更多細節但可能產生噪點 |
| `high_threshold`  | FLOAT     | 控件       | 0.8    | 0.01-0.99  | 高閾值，決定保留多強烈的邊緣。較高值僅保留最明顯的輪廓線條 |

## 輸出結果

| 輸出名稱 | 資料類型 | 描述 |
|----------|-----------|------|
| `image`  | IMAGE     | 黑白邊緣影像，白色線條是檢測到的邊緣，黑色區域是沒有邊緣的部分 |
