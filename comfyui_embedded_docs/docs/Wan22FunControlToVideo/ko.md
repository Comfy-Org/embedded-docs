...
|-----------|-----------|----------|-------|-------------|
| `positive` | CONDITIONING | 예 | - | 비디오 생성을 안내하는 양성 컨디셔닝 입력 |
| `negative` | CONDITIONING | 예 | - | 비디오 생성을 안내하는 음성 컨디셔닝 입력 |
...
| `batch_size` | INT | 예 | 1 ~ 4096 | 생성할 비디오 시퀀스 수(기본값: 1) |
| `ref_image` | IMAGE | 아니요 | - | 시각적 안내를 제공하는 선택적 참조 이미지 |
| `control_video` | IMAGE | 아니요 | - | 생성 과정을 안내하는 선택적 제어 비디오 |
...
| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `positive` | CONDITIONING | 연결 잠재, 마스크 및 선택적 참조 잠재를 포함한 비디오별 잠재 데이터가 적용된 수정된 양성 컨디셔닝 |
| `negative` | CONDITIONING | 연결 잠재, 마스크 및 선택적 참조 잠재를 포함한 비디오별 잠재 데이터가 적용된 수정된 음성 컨디셔닝 |
| `latent` | LATENT | 배치 크기, 잠재 채널 및 공간/시간적 스케일링을 기반으로 비디오 생성에 적합한 차원을 가진 빈 잠재 텐서 |
