...
|-----------|-----------|----------|-------|-------------|
| `긍정 프롬프트` | CONDITIONING | 예 | - | 비디오 생성을 안내하는 양성 컨디셔닝 입력 |
| `부정 프롬프트` | CONDITIONING | 예 | - | 비디오 생성을 안내하는 음성 컨디셔닝 입력 |
...
| `배치 크기` | INT | 예 | 1 ~ 4096 | 생성할 비디오 시퀀스 수(기본값: 1) |
| `참조 이미지` | IMAGE | 아니요 | - | 시각적 안내를 제공하는 선택적 참조 이미지 |
| `제어 비디오` | IMAGE | 아니요 | - | 생성 과정을 안내하는 선택적 제어 비디오 |
...
| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `부정 프롬프트` | CONDITIONING | 연결 잠재, 마스크 및 선택적 참조 잠재를 포함한 비디오별 잠재 데이터가 적용된 수정된 양성 컨디셔닝 |
| `잠재 공간` | CONDITIONING | 연결 잠재, 마스크 및 선택적 참조 잠재를 포함한 비디오별 잠재 데이터가 적용된 수정된 음성 컨디셔닝 |
