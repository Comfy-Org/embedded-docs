...
|-----------|-----------|----------|-------|-------------|
| `control_net` | CONTROL_NET | 예 | - | 새 유형 설정으로 수정할 제어 네트워크입니다 |
| `type` | STRING | 예 | `"auto"`<br>사용 가능한 모든 UNION_CONTROLNET_TYPES 키 | 적용할 제어 네트워크 유형입니다. 자동 유형 감지를 위해 "auto"를 사용하거나 사용 가능한 옵션에서 특정 제어 네트워크 유형을 선택합니다 |

## 출력

| 출력 이름 | 데이터 유형 | 설명 |
|-------------|-----------|-------------|
| `control_net` | CONTROL_NET | 지정된 유형 설정이 적용된 수정된 제어 네트워크입니다 |
