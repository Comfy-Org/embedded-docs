...
Le nœud SetUnionControlNetType vous permet de choisir le type de contrôle utilisé par un réseau de contrôle. Il prend un réseau de contrôle existant et crée une copie modifiée avec le type de contrôle sélectionné, sans modifier le réseau de contrôle d'origine. Lorsque « auto » est sélectionné, le type de contrôle stocké est effacé afin que le type puisse être détecté automatiquement.
...
| --- | --- | --- |
| `control_net` | Le réseau de contrôle modifié avec le réglage de type spécifié appliqué | CONTROL_NET |
