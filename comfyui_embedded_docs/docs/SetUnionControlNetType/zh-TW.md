...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `control_net` | CONTROL_NET | 是 | - | 要修改並套用新類型設定的控制網路 |
| `type` | STRING | 是 | `"auto"`<br>所有可用的 UNION_CONTROLNET_TYPES 鍵 | 要套用的控制網路類型。使用 "auto" 進行自動類型偵測，或從可用選項中選擇特定的控制網路類型 |

## 輸出

| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `control_net` | CONTROL_NET | 已套用指定類型設定的修改版控制網路 |
