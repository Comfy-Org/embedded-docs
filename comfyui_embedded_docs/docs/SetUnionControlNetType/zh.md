...
|-----------|-----------|----------|-------|-------------|
| `control_net` | CONTROL_NET | 是 | - | 需要修改类型设置的控制网络 |
| `type` | STRING | 是 | `"auto"`<br>所有可用的 UNION_CONTROLNET_TYPES 键 | 要应用的控制网络类型。使用 "auto" 进行自动类型检测，或从可用选项中选择特定的控制网络类型 |

## 输出

| 输出名称 | 数据类型 | 描述 |
|-------------|-----------|-------------|
| `control_net` | CONTROL_NET | 已应用指定类型设置的修改后控制网络 |
