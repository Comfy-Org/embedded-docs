...
`GenerateTracks` 節點會為影片產生多條平行運動路徑。它會先定義一條從起點到終點的主路徑，再產生一組與此路徑平行且間距相等的軌道。您可以控制路徑的形狀（直線或貝茲曲線）、沿路徑移動的速度，以及軌道在哪些影格中可見。
...
| `num_frames` | 要產生軌道位置的總影格數。預設值為 81。 | INT | 是 | 1 - 1024 |
| `num_tracks` | 要產生的平行軌道數量。預設值為 5。 | INT | 是 | 1 - 100 |
| `track_spread` | 軌道之間的標準化距離。軌道會垂直於運動方向展開。預設值為 0.025。 | FLOAT | 是 | 0.0 - 1.0 |
...
| `interpolation` | 控制沿路徑移動的時間／速度。預設值為 "linear"。使用 "constant" 時，所有點都會停留在起點位置。 | COMBO | 是 | `"linear"`<br>`"ease_in"`<br>`"ease_out"`<br>`"ease_in_out"`<br>`"constant"` |
| `track_mask` | 可選的遮罩，用於指示可見影格。 | MASK | 否 | - |

**注意：** `mid_x` 和 `mid_y` 參數僅在 `bezier` 參數設定為 `True` 時使用。當 `bezier` 為 `False` 時，路徑為從起點到終點的直線。

## 輸出

| 輸出名稱 | 描述 | 資料類型 |
| --- | --- | --- |
| `TRACKS` | 包含所有影格中所有軌道的生成路徑座標與可見性資訊的軌道物件。 | TRACKS |
| `track_length` | 產生軌道的影格數，與輸入的 `num_frames` 相符。 | INT |
