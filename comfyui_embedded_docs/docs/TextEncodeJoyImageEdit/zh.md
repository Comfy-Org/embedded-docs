...
| `vae` | 用于将图像编码到潜在空间的 VAE 模型 | VAE | 否 | - |
| `图像` | 可选的图像，包含在条件数据中，最多 6 张。每张图像作为独立输入连接（image_0 到 image_5） | IMAGE | 否 | 0 到 6 张图像 |

## 输出
`images` 输入是一个自动增长组，接受 0 到 6 张图像。当未连接任何图像时，条件数据仅基于文本提示。
| 输出名 | 描述 | 数据类型 |
|-------------|-------------|-----------|
