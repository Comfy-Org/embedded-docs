...
|-----------|-----------|----------|-------|-------------|
| `model` | MODEL | 예 | - | 적응형 투영 안내를 적용할 확산 모델 |
| `eta` | FLOAT | 예 | -10.0 ~ 10.0 | 평행 안내 벡터의 크기를 제어합니다. 설정값 1에서 기본 CFG 동작을 나타냅니다(기본값: 1.0). |
| `norm_threshold` | FLOAT | 예 | 0.0 ~ 50.0 | 안내 벡터를 이 값으로 정규화하며, 설정값 0에서는 정규화가 비활성화됩니다(기본값: 5.0). |
| `momentum` | FLOAT | 예 | -5.0 ~ 1.0 | 확산 과정 중 안내의 이동 평균을 제어하며, 설정값 0에서는 비활성화됩니다(기본값: 0.0). |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-----------|-----------|-------------|
| `model` | MODEL | 샘플링 과정에 적응형 투영 안내가 적용된 수정된 모델을 반환합니다 |
