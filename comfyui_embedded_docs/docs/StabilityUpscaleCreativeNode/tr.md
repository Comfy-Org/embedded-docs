...
| `prompt` | STRING | Evet | - | Çıktı görüntüsünde görmek istedikleriniz. Öğeleri, renkleri ve konuları net bir şekilde tanımlayan güçlü, betimleyici bir prompt daha iyi sonuçlara yol açacaktır. (varsayılan: boş dize) |
| `creativity` | FLOAT | Evet | 0.1-0.5 | Başlangıç görüntüsü tarafından ağır şekilde koşullandırılmayan ek detaylar oluşturma olasılığını kontrol eder. (varsayılan: 0.3) |
| `style_preset` | COMBO | Evet | Birden fazla seçenek mevcut | İsteğe bağlı olarak oluşturulan görüntünün istediğiniz tarzı. Seçenekler arasında Stability AI'den çeşitli stil ön ayarları bulunur. |
| `seed` | INT | Evet | 0-4294967294 | Gürültü oluşturmak için kullanılan rastgele tohum değeri. (varsayılan: 0) |
| `negative_prompt` | STRING | Hayır | - | Çıktı görüntüsünde görmek *istemediklerinizin* anahtar kelimeleri. Bu gelişmiş bir özelliktir. (varsayılan: boş dize) |
