...
| `시작 프레임` | IMAGE | 예 | - | 생성된 비디오의 첫 번째 프레임으로 사용될 이미지입니다. 이미지는 최소 300x300픽셀이어야 하며, 종횡비가 1:2.5에서 2.5:1 사이여야 합니다. |
| `프롬프트` | STRING | 예 | - | 긍정 텍스트 프롬프트입니다. 생성하려는 비디오 콘텐츠를 설명합니다. 프롬프트는 1자에서 2500자 사이여야 합니다. |
| `모드` | COMBO | 예 | `"pro"` | 비디오 생성을 위한 작동 모드입니다. |
| `길이` | COMBO | 예 | `5`<br>`10` | 생성할 비디오의 길이(초)입니다. |
| `오디오 생성` | BOOLEAN | 아니요 | - | 활성화하면 노드가 비디오와 함께 오디오를 생성합니다. 비활성화하면 비디오가 무음이 됩니다. (기본값: True) |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `video` | VIDEO | 생성된 비디오 파일로, `오디오 생성` 입력에 따라 오디오가 포함될 수 있습니다. |
