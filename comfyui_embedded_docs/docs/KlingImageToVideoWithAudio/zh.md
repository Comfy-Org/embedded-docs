...
| `start_frame` | IMAGE | 是 | - | 作为生成视频首帧的图像。图像尺寸必须至少为 300x300 像素，宽高比需在 1:2.5 到 2.5:1 之间。 |
| `prompt` | STRING | 是 | - | 正向文本提示。描述您想要生成的视频内容。提示长度必须在 1 到 2500 个字符之间。 |
| `mode` | COMBO | 是 | `"pro"` | 视频生成的操作模式。 |
| `duration` | COMBO | 是 | `5`<br>`10` | 要生成的视频长度，单位为秒。 |
| `generate_audio` | BOOLEAN | 否 | - | 启用时，节点将生成音频配合视频；禁用时，视频将静音。（默认值：True） |

## 输出

| 输出名称 | 数据类型 | 描述 |
|-------------|-----------|-------------|
| `video` | VIDEO | 生成的视频文件，根据 `generate_audio` 输入可能包含音频。 |
