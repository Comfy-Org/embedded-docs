...
| `start_frame` | IMAGE | 是 | - | 將作為生成影片第一幀的圖片。圖片必須至少為 300x300 像素，且長寬比介於 1:2.5 到 2.5:1 之間。 |
| `prompt` | STRING | 是 | - | 正向文字提示。描述您想要生成的影片內容。提示長度必須在 1 到 2500 個字元之間。 |
| `mode` | COMBO | 是 | `"pro"` | 影片生成的操作模式。 |
| `duration` | COMBO | 是 | `5`<br>`10` | 要生成的影片長度，以秒為單位。 |
| `generate_audio` | BOOLEAN | 否 | - | 啟用時，節點將生成音頻來配合影片。停用時，影片將為無聲狀態。（預設值：True） |

## 輸出

| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `video` | VIDEO | 生成的影片檔案，根據 `generate_audio` 輸入的設定，可能包含音頻。 |
