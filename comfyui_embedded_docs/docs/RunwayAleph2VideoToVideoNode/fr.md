...
|-----------|-------------|-----------------|--------|-------|
| `prompt` | Décrit ce qui doit apparaître dans la sortie (1 à 1000 caractères). | STRING | Oui | 1 à 1000 caractères |
| `video` | Vidéo d'entrée à éditer. Doit durer 2 à 30 secondes à 30 ips ou moins. | VIDEO | Oui | Durée de 2 à 30 secondes, 30 ips max |
| `seed` | Graine aléatoire pour la génération (par défaut : 0). | INT | Oui | 0 à 4294967295 |
| `public_figure_threshold` | Modération du contenu pour les personnalités publiques reconnaissables (par défaut : "low"). | COMBO | Oui | "auto"<br>"low" |
...
| `prompt_images` | Images de guidage ancrées à la vidéo de sortie, provenant des nœuds Aleph2 Prompt Image (jusqu'à 5). Utilisez les keyframes ou les images d'invite, pas les deux. | PROMPT_IMAGE | Non | Jusqu'à 5 éléments |
...
|---------------|-------------|-----------------|
| `video` | La vidéo éditée générée par le modèle Aleph2. | VIDEO |
