...
| `batch_size` | INT | Sim | 1 a 4096 | O número de amostras latentes de áudio no lote. O valor padrão é 1. |
| `audio_vae` | VAE | Sim | N/A | O modelo Audio VAE para obter a configuração. Este parâmetro é obrigatório. |
...
|---------------|--------------|-----------|
| `Latent` | LATENT | Um tensor latente de áudio vazio com a estrutura (batch_size, z_channels, num_audio_latents, audio_freq) configurado para corresponder ao Audio VAE de entrada. A saída também inclui um campo `type` definido como "audio". |
