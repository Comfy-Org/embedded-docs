...
| `batch_size` | INT | 예 | 1 ~ 4096 | 배치 내 잠재 오디오 샘플 수입니다. 기본값은 1입니다. |
| `audio_vae` | VAE | 예 | 해당 없음 | 구성을 가져올 오디오 VAE 모델입니다. 이 매개변수는 필수입니다. |
...
| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `Latent` | LATENT | 입력 오디오 VAE와 일치하도록 구성된 (batch_size, z_channels, num_audio_latents, audio_freq) 구조의 빈 잠재 오디오 텐서입니다. 출력에는 "audio"로 설정된 `type` 필드도 포함됩니다. |
