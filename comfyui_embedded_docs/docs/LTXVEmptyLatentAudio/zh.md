...
| 参数 | 数据类型 | 是否必填 | 范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `frames_number` | INT | 是 | 1 至 1000 | 帧数。默认值为 97。 |
| `frame_rate` | INT | 是 | 1 至 1000 | 每秒帧数。默认值为 25。 |
| `batch_size` | INT | 是 | 1 至 4096 | 批次中的潜变量音频样本数量。默认值为 1。 |
| `audio_vae` | VAE | 是 | 不适用 | 用于获取配置的音频 VAE 模型。此参数为必填项。 |
