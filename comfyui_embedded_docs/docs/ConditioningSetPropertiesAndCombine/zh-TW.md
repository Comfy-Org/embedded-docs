...
| `strength` | FLOAT | 必要 | 1.0 | 0.0 - 10.0 | 控制新條件屬性的強度 |
| `set_cond_area` | STRING | 必要 | default | ["default", "mask bounds"] | 決定條件區域的套用方式 |
| `mask` | MASK | 可選 | - | - | 可選遮罩，用於定義條件的特定區域 |
| `hooks` | HOOKS | 可選 | - | - | 用於自訂處理的可選鉤子函數 |
| `timesteps` | TIMESTEPS_RANGE | 可選 | - | - | 可選時間步長範圍，用於控制條件套用的時機 |
