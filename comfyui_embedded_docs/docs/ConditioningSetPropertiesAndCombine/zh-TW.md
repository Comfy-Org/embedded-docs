...
| `strength` | FLOAT | 必填 | 1.0 | 0.0 - 10.0 | 控制新條件屬性的強度 |
| `set_cond_area` | STRING | 必填 | default | ["default", "mask bounds"] | 決定條件區域的應用方式 |
| `mask` | MASK | 選填 | - | - | 用於定義特定條件區域的選用遮罩 |
| `hooks` | HOOKS | 選填 | - | - | 用於自訂處理的選用掛鉤函數 |
| `timesteps` | TIMESTEPS_RANGE | 選填 | - | - | 用於控制條件應用時機的選用時間步範圍 |
