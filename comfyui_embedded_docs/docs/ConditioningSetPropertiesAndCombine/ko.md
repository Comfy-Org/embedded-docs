...
|-----------|-----------|------------|---------|-------|-------------|
| `조건` | CONDITIONING | 필수 | - | - | 수정할 원본 컨디셔닝 데이터 |
| `새 조건` | CONDITIONING | 필수 | - | - | 적용할 속성을 제공하는 새 컨디셔닝 데이터 |
| `강도` | FLOAT | 필수 | 1.0 | 0.0 - 10.0 | 새 컨디셔닝 속성의 강도를 제어합니다 |
| `조건 영역 설정` | STRING | 필수 | default | ["default", "mask bounds"] | 컨디셔닝 영역이 적용되는 방식을 결정합니다 |
| `마스크` | MASK | 선택 사항 | - | - | 컨디셔닝을 위한 특정 영역을 정의하는 선택적 마스크 |
| `후크` | HOOKS | 선택 사항 | - | - | 사용자 지정 처리를 위한 선택적 후크 함수 |
| `타임스텝 범위` | TIMESTEPS_RANGE | 선택 사항 | - | - | 컨디셔닝 적용 시점을 제어하는 선택적 타임스텝 범위 |
