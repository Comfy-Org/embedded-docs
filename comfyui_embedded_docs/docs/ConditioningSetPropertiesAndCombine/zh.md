...
|-----------|-----------|------------|---------|-------|-------------|
| `cond` | CONDITIONING | 必需 | - | - | 要修改的原始条件数据 |
| `cond_NEW` | CONDITIONING | 必需 | - | - | 提供要应用属性的新条件数据 |
| `strength` | FLOAT | 必需 | 1.0 | 0.0 - 10.0 | 控制新条件属性的强度 |
| `set_cond_area` | STRING | 必需 | default | ["default", "mask bounds"] | 确定条件区域的应用方式 |
| `mask` | MASK | 可选 | - | - | 可选遮罩，用于定义条件应用的特定区域 |
| `hooks` | HOOKS | 可选 | - | - | 可选钩子函数，用于自定义处理 |
| `timesteps` | TIMESTEPS_RANGE | 可选 | - | - | 可选时间步范围，用于控制条件应用的时间 |
