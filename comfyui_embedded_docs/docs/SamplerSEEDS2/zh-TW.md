...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `solver_type` | COMBO | 是 | `"phi_1"`<br>`"phi_2"` | 選擇取樣器的底層求解器演算法。 |
| `eta` | FLOAT | 否 | 0.0 - 100.0 | 隨機強度（預設值：1.0）。 |
| `s_noise` | FLOAT | 否 | 0.0 - 100.0 | SDE 噪聲倍數（預設值：1.0）。 |
| `r` | FLOAT | 否 | 0.01 - 1.0 | 中間階段（c2 節點）的相對步長（預設值：0.5）。 |

## 輸出

| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `sampler` | SAMPLER | 一個已配置的取樣器物件，可傳遞給其他取樣節點。 |
