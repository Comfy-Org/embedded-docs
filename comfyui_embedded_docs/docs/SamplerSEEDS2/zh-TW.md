...
此節點提供了一個可配置的取樣器，用於生成圖像。它實作了 SEEDS-2 演算法，這是一種隨機微分方程（SDE）求解器。透過調整其參數，您可以將其配置為類似多種特定取樣器的行為，包括 `seeds_2`、`exp_heun_2_x0` 和 `exp_heun_2_x0_sde`。

## 輸入

| 參數 | 描述 | 資料類型 | 必填 | 範圍 |
| --- | --- | --- | --- | --- |
| `solver_type` | 為取樣器選擇底層求解器演算法。 | COMBO | 是 | `"phi_1"`<br>`"phi_2"` |
| `eta` | 隨機強度（預設值：1.0）。 | FLOAT | 否 | 0.0 - 100.0 |
| `s_noise` | SDE 雜訊倍率（預設值：1.0）。 | FLOAT | 否 | 0.0 - 100.0 |
