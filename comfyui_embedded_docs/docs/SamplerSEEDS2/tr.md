...
Bu düğüm, görüntü üretimi için yapılandırılabilir bir örnekleyici sağlar. Stokastik diferansiyel denklem (SDE) çözücüsüne dayanır ve ayarlarına bağlı olarak `seeds_2`, `exp_heun_2_x0` ve `exp_heun_2_x0_sde` dahil olmak üzere birkaç belirli örnekleyici gibi davranabilir.
...
| `r` | Ara aşama (c2 düğümü) için göreli adım boyutu (varsayılan: 0.5). | FLOAT | Hayır | 0.01 - 1.0 |

**Not:** Düğüm açıklaması aşağıdaki örnekleyici ön ayarlarını tanımlar:
...
| --- | --- | --- |
| `sampler` | Diğer örnekleme düğümlerine aktarılabilecek yapılandırılmış bir örnekleyici nesnesi. | SAMPLER |
