...
Ce nœud fournit un échantillonneur configurable pour la génération d'images. Il est basé sur un solveur d'équations différentielles stochastiques (SDE) et peut se comporter comme plusieurs échantillonneurs spécifiques selon ses réglages, notamment `seeds_2`, `exp_heun_2_x0` et `exp_heun_2_x0_sde`.
...
| `eta` | Intensité stochastique (par défaut : 1.0). | FLOAT | Non | 0.0 - 100.0 |
...
**Remarque :** La description du nœud définit les préréglages d'échantillonneur suivants :
