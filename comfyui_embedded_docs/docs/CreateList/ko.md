...
|-----------|-----------|----------|-------|-------------|
| `input_*` | 다양함 | 예 | 모든 값 | 가변 개수의 입력 슬롯입니다. 더하기(+) 아이콘을 클릭하여 입력을 추가할 수 있습니다. 모든 입력은 동일한 데이터 타입(예: 모두 IMAGE 또는 모두 STRING)이어야 합니다. |
...
| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `list` | 다양함 | 연결된 입력의 모든 항목을 제공된 순서대로 연결한 단일 목록입니다. 출력 데이터 타입은 입력 데이터 타입과 일치합니다. |
