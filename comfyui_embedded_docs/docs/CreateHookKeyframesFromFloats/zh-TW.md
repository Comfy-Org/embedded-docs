...
此節點從一系列浮點強度值建立掛鉤關鍵影格，將它們均勻分佈在指定的起始和結束百分比之間。它會產生一系列關鍵影格，每個強度值都會被分配到動畫時間軸中的特定百分比位置。此節點可以建立新的關鍵影格群組，也可以添加到現有的群組中，並提供列印生成關鍵影格的選項以供除錯使用。

## 輸入參數

| 參數名稱 | 資料類型 | 是否必填 | 數值範圍 | 參數說明 |
|-----------|-----------|----------|-------|-------------|
| `floats_strength` | FLOATS | 是 | -1 至 ∞ | 單一浮點數值或浮點數值列表，代表關鍵影格的強度值（預設值：-1） |
...
**注意：** `floats_strength` 參數接受單一浮點數值或可迭代的浮點數列表。關鍵影格會根據提供的強度值數量，在 `start_percent` 和 `end_percent` 之間線性分佈。

## 輸出結果

| 輸出名稱 | 資料類型 | 輸出說明 |
|-------------|-----------|-------------|
| `HOOK_KF` | HOOK_KEYFRAMES | 包含新建立關鍵影格的掛鉤關鍵影格群組，可以是新群組或附加到輸入的關鍵影格群組 |