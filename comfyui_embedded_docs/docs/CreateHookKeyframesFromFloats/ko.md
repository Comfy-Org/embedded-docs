...
|-----------|-----------|----------|-------|-------------|
| `floats_strength` | FLOATS | 예 | -1 ~ ∞ | 키프레임의 강도 값을 나타내는 단일 부동소수점 값 또는 부동소수점 값 목록입니다(기본값: -1) |
| `start_percent` | FLOAT | 예 | 0.0 ~ 1.0 | 타임라인에서 첫 번째 키프레임의 시작 백분율 위치입니다(기본값: 0.0) |
| `end_percent` | FLOAT | 예 | 0.0 ~ 1.0 | 타임라인에서 마지막 키프레임의 종료 백분율 위치입니다(기본값: 1.0) |
| `print_keyframes` | BOOLEAN | 예 | True/False | 활성화하면 생성된 키프레임 정보를 콘솔에 출력합니다(기본값: False) |
| `prev_hook_kf` | HOOK_KEYFRAMES | 아니요 | - | 새 키프레임을 추가할 기존 후크 키프레임 그룹입니다. 제공되지 않으면 새 그룹을 생성합니다 |
...
| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `HOOK_KF` | HOOK_KEYFRAMES | 새로 생성된 키프레임을 포함하는 후크 키프레임 그룹입니다. 새 그룹이거나 입력 키프레임 그룹에 추가된 그룹입니다 |
