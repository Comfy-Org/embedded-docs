...
|-----------|-----------|----------|-------|-------------|
| `실수 강도` | FLOATS | 예 | -1 ~ ∞ | 키프레임의 강도 값을 나타내는 단일 부동소수점 값 또는 부동소수점 값 목록입니다(기본값: -1) |
| `시작 퍼센트` | FLOAT | 예 | 0.0 ~ 1.0 | 타임라인에서 첫 번째 키프레임의 시작 백분율 위치입니다(기본값: 0.0) |
| `종료 퍼센트` | FLOAT | 예 | 0.0 ~ 1.0 | 타임라인에서 마지막 키프레임의 종료 백분율 위치입니다(기본값: 1.0) |
| `키프레임 출력` | BOOLEAN | 예 | True/False | 활성화하면 생성된 키프레임 정보를 콘솔에 출력합니다(기본값: False) |
| `이전 KF 후크` | HOOK_KEYFRAMES | 아니요 | - | 새 키프레임을 추가할 기존 후크 키프레임 그룹입니다. 제공되지 않으면 새 그룹을 생성합니다 |
