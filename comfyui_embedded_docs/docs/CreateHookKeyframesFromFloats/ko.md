...
|-----------|-----------|----------|-------|-------------|
| `floats_strength` | FLOATS | 예 | -1 ~ ∞ | 키프레임의 강도 값을 나타내는 단일 float 값 또는 float 값 목록 (기본값: -1) |
| `start_percent` | FLOAT | 예 | 0.0 ~ 1.0 | 타임라인에서 첫 번째 키프레임의 시작 백분율 위치 (기본값: 0.0) |
| `end_percent` | FLOAT | 예 | 0.0 ~ 1.0 | 타임라인에서 마지막 키프레임의 끝 백분율 위치 (기본값: 1.0) |
| `print_keyframes` | BOOLEAN | 예 | True/False | 활성화하면 생성된 키프레임 정보를 콘솔에 출력합니다 (기본값: False) |
| `prev_hook_kf` | HOOK_KEYFRAMES | 아니오 | - | 새로운 키프레임을 추가할 기존 훅 키프레임 그룹. 제공되지 않으면 새 그룹을 생성합니다 |
