...
|-----------|--------------|-----------|-------|-------------|
| `floats_strength` | FLOATS | Sí | -1 a ∞ | Un valor flotante único o una lista de valores flotantes que representan los valores de intensidad para los fotogramas clave (predeterminado: -1) |
| `start_percent` | FLOAT | Sí | 0.0 a 1.0 | La posición porcentual inicial para el primer fotograma clave en la línea de tiempo (predeterminado: 0.0) |
| `end_percent` | FLOAT | Sí | 0.0 a 1.0 | La posición porcentual final para el último fotograma clave en la línea de tiempo (predeterminado: 1.0) |
| `print_keyframes` | BOOLEAN | Sí | True/False | Cuando está habilitado, imprime la información de los fotogramas clave generados en la consola (predeterminado: False) |
