...
| 参数 | 数据类型 | 是否必填 | 范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `floats_strength` | FLOATS | 是 | -1 到 ∞ | 表示关键帧强度值的单个浮点值或浮点值列表（默认值：-1） |
| `start_percent` | FLOAT | 是 | 0.0 到 1.0 | 时间轴上第一个关键帧的起始百分比位置（默认值：0.0） |
| `end_percent` | FLOAT | 是 | 0.0 到 1.0 | 时间轴上最后一个关键帧的结束百分比位置（默认值：1.0） |
| `print_keyframes` | BOOLEAN | 是 | True/False | 启用时，将生成的关键帧信息打印到控制台（默认值：False） |
| `prev_hook_kf` | HOOK_KEYFRAMES | 否 | - | 要将新关键帧添加到的现有钩子关键帧组，如果未提供则创建新组 |
