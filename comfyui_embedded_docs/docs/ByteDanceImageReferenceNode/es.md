...
|-----------|--------------|-----------|-------|-------------|
| `model` | STRING | Sí | `"seedance-1-0-pro-250528"`<br>`"seedance-1-0-lite-i2v-250428"` | El modelo de IA a utilizar para la generación de video (predeterminado: `"seedance-1-0-lite-i2v-250428"`). |
| `prompt` | STRING | Sí | - | El texto de descripción utilizado para generar el video. |
| `images` | IMAGE | Sí | - | De una a cuatro imágenes. Cada imagen debe tener entre 300x300 y 6000x6000 píxeles, con una relación de aspecto entre 0.4 y 2.5. |
| `resolution` | STRING | Sí | `"480p"`<br>`"720p"` | La resolución del video de salida. |
| `aspect_ratio` | STRING | Sí | `"adaptive"`<br>`"16:9"`<br>`"4:3"`<br>`"1:1"`<br>`"3:4"`<br>`"9:16"`<br>`"21:9"` | La relación de aspecto del video de salida (predeterminado: `"adaptive"`). |
| `duration` | INT | Sí | 3 - 12 | La duración del video de salida en segundos (predeterminado: 5). |
