...
|-----------|-----------|----------|-------|-------------|
| `positive` | CONDITIONING | Sí | - | El acondicionamiento positivo que guía la generación hacia el contenido deseado |
| `negative` | CONDITIONING | Sí | - | El acondicionamiento negativo que guía la generación lejos del contenido no deseado |
...
| `strength` | FLOAT | Sí | 0.0 a 10.0 | La fuerza del efecto ControlNet (valor por defecto: 1.0) |
| `start_percent` | FLOAT | Sí | 0.0 a 1.0 | El punto de inicio (como porcentaje) de cuándo comienza la influencia del ControlNet durante la generación (valor por defecto: 0.0) |
| `end_percent` | FLOAT | Sí | 0.0 a 1.0 | El punto final (como porcentaje) de cuándo se detiene la influencia del ControlNet durante la generación (valor por defecto: 1.0) |
...
|-------------|-----------|-------------|
| `positive` | CONDITIONING | El acondicionamiento positivo modificado con ControlNet aplicado para inpainting |
| `negative` | CONDITIONING | El acondicionamiento negativo modificado con ControlNet aplicado para inpainting |