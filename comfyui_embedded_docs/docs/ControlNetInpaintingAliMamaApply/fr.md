...
Le nœud ControlNetInpaintingAliMamaApply applique le conditionnement ControlNet pour les tâches d'inpainting en combinant les conditionnements positif et négatif avec une image de contrôle et un masque. Il traite l'image et le masque d'entrée pour créer un conditionnement modifié qui guide le processus de génération, permettant de contrôler les zones de l'image qui sont inpaintées. Le nœud prend en charge le réglage de la force et les contrôles de temporisation pour ajuster finement l'influence du ControlNet à différentes étapes du processus de génération.
...
| `mask` | Le masque qui définit les zones de l'image à inpaint | MASK | Oui | - |
| `strength` | La force de l'effet ControlNet (par défaut : 1.0, pas : 0.01) | FLOAT | Oui | 0.0 à 10.0 |
| `start_percent` | Paramètre avancé. Le point de départ (en pourcentage) à partir duquel l'influence du ControlNet commence pendant la génération (par défaut : 0.0, pas : 0.001) | FLOAT | Oui | 0.0 à 1.0 |
| `end_percent` | Paramètre avancé. Le point de fin (en pourcentage) auquel l'influence du ControlNet s'arrête pendant la génération (par défaut : 1.0, pas : 0.001) | FLOAT | Oui | 0.0 à 1.0 |
...
| --- | --- | --- |
| `positive` | Le conditionnement positif modifié avec ControlNet appliqué pour l'inpainting | CONDITIONING |
| `negative` | Le conditionnement négatif modifié avec ControlNet appliqué pour l'inpainting | CONDITIONING |
