...
ControlNetInpaintingAliMamaApply 節點透過結合正負向條件與控制圖像和遮罩，為修補任務應用 ControlNet 條件控制。它處理輸入圖像和遮罩以建立修改後的條件，引導生成過程，從而精確控制圖像中需要修補的區域。該節點支援強度調整和時序控制，可在生成過程的不同階段微調 ControlNet 的影響程度。
...
| `負向` | CONDITIONING | 是 | - | 引導生成遠離不期望內容的負向條件 |
...
| `起始百分比` | FLOAT | 是 | 0.0 至 1.0 | ControlNet 影響在生成過程中開始的起始百分比（預設值：0.0） |
| `結束百分比` | FLOAT | 是 | 0.0 至 1.0 | ControlNet 影響在生成過程中結束的終止百分比（預設值：1.0） |

**注意：** 當 ControlNet 啟用 `concat_mask` 功能時，遮罩會在處理前進行反轉並應用到圖像上，且遮罩會包含在傳送給 ControlNet 的額外串接資料中。
