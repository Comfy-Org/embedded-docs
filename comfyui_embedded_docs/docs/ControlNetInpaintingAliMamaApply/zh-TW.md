...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `positive` | CONDITIONING | 是 | - | 引導生成朝向所需內容的正向條件化 |
| `negative` | CONDITIONING | 是 | - | 引導生成遠離不需要內容的負向條件化 |
| `control_net` | CONTROL_NET | 是 | - | 提供對生成過程額外控制的 ControlNet 模型 |
| `vae` | VAE | 是 | - | 用於編碼和解碼影像的 VAE（變分自編碼器） |
| `image` | IMAGE | 是 | - | 作為 ControlNet 控制引導的輸入影像 |
| `mask` | MASK | 是 | - | 定義影像中哪些區域應被修補的遮罩 |
| `strength` | FLOAT | 是 | 0.0 至 10.0 | ControlNet 效果的強度（預設值：1.0） |
| `start_percent` | FLOAT | 是 | 0.0 至 1.0 | ControlNet 影響在生成過程中開始的起點（以百分比表示）（預設值：0.0） |
| `end_percent` | FLOAT | 是 | 0.0 至 1.0 | ControlNet 影響在生成過程中結束的終點（以百分比表示）（預設值：1.0） |
...
| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `positive` | CONDITIONING | 已套用 ControlNet 進行修補的修改後正向條件化 |
| `negative` | CONDITIONING | 已套用 ControlNet 進行修補的修改後負向條件化 |
