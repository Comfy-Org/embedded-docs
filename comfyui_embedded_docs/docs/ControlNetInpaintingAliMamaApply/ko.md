...
|-----------|-----------|----------|-------|-------------|
| `긍정 조건` | CONDITIONING | 예 | - | 원하는 콘텐츠로 생성을 안내하는 포지티브 컨디셔닝 |
| `부정 조건` | CONDITIONING | 예 | - | 원하지 않는 콘텐츠에서 생성을 멀어지게 하는 네거티브 컨디셔닝 |
| `컨트롤넷` | CONTROL_NET | 예 | - | 생성에 대한 추가 제어를 제공하는 ControlNet 모델 |
| `vae` | VAE | 예 | - | 이미지 인코딩 및 디코딩에 사용되는 VAE(변이형 오토인코더) |
| `이미지` | IMAGE | 예 | - | ControlNet의 제어 안내 역할을 하는 입력 이미지 |
| `마스크` | MASK | 예 | - | 이미지에서 인페인팅할 영역을 정의하는 마스크 |
| `강도` | FLOAT | 예 | 0.0 ~ 10.0 | ControlNet 효과의 강도 (기본값: 1.0) |
| `시작 퍼센트` | FLOAT | 예 | 0.0 ~ 1.0 | 생성 중 ControlNet 영향이 시작되는 시점(백분율) (기본값: 0.0) |
| `종료 퍼센트` | FLOAT | 예 | 0.0 ~ 1.0 | 생성 중 ControlNet 영향이 종료되는 시점(백분율) (기본값: 1.0) |
...
| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `부정 조건` | CONDITIONING | 인페인팅을 위해 ControlNet이 적용된 수정된 포지티브 컨디셔닝 |
| `부정 조건` | CONDITIONING | 인페인팅을 위해 ControlNet이 적용된 수정된 네거티브 컨디셔닝 |
