...
|-----------|-----------|----------|-------|-------------|
| `string` | STRING | 예 | - | 치환이 수행될 입력 텍스트 문자열입니다 |
| `find` | STRING | 예 | - | 입력 텍스트 내에서 검색할 하위 문자열입니다 |
| `replace` | STRING | 예 | - | 발견된 모든 항목을 대체할 치환 텍스트입니다 |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `output` | STRING | 찾은 텍스트의 모든 항목이 치환 텍스트로 대체된 수정된 문자열입니다 |
