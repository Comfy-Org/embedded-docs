...
| 参数名 | 数据类型 | 必填 | 取值范围 | 描述 |
|--------|-----------|------|----------|------|
| `positive` | CONDITIONING | 是 | - | 用于指导视频生成的正向文本条件 |
| `negative` | CONDITIONING | 是 | - | 用于指导视频生成的负向文本条件 |
| `vae` | VAE | 是 | - | 用于将图像编码到潜在空间的 VAE 模型 |
| `width` | INT | 否 | 16 至 MAX_RESOLUTION | 输出视频宽度（默认：832，步长：16） |
| `height` | INT | 否 | 16 至 MAX_RESOLUTION | 输出视频高度（默认：480，步长：16） |
| `length` | INT | 否 | 1 至 MAX_RESOLUTION | 视频序列中的帧数（默认：81，步长：4） |
...
| 输出名称 | 数据类型 | 描述 |
|----------|-----------|------|
| `positive` | CONDITIONING | 应用了视频帧编码和 CLIP 视觉特征的正向条件 |
| `negative` | CONDITIONING | 应用了视频帧编码和 CLIP 视觉特征的负向条件 |
