...
| 參數 | 資料類型 | 必要 | 範圍 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `positive` | CONDITIONING | 是 | - | 用於引導影片生成的正面文字條件設定 |
| `negative` | CONDITIONING | 是 | - | 用於引導影片生成的負面文字條件設定 |
| `vae` | VAE | 是 | - | 用於將影像編碼至潛在空間的 VAE 模型 |
| `width` | INT | 是 | 16 至 MAX_RESOLUTION | 輸出影片寬度（預設值：832，步長：16） |
| `height` | INT | 是 | 16 至 MAX_RESOLUTION | 輸出影片高度（預設值：480，步長：16） |
| `length` | INT | 是 | 1 至 MAX_RESOLUTION | 影片序列中的影格數量（預設值：81，步長：4） |
...
| `start_image` | IMAGE | 否 | - | 影片序列的起始影格影像 |
| `end_image` | IMAGE | 否 | - | 影片序列的結束影格影像 |
...
| 輸出名稱 | 資料類型 | 描述 |
|-------------|-----------|-------------|
| `positive` | CONDITIONING | 已套用影片影格編碼和 CLIP 視覺特徵的正面條件設定 |
| `negative` | CONDITIONING | 已套用影片影格編碼和 CLIP 視覺特徵的負面條件設定 |
| `latent` | LATENT | 空白的潛在張量，其維度與指定的影片參數相符 |
