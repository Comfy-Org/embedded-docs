...
|-----------|-----------|----------|-------|-------------|
| `الصوت` | CUSTOM | نعم | - | الصوت المستهدف للتحويل. قم بالتوصيل من محدد الصوت أو استنساخ الصوت الفوري. |
| `الصوت` | AUDIO | نعم | - | الصوت المصدر المراد تحويله. |
| `الثبات` | FLOAT | لا | 0.0 - 1.0 | استقرار الصوت. القيم المنخفضة تعطي نطاقًا عاطفيًا أوسع، بينما تنتج القيم الأعلى كلامًا أكثر اتساقًا ولكن قد يكون رتيبًا (الافتراضي: 0.5). |
| `النموذج` | DYNAMICCOMBO | لا | `eleven_multilingual_sts_v2`<br>`eleven_english_sts_v2` | النموذج المستخدم لتحويل الكلام إلى كلام. يوفر كل خيار مجموعة محددة من إعدادات الصوت (تعزيز التشابه، الأسلوب، استخدام تعزيز المتحدث، السرعة). |
| `صيغة الإخراج` | COMBO | لا | `"mp3_44100_192"`<br>`"opus_48000_192"` | تنسيق إخراج الصوت (الافتراضي: "mp3_44100_192"). |
| `البذرة` | INT | لا | 0 - 4294967295 | البذرة لإمكانية إعادة إنتاج النتائج (الافتراضي: 0). |
| `إزالة الضوضاء الخلفية` | BOOLEAN | لا | - | إزالة الضوضاء الخلفية من الصوت المدخل باستخدام عزل الصوت (الافتراضي: False). |

## المخرجات

| اسم المخرج | نوع البيانات | الوصف |
|-------------|-----------|-------------|
| `الصوت` | AUDIO | الملف الصوتي المحول بالتنسيق المحدد للإخراج. |
