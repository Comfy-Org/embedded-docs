...
|-----------|-----------|----------|-------|-------------|
| `voice` | CUSTOM | 是 | - | 转换的目标声音。从语音选择器或即时语音克隆连接。 |
| `audio` | AUDIO | 是 | - | 待转换的源音频。 |
| `stability` | FLOAT | 否 | 0.0 - 1.0 | 语音稳定性。较低的值提供更广泛的情感范围，较高的值产生更一致但可能单调的语音（默认值：0.5）。 |
| `model` | DYNAMICCOMBO | 否 | `eleven_multilingual_sts_v2`<br>`eleven_english_sts_v2` | 用于语音转语音转换的模型。每个选项提供一组特定的语音设置（相似度增强、风格、使用说话人增强、速度）。 |
| `output_format` | COMBO | 否 | `"mp3_44100_192"`<br>`"opus_48000_192"` | 音频输出格式（默认值："mp3_44100_192"）。 |
| `seed` | INT | 否 | 0 - 4294967295 | 用于结果可复现的随机种子（默认值：0）。 |
