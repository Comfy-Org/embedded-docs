...
|-----------|-------------|-----------------|--------|-------|
| `modèle` | Modèle à utiliser. Cette sélection inclut également le prompt texte, jusqu’à 3 entrées d’images de référence et un prompt négatif facultatif. | MODEL | Oui | "qwen-image-3.0-pro"<br>"qwen-image-3.0" |
| `taille` | Résolution de sortie. « match input » réutilise la taille de la première image de référence, « auto » laisse le modèle choisir une taille avec le même rapport hauteur/largeur, « custom » définit une largeur et une hauteur explicites. | COMBO | Oui | "match input"<br>"auto"<br>"custom" |
...
| `extension de prompt` | Indique s’il faut enrichir le prompt avec l’assistance de l’IA. (par défaut : True) | BOOLEAN | Non | True<br>False |
| `filigrane` | Indique s’il faut ajouter un filigrane généré par l’IA au résultat. (par défaut : False) | BOOLEAN | Non | True<br>False |
