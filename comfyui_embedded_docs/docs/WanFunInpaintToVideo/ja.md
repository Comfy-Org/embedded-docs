...
| パラメータ | データ型 | 必須 | 範囲 | 説明 |
|-----------|-----------|----------|-------|-------------|
| `positive` | CONDITIONING | はい | - | ビデオ生成のためのポジティブなコンディショニングプロンプト |
...
| `batch_size` | INT | はい | 1 から 4096 | バッチで生成するビデオの数（デフォルト: 1） |
...
| 出力名 | データ型 | 説明 |
|-------------|-----------|-------------|
| `positive` | CONDITIONING | 処理されたポジティブなコンディショニング出力 |
| `negative` | CONDITIONING | 処理されたネガティブなコンディショニング出力 |
