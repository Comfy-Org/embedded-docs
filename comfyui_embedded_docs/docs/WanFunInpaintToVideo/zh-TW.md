...
WanFunInpaintToVideo 節點透過在起始與結束影像之間進行修補來建立影片序列。它接受正向與負向條件，以及可選的幀影像，以生成影片潛在表示。此節點可處理可配置尺寸與長度參數的影片生成。
...
| `負向` | 用於影片生成中要避免的負向條件提示 | CONDITIONING | 是 | - |
...
| `clip_vision_output` | 可選的 CLIP 視覺輸出，用作起始影像的條件 | CLIP_VISION_OUTPUT | 否 | - |
...
| 輸出名稱 | 描述 | 資料類型 |
| --- | --- | --- |
| `正向` | 處理後的正向條件輸出 | CONDITIONING |
| `負向` | 處理後的負向條件輸出 | CONDITIONING |
| `潛在空間` | 生成的影片潛在表示 | LATENT |
