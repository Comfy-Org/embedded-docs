...
| `length` | INT | 是 | 1 至 MAX_RESOLUTION | 视频序列的帧数（默认：81，步长：4） |
| `batch_size` | INT | 是 | 1 至 4096 | 单批次生成的视频数量（默认：1） |
| `clip_vision_output` | CLIP_VISION_OUTPUT | 否 | - | 用于附加条件提示的可选 CLIP 视觉输出 |
| `start_image` | IMAGE | 否 | - | 用于视频生成的可选起始帧图像 |
| `end_image` | IMAGE | 否 | - | 用于视频生成的可选结束帧图像 |
