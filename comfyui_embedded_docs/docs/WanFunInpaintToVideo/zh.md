...
| `negative` | CONDITIONING | 是 | - | 视频生成中需避免的负向条件提示 |
...
| `clip_vision_output` | CLIP_VISION_OUTPUT | 否 | - | 可选的 CLIP 视觉输出，用于额外条件控制 |
| `start_image` | IMAGE | 否 | - | 可选的视频生成起始帧图像 |
| `end_image` | IMAGE | 否 | - | 可选的视频生成结束帧图像 |

## 输出

| 输出名称 | 数据类型 | 描述 |
|-------------|-----------|-------------|
| `positive` | CONDITIONING | 处理后的正向条件输出 |
| `negative` | CONDITIONING | 处理后的负向条件输出 |
