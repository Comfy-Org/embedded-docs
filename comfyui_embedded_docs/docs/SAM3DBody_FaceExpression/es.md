...
Este nodo añade expresiones faciales a un cuerpo SAM3D detectando rostros en una imagen con el MediaPipe Face Landmarker, emparejando cada rostro detectado con una persona rastreada y asignando los 52 blendshapes de ARKit a los parámetros de expresión de 72 ejes de MHR. Luego vuelve a ejecutar el modelo corporal para que los vértices de la malla y los puntos clave de salida reflejen la nueva expresión.
...
| `input_threshold` | Zona muerta en la salida bruta de MediaPipe (por debajo = cero, por encima = reasignación lineal). Predeterminado: 0.02. | FLOAT | No | 0.0 a 0.5 (paso 0.01, predeterminado 0.02) |
| `blendshape_smooth_window` | Ventana gaussiana sobre la señal por fotograma de MediaPipe antes del mapeo a MHR. La salida bruta de MediaPipe oscila un 30-70% entre fotogramas en rostros estáticos. 1 = deshabilitado. Use valores impares. Predeterminado: 7. | INT | No | 1 a 31 (paso 2, predeterminado 7) |

Nota: Solo se aplica una resta de línea base por clip cuando al menos 30 fotogramas del clip contienen personas detectadas. Los huecos de detección de hasta 12 fotogramas por persona se rellenan mediante interpolación.

## Salidas

| Nombre de salida | Descripción | Tipo de datos |
|------------------|-------------|---------------|
| `mhr_pose_data` | Los datos de pose actualizados. Los parámetros de expresión de cada persona rastreada se reemplazan con la expresión facial mapeada, y los vértices de la malla y los puntos clave se regeneran para coincidir. | MHR_POSE_DATA |
