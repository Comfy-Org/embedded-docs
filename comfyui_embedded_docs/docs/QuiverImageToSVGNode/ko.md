...
| `model` | DYNAMICCOMBO | 예 | 여러 옵션 사용 가능 | SVG 벡터화에 사용할 모델입니다. 모델을 선택하면 해당 모델에 특화된 추가 매개변수가 표시됩니다: `target_size`(픽셀 단위 정사각형 크기 조정 목표, 기본값: 1024, 범위: 128-4096), `temperature`, `top_p`, `presence_penalty`. |
| `seed` | INT | 아니요 | 0 ~ 2147483647 | 노드 재실행 여부를 결정하는 시드입니다. 시드 값과 관계없이 실제 결과는 비결정적입니다. 이 매개변수는 "생성 후 제어" 기능이 있습니다(기본값: 0). |
