...
|-----------|--------------|-------------|-------|-----------|
| `images` | IMAGE | Sim | - | A sequência de imagens de entrada ou quadros de vídeo para visualizar os rastros. |
| `tracks` | TRACKS | Não | - | Os dados de rastreamento de movimento contendo caminhos de pontos e informações de visibilidade. Se não for fornecido, as imagens de entrada são retornadas sem alterações. |
| `line_resolution` | INT | Sim | 1 - 1024 | O número de quadros anteriores a serem usados ao desenhar a linha de trajetória para cada rastro (padrão: 24). |
...
|---------------|--------------|-----------|
| `IMAGE` | IMAGE | A sequência de imagens com os dados de rastreamento de movimento visualizados como sobreposições. Se nenhum `tracks` for fornecido, as imagens de entrada originais são retornadas. |
