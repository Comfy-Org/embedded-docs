...
| `pistes` | TRACKS | Non | - | Les données de suivi de mouvement contenant les trajectoires des points et les informations de visibilité. Si non fournies, les images d'entrée sont transmises sans modification. |
| `résolution_de_ligne` | INT | Oui | 1 - 1024 | Le nombre de trames précédentes à utiliser pour tracer la ligne de trajectoire de chaque piste (par défaut : 24). |
| `taille_du_cercle` | INT | Oui | 1 - 128 | La taille du cercle dessiné à la position actuelle de chaque piste (par défaut : 12). |
| `opacité` | FLOAT | Oui | 0.0 - 1.0 | L'opacité des superpositions de pistes dessinées (par défaut : 0.75). |
| `largeur_de_ligne` | INT | Oui | 1 - 128 | La largeur des lignes utilisées pour tracer les trajectoires des pistes (par défaut : 16). |
...
|---------------|-----------------|-------------|
| `IMAGE` | IMAGE | La séquence d'images avec les données de suivi de mouvement visualisées sous forme de superpositions. Si aucune `pistes` n'a été fournie, les images d'entrée originales sont retournées. |
