...
|-----------|-----------|----------|-------|-------------|
| `model` | STRING | 예 | `"SeedVR2"`<br>`"Ultimate"` | 업스케일에 사용할 AI 모델입니다. "SeedVR2"와 "Ultimate"는 서로 다른 품질과 가격 체계를 제공합니다. |
| `image` | IMAGE | 예 | | 업스케일할 입력 이미지입니다. |
| `target_resolution` | STRING | 예 | `"2K"`<br>`"4K"`<br>`"8K"` | 업스케일된 이미지의 원하는 출력 해상도입니다. |
