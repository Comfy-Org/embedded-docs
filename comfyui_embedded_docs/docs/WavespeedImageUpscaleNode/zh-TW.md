...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `model` | STRING | 是 | `"SeedVR2"`<br>`"Ultimate"` | 用於放大的 AI 模型。"SeedVR2" 和 "Ultimate" 提供不同的品質和價格等級。 |
| `image` | IMAGE | 是 | | 要放大的輸入影像。 |
| `target_resolution` | STRING | 是 | `"2K"`<br>`"4K"`<br>`"8K"` | 放大影像所需的輸出解析度。 |
