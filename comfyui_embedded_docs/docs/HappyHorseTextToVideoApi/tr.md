...
| `seed` | Üretim için kullanılacak tohum (seed). Aynı girdilerle aynı tohumu kullanmak aynı sonucu üretir. (varsayılan: 0). | INT | Evet | 0 ile 2147483647 arası |
...
| Parametre | Açıklama | Veri Türü | Gerekli | Aralık |
|-----------|-------------|-----------|----------|-------|
| `model.prompt` | Öğeleri ve görsel özellikleri tanımlayan istem. İngilizce ve Çince destekler. (varsayılan: ""). | STRING | Evet | - |
| `model.resolution` | Çıktı videosunun çözünürlüğü. | STRING | Evet | "720P"<br>"1080P" |
| `model.ratio` | Çıktı videosunun en-boy oranı. | STRING | Evet | "16:9"<br>"9:16"<br>"1:1"<br>"4:3"<br>"3:4"<br>"21:9"<br>"9:21"<br>"5:4"<br>"4:5" |
| `model.duration` | Videonun saniye cinsinden uzunluğu. (varsayılan: 5, min: 3, max: 15, step: 1). | INT | Evet | 3 ile 15 arası |
...
| `model.prompt` | Öğeleri ve görsel özellikleri tanımlayan istem. İngilizce ve Çince destekler. (varsayılan: ""). | STRING | Evet | - |
| `model.resolution` | Çıktı videosunun çözünürlüğü. | STRING | Evet | "720P"<br>"1080P" |
| `model.ratio` | Çıktı videosunun en-boy oranı. | STRING | Evet | "16:9"<br>"9:16"<br>"1:1"<br>"4:3"<br>"3:4" |
| `model.duration` | Videonun saniye cinsinden uzunluğu. (varsayılan: 5, min: 3, max: 15, step: 1). | INT | Evet | 3 ile 15 arası |
