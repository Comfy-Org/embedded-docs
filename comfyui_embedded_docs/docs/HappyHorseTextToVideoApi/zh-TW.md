...
| 參數 | 說明 | 資料類型 | 必填 | 範圍 |
|-----------|-------------|-----------|----------|-------|
| `提示詞` | 描述元素與視覺特徵的提示。支援英文與中文。（預設值：""）。 | STRING | 是 | - |
| `解析度` | 輸出影片的解析度。 | STRING | 是 | "720P"<br>"1080P" |
| `比例` | 輸出影片的長寬比。 | STRING | 是 | "16:9"<br>"9:16"<br>"1:1"<br>"4:3"<br>"3:4"<br>"21:9"<br>"9:21"<br>"5:4"<br>"4:5" |
| `時長` | 影片長度（秒）。（預設值：5，最小值：3，最大值：15，間距：1）。 | INT | 是 | 3 到 15 |
...
| `提示詞` | 描述元素與視覺特徵的提示。支援英文與中文。（預設值：""）。 | STRING | 是 | - |
| `解析度` | 輸出影片的解析度。 | STRING | 是 | "720P"<br>"1080P" |
| `比例` | 輸出影片的長寬比。 | STRING | 是 | "16:9"<br>"9:16"<br>"1:1"<br>"4:3"<br>"3:4" |
| `時長` | 影片長度（秒）。（預設值：5，最小值：3，最大值：15，間距：1）。 | INT | 是 | 3 到 15 |
