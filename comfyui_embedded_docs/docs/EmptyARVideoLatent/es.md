...
| --- | --- | --- | --- | --- |
| `width` | El ancho de los fotogramas del video en píxeles (predeterminado: 832) | INT | Sí | 16 a 8192 (step: 16) |
| `height` | La altura de los fotogramas del video en píxeles (predeterminado: 480) | INT | Sí | 16 a 8192 (step: 16) |
| `length` | El número de fotogramas del video (predeterminado: 81) | INT | Sí | 1 a 1024 (step: 4) |
...
| --- | --- | --- |
| `LATENT` | Un tensor latente relleno de ceros, que representa un espacio latente de video vacío con las dimensiones, la longitud y el tamaño de lote especificados. La forma del tensor es [batch_size, 16, lat_t, height/8, width/8], donde lat_t = ((length - 1) // 4) + 1 es el número de pasos de tiempo latentes derivados de la longitud solicitada. | LATENT |
