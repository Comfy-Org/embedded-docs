...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `width` | INT | 是 | 16 至 8192 (步長：16) | 影片幀的寬度（像素）（預設值：832） |
| `height` | INT | 是 | 16 至 8192 (步長：16) | 影片幀的高度（像素）（預設值：480） |
| `length` | INT | 是 | 1 至 1024 (步長：4) | 影片的幀數（預設值：81） |
| `batch_size` | INT | 是 | 1 至 64 | 單一批次中生成的影片數量（預設值：1） |
