...
|-----------|-----------|----------|-------|-------------|
| `圖像` | IMAGE | 是 | - | 要編輯的來源圖片。 |
| `提示詞` | STRING | 是 | - | 所需新外觀的文字描述（例如：「身穿閃亮盔甲的騎士」）。 |
| `alpha 模式` | COMBO | 是 | `"select"`<br>`"fill"`<br>`"custom"` | 處理 Alpha 遮罩的方式。「select」使用關鍵影格選取主體，「fill」直接取代整張圖片而不使用獨立遮罩，「custom」使用使用者提供的遮罩。 |
| `最大解析度` | COMBO | 是 | `"1080p"`<br>`"720p"` | 輸出圖片的最高解析度。較高解析度會消耗更多點數。 |
| `種子` | INT | 是 | - | 用於再現結果的種子值。 |
| `參考圖像` | IMAGE | 否 | - | 可選的參考圖片，用於引導新場景元素的風格或外觀。 |
...
| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `alpha` | IMAGE | 已編輯的圖片，場景元素已被切換。 |
| `alpha` | MASK | Beeble 使用的 Alpha 遮罩。在「fill」模式下為空，因為該模式沒有獨立遮罩。 |
