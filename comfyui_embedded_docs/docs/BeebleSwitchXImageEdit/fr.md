...
|-----------|-----------------|--------|-------|-------------|
| `image` | IMAGE | Oui | - | L'image source à modifier. |
| `invite` | STRING | Oui | - | Une description textuelle du nouvel aspect souhaité (par exemple, "un chevalier en armure étincelante"). |
| `mode alpha` | COMBO | Oui | `"select"`<br>`"fill"`<br>`"custom"` | Comment gérer le mat alpha. "select" utilise une image clé pour sélectionner le sujet, "fill" remplace l'image entière sans mat séparé, "custom" utilise un masque fourni par l'utilisateur. |
| `résolution maximale` | COMBO | Oui | `"1080p"`<br>`"720p"` | La résolution maximale de l'image de sortie. Une résolution plus élevée coûte plus de crédits. |
| `graine` | INT | Oui | - | Une valeur de graine pour la reproductibilité. |
| `image de référence` | IMAGE | Non | - | Une image de référence optionnelle pour guider le style ou l'apparence des nouveaux éléments de la scène. |
...
|---------------|-----------------|-------------|
| `alpha` | IMAGE | L'image modifiée avec les éléments de la scène changés. |
| `alpha` | MASK | Le mat alpha utilisé par Beeble. Vide pour le mode "fill", qui n'a pas de mat séparé. |
