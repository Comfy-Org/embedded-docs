...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `model` | MODEL | 是 | - | 將套用 LoRA 的擴散模型。 |
| `clip` | CLIP | 是 | - | 將套用 LoRA 的 CLIP 模型。 |
| `lora_name` | COMBO | 是 | *可用的 LoRA 檔案列表* | 要套用的 LoRA 檔案名稱。選項從 `loras` 資料夾中載入。 |
| `strength_model` | FLOAT | 是 | -100.0 至 100.0 | 修改擴散模型的強度。此值可為負數（預設值：1.0）。 |
| `strength_clip` | FLOAT | 是 | -100.0 至 100.0 | 修改 CLIP 模型的強度。此值可為負數（預設值：1.0）。 |
...
| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `MODEL` | MODEL | 以繞過模式套用 LoRA 後的擴散模型。 |
| `CLIP` | CLIP | 以繞過模式套用 LoRA 後的 CLIP 模型。 |
