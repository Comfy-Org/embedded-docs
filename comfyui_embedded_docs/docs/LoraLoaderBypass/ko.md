...
| `strength_model` | FLOAT | 예 | -100.0 ~ 100.0 | 확산 모델을 수정하는 강도입니다. 음수 값도 가능합니다(기본값: 1.0). |
| `strength_clip` | FLOAT | 예 | -100.0 ~ 100.0 | CLIP 모델을 수정하는 강도입니다. 음수 값도 가능합니다(기본값: 1.0). |
...
| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `MODEL` | MODEL | 우회 모드로 LoRA가 적용된 확산 모델입니다. |
| `CLIP` | CLIP | 우회 모드로 LoRA가 적용된 CLIP 모델입니다. |
