...
| Paramètre | Description | Type de données | Requis | Plage |
|-----------|-------------|-----------------|--------|-------|
| `size_preset` | Choisissez une taille recommandée. Sélectionnez `Custom` pour utiliser la largeur et la hauteur ci-dessous. | COMBO | Oui | Plusieurs préréglages spécifiques au modèle sont disponibles, y compris `Custom` |
| `width` | Largeur personnalisée de l'image. La valeur n'a d'effet que si `size_preset` est défini sur `Custom` (par défaut : 2048). | INT | Oui | 1024 à 3136 (pas 2) |
| `height` | Hauteur personnalisée de l'image. La valeur n'a d'effet que si `size_preset` est défini sur `Custom` (par défaut : 2048). | INT | Oui | 1024 à 2496 (pas 2) |
...
|-----------|-------------|-----------------|--------|-------|
| `size_preset` | Choisissez une taille recommandée. Sélectionnez `Custom` pour utiliser la largeur et la hauteur ci-dessous. | COMBO | Oui | Plusieurs préréglages spécifiques au modèle sont disponibles, y compris `Custom` |
| `width` | Largeur personnalisée de l'image. La valeur n'a d'effet que si `size_preset` est défini sur `Custom` (par défaut : 2048). | INT | Oui | 1024 à 6240 (pas 2) |
| `height` | Hauteur personnalisée de l'image. La valeur n'a d'effet que si `size_preset` est défini sur `Custom` (par défaut : 2048). | INT | Oui | 1024 à 4992 (pas 2) |
| `max_images` | Nombre maximal d'images à générer. Avec 1, exactement une image est produite. Avec >1, le modèle génère entre 1 et `max_images` images liées (p. ex., scènes narratives, variations de personnage). Le nombre total d'images (entrée + générées) ne peut pas dépasser 15. (par défaut : 1) | INT | Oui | 1 à 14 |
...
| `width` | Largeur personnalisée de l'image. La valeur n'a d'effet que si `size_preset` est défini sur `Custom` (par défaut : 2048). | INT | Oui | 1024 à 6240 (pas 2) |
| `height` | Hauteur personnalisée de l'image. La valeur n'a d'effet que si `size_preset` est défini sur `Custom` (par défaut : 2048). | INT | Oui | 1024 à 4992 (pas 2) |
| `max_images` | Nombre maximal d'images à générer. Avec 1, exactement une image est produite. Avec >1, le modèle génère entre 1 et `max_images` images liées (p. ex., scènes narratives, variations de personnage). Le nombre total d'images (entrée + générées) ne peut pas dépasser 15. (par défaut : 1) | INT | Oui | 1 à 10 |
| `fail_on_partial` | Si activé, interrompt l'exécution si des images demandées sont manquantes ou renvoient une erreur. (par défaut : False) | BOOLEAN | Oui | True / False |
...
| Paramètre | Description | Type de données | Requis | Plage |
|-----------|-------------|-----------------|--------|-------|
| `images` | Emplacement extensible : connectez 1..N éléments (p. ex. `image_1`, `image_2`, ...) ; la limite de nombre dépend du modèle sélectionné (voir les sections des modèles). Image(s) de référence facultative(s) pour la génération image-vers-image ou multi-référence. Sans image de référence, le nœud fonctionne en mode texte-vers-image. | IMAGE | Non | 0 à 10 images (`seedream 5.0 pro`, `seedream-4-5-251128`, `seedream-4-0-250828`)<br>0 à 14 images (`seedream 5.0 lite`) |
...
- `thinking` ne peut être désactivé que pour la génération texte-vers-image, pas lorsque des images de référence sont fournies.
