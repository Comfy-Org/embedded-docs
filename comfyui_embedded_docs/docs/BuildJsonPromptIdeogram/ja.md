...
| `背景` | 画像の背景または環境の必須説明です。値を指定することを強く推奨します。空文字列でも実行されますが、結果が予測不能になる可能性があります。（デフォルト：空） | STRING | いいえ | - |
...
| `美的要素` | 必須の美的キーワード（例：ムーディー、シネマティック、彩度低め）。値を指定することを強く推奨します。（デフォルト：空） | STRING | いいえ | - |
| `ライティング` | 必須の照明の説明（例：ゴールデンアワー、リムライト、劇的な影）。値を指定することを強く推奨します。（デフォルト：空） | STRING | いいえ | - |
| `媒体` | 必須のメディアタイプ（例：photograph、illustration、3d_render、painting、graphic_design）。スタイルがphotoの場合、photographに設定してください。値を指定することを強く推奨します。（デフォルト：空） | STRING | いいえ | - |
