...
| `배경` | 이미지 배경 또는 환경에 대한 필수 설명입니다. 값을 제공하는 것이 강력히 권장됩니다. 값이 없으면 빈 문자열로 실행되지만 결과를 예측할 수 없습니다. (기본값: 비어 있음) | STRING | 아니요 | - |
...
| `미적 요소` | 필수 미적 키워드입니다(예: 무드 있는, 영화 같은, 채도 낮춤). 값을 제공하는 것이 강력히 권장됩니다. (기본값: 비어 있음) | STRING | 아니요 | - |
| `조명` | 필수 조명 설명(예: 골든 아워, 림 라이트, 드라마틱한 그림자). 값을 제공하는 것이 강력히 권장됩니다. (기본값: 비어 있음) | STRING | 아니요 | - |
| `매체` | 필수 매체 유형(예: 사진, 일러스트레이션, 3d_render, 회화, 그래픽 디자인). 스타일이 photo일 때 photograph로 설정하세요. 값을 제공하는 것이 강력히 권장됩니다. (기본값: 비어 있음) | STRING | 아니요 | - |
