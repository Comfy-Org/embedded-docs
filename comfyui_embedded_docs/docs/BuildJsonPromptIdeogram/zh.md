...
| `背景` | 图像背景或环境的必填描述。强烈建议提供值，否则节点会在空字符串下运行，结果可能无法预测。（默认：空） | STRING | 否 | - |
...
| `美学` | 必填的美学关键词（例如情绪化、电影感、去饱和）。强烈建议提供值。（默认：空） | STRING | 否 | - |
| `光照` | 必填的照明描述（例如黄金时刻、轮廓光、戏剧性阴影）。强烈建议提供值。（默认：空） | STRING | 否 | - |
| `媒介` | 必填的媒介类型（例如照片、插画、3D 渲染、绘画、平面设计）。当 `风格` = photo 时，设置为 photograph。强烈建议提供值。（默认：空） | STRING | 否 | - |
