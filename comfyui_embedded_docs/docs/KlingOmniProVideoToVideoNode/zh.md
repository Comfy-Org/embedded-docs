...
| `prompt` | STRING | 是 | 不适用 | 描述视频内容的文本提示。可包含正面和负面描述。 |
| `aspect_ratio` | COMBO | 是 | `"16:9"`<br>`"9:16"`<br>`"1:1"` | 生成视频所需的宽高比。 |
| `duration` | INT | 是 | 3 到 10 | 生成视频的时长（秒），默认值为 3。 |
| `reference_video` | VIDEO | 是 | 不适用 | 用作参考的视频。 |
| `keep_original_sound` | BOOLEAN | 是 | 不适用 | 决定输出中是否保留参考视频的音频（默认值：True）。 |
| `reference_images` | IMAGE | 否 | 不适用 | 最多 4 张额外的参考图像。 |
