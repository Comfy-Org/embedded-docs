...
| `프롬프트` | STRING | 예 | 해당 없음 | 비디오 콘텐츠를 설명하는 텍스트 프롬프트입니다. 긍정적 설명과 부정적 설명을 모두 포함할 수 있습니다. |
| `종횡비` | COMBO | 예 | `"16:9"`<br>`"9:16"`<br>`"1:1"` | 생성된 비디오의 원하는 화면 비율입니다. |
| `길이` | INT | 예 | 3~10 | 생성된 비디오의 길이(초)입니다(기본값: 3). |
| `참조 비디오` | VIDEO | 예 | 해당 없음 | 참조로 사용할 비디오입니다. |
| `원본 사운드 유지` | BOOLEAN | 예 | 해당 없음 | 출력에서 참조 비디오의 오디오를 유지할지 여부를 결정합니다(기본값: True). |
| `참조 이미지` | IMAGE | 아니요 | 해당 없음 | 최대 4개의 추가 참조 이미지입니다. |
| `해상도` | COMBO | 아니요 | `"1080p"`<br>`"720p"` | 생성된 비디오의 해상도입니다(기본값: "1080p"). |
