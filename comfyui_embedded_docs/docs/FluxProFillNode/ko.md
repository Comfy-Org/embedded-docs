...
|-----------|-----------|----------|-------|-------------|
| `이미지` | IMAGE | 예 | - | 인페인트할 입력 이미지 |
| `마스크` | MASK | 예 | - | 이미지에서 채워야 할 영역을 정의하는 마스크 |
| `프롬프트` | STRING | 아니요 | - | 이미지 생성을 위한 프롬프트 (기본값: 빈 문자열) |
| `프롬프트 업샘플링` | BOOLEAN | 아니요 | - | 프롬프트에 업샘플링을 수행할지 여부입니다. 활성화하면 프롬프트가 자동으로 수정되어 더 창의적인 생성을 유도하지만, 결과는 비결정적입니다(동일한 시드로도 완전히 동일한 결과가 생성되지 않습니다). (기본값: false) |
