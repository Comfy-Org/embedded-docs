...
| `dim` | COMBO | 是 | "x"<br>"y"<br>"t" | 裁剪潜在样本时沿用的维度 |
| `index` | INT | 是 | -16384 到 16384 | 裁剪的起始位置（默认值：0）。正值从起始位置开始计数，负值从末尾开始计数。节点会自动将索引限制在潜在样本的有效范围内 |
| `amount` | INT | 是 | 1 到 16384 | 沿指定维度提取的元素数量（默认值：1）。如果该值超出起始索引后的可用数据范围，节点会自动减小该值 |
