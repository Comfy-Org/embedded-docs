...
|-----------|-------------|-------------|
| `القيمة`   | FLOAT       | يحدد قيمة الكثافة للقناع، مما يؤثر على مظهره العام وفائدته في العمليات اللاحقة. |
| `العرض`   | INT         | يحدد عرض القناع المُنشأ، مما يؤثر بشكل مباشر على حجمه ونسبة أبعاده. |
| `الارتفاع`  | INT         | يضبط ارتفاع القناع المُنشأ، مما يؤثر على حجمه ونسبة أبعاده. |

## المخرجات

| المعامل | نوع البيانات | الوصف |
|-----------|-------------|-------------|
| `mask`    | MASK        | يُخرج قناعًا موحدًا بالأبعاد والقيمة المحددين. |