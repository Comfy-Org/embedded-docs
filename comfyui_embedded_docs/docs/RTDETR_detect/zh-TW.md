...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `model` | MODEL | 是 | N/A | 用於物件檢測的 RT-DETR 模型。 |
| `image` | IMAGE | 是 | N/A | 要檢測物件的輸入影像。節點最多以 32 張影像為一批進行處理。 |
| `threshold` | FLOAT | 否 | N/A | 檢測結果必須達到的最低置信度分數才會被納入結果（預設值：0.5）。 |
...
| `max_detections` | INT | 否 | N/A | 每張影像返回的最大檢測數量。按置信度分數降序排列（預設值：100）。 |
