...
| `threshold` | Минимальный порог уверенности, который должно иметь обнаружение, чтобы быть включённым в результаты (по умолчанию: 0.5). | FLOAT | Нет | N/A |
...
| `max_detections` | Максимальное количество обнаружений, возвращаемых для каждого изображения. В порядке убывания показателя уверенности (по умолчанию: 100). | INT | Нет | N/A |
