...
The RT-DETR Detect node выполняет обнаружение объектов на входных изображениях с использованием модели RT-DETR. Он идентифицирует объекты, возвращает ограничивающие рамки вокруг них и назначает им метки в соответствии с классами набора данных COCO. Вы можете фильтровать результаты по показателю уверенности, классу объекта и ограничивать общее количество обнаружений.
...
| `image` | Входное изображение (изображения), на котором(ых) выполняется обнаружение объектов. Узел обрабатывает изображения пакетами размером до 32. | IMAGE | Да | N/A |
...
| `max_detections` | Максимальное количество обнаружений, возвращаемых для каждого изображения. В порядке убывания показателя уверенности (по умолчанию: 100). | INT | Да | N/A |

## Выходы

| Имя выхода | Описание | Тип данных |
| --- | --- | --- |
| `bboxes` | Список ограничивающих рамок для каждого входного изображения. Каждая рамка содержит координаты (x, y, ширина, высота), метку класса и показатель уверенности. | BOUNDINGBOX |
