...
The RT-DETR Detect node performs object detection on input images using an RT-DETR model. It finds objects in the image and returns bounding box coordinates for each detection, labeled with the corresponding COCO dataset class. You can filter the results by confidence score and object class, and limit the total number of detections returned per image.
