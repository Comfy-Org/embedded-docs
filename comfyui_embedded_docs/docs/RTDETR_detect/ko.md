...
RT-DETR 감지 노드는 RT-DETR 모델을 사용하여 입력 이미지에서 객체 감지를 수행합니다. 이미지에서 객체를 찾아 각 감지에 대한 경계 상자 좌표를 반환하며, 해당 COCO 데이터 세트 클래스로 레이블이 지정됩니다. 신뢰도 점수와 객체 클래스로 결과를 필터링하고 이미지당 반환되는 총 감지 수를 제한할 수 있습니다.
