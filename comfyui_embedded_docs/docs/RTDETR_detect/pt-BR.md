...
O nó RT-DETR Detect realiza detecção de objetos em imagens de entrada usando um modelo RT-DETR. Ele encontra objetos na imagem e retorna as coordenadas das caixas delimitadoras (bounding boxes) para cada detecção, rotuladas com a classe correspondente do conjunto de dados COCO. Você pode filtrar os resultados por pontuação de confiança e classe de objeto, além de limitar o número total de detecções retornadas por imagem.
