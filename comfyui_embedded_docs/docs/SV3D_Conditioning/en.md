...
Note: The camera azimuth starts at 0 degrees and increases by a constant amount each frame so that the camera completes a full 360-degree orbit around the object across the generated frames. The `elevation` value stays constant for every frame.
...
| `negative` | The negative conditioning data with zeroed embeddings and latents for contrastive generation | CONDITIONING |
