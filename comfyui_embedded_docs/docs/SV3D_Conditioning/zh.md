...
| `init_image` | IMAGE | 是 | - | 作为 3D 视频生成起点的初始图像 |
| `vae` | VAE | 是 | - | 用于将图像编码到潜在空间的 VAE 模型 |
| `width` | INT | 否 | 16 到 MAX_RESOLUTION | 生成视频帧的输出宽度（默认：576，必须能被 8 整除） |
| `height` | INT | 否 | 16 到 MAX_RESOLUTION | 生成视频帧的输出高度（默认：576，必须能被 8 整除） |
| `video_frames` | INT | 否 | 1 到 4096 | 视频序列要生成的帧数（默认：21） |
| `elevation` | FLOAT | 否 | -90.0 到 90.0 | 3D 视图中相机仰角的角度（单位：度，默认：0.0） |

## 输出参数

| 输出名称 | 数据类型 | 描述 |
|----------|-----------|------|
| `positive` | CONDITIONING | 包含图像嵌入和相机参数的正向条件数据，用于生成过程 |
| `negative` | CONDITIONING | 带有零值嵌入的负向条件数据，用于对比生成 |
