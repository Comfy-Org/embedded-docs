...
| Paramètre | Description | Type de données |
| --- | --- | --- |
| `model` | Le modèle modifié avec des échelles de conditionnement et de non-conditionnement ajustées. Ce modèle est censé produire des sorties aux caractéristiques potentiellement améliorées grâce au redimensionnement appliqué. | MODEL |
