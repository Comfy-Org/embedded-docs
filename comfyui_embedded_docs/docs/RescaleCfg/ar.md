...
|--------------|---------------|--------|
| `النموذج`      | MODEL         | يمثل معامل `model` النموذج التوليدي الذي سيتم تعديله. يعد هذا المعامل حاسمًا حيث تطبق العقدة دالة إعادة قياس على مخرجات النموذج، مما يؤثر مباشرة على عملية التوليد. |
| `المُضَاعِف` | `FLOAT`       | يتحكم معامل `multiplier` في مدى إعادة القياس المطبقة على مخرجات النموذج. فهو يحدد التوازن بين المكونات الأصلية والمقاسة مرة أخرى، مما يؤثر على خصائص المخرجات النهائية. |
