...
| `model` | MODEL | Oui | - | Le modèle de diffusion vidéo de base. |
...
| `audio_scale` | FLOAT | Non | -10.0 - 10.0 | Facteur d’échelle appliqué au conditionnement audio. (par défaut : 1.0) |
| `previous_frames` | IMAGE | Non | - | Images vidéo précédentes optionnelles pour étendre la séquence. |
...
| `positive` | CONDITIONING | Le conditionnement positif, potentiellement modifié avec un contexte supplémentaire (ex. : image de départ, CLIP vision). |
