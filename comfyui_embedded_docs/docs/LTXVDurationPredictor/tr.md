...
| `pozitif` | Süre tahmini için prompt'un metin gömmelerini ve meta verilerini sağlayan koşullandırma. | CONDITIONING | Evet | N/A |
| `duration_head` | ModelPatchLoader ile yüklenen LTX 2.4 süre başlığı. Bir LTX süre başlığı olmalıdır. | MODEL_PATCH | Evet | N/A |
| `kare_hızı` | Saniyeyi kareye dönüştürmek için kullanılan saniyedeki kare sayısı cinsinden kare hızı (varsayılan: 24.0). | FLOAT | Evet | 1.0 ila 120.0 |
| `min_saniye` | Tahmini bir kare sayısına dönüştürürken kullanılan saniye cinsinden minimum süre (varsayılan: 1.0). | FLOAT | Evet | 0.5 ila 120.0 |
| `maks_saniye` | Tahmini bir kare sayısına dönüştürürken kullanılan saniye cinsinden maksimum süre (varsayılan: 20.0). | FLOAT | Evet | 0.5 ila 120.0 |
...
|-------------|-------------|-----------|
| `kare_sayısı` | Tahmin edilen sürenin kare sayısına dönüştürülmüş ve VAE'nin 8k+1 kare ızgarasına hizalanmış hali. | INT |
| `saniye` | Ham (sınırlanmamış) tahmin edilen süre. Kare ızgarasına hizalanmadan önceki değerdir. | FLOAT |
