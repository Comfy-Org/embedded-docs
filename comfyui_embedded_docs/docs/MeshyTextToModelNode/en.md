...
|-----------|-------------|-----------|----------|-------|
| `model` | Specifies the AI model version to use for generation. | COMBO | Yes | `"meshy-7.1"`<br>`"meshy-7"`<br>`"meshy-6"`<br>`"latest"` |
...
| `symmetry_mode` | Controls symmetry in the generated model. This is an advanced parameter. Options are `"auto"`, `"on"`, and `"off"`. | COMBO | Yes | `"auto"`<br>`"on"`<br>`"off"` |
...
| `ultra_resolution` | Resolution of the ultra pass: `"2k"` runs it at 2048³, `"4k"` at 4096³ for the finest surface detail. 4k requires the `"meshy-7.1"` or `"latest"` model. Used only when `ultra_mode` is enabled (default: `"2k"`). | COMBO | No | `"2k"`<br>`"4k"` |
...
When `ultra_mode` is enabled, the `model` parameter must be set to `"meshy-7.1"`, `"meshy-7"` or `"latest"`; any other model raises an error. With `"meshy-7"` the ultra pass always runs at 2048³, and the `"4k"` ultra resolution requires `"meshy-7.1"` or `"latest"`.
