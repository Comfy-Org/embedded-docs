...
| 参数 | 数据类型 | 是否必填 | 范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `positive` | CONDITIONING | 是 | - | 正向条件提示，用于引导视频应包含的内容 |
| `negative` | CONDITIONING | 是 | - | 负向条件提示，用于指定视频应避免的内容 |
| `vae` | VAE | 是 | - | 用于编码和解码视频帧的变分自编码器 |
| `length` | INT | 是 | 1 至 MAX_RESOLUTION | 要生成的视频序列总帧数（默认值：77，步长：4） |
| `video_latent` | LATENT | 是 | - | 作为扩展起点的初始视频潜在表示 |
| `audio_encoder_output` | AUDIOENCODEROUTPUT | 否 | - | 可选的音频嵌入，可根据声音特征影响视频生成 |
| `ref_image` | IMAGE | 否 | - | 可选的参考图像，为视频生成提供视觉引导 |
| `control_video` | IMAGE | 否 | - | 可选的控制视频，可引导生成视频的运动和风格 |

## 输出

| 输出名称 | 数据类型 | 描述 |
|-------------|-----------|-------------|
| `positive` | CONDITIONING | 已应用视频上下文的处理后的正向条件 |
| `negative` | CONDITIONING | 已应用视频上下文的处理后的负向条件 |
