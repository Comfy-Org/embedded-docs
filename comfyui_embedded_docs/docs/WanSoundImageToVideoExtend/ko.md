...
|-----------|-----------|----------|-------|-------------|
| `긍정 프롬프트` | CONDITIONING | 예 | - | 비디오에 포함되어야 할 내용을 안내하는 긍정 조건화 프롬프트 |
| `부정 프롬프트` | CONDITIONING | 예 | - | 비디오에서 제외되어야 할 내용을 지정하는 부정 조건화 프롬프트 |
| `VAE` | VAE | 예 | - | 비디오 프레임의 인코딩 및 디코딩에 사용되는 변분 오토인코더 |
| `길이` | INT | 예 | 1 ~ MAX_RESOLUTION | 비디오 시퀀스에 생성할 총 프레임 수 (기본값: 77, 단계: 4) |
| `비디오 잠재 공간` | LATENT | 예 | - | 확장의 시작점이 되는 초기 비디오 잠재 표현 |
| `오디오 인코더 출력` | AUDIOENCODEROUTPUT | 아니요 | - | 사운드 특성에 기반하여 비디오 생성에 영향을 줄 수 있는 선택적 오디오 임베딩 |
| `참조 이미지` | IMAGE | 아니요 | - | 비디오 생성을 위한 시각적 안내를 제공하는 선택적 참조 이미지 |
| `제어 비디오` | IMAGE | 아니요 | - | 생성된 비디오의 움직임과 스타일을 안내할 수 있는 선택적 제어 비디오 |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `부정 프롬프트` | CONDITIONING | 비디오 컨텍스트가 적용된 처리된 긍정 조건화 |
| `잠재 공간` | CONDITIONING | 비디오 컨텍스트가 적용된 처리된 부정 조건화 |
