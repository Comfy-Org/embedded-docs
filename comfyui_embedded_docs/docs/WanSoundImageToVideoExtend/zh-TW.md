...
| 參數 | 資料類型 | 必要 | 範圍 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `positive` | CONDITIONING | 是 | - | 正向條件提示，引導影片應包含的內容 |
| `negative` | CONDITIONING | 是 | - | 負向條件提示，指定影片應避免的內容 |
| `vae` | VAE | 是 | - | 用於編碼和解碼影片影格的變分自編碼器 |
| `length` | INT | 是 | 1 至 MAX_RESOLUTION | 要生成的影片序列總影格數（預設值：77，步長：4） |
| `video_latent` | LATENT | 是 | - | 作為擴展起點的初始影片潛在表示 |
| `audio_encoder_output` | AUDIOENCODEROUTPUT | 否 | - | 可選的音訊嵌入，可根據聲音特徵影響影片生成 |
| `ref_image` | IMAGE | 否 | - | 可選的參考影像，為影片生成提供視覺引導 |
...
| 輸出名稱 | 資料類型 | 描述 |
|-------------|-----------|-------------|
| `positive` | CONDITIONING | 已套用影片上下文的處理後正向條件 |
| `negative` | CONDITIONING | 已套用影片上下文的處理後負向條件 |
