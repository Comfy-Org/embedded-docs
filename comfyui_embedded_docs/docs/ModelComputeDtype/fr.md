...
Le nœud ModelComputeDtype change le type de données de calcul (précision) utilisé par un modèle pendant le traitement. Il crée une copie du modèle d'entrée et applique le réglage de précision sélectionné, ce qui peut aider à optimiser l'utilisation de la mémoire et les performances selon votre matériel. Ceci est utile pour déboguer et tester différentes configurations de précision.
...
| `dtype` | Le type de données de calcul à appliquer au modèle (défaut : "default"). Ce paramètre est marqué comme une option avancée. | COMBO | Oui | "default"<br>"fp32"<br>"fp16"<br>"bf16" |
