...
ModelComputeDtype 節點允許您在推理過程中變更模型使用的計算資料類型。它會建立輸入模型的副本並套用指定的資料類型設定，這有助於根據您的硬體能力優化記憶體使用和效能。此功能在除錯和測試不同精度設定時特別有用。
...
| `dtype` | STRING | 是 | "default"<br>"fp32"<br>"fp16"<br>"bf16" | 要套用至模型的計算資料類型 |
