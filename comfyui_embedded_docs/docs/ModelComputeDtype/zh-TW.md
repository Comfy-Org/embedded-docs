...
ModelComputeDtype 節點會更改模型在處理期間所使用的計算資料型別（精度）。它會建立輸入模型的副本，並套用所選的精度設定，這有助於根據您的硬體最佳化記憶體使用量與效能。這對於偵錯及測試不同的精度設定非常有用。

## 輸入

| 參數 | 描述 | 資料類型 | 必填 | 數值範圍 |
| --- | --- | --- | --- | --- |
| `model` | 要套用新的計算資料型別的輸入模型 | MODEL | 是 | - |
| `dtype` | 要套用至模型的計算資料型別（預設值：`"default"`）。此參數在 UI 中標記為進階設定。 | COMBO | 是 | `"default"`<br>`"fp32"`<br>`"fp16"`<br>`"bf16"` |
