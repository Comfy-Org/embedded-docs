...
|-----------|-----------|----------|-------|-------------|
| `model` | MODEL | 예 | - | 새로운 연산 데이터 타입을 적용할 입력 모델 |
| `dtype` | STRING | 예 | "default"<br>"fp32"<br>"fp16"<br>"bf16" | 모델에 적용할 연산 데이터 타입 (기본값: "default") |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `model` | MODEL | 새로운 연산 데이터 타입이 적용된 수정된 모델 |
