...
|----------|-------------|------------|---------|------|-------|
| `conditioning` | CONDITIONING | 필수 | - | - | 수정할 조건화 데이터 |
| `width` | FLOAT | 필수 | 1.0 | 0.0 - 1.0 | 전체 너비 대비 영역의 너비 백분율 |
| `height` | FLOAT | 필수 | 1.0 | 0.0 - 1.0 | 전체 높이 대비 영역의 높이 백분율 |
...
| `strength` | FLOAT | 필수 | 1.0 | 0.0 - 10.0 | 정의된 영역 내 조건화에 적용되는 강도 배수 |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-----------|-------------|-------|
| `conditioning` | CONDITIONING | 지정된 영역과 강도 설정이 적용된 수정된 조건화 데이터 |