...
|-----------|-----------|------------|---------|-------|-------------|
| `條件設定` | CONDITIONING | 必要 | - | - | 要修改的 conditioning 資料 |
| `寬度` | FLOAT | 必要 | 1.0 | 0.0 - 1.0 | 區域寬度佔總寬度的百分比 |
| `高度` | FLOAT | 必要 | 1.0 | 0.0 - 1.0 | 區域高度佔總高度的百分比 |
| `時間軸` | FLOAT | 必要 | 1.0 | 0.0 - 1.0 | 區域的時間持續時間佔總影片長度的百分比 |
| `X 座標` | FLOAT | 必要 | 0.0 | 0.0 - 1.0 | 區域的水平起始位置百分比 |
| `Y 座標` | FLOAT | 必要 | 0.0 | 0.0 - 1.0 | 區域的垂直起始位置百分比 |
| `Z 座標` | FLOAT | 必要 | 0.0 | 0.0 - 1.0 | 區域在影片時間軸上的時間起始位置百分比 |
| `強度` | FLOAT | 必要 | 1.0 | 0.0 - 10.0 | 套用於定義區域內 conditioning 的強度乘數 |

## 輸出

| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `條件設定` | CONDITIONING | 已套用指定區域與強度設定的修改後 conditioning 資料 |
