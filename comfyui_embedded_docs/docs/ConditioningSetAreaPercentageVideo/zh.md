...
|-----------|-----------|------------|---------|-------|-------------|
| `conditioning` | CONDITIONING | 必需 | - | - | 需要修改的 conditioning 数据 |
| `width` | FLOAT | 必需 | 1.0 | 0.0 - 1.0 | 区域宽度占总宽度的百分比 |
| `height` | FLOAT | 必需 | 1.0 | 0.0 - 1.0 | 区域高度占总高度的百分比 |
| `temporal` | FLOAT | 必需 | 1.0 | 0.0 - 1.0 | 区域时间跨度占总视频长度的百分比 |
| `x` | FLOAT | 必需 | 0.0 | 0.0 - 1.0 | 区域水平起始位置的百分比 |
| `y` | FLOAT | 必需 | 0.0 | 0.0 - 1.0 | 区域垂直起始位置的百分比 |
| `z` | FLOAT | 必需 | 0.0 | 0.0 - 1.0 | 区域时间起始位置占视频时间线的百分比 |
| `strength` | FLOAT | 必需 | 1.0 | 0.0 - 10.0 | 应用于定义区域内 conditioning 的强度乘数 |

## 输出

| 输出名称 | 数据类型 | 描述 |
|-------------|-----------|-------------|
| `conditioning` | CONDITIONING | 已应用指定区域和强度设置的修改后 conditioning 数据 |
