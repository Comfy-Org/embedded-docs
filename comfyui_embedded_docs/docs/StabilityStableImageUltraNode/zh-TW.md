...
| `長寬比` | COMBO | 是 | `"1:1"`<br>`"16:9"`<br>`"21:9"`<br>`"2:3"`<br>`"3:2"`<br>`"4:5"`<br>`"5:4"`<br>`"9:16"`<br>`"9:21"` | 生成圖像的長寬比（預設值："1:1"）。 |
...
| `種子` | INT | 是 | 0 - 4294967294 | 用於產生噪點的隨機種子。 |
| `影像` | IMAGE | 否 | - | 用於圖像到圖像生成的可選輸入圖像。 |
| `負向提示詞` | STRING | 否 | - | 描述您不希望出現在輸出圖像中的內容的文字。這是一個進階功能。 |
