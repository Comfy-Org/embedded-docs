...
|-----------|--------------|-------------|-------|-------------|
| `model` | MODEL | Sí | - | El modelo al que se aplicarán las opciones de Chroma Radiance |
| `preserve_wrapper` | BOOLEAN | No | - | Cuando está habilitado, delegará en un envoltorio de función de modelo existente si existe. Generalmente debe dejarse habilitado. (predeterminado: True) |
| `start_sigma` | FLOAT | No | 0.0 a 1.0 | Primer sigma en el que estas opciones estarán activas. (predeterminado: 1.0) |
| `end_sigma` | FLOAT | No | 0.0 a 1.0 | Último sigma en el que estas opciones estarán activas. (predeterminado: 0.0) |
| `nerf_tile_size` | INT | No | -1 y superior | Permite sobrescribir el tamaño predeterminado del mosaico NeRF. -1 significa usar el valor predeterminado (32). 0 significa usar el modo sin mosaico (puede requerir mucha VRAM). (predeterminado: -1) |
