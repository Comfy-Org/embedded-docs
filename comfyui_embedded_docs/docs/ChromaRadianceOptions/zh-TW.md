...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `模型` | MODEL | 是 | - | 要套用 Chroma Radiance 選項的模型 |
| `保留包裝器` | BOOLEAN | 否 | - | 啟用時，若存在現有模型函數包裝器，將委派給該包裝器。通常應保持啟用。（預設值：True） |
| `起始 sigma` | FLOAT | 否 | 0.0 至 1.0 | 這些選項開始生效的第一個 sigma 值。（預設值：1.0） |
| `結束 sigma` | FLOAT | 否 | 0.0 至 1.0 | 這些選項結束生效的最後一個 sigma 值。（預設值：0.0） |
| `NeRF 圖塊大小` | INT | 否 | -1 及以上 | 允許覆寫預設的 NeRF 區塊大小。-1 表示使用預設值 (32)。0 表示使用非區塊模式（可能需要大量 VRAM）。（預設值：-1） |
...
| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `模型` | MODEL | 已套用 Chroma Radiance 選項的修改後模型 |
