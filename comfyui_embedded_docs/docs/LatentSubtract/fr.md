...
|--------------|-----------------|-------------|
| `samples1`   | `LATENT`        | Le premier ensemble d'échantillons latents à partir duquel la soustraction est effectuée. Il sert de base à l'opération de soustraction. |
| `samples2`   | `LATENT`        | Le second ensemble d'échantillons latents qui sera soustrait du premier ensemble. Cette opération peut modifier la sortie du modèle génératif résultant en supprimant des attributs ou des caractéristiques. |
