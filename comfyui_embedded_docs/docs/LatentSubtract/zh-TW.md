...
| 參數名稱     | 資料類型    | 描述 |
|--------------|-------------|-------------|
| `samples1`   | `LATENT`    | 將被減去的首組潛在樣本，作為減法運算的基礎。 |
| `samples2`   | `LATENT`    | 將從首組樣本中減去的第二組潛在樣本。此操作可透過移除屬性或特徵來改變生成模型的最終輸出。 |

## 輸出參數

| 參數名稱 | 資料類型    | 描述 |
|-----------|-------------|-------------|
| `latent`  | `LATENT`    | 從首組潛在樣本中減去第二組潛在樣本的結果。此修改後的潛在表示可用於後續的生成任務。 |