...
| 參數         | 資料類型 | 說明 |
|--------------|----------|------|
| `samples1`   | `LATENT` | 作為被減數的第一組潛在樣本，是減法運算的基礎。 |
| `samples2`   | `LATENT` | 將從第一組樣本中減去的第二組潛在樣本。此操作可透過移除屬性或特徵來改變生成模型的輸出結果。 |

## 輸出
| 參數      | 資料類型 | 說明 |
|-----------|----------|------|
| `latent`  | `LATENT` | 從第一組潛在樣本中減去第二組潛在樣本的結果。此修改後的潛在表示可用於進一步的生成任務。 |