# Kling 文本到视频音频

Kling Text to Video with Audio 节点可根据文本描述生成短视频。它向 Kling AI 服务发送请求，该服务处理提示词并返回视频文件。此节点还可以根据文本为视频生成配套音频。

## 输入

| 参数 | 描述 | 数据类型 | 必填 | 范围 |
|-----------|-------------|-----------|----------|-------|
| `模型` | 用于视频生成的特定 AI 模型。 | COMBO | 是 | `"kling-v2-6"` |
| `提示词` | 正向文本提示词。用于生成视频的描述。必须介于 1 到 2500 个字符之间。 | STRING | 是 | - |
| `模式` | 视频生成的操作模式。 | COMBO | 是 | `"pro"` |
| `宽高比` | 生成视频所需的宽高比。 | COMBO | 是 | `"16:9"`<br>`"9:16"`<br>`"1:1"` |
| `时长` | 视频时长，以秒为单位。 | COMBO | 是 | `5`<br>`10` |
| `生成音频` | 控制是否为视频生成音频。启用后，AI 将根据提示词创建声音（默认：`True`）。 | BOOLEAN | 否 | - |

## 输出

| 输出名称 | 描述 | 数据类型 |
|-------------|-------------|-----------|
| `output` | 生成的视频文件。 | VIDEO |

> 本文档由 AI 生成。如果您发现任何错误或有改进建议，欢迎贡献！ [在 GitHub 上编辑](https://github.com/Comfy-Org/embedded-docs/blob/main/comfyui_embedded_docs/docs/KlingTextToVideoWithAudio/zh.md)

---
**Source fingerprint (SHA-256):** `ddd2f3c1799abac067a05f3f5d6442ad4fe023d2f4f9afbde2894ca66854977e`
