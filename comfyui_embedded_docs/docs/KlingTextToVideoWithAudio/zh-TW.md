> 本文檔由 AI 生成。如果您發現任何錯誤或有改進建議，歡迎貢獻！ [在 GitHub 上編輯](https://github.com/Comfy-Org/embedded-docs/blob/main/comfyui_embedded_docs/docs/KlingTextToVideoWithAudio/zh-TW.md)

# Kling 文字轉影片（含音訊）節點

Kling 文字轉影片（含音訊）節點可根據文字描述生成一段短影片。它會向 Kling AI 服務發送請求，由該服務處理提示詞並回傳影片檔案。此節點還能根據文字內容為影片生成配套音訊。

## 輸入

| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `model_name` | COMBO | 是 | `"kling-v2-6"` | 用於影片生成的特定 AI 模型。 |
| `prompt` | STRING | 是 | - | 正向文字提示詞。用於生成影片的描述內容。長度必須在 1 到 2500 個字元之間。 |
| `mode` | COMBO | 是 | `"pro"` | 影片生成的操作模式。 |
| `aspect_ratio` | COMBO | 是 | `"16:9"`<br>`"9:16"`<br>`"1:1"` | 生成影片所需的寬高比。 |
| `duration` | COMBO | 是 | `5`<br>`10` | 影片的長度（以秒為單位）。 |
| `generate_audio` | BOOLEAN | 否 | - | 控制是否為影片生成音訊。啟用時，AI 會根據提示詞創建聲音。（預設值：`True`） |

## 輸出

| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `output` | VIDEO | 生成的影片檔案。 |

---
**Source fingerprint (SHA-256):** `eff4549816c347a090e2f6e8ae8ba832bd2c5b7aef7c729b51c9d72b7a814d5a`
