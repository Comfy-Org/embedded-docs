...
| `提示詞` | STRING | 是 | - | 正向文字提示詞。用於生成影片的描述內容。長度必須在 1 到 2500 個字元之間。 |
| `模式` | COMBO | 是 | `"pro"` | 影片生成的操作模式。 |
| `長寬比` | COMBO | 是 | `"16:9"`<br>`"9:16"`<br>`"1:1"` | 生成影片所需的寬高比。 |
| `時長` | COMBO | 是 | `5`<br>`10` | 影片的長度（以秒為單位）。 |
| `生成音訊` | BOOLEAN | 否 | - | 控制是否為影片生成音訊。啟用時，AI 會根據提示詞創建聲音。（預設值：`True`） |
