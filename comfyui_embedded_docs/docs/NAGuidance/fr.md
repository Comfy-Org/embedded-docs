...
|-----------|-----------------|--------|-------|-------------|
| `modèle` | MODEL | Oui | - | Le modèle auquel appliquer le guidage d'attention normalisé. |
| `facteur_nag` | FLOAT | Oui | 0.0 - 50.0 | Le facteur d'échelle du guidage. Des valeurs plus élevées éloignent davantage la génération du prompt négatif. (par défaut : 5.0) |
| `alpha_nag` | FLOAT | Oui | 0.0 - 1.0 | Le facteur de fusion pour l'attention normalisée. Une valeur de 1.0 remplace complètement l'attention d'origine, tandis que 0.0 n'a aucun effet. (par défaut : 0.5) |
| `tau_nag` | FLOAT | Oui | 1.0 - 10.0 | Un facteur d'échelle utilisé pour limiter le ratio de normalisation. (par défaut : 1.5) |

## Sorties

| Nom de la sortie | Type de données | Description |
|------------------|-----------------|-------------|
| `modèle` | MODEL | Le modèle modifié avec le guidage d'attention normalisé activé. |
