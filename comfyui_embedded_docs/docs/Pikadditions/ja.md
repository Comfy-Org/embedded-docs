...
| パラメータ | データ型 | 必須 | 範囲 | 説明 |
|-----------|-----------|----------|-------|-------------|
| `video` | VIDEO | はい | - | 画像を追加する対象の動画。 |
| `image` | IMAGE | はい | - | 動画に追加する画像。 |
| `prompt_text` | STRING | はい | - | 動画に追加する内容のテキストによる説明。 |
| `negative_prompt` | STRING | はい | - | 動画で避けたい内容のテキストによる説明。 |
| `seed` | INT | はい | 0 から 4294967295 | 再現性のある結果のためのランダムシード値。 |
