...
|-----------|-------------|---------------|-----------|-------|
| `model` | Modelo a utilizar para la generación de video. (predeterminado: "MiniMax H3"). Al seleccionar este modelo, también se proporcionan el prompt de texto, la resolución, la relación de aspecto y la duración del video generado (consulte Entradas de MiniMax H3 más abajo). | COMBO | Sí | `"MiniMax H3"` |
...
| `watermark` | Indica si se debe añadir una marca de agua AIGC al video. (predeterminado: false) | BOOLEAN | No | true<br>false |
