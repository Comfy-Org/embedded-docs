...
|-----------|-----------|----------|-------|-------------|
| `너비` | INT | 예 | 16 ~ MAX_RESOLUTION | 잠재 비디오의 픽셀 단위 너비 (기본값: 1280, 16으로 나누어 떨어져야 함) |
| `높이` | INT | 예 | 16 ~ MAX_RESOLUTION | 잠재 비디오의 픽셀 단위 높이 (기본값: 704, 16으로 나누어 떨어져야 함) |
| `길이` | INT | 예 | 1 ~ MAX_RESOLUTION | 잠재 비디오의 프레임 수 (기본값: 121, 8로 나누어 떨어져야 함) |
| `배치 크기` | INT | 아니요 | 1 ~ 4096 | 한 배치에서 생성할 잠재 비디오의 수 (기본값: 1) |
