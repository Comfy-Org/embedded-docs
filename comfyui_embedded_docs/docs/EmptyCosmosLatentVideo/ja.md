...
| パラメータ | データ型 | 必須 | 範囲 | 説明 |
|-----------|-----------|----------|-------|-------------|
| `width` | INT | はい | 16 ～ MAX_RESOLUTION | 潜在ビデオの幅（ピクセル単位、デフォルト: 1280、16で割り切れる必要があります） |
| `height` | INT | はい | 16 ～ MAX_RESOLUTION | 潜在ビデオの高さ（ピクセル単位、デフォルト: 704、16で割り切れる必要があります） |
| `length` | INT | はい | 1 ～ MAX_RESOLUTION | 潜在ビデオのフレーム数（デフォルト: 121、8で割り切れる必要があります） |
| `batch_size` | INT | いいえ | 1 ～ 4096 | 1バッチで生成する潜在ビデオの数（デフォルト: 1） |
