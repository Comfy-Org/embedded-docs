...
| 参数 | 数据类型 | 是否必填 | 范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `width` | INT | 是 | 16 至 MAX_RESOLUTION | 潜在视频的宽度（像素），默认值：1280，必须能被 16 整除 |
| `height` | INT | 是 | 16 至 MAX_RESOLUTION | 潜在视频的高度（像素），默认值：704，必须能被 16 整除 |
| `length` | INT | 是 | 1 至 MAX_RESOLUTION | 潜在视频的帧数，默认值：121，必须能被 8 整除 |
| `batch_size` | INT | 否 | 1 至 4096 | 每批次生成的潜在视频数量，默认值：1 |
