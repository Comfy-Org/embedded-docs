...
| 参数 | 描述 | 数据类型 | 是否必填 | 范围 |
|-----------|-------------|-----------|----------|-------|
| `text` | 要保存到文件的文本内容。此输入必须连接自另一个节点。 | STRING | 是 | - |
| `filename_prefix` | 输出文件名的前缀。会附加 5 位计数器以防止覆盖现有文件（默认值："ComfyUI"）。 | STRING | 否 | - |
| `format` | 保存文本的文件格式（默认值："txt"）。选择 "json" 时，有效的 JSON 文本会以 2 空格缩进美化输出；否则按原样保存文本。 | COMBO | 否 | `"txt"`<br>`"md"`<br>`"json"` |
