...
Save Text düğümü, metin içeriğini çıktı dizinindeki bir dosyaya yazar. Üç farklı dosya biçiminde kaydetmeyi destekler: düz metin (.txt), Markdown (.md) ve JSON (.json). JSON olarak kaydederken, düğüm giriş metnini geçerli bir JSON olarak ayrıştırmaya çalışır ve uygun girintileme ile biçimlendirir.
