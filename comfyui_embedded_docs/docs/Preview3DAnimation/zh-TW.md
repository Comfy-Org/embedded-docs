...
| model_file    | STRING | 位於 `ComfyUI/output/` 下的模型檔案路徑   |

## 畫布區域描述

目前 ComfyUI 前端中的 3D 相關節點共享同一個畫布元件，因此它們的基本操作除了一些功能差異外大多一致。
...
![Load 3D 節點介面](../Preview3D/asset/preview3d_canvas.jpg)
...
- 2. 模型：模型渲染模式、紋理材質、上方方向設定
- 3. 攝影機：正交視圖和透視視圖切換，設定透視角度
- 4. 光源：場景全域照明強度
