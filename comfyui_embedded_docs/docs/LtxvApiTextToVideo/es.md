...
|-----------|--------------|-----------|-------|-------------|
| `model` | COMBO | Sí | `"LTX-2 (Fast)"`<br>`"LTX-2 (Quality)"`<br>`"LTX-2 (Turbo)"` | El modelo de IA a utilizar para la generación de video. Los modelos disponibles se obtienen del `MODELS_MAP` del código fuente. |
...
| `duration` | COMBO | Sí | `6`<br>`8`<br>`10`<br>`12`<br>`14`<br>`16`<br>`18`<br>`20` | La duración del video generado en segundos (predeterminado: 8). |
...
| `generate_audio` | BOOLEAN | No | - | Cuando está habilitado, el video generado incluirá audio creado por IA que coincida con la escena (predeterminado: False). |
