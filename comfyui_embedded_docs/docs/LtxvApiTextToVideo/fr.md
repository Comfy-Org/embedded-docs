...
| --- | --- | --- | --- | --- |
| `model` | Le modèle d'IA à utiliser pour la génération vidéo. « LTX-2 (Pro) » offre une qualité supérieure, tandis que « LTX-2 (Fast) » est optimisé pour la rapidité. | COMBO | Oui | `"LTX-2 (Pro)"`<br>`"LTX-2 (Fast)"` |
| `prompt` | La description textuelle que l'IA utilisera pour générer la vidéo. Ce champ accepte plusieurs lignes de texte. | STRING | Oui | - |
| `duration` | La durée de la vidéo générée en secondes (défaut : 8). | COMBO | Oui | `6`<br>`8`<br>`10`<br>`12`<br>`14`<br>`16`<br>`18`<br>`20` |
...
| `fps` | Le nombre d'images par seconde de la vidéo (défaut : 25). | COMBO | Oui | `25`<br>`50` |
| `generate_audio` | Lorsque ce paramètre est défini sur true, la vidéo générée inclut un son généré par IA correspondant à la scène (défaut : False). | BOOLEAN | Non | `True`<br>`False` |
