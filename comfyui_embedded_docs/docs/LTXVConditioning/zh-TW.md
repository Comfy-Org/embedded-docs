...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `正向` | CONDITIONING | 是 | - | 將接收影格率資訊的正向條件輸入 |
| `負向` | CONDITIONING | 是 | - | 將接收影格率資訊的負向條件輸入 |
| `影格率` | FLOAT | 是 | 0.0 - 1000.0 | 要套用至兩組條件設定的影格率值（預設值：25.0） |

## 輸出

| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `負向` | CONDITIONING | 已套用影格率資訊的正向條件 |
| `負向` | CONDITIONING | 已套用影格率資訊的負向條件 |
