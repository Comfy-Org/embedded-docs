...
| 参数 | 数据类型 | 是否必填 | 范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `positive` | CONDITIONING | 是 | - | 将接收帧率信息的正向条件输入 |
| `negative` | CONDITIONING | 是 | - | 将接收帧率信息的负向条件输入 |
| `frame_rate` | FLOAT | 是 | 0.0 - 1000.0 | 应用于两个条件集的帧率值（默认值：25.0） |

## 输出

| 输出名称 | 数据类型 | 描述 |
|-------------|-----------|-------------|
| `positive` | CONDITIONING | 已应用帧率信息的正向条件 |
| `negative` | CONDITIONING | 已应用帧率信息的负向条件 |
