...
| 參數 | 說明 | 資料類型 | 必要 | 範圍 |
|-----------|-------------|-----------|----------|-------|
| `video` | 要編輯的來源影片。最長 18 秒。 | VIDEO | 是 | - |
| `prompt` | 描述所需的編輯內容。 | STRING | 是 | - |
| `resolution` | 編輯後影片的輸出解析度。 | COMBO | 是 | `"360p"`<br>`"540p"`<br>`"720p"`<br>`"1080p"` |
| `strength` | 保留原始內容與重新想像來源內容的強度。"auto" 讓 Ray 3.2 自行選擇；adhere_* 保留最多原始內容，flex_* 較為平衡，reimagine_* 變動最大。（預設值："auto"） | COMBO | 是 | `"auto"`<br>`"adhere_1"`<br>`"adhere_2"`<br>`"adhere_3"`<br>`"flex_1"`<br>`"flex_2"`<br>`"flex_3"`<br>`"reimagine_1"`<br>`"reimagine_2"`<br>`"reimagine_3"` |
| `seed` | 用於再現結果的隨機種子。 | INT | 是 | - |
