...
|-----------|-----------|----------|-------|-------------|
| `model` | COMBO | 是 | "wan2.5-t2v-preview"<br>"wan2.6-t2v" | 使用的模型（默认值："wan2.6-t2v"） |
| `prompt` | STRING | 是 | - | 描述元素和视觉特征的提示词。支持中文和英文（默认值：""） |
| `negative_prompt` | STRING | 否 | - | 描述需要避免内容的负面提示词（默认值：""） |
...
| `duration` | INT | 否 | 5-15（步长为5） | 视频时长（秒）。15秒时长仅适用于 Wan 2.6 模型（默认值：5） |
| `audio` | AUDIO | 否 | - | 音频必须包含清晰响亮的人声，无多余噪音或背景音乐 |
| `seed` | INT | 否 | 0-2147483647 | 用于生成的随机种子（默认值：0） |
| `generate_audio` | BOOLEAN | 否 | - | 如果未提供音频输入，则自动生成音频（默认值：False） |
| `prompt_extend` | BOOLEAN | 否 | - | 是否使用 AI 辅助增强提示词（默认值：True） |
| `watermark` | BOOLEAN | 否 | - | 是否在结果中添加 AI 生成水印（默认值：False） |
| `shot_type` | COMBO | 否 | "single"<br>"multi" | 指定生成视频的镜头类型，即视频是单一连续镜头还是包含剪辑的多镜头。此参数仅在 prompt_extend 为 True 时生效（默认值："single"） |
