...
| 參數 | 資料類型 | 必填 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `model` | COMBO | 是 | "wan2.5-t2v-preview"<br>"wan2.6-t2v" | 要使用的模型（預設值："wan2.6-t2v"） |
| `prompt` | STRING | 是 | - | 描述元素和視覺特徵的提示詞。支援英文和中文（預設值：""） |
| `negative_prompt` | STRING | 否 | - | 描述應避免內容的負面提示詞（預設值：""） |
...
| `duration` | INT | 否 | 5-15（以 5 為步進） | 影片時長（秒）。15 秒時長僅適用於 Wan 2.6 模型（預設值：5） |
| `audio` | AUDIO | 否 | - | 音訊必須包含清晰響亮的語音，無雜音或背景音樂 |
...
| `shot_type` | COMBO | 否 | "single"<br>"multi" | 指定生成影片的鏡頭類型，即影片是單一連續鏡頭還是包含剪輯的多個鏡頭。此參數僅在 prompt_extend 為 True 時生效（預設值："single"） |
