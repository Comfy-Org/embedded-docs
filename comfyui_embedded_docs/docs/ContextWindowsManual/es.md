...
|-----------|--------------|-------------|-------|-------------|
| `model` | MODEL | Sí | - | El modelo al que se le aplicarán las ventanas de contexto durante el muestreo. |
| `context_length` | INT | No | 1+ | La longitud de la ventana de contexto (predeterminado: 16). |
| `context_overlap` | INT | No | 0+ | La superposición de la ventana de contexto (predeterminado: 4). |
...
| `context_stride` | INT | No | 1+ | El paso de la ventana de contexto; solo aplicable a programaciones uniformes (predeterminado: 1). |
| `closed_loop` | BOOLEAN | No | - | Si se cierra el bucle de la ventana de contexto; solo aplicable a programaciones en bucle (predeterminado: Falso). |
| `fuse_method` | COMBO | No | `PYRAMID`<br>`LIST_STATIC` | El método a utilizar para fusionar las ventanas de contexto (predeterminado: PYRAMID). |
| `dim` | INT | No | 0-5 | La dimensión a la que se aplicarán las ventanas de contexto (predeterminado: 0). |
| `freenoise` | BOOLEAN | No | - | Si se aplica la mezcla de ruido FreeNoise, mejora la combinación de ventanas (predeterminado: Falso). |
