...
| `context_stride` | INT | Hayır | 1+ | Bağlam penceresinin adım aralığı; yalnızca tekdüze zamanlamalar için geçerlidir (varsayılan: 1). |
| `closed_loop` | BOOLEAN | Hayır | - | Bağlam penceresi döngüsünün kapatılıp kapatılmayacağı; yalnızca döngülü zamanlamalar için geçerlidir (varsayılan: False). |
...
| `freenoise` | BOOLEAN | Hayır | - | FreeNoise gürültü karıştırmanın uygulanıp uygulanmayacağı, pencere harmanlamasını iyileştirir (varsayılan: False). |
