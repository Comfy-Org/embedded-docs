...
| `prompt` | Video içeriğini tanımlayan metin istemi. Hem olumlu hem de olumsuz açıklamalar içerebilir. Storyboard'lar etkinleştirildiğinde yok sayılır. | STRING | Evet | - |
| `duration` | Oluşturulan videonun istenen uzunluğu (saniye cinsinden, varsayılan: 5). | INT | Evet | 3 ila 15 |
| `first_frame` | Video dizisi için başlangıç görüntüsü. | IMAGE | Evet | - |
| `end_frame` | Video için isteğe bağlı bitiş karesi. `reference_images` ile aynı anda kullanılamaz. Storyboard'larla çalışmaz. | IMAGE | Hayır | - |
...
| `hikaye_tahtaları` | Her biri için ayrı istemler ve süreler içeren bir dizi video bölümü oluşturun. Yalnızca `kling-v3-omni` için desteklenir. | DYNAMIC_COMBO | Hayır | `"disabled"`<br>`"1 storyboard"`<br>`"2 storyboards"`<br>`"3 storyboards"`<br>`"4 storyboards"`<br>`"5 storyboards"`<br>`"6 storyboards"` |
...
`storyboards` `"disabled"` dışında bir değere ayarlandığında, seçilen her bölüm için aşağıdaki girdiler eklenir (N, 1 ile seçilen storyboard sayısı arasında değişir):

| Parametre | Açıklama | Veri Türü | Gerekli | Aralık |
|-----------|-------------|-----------|----------|-------|
| `storyboard_N_prompt` | Storyboard bölümü N için istem. Maksimum 512 karakter. (varsayılan: "") | STRING | Evet | - |
| `storyboard_N_duration` | Storyboard bölümü N için süre (saniye cinsinden, varsayılan: 4). | INT | Evet | 1 ila 15 |
...
* Tüm girdi görüntüleri (`first_frame`, `end_frame` ve tüm `reference_images`) genişlik ve yükseklikte minimum 300 piksel boyutuna sahip olmalıdır.
...
* İstem, girdi görüntülerine `@image`, `@image1`, `@image2` vb. yer tutucuları kullanarak başvurabilir; bunlar otomatik olarak API uyumlu görüntü referans formatına dönüştürülür.
