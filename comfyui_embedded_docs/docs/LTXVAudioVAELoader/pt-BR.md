...
O nó LTXV Audio VAE Loader carrega um modelo pré-treinado de Autoencoder Variacional de Áudio (VAE) a partir de um arquivo de checkpoint. Ele lê o checkpoint especificado, mantém os pesos do VAE de áudio e do vocoder e prepara o modelo para uso em fluxos de trabalho de geração ou processamento de áudio no ComfyUI.
...
|-------------|-------------|-----------|
| `Audio VAE` | O modelo de Autoencoder Variacional de Áudio (VAE) carregado, pronto para ser conectado a outros nós de processamento de áudio. | VAE |
