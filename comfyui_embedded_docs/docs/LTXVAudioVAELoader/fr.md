...
|-----------|-----------------|--------|-------|-------------|
| `ckpt_name` | STRING | Oui | Tous les fichiers du dossier `checkpoints`.<br>*Exemple : `"audio_vae.safetensors"`* | Point de contrôle Audio VAE à charger. Il s'agit d'une liste déroulante remplie avec tous les fichiers trouvés dans votre répertoire `checkpoints` de ComfyUI. |
