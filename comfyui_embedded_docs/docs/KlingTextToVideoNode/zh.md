...
| `negative_prompt` | STRING | 是 | - | 反向文本提示词 |
| `cfg_scale` | FLOAT | 否 | 0.0 至 1.0 | 配置缩放值（默认值：1.0） |
| `aspect_ratio` | COMBO | 否 | 来自 KlingVideoGenAspectRatio 的选项 | 视频宽高比设置（默认值："16:9"） |
| `mode` | COMBO | 否 | 提供多个选项 | 用于视频生成的配置，格式为：模式 / 时长 / 模型名称。（默认值：modes[8]） |
