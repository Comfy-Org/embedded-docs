...
| `cfg 스케일` | FLOAT | 아니요 | 0.0 ~ 1.0 | 구성 스케일 값 (기본값: 1.0) |
| `종횡비` | COMBO | 아니요 | KlingVideoGenAspectRatio 옵션 | 비디오 화면 비율 설정 (기본값: "16:9") |
| `모드` | COMBO | 아니요 | 여러 옵션 사용 가능 | 비디오 생성에 사용할 구성으로, 형식은 다음과 같습니다: 모드 / 지속 시간 / 모델_이름. (기본값: modes[8]) |
