...
| `加速模式` | BOOLEAN | 是 | - | 是否使用 turbo 模式（生成速度更快，但品質可能較低）（預設：False） |
| `長寬比` | COMBO | 否 | "1:1"<br>"16:9"<br>"9:16"<br>"4:3"<br>"3:4"<br>"3:2"<br>"2:3" | 圖像生成的寬高比（預設："1:1"） |
| `MagicPrompt 選項` | COMBO | 否 | "AUTO"<br>"ON"<br>"OFF" | 決定生成時是否使用 MagicPrompt（預設："AUTO"） |
| `種子值` | INT | 否 | 0-2147483647 | 用於生成的隨機種子值（預設：0） |
| `排除提示詞` | STRING | 否 | - | 描述應從圖像中排除的內容（預設：空） |
