...
|-----------|-----------|----------|-------|-------------|
| `提示詞` | STRING | 是 | - | 用於圖像生成的提示文字（預設：空值） |
| `加速模式` | BOOLEAN | 是 | - | 是否使用加速模式（生成速度更快，可能降低品質）（預設：False） |
| `長寬比` | COMBO | 否 | "1:1"<br>"16:9"<br>"9:16"<br>"4:3"<br>"3:4"<br>"3:2"<br>"2:3" | 圖像生成的長寬比（預設："1:1"） |
| `MagicPrompt 選項` | COMBO | 否 | "AUTO"<br>"ON"<br>"OFF" | 決定是否在生成過程中使用 MagicPrompt（預設："AUTO"） |
| `種子值` | INT | 否 | 0-2147483647 | 用於生成過程的隨機種子值（預設：0） |
| `排除提示詞` | STRING | 否 | - | 描述圖像中應排除的內容（預設：空值） |
| `影像數量` | INT | 否 | 1-8 | 要生成的圖像數量（預設：1） |

**注意：** `num_images` 參數在每個生成請求中最多限制為 8 張圖像。
