> 本文档由 AI 生成。如果您发现任何错误或有改进建议，欢迎贡献！ [在 GitHub 上编辑](https://github.com/Comfy-Org/embedded-docs/blob/main/comfyui_embedded_docs/docs/CreateHookKeyframe/zh.md)

创建钩子关键帧节点允许您在生成过程中定义钩子行为发生变化的特定点。它会创建关键帧，在生成进度的特定百分比处修改钩子的强度，并且这些关键帧可以串联在一起，形成复杂的调度模式。

## 输入

| 参数 | 数据类型 | 是否必填 | 范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `strength_mult` | FLOAT | 是 | -20.0 至 20.0 | 此关键帧处钩子强度的乘数（默认值：1.0） |
| `start_percent` | FLOAT | 是 | 0.0 至 1.0 | 此关键帧在生成过程中生效的百分比节点（默认值：0.0） |
| `prev_hook_kf` | HOOK_KEYFRAMES | 否 | - | 可选的先前钩子关键帧组，用于将当前关键帧添加至其中 |

## 输出

| 输出名称 | 数据类型 | 描述 |
|-------------|-----------|-------------|
| `HOOK_KF` | HOOK_KEYFRAMES | 包含新创建关键帧的一组钩子关键帧 |

---
**Source fingerprint (SHA-256):** `51893311a0623cafcf8c2d8af00e4005ca2fea2df9474e87d7d4b332b38435c3`
