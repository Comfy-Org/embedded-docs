...
| 参数 | 数据类型 | 必填 | 取值范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `strength_mult` | FLOAT | 是 | -20.0 到 20.0 | 此关键帧处钩子强度的乘数（默认值：1.0） |
| `start_percent` | FLOAT | 是 | 0.0 到 1.0 | 生成过程中此关键帧生效的百分比点（默认值：0.0） |
| `prev_hook_kf` | HOOK_KEYFRAMES | 否 | - | 可选的先前钩子关键帧组，用于将当前关键帧添加到该组中 |
