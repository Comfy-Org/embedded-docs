...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `strength_mult` | FLOAT | 是 | -20.0 至 20.0 | 此關鍵影格處鉤子強度的乘數（預設值：1.0） |
| `start_percent` | FLOAT | 是 | 0.0 至 1.0 | 此關鍵影格在生成過程中生效的百分比點（預設值：0.0） |
| `prev_hook_kf` | HOOK_KEYFRAMES | 否 | - | 可選的先前鉤子關鍵影格群組，用於將此關鍵影格加入其中 |

## 輸出

| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `HOOK_KF` | HOOK_KEYFRAMES | 包含新建立關鍵影格在內的一組鉤子關鍵影格 |
