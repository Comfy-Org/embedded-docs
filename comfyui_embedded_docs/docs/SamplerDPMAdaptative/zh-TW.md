...
SamplerDPMAdaptative 節點實現了一種自適應 DPM（擴散概率模型）採樣器，能夠在採樣過程中自動調整步長。它使用基於容錯率的誤差控制來確定最佳步長，在計算效率與採樣準確度之間取得平衡。這種自適應方法有助於維持生成品質，同時可能減少所需的採樣步數。
...
| `rtol` | FLOAT | 是 | 0.0-100.0 | 誤差控制的相對容錯率（預設值：0.05） |
| `atol` | FLOAT | 是 | 0.0-100.0 | 誤差控制的絕對容錯率（預設值：0.0078） |
| `h_init` | FLOAT | 是 | 0.0-100.0 | 初始步長大小（預設值：0.05） |
