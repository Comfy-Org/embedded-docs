...
SamplerDPMAdaptative 节点实现了一个自适应 DPM（扩散概率模型）采样器，可在采样过程中自动调整步长。它使用基于容差的误差控制来确定最优步长，在计算效率与采样精度之间取得平衡。这种自适应方法有助于在保持质量的同时，潜在地减少所需的步数。
...
所有输入均为高级参数，用于微调自适应采样的行为。除 `order` 仅限取整数值 2 或 3 之外，所有数值输入均允许使用小数值，最小值为 0.0，最大值为 100.0。

## 输出

| 输出名称 | 描述 | 数据类型 |
| --- | --- | --- |
| `sampler` | 返回一个已配置好的 DPM 自适应采样器实例 | SAMPLER |
