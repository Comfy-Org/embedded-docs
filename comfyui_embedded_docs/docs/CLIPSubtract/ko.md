...
| 매개변수 | 데이터 타입 | 입력 형태 | 기본값 | 범위 | 설명 |
|-----------|-----------|------------|---------|-------|-------------|
| `clip1` | CLIP | 필수 | - | - | 수정될 기준 CLIP 모델입니다 |
| `clip2` | CLIP | 필수 | - | - | 기준 모델에서 주요 패치가 차감될 CLIP 모델입니다 |
| `multiplier` | FLOAT | 필수 | 1.0 | -10.0 ~ 10.0, 간격 0.01 | 뺄셈 연산의 강도를 제어합니다. 양수 값은 clip2의 특성을 차감하고, 음수 값은 특성을 추가합니다. |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `CLIP` | CLIP | 뺄셈 연산 후 생성된 결과 CLIP 모델입니다 |
