...
| `rank` | INT | 예 | 1-4096 | LoRA의 순위 값으로, 크기와 복잡성을 제어합니다 (기본값: 8) |
| `lora_type` | COMBO | 예 | `"standard"`<br>`"locon"`<br>`"loha"`<br>`"lokr"`<br>`"dylora"` | 생성할 LoRA 유형 (기본값: "standard") |
| `bias_diff` | BOOLEAN | 예 | - | LoRA 계산에 편향 차이를 포함할지 여부 (기본값: True) |
| `model_diff` | MODEL | 아니요 | - | LoRA로 변환할 ModelSubtract 출력 |
