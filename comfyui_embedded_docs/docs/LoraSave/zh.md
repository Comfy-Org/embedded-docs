...
| 参数 | 数据类型 | 必填 | 取值范围 | 描述 |
|------|-----------|------|----------|------|
| `filename_prefix` | STRING | 是 | - | 输出文件名的前缀（默认："loras/ComfyUI_extracted_lora"） |
| `rank` | INT | 是 | 1-4096 | LoRA 的秩值，控制大小和复杂度（默认：8） |
| `lora_type` | COMBO | 是 | 多个可用选项 | 要创建的 LoRA 类型，提供多种可用选项 |
| `bias_diff` | BOOLEAN | 是 | - | 是否在 LoRA 计算中包含偏置差异（默认：True） |
