...
|-----------|-----------|----------|-------|-------------|
| `video` | VIDEO | 예 | 해당 없음 | 업스케일할 입력 비디오 파일입니다. MP4 컨테이너 형식이어야 하며, 재생 시간은 5초에서 10분 사이여야 합니다. |
| `target_resolution` | STRING | 예 | `"720p"`<br>`"1080p"`<br>`"2K"`<br>`"4K"` | 업스케일된 출력 비디오의 원하는 해상도입니다. |
...
| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `output` | VIDEO | 선택한 대상 해상도로 업스케일된 비디오 파일입니다. |
