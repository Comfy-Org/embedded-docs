...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `video` | VIDEO | 是 | 不適用 | 要進行放大的輸入影片檔案。必須為 MP4 容器格式，且時長介於 5 秒至 10 分鐘之間。 |
| `target_resolution` | STRING | 是 | `"720p"`<br>`"1080p"`<br>`"2K"`<br>`"4K"` | 放大後輸出影片的目標解析度。 |
...
| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `output` | VIDEO | 以選定目標解析度輸出的放大後影片檔案。 |
