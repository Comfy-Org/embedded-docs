...
|-----------|-------------|-------------|
| `destination` | `LATENT`    | Üzerine başka bir gizli temsilin birleştirileceği gizli temsil. Birleştirme işlemi için temel katman olarak işlev görür. |
| `source` | `LATENT`    | Hedef üzerine birleştirilecek gizli temsil. Bu kaynak katman, belirtilen parametrelere göre yeniden boyutlandırılabilir ve konumlandırılabilir. |
...
| `resize_source` | `BOOLEAN` | Kaynak gizli temsilin, birleştirmeden önce hedefin boyutlarına uyacak şekilde yeniden boyutlandırılıp boyutlandırılmayacağını belirten bir bayrak. |
| `mask` | `MASK`     | Kaynağın hedef üzerinde nasıl karıştırılacağını kontrol etmek için kullanılabilecek isteğe bağlı bir maske. Maske, kaynağın hangi kısımlarının nihai birleşimde görüneceğini tanımlar. |
