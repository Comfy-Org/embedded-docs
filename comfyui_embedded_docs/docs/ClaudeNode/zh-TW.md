...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `prompt` | STRING | 是 | 不適用 | 輸入到模型的文字。（預設：空字串） |
| `model` | COMBO | 是 | `"Opus 4.7"`<br>`"Opus 4.6"`<br>`"Sonnet 4.6"`<br>`"Sonnet 4.5"`<br>`"Haiku 4.5"` | 用於生成回應的 Claude 模型。 |
| `seed` | INT | 是 | 0 到 2147483647 | 種子控制節點是否應重新執行；無論種子為何，結果皆非確定性。（預設：0） |
| `images` | IMAGE | 否 | 0 到 20 張圖片 | 可選的圖片，作為模型的上下文。最多 20 張圖片。 |
| `system_prompt` | STRING | 否 | 不適用 | 決定模型行為的基礎指令。（預設：空字串） |
...
| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `output` | STRING | 從 Claude 模型生成的文字回應。若未生成文字，則返回「來自 Claude 模型的空回應。」 |
