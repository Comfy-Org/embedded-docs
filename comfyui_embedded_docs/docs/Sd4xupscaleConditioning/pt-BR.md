...
| `positive`           | `CONDITIONING`     | Elementos de condicionamento positivo que guiam o processo de ampliação em direção a atributos ou características desejadas nas imagens de saída. |
...
| `scale_ratio`        | `FLOAT`            | Determina o fator pelo qual a resolução da imagem é aumentada. Uma proporção de escala maior resulta em uma imagem de saída maior, permitindo maior detalhe e clareza. |
| `noise_augmentation` | `FLOAT`            | Controla o nível de aumento de ruído aplicado durante o processo de ampliação. Isso pode ser usado para introduzir variabilidade e melhorar a robustez das imagens de saída. |
...
| `latent`      | `LATENT`     | Uma representação latente gerada durante o processo de ampliação, que pode ser utilizada em processamento adicional ou treinamento de modelo. |