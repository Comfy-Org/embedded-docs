...
|---------|----------|-------|--------|---------|
| `latents` | LATENT | بله | N/A | لیستی از دیکشنری‌های نهفته (latent dicts) برای دسته‌بندی بر اساس وضوح. |
| `conditioning` | CONDITIONING | بله | N/A | لیستی از لیست‌های شرطی‌سازی (باید با طول `latents` مطابقت داشته باشد). |
...
|-----------|----------|---------|
| `latents` | LATENT | لیستی از دیکشنری‌های نهفته دسته‌بندی‌شده، یکی برای هر سطل وضوح (resolution bucket). |
| `conditioning` | CONDITIONING | لیستی از لیست‌های شرطی‌سازی، یکی برای هر سطل وضوح. |
