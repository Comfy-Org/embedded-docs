...
| 参数 | 数据类型 | 是否必填 | 范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `latents` | LATENT | 是 | 不适用 | 按分辨率分桶的潜在字典列表。 |
| `conditioning` | CONDITIONING | 是 | 不适用 | 条件列表的列表（必须与 `latents` 长度匹配）。 |
...
| 输出名称 | 数据类型 | 描述 |
|-------------|-----------|-------------|
| `latents` | LATENT | 按分辨率分桶后的批处理潜在字典列表，每个分辨率桶对应一个。 |
| `conditioning` | CONDITIONING | 条件列表的列表，每个分辨率桶对应一个。 |
