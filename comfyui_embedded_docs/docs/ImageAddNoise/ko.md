...
|-----------|-----------|----------|-------|-------------|
| `이미지` | IMAGE | 예 | - | 노이즈를 추가할 입력 이미지 |
| `시드` | INT | 예 | 0 ~ 18446744073709551615 | 노이즈 생성에 사용되는 난수 시드 (기본값: 0) |
| `강도` | FLOAT | 예 | 0.0 ~ 1.0 | 노이즈 효과의 강도를 제어합니다 (기본값: 0.5) |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `이미지` | IMAGE | 노이즈가 적용된 출력 이미지 |
