...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `圖像` | IMAGE | 是 | - | 要調整大小並填充的輸入影像 |
| `目標寬度` | INT | 是 | 1 至 MAX_RESOLUTION | 輸出影像的目標寬度（預設值：512） |
| `目標高度` | INT | 是 | 1 至 MAX_RESOLUTION | 輸出影像的目標高度（預設值：512） |
| `填充顏色` | COMBO | 是 | "white"<br>"black" | 用於調整大小後影像周圍填充區域的顏色（預設值："white"） |
| `插值方法` | COMBO | 是 | "area"<br>"bicubic"<br>"nearest-exact"<br>"bilinear"<br>"lanczos" | 用於調整影像大小的插值方法（預設值："area"） |

## 輸出

| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `圖像` | IMAGE | 調整大小並填充後的輸出影像 |
