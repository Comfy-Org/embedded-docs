...
|-----------|-----------|----------|-------|-------------|
| `모델` | MODEL | 예 | - | LazyCache를 추가할 모델입니다. |
| `재사용 임계값` | FLOAT | 아니요 | 0.0 - 3.0 | 캐시된 단계를 재사용하기 위한 임계값입니다(기본값: 0.2). |
| `시작 백분율` | FLOAT | 아니요 | 0.0 - 1.0 | LazyCache 사용을 시작할 상대적 샘플링 단계입니다(기본값: 0.15). |
| `종료 백분율` | FLOAT | 아니요 | 0.0 - 1.0 | LazyCache 사용을 종료할 상대적 샘플링 단계입니다(기본값: 0.95). |
| `상세 정보` | BOOLEAN | 아니요 | - | 상세 정보를 로그로 출력할지 여부입니다(기본값: False). |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `모델` | MODEL | LazyCache 기능이 추가된 모델입니다. |
