...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `模型` | MODEL | 是 | - | 要加入 LazyCache 功能的模型。 |
| `重複使用閾值` | FLOAT | 否 | 0.0 - 3.0 | 重複使用快取步驟的閾值（預設值：0.2）。 |
| `起始百分比` | FLOAT | 否 | 0.0 - 1.0 | 開始使用 LazyCache 的相對取樣步驟（預設值：0.15）。 |
| `結束百分比` | FLOAT | 否 | 0.0 - 1.0 | 結束使用 LazyCache 的相對取樣步驟（預設值：0.95）。 |
