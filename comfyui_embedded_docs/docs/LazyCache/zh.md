...
| `start_percent` | FLOAT | 否 | 0.0 - 1.0 | 开始使用 LazyCache 的相对采样步骤（默认值：0.15）。 |
| `end_percent` | FLOAT | 否 | 0.0 - 1.0 | 结束使用 LazyCache 的相对采样步骤（默认值：0.95）。 |
| `verbose` | BOOLEAN | 否 | - | 是否记录详细信息（默认值：False）。 |
