...
|-----------|--------------|-------------|-------|-------------|
| `modelo` | MODEL | Sí | - | El modelo al que se le añadirá LazyCache. |
| `umbral_reutilización` | FLOAT | No | 0.0 - 3.0 | El umbral para reutilizar pasos almacenados en caché (predeterminado: 0.2). |
| `porcentaje_inicio` | FLOAT | No | 0.0 - 1.0 | El paso de muestreo relativo para comenzar a usar LazyCache (predeterminado: 0.15). |
| `porcentaje_fin` | FLOAT | No | 0.0 - 1.0 | El paso de muestreo relativo para finalizar el uso de LazyCache (predeterminado: 0.95). |
...
|------------------|--------------|-------------|
| `modelo` | MODEL | El modelo con la funcionalidad LazyCache añadida. |
