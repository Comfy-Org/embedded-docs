...
| 参数 | 数据类型 | 必填 | 范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `model` | COMBO | 是 | "sora-2"<br>"sora-2-pro" | 用于视频生成的 OpenAI Sora 模型（默认值："sora-2"） |
| `prompt` | STRING | 是 | - | 引导文本；如果存在输入图像，则可为空（默认值：空） |
| `size` | COMBO | 是 | "720x1280"<br>"1280x720"<br>"1024x1792"<br>"1792x1024" | 生成视频的分辨率（默认值："1280x720"） |
| `duration` | COMBO | 是 | 4<br>8<br>12 | 生成视频的时长，单位为秒（默认值：8） |
| `image` | IMAGE | 否 | - | 用于视频生成的可选输入图像 |
| `seed` | INT | 否 | 0 到 2147483647 | 用于确定节点是否应重新运行的种子；无论种子如何，实际结果都是非确定性的（默认值：0） |
