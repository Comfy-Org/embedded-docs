...
| `prompt` | STRING | 是 | - | 引導文字；若提供輸入圖片則可為空（預設值：空） |
| `size` | COMBO | 是 | "720x1280"<br>"1280x720"<br>"1024x1792"<br>"1792x1024" | 生成影片的解析度（預設值："1280x720"） |
| `duration` | COMBO | 是 | 4<br>8<br>12 | 生成影片的長度，單位為秒（預設值：8） |
| `image` | IMAGE | 否 | - | 用於影片生成的可選輸入圖片 |
| `seed` | INT | 否 | 0 到 2147483647 | 決定節點是否應重新執行的種子值；無論種子值為何，實際結果皆為非確定性（預設值：0） |
