...
|---------|----------|-------|--------|---------|
| `مدل` | MODEL | بله | - | مدل انتشار که بهینه‌سازی HyperTile روی آن اعمال می‌شود |
| `اندازه کاشی` | INT | خیر | 1 - 2048 | اندازه هدف کاشی برای پردازش (پیش‌فرض: 256). اندازه مؤثر کاشی به نزدیک‌ترین مضرب 8 گرد می‌شود، با حداقل 32. |
| `اندازه تعویض` | INT | خیر | 1 - 128 | نحوه بازآرایی کاشی‌ها را در طول پردازش برای بهبود کارایی کنترل می‌کند (پیش‌فرض: 2) |
| `حداکثر عمق` | INT | خیر | 0 - 10 | حداکثر سطح عمق (مقیاس تفکیک‌پذیری) برای اعمال کاشی‌بندی. مقدار 0 کاشی‌بندی را فقط در بالاترین تفکیک‌پذیری اعمال می‌کند (پیش‌فرض: 0) |
| `مقیاس عمق` | BOOLEAN | خیر | True / False | وقتی فعال باشد، اندازه کاشی در سطوح عمق پایین‌تر به صورت متناسب مقیاس‌دهی می‌شود. این می‌تواند به حفظ کیفیت در تفکیک‌پذیری‌های پایین‌تر کمک کند (پیش‌فرض: False) |

## خروجی‌ها

| نام خروجی | نوع داده | توضیحات |
|-----------|----------|---------|
| `مدل` | MODEL | مدل اصلاح‌شده با بهینه‌سازی HyperTile اعمال‌شده |
