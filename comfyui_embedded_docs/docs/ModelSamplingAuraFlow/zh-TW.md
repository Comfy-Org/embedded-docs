...
The ModelSamplingAuraFlow 節點將專門的採樣配置套用至擴散模型，特別針對 AuraFlow 模型架構設計。它透過套用 `shift` 參數來調整採樣分佈，從而修改模型的採樣行為。此節點繼承自 SD3 模型採樣框架，並提供對採樣過程的精細控制。
...
| `shift` | 套用至採樣分佈的偏移值。預設值：1.73。步長：0.01。 | FLOAT | 是 | 0.0 - 100.0 |
