...
| `find` | STRING | Oui | - | Texte à rechercher (par défaut : chaîne vide). |
| `replace` | STRING | Oui | - | Texte de remplacement (par défaut : chaîne vide). |

## Sorties

| Nom de la sortie | Type de données | Description |
|------------------|-----------------|-------------|
| `text` | STRING | Le texte traité, où toutes les occurrences du texte `find` ont été remplacées par le texte `replace`. |
