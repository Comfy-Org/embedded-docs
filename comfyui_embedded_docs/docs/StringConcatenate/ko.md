...
|-----------|-----------|----------|-------|-------------|
| `문자열_a` | STRING | 예 | - | 연결할 첫 번째 텍스트 문자열 |
| `문자열_b` | STRING | 예 | - | 연결할 두 번째 텍스트 문자열 |
| `구분자` | STRING | 아니요 | - | 두 입력 문자열 사이에 삽입할 문자 또는 문자열 (기본값: 빈 문자열) |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `output` | STRING | string_a와 string_b 사이에 구분자가 삽입되어 결합된 문자열 |
