...
| `resize_type` | COMBO | Sim | `SCALE_BY`<br>`SCALE_DIMENSIONS`<br>`SCALE_LONGER_DIMENSION`<br>`SCALE_SHORTER_DIMENSION`<br>`SCALE_WIDTH`<br>`SCALE_HEIGHT`<br>`SCALE_TOTAL_PIXELS`<br>`MATCH_SIZE` | O método usado para determinar o novo tamanho. Os parâmetros obrigatórios mudam conforme o tipo selecionado. |
...
| `scale_method` | COMBO | Sim | `"nearest-exact"`<br>`"bilinear"`<br>`"area"`<br>`"bicubic"`<br>`"lanczos"` | O algoritmo de interpolação usado para escalonamento (padrão: "area"). |
