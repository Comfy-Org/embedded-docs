...
| パラメータ | データ型 | 必須 | 範囲 | 説明 |
|-----------|-----------|----------|-------|-------------|
| `model` | MODEL | はい | - | 垂直ネガティブガイダンスを適用するモデル |
| `empty_conditioning` | CONDITIONING | はい | - | ネガティブガイダンス計算に使用される空の条件付け |
| `neg_scale` | FLOAT | いいえ | 0.0 - 100.0 | ネガティブガイダンスのスケーリング係数（デフォルト: 1.0） |

## 出力

| 出力名 | データ型 | 説明 |
|-------------|-----------|-------------|
| `model` | MODEL | 垂直ネガティブガイダンスが適用された修正済みモデル |
