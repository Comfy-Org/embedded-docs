...
|-----------|-----------|----------|-------|-------------|
| `bg_removal_name` | STRING | 예 | 사용 가능한 모델 파일 목록 | 이미지에서 배경을 제거하는 데 사용할 모델입니다. 사용 가능한 배경 제거 모델 파일 목록에서 선택하세요. |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `bg_model` | BACKGROUND_REMOVAL | 로드된 배경 제거 모델로, 다른 노드에서 이미지 처리를 위해 사용할 준비가 되었습니다. |
