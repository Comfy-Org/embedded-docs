...
SamplerEulerAncestralCFGPP ノードは、画像生成において classifier-free guidance (CFG++) を備えた Euler Ancestral 法を使用するサンプラーを作成します。このサンプラーは、祖先サンプリング技術とガイダンス条件付けを組み合わせることで、コヒーレンスを維持しながら多様な画像バリエーションを生成し、ノイズとステップサイズの調整を制御するパラメーターによる微調整を可能にします。

## 入力

| パラメータ | 説明 | データ型 | 必須 | 範囲 |
| --- | --- | --- | --- | --- |
| `eta` | サンプリング中のステップサイズを制御します。値が大きいほど更新がより積極的になります（デフォルト：1.0） | FLOAT | はい | 0.0 - 1.0 |
