...
此節點建立一個取樣器，使用 Euler Ancestral 方法搭配無分類器引導（CFG++）來進行影像生成。此取樣器結合祖先取樣技術與引導條件化，以產生多樣化的影像變化，同時維持連貫性，並可透過控制雜訊與步長調整的參數進行微調。

## 輸入

| 參數 | 描述 | 資料型態 | 必填 | 範圍 |
| --- | --- | --- | --- | --- |
| `eta` | 控制取樣期間的步長，較高的數值會導致更激進的更新（預設值：1.0） | FLOAT | 是 | 0.0 - 1.0 |
...
| 輸出名稱 | 描述 | 資料型態 |
| --- | --- | --- |
| `sampler` | 傳回一個已配置的取樣器物件，可用於影像生成管線中 | SAMPLER |
