...
|-----------|-------------|-----------|----------|-------|
| `clip_name` | The filename of the text encoder model to load. This must be a file located in the `ComfyUI/models/text_encoders/` directory. | COMBO | Yes | List of files found in the `text_encoders` folder |
...
| `device` | The device to load the model onto. `"default"` uses the default device (typically the GPU if available), while `"cpu"` forces CPU loading. This is an advanced option (default: `"default"`). | COMBO | No | `"default"`<br>`"cpu"` |

### Supported Type-to-Encoder Mappings

The `type` parameter selects the correct encoder for a given model architecture. The following common mappings are listed in the node's description:
