...
| `text` | 이미지 위에 오버레이할 텍스트 (기본값: ""). 여러 줄을 지원합니다. `\n` 및 `\t` 이스케이프 시퀀스는 줄바꿈과 탭으로 변환되며, 텍스트는 이미지 너비에 맞게 자동으로 줄바꿈됩니다. | STRING | 예 | |
| `font_size` | 이미지 높이 대비 글꼴 크기 백분율 (기본값: 5.0) | FLOAT | 예 | 0.5 ~ 50.0 (0.5 간격) |
