...
|-----------|-----------|----------|-------|-------------|
| `이미지` | IMAGE | 예 | - | 3D 모델 생성에 사용할 입력 이미지입니다. 여러 이미지를 제공할 수 있습니다. |
| `시드` | INT | 예 | - | 생성 일관성을 위한 무작위 시드 값입니다. |
| `재질 유형` | STRING | 예 | - | 3D 모델에 적용할 재질 유형입니다. |
| `폴리곤 수` | STRING | 예 | - | 생성된 3D 모델의 목표 폴리곤 수입니다. 메시 품질과 세부 수준을 결정합니다. |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `GLB` | STRING | 다운로드된 3D 모델의 파일 경로입니다(하위 호환성 전용). |
