...
Este nó prepara o condicionamento de imagem para o pipeline de geração 3D Trellis2. Ele extrai características visuais da imagem de entrada com um modelo de visão DINOv3 em duas resoluções, organiza-as em mapas de características por estágio (opcionalmente aprimorados com um modelo NAF) e combina-as com dados de câmera derivados do campo de visão horizontal. Ele gera um par de condicionamentos positivo e negativo, onde o negativo usa características zeradas para orientação sem classificador.

## Entradas

| Parâmetro | Descrição | Tipo de Dado | Obrigatório | Faixa |
|-----------|-------------|--------------|-------------|-------|
| `clip_vision_model` | ClipVision DINOv3 ViT-L/16. | CLIP_VISION | Sim | — |
| `image` | Imagem pré-processada de ImageCropToMask (pad_factor=1.1 para Pixal3D). | IMAGE | Sim | — |
...
Nota: O valor de `camera_angle_x` é convertido para radianos internamente e usado para calcular a distância da câmera para a matriz de transformação de projeção. Quando o modelo de visão fornecido inclui um componente NAF, o nó adicionalmente produz mapas de características de alta resolução para os estágios de forma e textura.

## Saídas

| Nome da Saída | Descrição | Tipo de Dado |
|---------------|-------------|--------------|
| `positive` | Condicionamento positivo contendo os mapas de características derivados da imagem e os dados de projeção para a geração com Trellis2. | CONDITIONING |
| `negative` | Condicionamento negativo com tensores de características zerados, usado para orientação sem classificador. | CONDITIONING |
