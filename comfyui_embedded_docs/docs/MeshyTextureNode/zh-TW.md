> 本文檔由 AI 生成。如果您發現任何錯誤或有改進建議，歡迎貢獻！ [在 GitHub 上編輯](https://github.com/Comfy-Org/embedded-docs/blob/main/comfyui_embedded_docs/docs/MeshyTextureNode/zh-TW.md)

# Meshy：紋理節點

Meshy：紋理節點可將 AI 生成的紋理應用到 3D 模型上。它接收來自先前 Meshy 3D 生成或轉換節點的任務 ID，並使用文字描述或參考影像為模型創建新的紋理。該節點以 GLB 和 FBX 檔案格式輸出帶有紋理的模型。

## 輸入

| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `model` | COMBO | 是 | `"latest"` | 用於紋理處理的 AI 模型版本。目前僅提供「最新」版本。 |
| `meshy_task_id` | MESHY_TASK_ID | 是 | - | 來自先前 Meshy 3D 生成或轉換任務的唯一識別碼（任務 ID）。這提供了要進行紋理處理的基礎 3D 模型。 |
| `enable_original_uv` | BOOLEAN | 否 | - | 使用模型的原始 UV 而非生成新的 UV。啟用時（預設值：`True`），Meshy 會保留上傳模型中的現有紋理。如果模型沒有原始 UV，輸出品質可能不佳。 |
| `pbr` | BOOLEAN | 否 | - | 為紋理模型啟用基於物理的渲染（PBR）材質輸出（預設值：`False`）。 |
| `text_style_prompt` | STRING | 否 | - | 使用文字描述您期望的物體紋理風格。最多 600 個字元。不能與 `image_style` 同時使用。 |
| `image_style` | IMAGE | 否 | - | 用於引導紋理處理過程的 2D 影像。不能與 `text_style_prompt` 同時使用。 |

**參數限制：**

* 您必須提供 `text_style_prompt` 或 `image_style` 其中之一，但不能同時提供兩者。
* `text_style_prompt` 最多限制為 600 個字元。

## 輸出

| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `model_file` | STRING | 生成的 GLB 模型檔案名稱。此輸出為向後相容而提供。 |
| `meshy_task_id` | MODEL_TASK_ID | 此紋理處理作業的唯一任務識別碼，可用於引用結果。 |
| `GLB` | FILE3DGLB | 以 GLB 檔案格式儲存的紋理 3D 模型。 |
| `FBX` | FILE3DFBX | 以 FBX 檔案格式儲存的紋理 3D 模型。 |

---
**Source fingerprint (SHA-256):** `380b682a8290c69e71a204c8c3d6c2d4fb2c15f4bc1679b98c7fc4fd9ec9e1b3`
