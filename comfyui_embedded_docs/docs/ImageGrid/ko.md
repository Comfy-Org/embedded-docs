...
|-----------|-----------|----------|-------|-------------|
| `images` | IMAGE | 예 | - | 그리드로 배열할 이미지 목록입니다. 이 노드가 작동하려면 최소 하나의 이미지가 필요합니다. |
| `columns` | INT | 아니요 | 1 - 20 | 그리드의 열 수입니다(기본값: 4). |
| `cell_width` | INT | 아니요 | 32 - 2048 | 그리드 각 셀의 너비(픽셀 단위)입니다(기본값: 256). |
| `cell_height` | INT | 아니요 | 32 - 2048 | 그리드 각 셀의 높이(픽셀 단위)입니다(기본값: 256). |
| `padding` | INT | 아니요 | 0 - 50 | 그리드 내 이미지 사이에 배치할 여백(픽셀 단위)입니다(기본값: 4). |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `image` | IMAGE | 모든 입력 이미지가 그리드로 배열된 단일 출력 이미지입니다. |
