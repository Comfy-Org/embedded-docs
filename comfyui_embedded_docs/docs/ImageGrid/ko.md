...
|-----------|-----------|----------|-------|-------------|
| `이미지` | IMAGE | 예 | - | 그리드로 배열할 이미지 목록입니다. 이 노드가 작동하려면 최소 하나의 이미지가 필요합니다. |
| `열` | INT | 아니요 | 1 - 20 | 그리드의 열 수입니다(기본값: 4). |
| `셀 너비` | INT | 아니요 | 32 - 2048 | 그리드 각 셀의 너비(픽셀 단위)입니다(기본값: 256). |
| `셀 높이` | INT | 아니요 | 32 - 2048 | 그리드 각 셀의 높이(픽셀 단위)입니다(기본값: 256). |
| `패딩` | INT | 아니요 | 0 - 50 | 그리드 내 이미지 사이에 배치할 여백(픽셀 단위)입니다(기본값: 4). |
