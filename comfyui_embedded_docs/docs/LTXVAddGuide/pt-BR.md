...
|-----------|--------------|-------------|-------|-----------|
| `positive` | CONDITIONING | Sim | - | Entrada de condicionamento positivo a ser modificada com orientação de quadro-chave |
| `negative` | CONDITIONING | Sim | - | Entrada de condicionamento negativo a ser modificada com orientação de quadro-chave |
| `vae` | VAE | Sim | - | Modelo VAE usado para codificar os quadros de imagem/vídeo de entrada |
| `latent` | LATENT | Sim | - | Sequência latente de entrada que receberá os quadros de condicionamento |
| `image` | IMAGE | Sim | - | Imagem ou vídeo para condicionar o vídeo latente. Deve ter 8*n + 1 quadros. Se o vídeo não tiver 8*n + 1 quadros, ele será cortado para o valor mais próximo de 8*n + 1 quadros. |
...
| `strength` | FLOAT | Não | 0.0 a 1.0 | Intensidade da influência do condicionamento, onde 1.0 aplica condicionamento total e 0.0 não aplica condicionamento (padrão: 1.0) |
...
|---------------|--------------|-----------|
| `positive` | CONDITIONING | Condicionamento positivo atualizado com informações de orientação de quadro-chave |
| `negative` | CONDITIONING | Condicionamento negativo atualizado com informações de orientação de quadro-chave |
