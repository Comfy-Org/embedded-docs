...
|-----------|-----------|----------|-------|-------------|
| `positive` | CONDITIONING | Да | - | Позитивное кондиционирование, которое будет модифицировано с помощью ключевых кадров |
| `negative` | CONDITIONING | Да | - | Негативное кондиционирование, которое будет модифицировано с помощью ключевых кадров |
| `vae` | VAE | Да | - | Модель VAE, используемая для кодирования входных изображений/видеокадров |
| `latent` | LATENT | Да | - | Входная латентная последовательность, которая получит кадры кондиционирования |
| `image` | IMAGE | Да | - | Изображение или видео для кондиционирования латентного видео. Должно содержать 8*n + 1 кадров. Если видео содержит не 8*n + 1 кадров, оно будет обрезано до ближайшего значения 8*n + 1 кадров. |
| `frame_idx` | INT | Нет | -9999 до 9999 | Индекс кадра, с которого начинается кондиционирование. Для однокадровых изображений или видео с 1-8 кадрами допустимо любое значение frame_idx. Для видео с 9+ кадрами frame_idx должен быть кратен 8, иначе он будет округлен вниз до ближайшего кратного 8. Отрицательные значения отсчитываются с конца видео. (по умолчанию: 0) |
| `strength` | FLOAT | Нет | 0.0 до 1.0 | Сила влияния кондиционирования, где 1.0 применяет полное кондиционирование, а 0.0 не применяет кондиционирование (по умолчанию: 1.0) |
...
|-------------|-----------|-------------|
| `positive` | CONDITIONING | Позитивное кондиционирование, обновленное с информацией о ключевых кадрах |
| `negative` | CONDITIONING | Негативное кондиционирование, обновленное с информацией о ключевых кадрах |
| `latent` | LATENT | Латентная последовательность с включенными кадрами кондиционирования и обновленной маской шума |