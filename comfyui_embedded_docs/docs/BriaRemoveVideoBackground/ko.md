...
| `background_color` | STRING | 예 | `"Black"`<br>`"White"`<br>`"Gray"`<br>`"Red"`<br>`"Green"`<br>`"Blue"`<br>`"Yellow"`<br>`"Cyan"`<br>`"Magenta"`<br>`"Orange"` | 출력 비디오의 새 배경으로 사용할 단색입니다. |
| `seed` | INT | 아니요 | 0 ~ 2147483647 | 노드 재실행 여부를 제어하는 시드 값입니다. 시드 값과 관계없이 결과는 비결정적입니다. (기본값: 0) |
