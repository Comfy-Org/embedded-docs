...
이 노드는 Bria AI 서비스를 사용하여 비디오에서 배경을 제거합니다. 입력 비디오를 처리하고 원본 배경을 선택한 단색으로 교체합니다. 이 작업은 외부 API를 통해 수행되며, 결과는 새 비디오 파일로 반환됩니다.

## 입력

| 매개변수 | 설명 | 데이터 타입 | 필수 | 범위 |
|-----------|-------------|-----------|----------|-------|
| `video` | 배경을 제거할 입력 비디오 파일입니다. | VIDEO | 예 | N/A |
...
| `seed` | `seed`는 노드를 다시 실행할지 여부를 제어합니다. 결과는 `seed`와 관계없이 비결정적입니다. (기본값: 0) | INT | 예 | 0  ~  2147483647 |

**참고:** 입력 비디오의 길이는 60초 이하여야 합니다. `seed` 값을 변경하지 않아도 결과는 비결정적입니다.

## 출력

| 출력 이름 | 설명 | 데이터 타입 |
|-------------|-------------|-----------|
| `output` | 배경이 제거되고 선택한 색상으로 대체된 처리된 비디오 파일이며, H.264 코덱의 MP4로 인코딩됩니다. | VIDEO |
