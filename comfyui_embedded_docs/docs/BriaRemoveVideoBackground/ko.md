...
이 노드는 Bria AI 서비스를 사용하여 비디오에서 배경을 제거합니다. 입력 비디오를 처리하고 원본 배경을 선택한 단색으로 대체합니다. 이 작업은 외부 API를 통해 수행되며, 결과는 새 비디오 파일로 반환됩니다.
...
| `seed` | 시드는 노드 재실행 여부를 제어합니다. 시드와 무관하게 결과는 비결정적입니다. (기본값: 0) | INT | 예 | 0 to 2147483647 |
...
| 출력 이름 | 설명 | 데이터 타입 |
|-------------|-------------|-----------|
| `output` | 배경이 제거되고 선택한 색상으로 대체된 처리된 비디오 파일입니다. 출력 비디오는 H.264 코덱의 MP4로 인코딩됩니다. | VIDEO |
