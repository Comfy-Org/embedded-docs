...
|---------|----------|--------|--------|----------|
| `samples` | LATENT | بله | | نمایش نهفته ورودی ویدیو که باید بزرگ‌نمایی شود. |
| `upscale_model` | LATENT_UPSCALE_MODEL | بله | | مدل بارگذاری‌شده‌ای که برای انجام بزرگ‌نمایی ۲ برابری روی داده‌های نهفته استفاده می‌شود. |
| `vae` | VAE | بله | | مدل VAE که برای نرمال‌زدایی از داده‌های نهفته ورودی قبل از بزرگ‌نمایی و نرمال‌سازی مجدد داده‌های نهفته خروجی پس از آن استفاده می‌شود. |

## خروجی‌ها

| نام خروجی | نوع داده | توضیحات |
|-----------|----------|----------|
| `LATENT` | LATENT | نمایش نهفته بزرگ‌نمایی‌شده با ابعاد فضایی دو برابر نسبت به ورودی. خروجی دارای همان اندازه دسته (batch size)، تعداد کانال‌ها و طول زمانی ورودی است. `noise_mask` از ورودی، در صورت وجود، از خروجی حذف می‌شود. |
