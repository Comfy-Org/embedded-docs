...
|-----------|-----------|----------|-------|-------------|
| `긍정 조건` | CONDITIONING | 예 | - | 오디오 타이밍 정보로 수정될 긍정 조건 입력 |
| `부정 조건` | CONDITIONING | 예 | - | 오디오 타이밍 정보로 수정될 부정 조건 입력 |
| `시작(초)` | FLOAT | 예 | 0.0 ~ 1000.0 | 오디오 생성을 위한 시작 시간(초) (기본값: 0.0) |
| `전체(초)` | FLOAT | 예 | 0.0 ~ 1000.0 | 오디오 생성을 위한 전체 지속 시간(초) (기본값: 47.0) |

## 출력

| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `negative` | CONDITIONING | 오디오 타이밍 정보가 적용된 수정된 긍정 조건 |
| `부정 조건` | CONDITIONING | 오디오 타이밍 정보가 적용된 수정된 부정 조건 |
