...
| 参数 | 数据类型 | 是否必填 | 范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `positive` | CONDITIONING | 是 | - | 需要添加音频时间信息的正向条件输入 |
| `negative` | CONDITIONING | 是 | - | 需要添加音频时间信息的负向条件输入 |
| `seconds_start` | FLOAT | 是 | 0.0 至 1000.0 | 音频生成的起始时间（秒），默认值：0.0 |
| `seconds_total` | FLOAT | 是 | 0.0 至 1000.0 | 音频生成的总时长（秒），默认值：47.0 |

## 输出

| 输出名称 | 数据类型 | 描述 |
|-------------|-----------|-------------|
| `positive` | CONDITIONING | 已应用音频时间信息的修改后正向条件 |
| `negative` | CONDITIONING | 已应用音频时间信息的修改后负向条件 |
