...
| `aprimoramento_de_rostos` | Aprimora rostos (se houver) durante o processamento (padrão: True). | BOOLEAN | Não | - |
| `criatividade_no_aprimoramento_de_rostos` | Define o nível de criatividade para o aprimoramento de rostos (padrão: 0.0). | FLOAT | Não | 0.0 - 1.0 |
| `força_do_aprimoramento_de_rostos` | Controla quão nítidos os rostos aprimorados ficam em relação ao fundo (padrão: 1.0). | FLOAT | Não | 0.0 - 1.0 |
| `cortar_para_preencher` | Por padrão, a imagem é exibida com tarjas (letterbox) quando a proporção da saída difere. Ative para recortar a imagem a fim de preencher as dimensões de saída (padrão: False). | BOOLEAN | Não | - |
| `largura_de_saida` | O valor zero significa calcular automaticamente (normalmente será o tamanho original ou output_height, se especificado) (padrão: 0). | INT | Não | 0 - 32000 |
| `altura_de_saida` | O valor zero significa gerar na mesma altura que o original ou a largura de saída (padrão: 0). | INT | Não | 0 - 32000 |
| `criatividade` | Controla o nível geral de criatividade do aprimoramento (padrão: 3). | INT | Não | 1 - 9 |
| `preservação_de_rostos` | Preserva a identidade facial dos sujeitos (padrão: True). | BOOLEAN | Não | - |
