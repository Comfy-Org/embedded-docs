...
| --- | --- | --- | --- | --- |
| `model` | The HiDream-O1 model to apply the seam smoothing wrapper to. | MODEL | Yes | - |
...
| `pattern` | Shift layout. `single_shift`: one pass at the natural patch grid plus others offset. `symmetric`: all passes off-grid, shifts split around origin (default: `"single_shift"`). | COMBO | Yes | `"single_shift"`<br>`"symmetric"` |
...
- The ramp options of `passes` (`ramp_2_4`, `ramp_2_4_8`) increase the number of passes as sampling progresses toward `end_percent` within the gated range, so they are only meaningful when `start_percent` and `end_percent` define a non-empty range.
- The averaged result is blended back into the model output only away from the image borders: a mask keeps the original prediction in the 32-pixel strip along each edge (with a 4-pixel feather), avoiding the wraparound contamination caused by the shifted passes.

## Outputs

| Output Name | Description | Data Type |
| --- | --- | --- |
| `model` | The modified model with the patch seam smoothing wrapper applied. | MODEL |
