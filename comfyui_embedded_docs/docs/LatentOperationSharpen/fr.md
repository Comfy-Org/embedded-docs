...
Le nœud `LatentOperationSharpen` crée une opération d’accentuation pour les représentations latentes à l’aide d’un noyau basé sur une gaussienne. Il normalise les données latentes, applique un noyau d’accentuation personnalisé par convolution, puis restaure la luminance d’origine, ce qui améliore les détails et les contours dans la représentation de l’espace latent.
...
Les trois entrées sont des paramètres avancés et disposent de valeurs par défaut, de sorte que le nœud peut être utilisé sans les modifier. Ce nœud est marqué comme expérimental.

## Sorties

| Nom de sortie | Description | Type de données |
| --- | --- | --- |
| `operation` | Retourne une opération d’accentuation qui peut être appliquée aux données latentes | LATENT_OPERATION |
