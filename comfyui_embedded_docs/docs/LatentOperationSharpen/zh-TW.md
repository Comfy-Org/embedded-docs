...
LatentOperationSharpen 節點使用高斯核心對潛在表徵應用銳化效果。其運作原理是將潛在資料標準化，應用自訂銳化核心進行卷積運算，然後恢復原始亮度。這能增強潛在空間表徵中的細節和邊緣。
...
| `alpha` | FLOAT | 否 | 0.0-5.0 | 銳化強度係數（預設值：0.1） |

## 輸出結果

| 輸出名稱 | 資料類型 | 說明 |
|-------------|-----------|-------------|
| `operation` | LATENT_OPERATION | 返回可應用於潛在資料的銳化操作 |