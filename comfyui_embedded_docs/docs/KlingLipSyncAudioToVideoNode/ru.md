...
| `audio` | AUDIO | Да | - | Аудиофайл, содержащий вокал для синхронизации с видео |
| `voice_language` | COMBO | Нет | `"en"`<br>`"zh"`<br>`"es"`<br>`"fr"`<br>`"de"`<br>`"it"`<br>`"pt"`<br>`"pl"`<br>`"tr"`<br>`"ru"`<br>`"nl"`<br>`"cs"`<br>`"ar"`<br>`"ja"`<br>`"hu"`<br>`"ko"` | Язык голоса в аудиофайле (по умолчанию: "en") |
