...
| 参数 | 数据类型 | 是否必需 | 范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `video` | VIDEO | 是 | - | 包含待同步嘴部动作面部的视频文件 |
| `audio` | AUDIO | 是 | - | 包含待与视频同步语音的音频文件 |
| `voice_language` | COMBO | 是 | `"en"`<br>`"zh"`<br>`"es"`<br>`"fr"`<br>`"de"`<br>`"it"`<br>`"pt"`<br>`"pl"`<br>`"tr"`<br>`"ru"`<br>`"nl"`<br>`"cs"`<br>`"ar"`<br>`"ja"`<br>`"hu"`<br>`"ko"` | 音频文件中语音的语言（默认值："en"） |
...
| 输出名称 | 数据类型 | 描述 |
|-------------|-----------|-------------|
| `output` | VIDEO | 经过唇形同步嘴部动作处理后的视频 |
| `video_id` | STRING | 处理后的视频的唯一标识符 |
