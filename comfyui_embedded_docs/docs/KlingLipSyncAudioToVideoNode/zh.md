...
| 参数名 | 数据类型 | 必填 | 取值范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `video` | VIDEO | 是 | - | 包含待同步口型人脸的视频文件 |
| `audio` | AUDIO | 是 | - | 包含需要与视频同步人声的音频文件 |
| `voice_language` | COMBO | 否 | `"en"`<br>`"zh"`<br>`"es"`<br>`"fr"`<br>`"de"`<br>`"it"`<br>`"pt"`<br>`"pl"`<br>`"tr"`<br>`"ru"`<br>`"nl"`<br>`"cs"`<br>`"ar"`<br>`"ja"`<br>`"hu"`<br>`"ko"` | 音频文件中人声的语言（默认值："en"） |
...
| 输出名称 | 数据类型 | 描述 |
|-------------|-----------|-------------|
| `output` | VIDEO | 经过口型同步处理后的视频 |
| `video_id` | STRING | 已处理视频的唯一标识符 |
