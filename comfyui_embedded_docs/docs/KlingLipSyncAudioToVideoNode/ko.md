...
|-----------|-----------|----------|-------|-------------|
| `비디오` | VIDEO | 예 | - | 립싱크할 얼굴이 포함된 비디오 파일 |
| `오디오` | AUDIO | 예 | - | 비디오와 동기화할 음성이 포함된 오디오 파일 |
| `음성 언어` | COMBO | 예 | `"en"`<br>`"zh"`<br>`"es"`<br>`"fr"`<br>`"de"`<br>`"it"`<br>`"pt"`<br>`"pl"`<br>`"tr"`<br>`"ru"`<br>`"nl"`<br>`"cs"`<br>`"ar"`<br>`"ja"`<br>`"hu"`<br>`"ko"` | 오디오 파일에 포함된 음성의 언어 (기본값: "en") |
...
| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `비디오 ID` | VIDEO | 립싱크된 입 움직임이 적용된 처리된 비디오 |
