...
| 參數 | 資料類型 | 必要 | 範圍 | 說明 |
|-----------|-----------|----------|-------|-------------|
| `影片` | VIDEO | 是 | - | 包含要進行唇形同步的臉部之影片檔案 |
| `音訊` | AUDIO | 是 | - | 包含要與影片同步的語音之音訊檔案 |
| `語音語言` | COMBO | 是 | `"en"`<br>`"zh"`<br>`"es"`<br>`"fr"`<br>`"de"`<br>`"it"`<br>`"pt"`<br>`"pl"`<br>`"tr"`<br>`"ru"`<br>`"nl"`<br>`"cs"`<br>`"ar"`<br>`"ja"`<br>`"hu"`<br>`"ko"` | 音訊檔案中語音的語言（預設值："en"） |
