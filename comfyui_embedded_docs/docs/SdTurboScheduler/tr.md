...
| `adımlar` | `INT` | Adımlar parametresi, oluşturulacak sigma dizisinin uzunluğunu belirler ve arındırma sürecinin ayrıntı düzeyini doğrudan etkiler. |
| `gürültü_azaltma` | `FLOAT` | Arındırma parametresi, sigma dizisinin başlangıç noktasını ayarlayarak görüntü oluşturma sırasında uygulanan arındırma seviyesi üzerinde daha hassas kontrol sağlar. |
