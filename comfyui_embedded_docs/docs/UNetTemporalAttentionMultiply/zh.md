...
| 参数 | 数据类型 | 是否必填 | 范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `model` | MODEL | 是 | - | 待使用注意力乘法因子修改的输入模型 |
| `self_structural` | FLOAT | 否 | 0.0 - 10.0 | 自注意力结构组件的乘法因子（默认值：1.0） |
| `self_temporal` | FLOAT | 否 | 0.0 - 10.0 | 自注意力时序组件的乘法因子（默认值：1.0） |
| `cross_structural` | FLOAT | 否 | 0.0 - 10.0 | 交叉注意力结构组件的乘法因子（默认值：1.0） |
| `cross_temporal` | FLOAT | 否 | 0.0 - 10.0 | 交叉注意力时序组件的乘法因子（默认值：1.0） |

## 输出

| 输出名称 | 数据类型 | 描述 |
|-------------|-----------|-------------|
| `model` | MODEL | 经过注意力权重调整后的修改模型 |
