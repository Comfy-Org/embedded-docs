...
| 参数名 | 数据类型 | 必填 | 取值范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `model` | MODEL | 是 | - | 需要应用注意力乘数进行修改的输入模型 |
| `self_structural` | FLOAT | 否 | 0.0 - 10.0 | 自注意力结构组件的乘数（默认值：1.0） |
| `self_temporal` | FLOAT | 否 | 0.0 - 10.0 | 自注意力时序组件的乘数（默认值：1.0） |
| `cross_structural` | FLOAT | 否 | 0.0 - 10.0 | 交叉注意力结构组件的乘数（默认值：1.0） |
| `cross_temporal` | FLOAT | 否 | 0.0 - 10.0 | 交叉注意力时序组件的乘数（默认值：1.0） |
