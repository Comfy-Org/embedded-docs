...
| `empty_padding` | Contrôle la gestion des entrées de texte vides. Lorsqu'elle est définie sur « none », les entrées de texte vides pour `clip_g`, `clip_l` ou `t5xxl` produiront des listes de jetons vides au lieu d'un remplissage. Il s'agit d'un paramètre avancé (par défaut : « none »). | COMBO | Oui | `"none"`<br>`"empty_prompt"` |

**Contraintes des paramètres :**

- Lorsque `empty_padding` est défini sur « none », les entrées de texte vides pour `clip_g`, `clip_l` ou `t5xxl` produiront des listes de jetons vides au lieu d'un remplissage.
...
|---------------|-------------|-----------------|
| `CONDITIONING` | Les données de conditionnement de texte encodées prêtes à être utilisées dans les pipelines de génération SD3 | CONDITIONING |
