...
| `전략` | uniform: 균등 간격, head: 처음 N개, tail: 마지막 N개, random: 무작위 정렬(기본값: "uniform"). | COMBO | 예 | "uniform"<br>"head"<br>"tail"<br>"random" |
...
- `strategy`가 `"uniform"`이고 `num_frames`가 1이면 비디오의 중간 프레임이 선택됩니다.
- 인식할 수 없는 `strategy` 값은 오류를 발생시킵니다.
